...
async function runCodeInHarness(options = {}) {
	...
	const runCode = async () => {
		...
		const findModule = (modulePath) => {
			...
			if (!module) {
				...
				if (modulePath.endsWith("json")) {
					...
					module = new SyntheticModule(["default"], evaluate, {
						context,
					});
				} else {
					module = new SourceTextModule(code + unique(), {
						context,
		...
		if (isModule) {
			const module = new SourceTextModule(code + unique(), {
				context,
				importModuleDynamically,
			});
...
function createHarnessForTest(includes) {
	...
	if (includes) {
		for (const include of includes) {
			...
			if (!content)
				throw new Error(`Included file is missing: ${include}`);
			harness += content;
		}
	}
	...
	if (req.method === "POST") {
		...
		req.on("end", async () => {
			...
			try {
				...
				if (
					parseInt(process.version.split(".")[0].replace("v", "")) <
					20
				) {
					return res.end(
						"Please upgrade the Node.js version to 20 or later.",
					);
				}
