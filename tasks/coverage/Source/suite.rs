...
pub trait Suite<T:Case> {
	...
	fn skip_test_path(&self, _path:&Path) -> bool { false }
...
pub trait Case: Sized + Sync + Send + UnwindSafe {
	...
	fn execute(&mut self, source_type:SourceType) -> TestResult {
		...
		let source_text = if self.always_strict() {
			// To run in strict mode, the test contents must be modified prior
			// to execution-- a "use strict" directive must be inserted as
			// the initial character sequence of the file, followed by a
