...
pub trait Suite<T: Case> {
	...
	fn read_test_cases(&mut self, name: &str, args: &AppArgs) {
		...
		let get_paths = || {
			...
			WalkDir::new(&cases_path)
				...
				.filter(|path| {
					filter.map_or(true, |query| {
						path.to_string_lossy().contains(query)
					})
				})
				.collect::<Vec<_>>()
		};
	...
	fn coverage_report(&self) -> CoverageReport<T> {
		...
		let parsed_positives =
			positives.iter().filter(|case| case.test_parsed()).count();

		let mut failed_positives = positives
			.into_iter()
			.filter(|case| !case.test_passed())
			.collect::<Vec<_>>();
		...
		let mut failed_negatives = negatives
			.into_iter()
			.filter(|case| !case.test_passed())
			.collect::<Vec<_>>();
	...
	fn print_coverage<W: Write>(
		...
		let parsed_diff =
			(*parsed_positives as f64 / *all_positives as f64) * 100.0;
		let positive_diff =
			(*passed_positives as f64 / *all_positives as f64) * 100.0;
		let negative_diff =
			(*passed_negatives as f64 / *all_negatives as f64) * 100.0;
	...
	fn snapshot_errors(
		...
		let mut tests = self
			...
			.iter()
			.filter(|case| {
				matches!(case.test_result(), TestResult::CorrectError(_, _))
			})
			.collect::<Vec<_>>();
...
pub trait Case: Sized + Sync + Send + UnwindSafe {
	...
	fn test_parsed(&self) -> bool {
		...
		assert!(!matches!(result, TestResult::ToBeRun), "test should be run");
		match result {
			TestResult::ParseError(_, panicked)
			| TestResult::CorrectError(_, panicked) => !panicked,
			_ => true,
		}
	}
	...
	fn execute(&mut self, source_type: SourceType) -> TestResult {
		...
		match result {
			Err(err) if should_fail => {
				TestResult::CorrectError(err, driver.panicked)
			},
			Err(err) if !should_fail => {
				TestResult::ParseError(err, driver.panicked)
			},
	...
	fn print<W: Write>(
		...
		let path =
			normalize_path(Path::new("tasks/coverage").join(self.path()));
		match self.test_result() {
			TestResult::ParseError(error, _) => {
				writer.write_all(
					format!("Expect to Parse: {path}\n").as_bytes(),
				)?;
			...
			TestResult::GenericError(case, error) => {
				...
				writer.write_all(
					format!("{case} error: {error}\n\n").as_bytes(),
				)?;
			},
			TestResult::IncorrectlyPassed => {
				writer.write_all(
					format!("Expect Syntax Error: {path}\n").as_bytes(),
				)?;
			},
			TestResult::Passed
			| TestResult::ToBeRun
			| TestResult::CorrectError(..) => {},
		}
		Ok(())
	}
