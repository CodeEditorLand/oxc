...
use oxc::{
	...
	span::{SourceType, Span, cmp::ContentEq},
...
impl CompilerInterface for Driver {
	...
	fn transform_options(&self) -> Option<&TransformOptions> { self.transform.as_ref() }
				...
				Ok(pattern2) => {
					...
					if !pattern2.content_eq(pattern) {
						self.errors.push(OxcDiagnostic::error(format!(
							"Regular Expression content mismatch for `{}`: `{pattern}` == \
							 `{pattern2}`",
