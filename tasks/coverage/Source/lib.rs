...
use crate::{
	...
	tools::{
		...
		transformer::{
			TransformerBabelCase,
			TransformerMiscCase,
			TransformerTest262Case,
...
pub fn workspace_root() -> PathBuf { project_root().join("tasks").join("coverage") }

fn snap_root() -> PathBuf { workspace_root().join("snapshots") }
...
impl AppArgs {
	fn should_print_detail(&self) -> bool { self.filter.is_some() || self.detail }
...
fn test() { AppArgs::default().run_default() }
