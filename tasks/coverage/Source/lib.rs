...
use crate::{
	...
	tools::{
		codegen::{
			CodegenBabelCase, CodegenMiscCase, CodegenTest262Case,
			CodegenTypeScriptCase,
		},
		minifier::{MinifierBabelCase, MinifierTest262Case},
		prettier::{
			PrettierBabelCase, PrettierMiscCase, PrettierTest262Case,
			PrettierTypeScriptCase,
		},
		semantic::{
			SemanticBabelCase, SemanticMiscCase, SemanticTest262Case,
			SemanticTypeScriptCase,
		},
		transformer::{
	...
	typescript::{
		TranspileRunner, TypeScriptCase, TypeScriptSuite,
		TypeScriptTranspileCase,
	},
};
...
impl AppArgs {
	...
	pub fn run_codegen_runtime(&self) {
		...
		Test262Suite::<CodegenRuntimeTest262Case>::new()
			.run_async("codegen_runtime_test262", self);
	...
	pub fn run_sync_v8_test262_status(&self) {
		...
		fs::write(
			workspace_root().join(V8_TEST_262_FAILED_TESTS_PATH),
			tests.join("\n"),
		)
...
fn test() {
	let args =
		AppArgs { debug: false, filter: None, detail: false, diff: false };
	args.run_all()
}
