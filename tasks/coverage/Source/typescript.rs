...
impl TypeScriptCase {
	...
	pub fn meta(&self) -> &TypeScriptTestMeta { &self.meta }
}
...
impl Case for TypeScriptCase {
	fn new(path:PathBuf, code:String) -> Self {
		...
		Self {
			...
			// future, need to handle multi-file test cases
			// Use meta.tests[0].content.clone() instead of code to get without
			// meta options code
	...
	fn should_fail(&self) -> bool { !self.meta.error_files.is_empty() }
	...
	fn run(&mut self) { self.result = self.execute(self.source_type); }
}
...
impl TypeScriptTestMeta {
	...
	/// These files start with `// @<option-name>: <option-value>` and are
	/// followed by the file's content. This function extracts the individual
	/// files with their content and drops unsupported files.
