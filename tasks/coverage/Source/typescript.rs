...
impl<T: Case> TypeScriptSuite<T> {
	pub fn new() -> Self {
		Self {
			test_root: project_root().join(TESTS_ROOT).join("cases"),
			test_cases: vec![],
		}
	}
}
...
impl<T: Case> Suite<T> for TypeScriptSuite<T> {
	...
	fn skip_test_path(&self, path: &Path) -> bool {
		...
		let supported_paths =
			["conformance"].iter().any(|p| path.to_string_lossy().contains(p));
		#[cfg(not(any(coverage, coverage_nightly)))]
		let supported_paths = ["conformance", "compiler"]
			.iter()
...
impl Case for TypeScriptCase {
	fn new(path: PathBuf, code: String) -> Self {
		...
		let is_module =
			["esnext", "es2022", "es2020", "es2015"].into_iter().any(
				|module| compiler_options.modules.contains(&module.to_string()),
			);
...
impl TypeScriptTestMeta {
	...
	pub fn from_source(path: &Path, code: &str) -> Self {
		let mut current_file_options: HashMap<String, String> =
			HashMap::default();
			...
			if let Some(option) = META_OPTIONS.captures(line) {
				...
				if meta_name != "filename" {
					current_file_options
						.insert(meta_name.clone(), meta_value.to_string());
					continue;
				}
		...
		let file_name =
			if !test_unit_data.is_empty() || current_file_name.is_some() {
	...
	fn get_error_files(path: &Path, options: &CompilerOptions) -> Vec<String> {
		...
		suffixes.extend(
			options.modules.iter().map(|module| format!("(module={module})")),
		);
		suffixes.extend(
			options.targets.iter().map(|target| format!("(target={target})")),
		);
		suffixes.extend(options.jsx.iter().map(|jsx| format!("(jsx={jsx})")));
		let mut error_files = vec![];
		for suffix in suffixes {
			let error_path =
				root.join(format!("{file_name}{suffix}.errors.txt"));
...
impl CompilerOptions {
	pub fn new(options: &HashMap<String, String>) -> Self {
		Self {
			...
			declaration: Self::value_to_boolean(
				options.get("declaration"),
				false,
			),
			always_strict: Self::value_to_boolean(
				options.get("alwaysstrict"),
				false,
			),
			...
			allow_unused_labels: Self::value_to_boolean(
