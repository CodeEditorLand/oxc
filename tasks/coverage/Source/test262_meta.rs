...
impl Phase {
	pub fn is_runtime(self) -> bool { matches!(self, Self::Runtime) }
...
impl MetaData {
	pub fn from_str(s:&str) -> Self {
		...
		Self {
			...
			// yaml["esid"].as_str().map(Into::into), es5id: yaml["es5id"].
			// as_str().map(Into::into), es6id: yaml["es6id"].as_str().
			// map(Into::into), info: yaml["info"].as_str().
