...
impl fmt::Write for HtmlWriter {
	...
	#[inline]
	fn write_char(&mut self, c:char) -> fmt::Result { self.inner.get_mut().write_char(c) }

	#[inline]
	fn write_str(&mut self, s:&str) -> fmt::Result { self.inner.get_mut().write_str(s) }
}

impl From<HtmlWriter> for String {
	#[inline]
	fn from(html:HtmlWriter) -> Self { html.into_inner() }
}
...
impl HtmlWriter {
	...
	pub fn into_inner(self) -> String { self.inner.into_inner() }

	/// Render an HTML tag with some children.
	///
	/// In most cases, you shouldn't use this method directly. Instead, prefer
	/// one of the tag-specific convenience methods like [`HtmlWriter::div`].
	/// Feel free to add any missing implementations that you need.
	///
	/// Also works with JSX (or really any XML). Does not support self-closing
	/// tags.
	///
	/// - `tag`:   The HTML tag name
	/// - `attrs`: Raw `attr="value"` string to insert into the opening tag
	/// - `inner`: A closure that produces content to render in between the
