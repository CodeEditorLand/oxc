...
fn root() -> PathBuf { project_root().join("tasks").join("prettier_conformance") }

fn fixtures_root() -> PathBuf { root().join("prettier").join("tests").join("format") }

fn snap_root() -> PathBuf { root().join("snapshots") }
...
`;"#
		...
		// put it here but not in below if-statement to help detect no matched input
		// cases.
...
mod tests {
	...
	use crate::{SNAP_RELATIVE_PATH, TestRunner, fixtures_root};
