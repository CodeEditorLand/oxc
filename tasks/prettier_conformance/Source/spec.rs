...
impl VisitMut<'_> for SpecParser {
	fn visit_call_expression(&mut self, expr:&mut CallExpression<'_>) {
		...
		// The `runFormatTest` function used on prettier's test cases. We need to
		// collect all `run_spec` calls And then parse the arguments to get the
		// options and parsers Finally we use this information to generate the
		// snapshot tests
