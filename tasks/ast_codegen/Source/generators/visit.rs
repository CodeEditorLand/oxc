...
impl Generator for VisitGenerator {
	fn name(&self) -> &'static str { "VisitGenerator" }
...
static CLIPPY_ALLOW:&str = "\
    unused_variables,clippy::extra_unused_type_parameters,clippy::explicit_iter_loop,\
                            clippy::self_named_module_files,clippy::semicolon_if_nothing_returned,\
                            clippy::match_wildcard_for_single_variants";

fn generate_visit(ctx:&CodegenCtx) -> TokenStream {
	let header = generated_header!();
	// we evaluate it outside of quote to take advantage of expression
	// evaluation otherwise the `\n\` wouldn't work!
...
impl<'a> VisitBuilder<'a> {
	...
	where
		T: ToTokens, {
		if self.is_mut { quote!(&mut #tk) } else { quote!(&#tk) }
	}
	...
	fn get_iter(&self) -> TokenStream { if self.is_mut { quote!(iter_mut) } else { quote!(iter) } }
	...
	fn get_visitor(
		...
		let (ident, as_type) = {
			...
			(
				ident.clone(),
				if collection { parse_quote!(Vec<'a, #as_type>) } else { as_type },
			)
		};
		...
		let ident_snake = {
			...
			let it = if collection {
				...
				// name: `visit_formal_parameters`. and edge case for
				// `Vec<TSImportAttribute>` to avoid conflicts with
				// `TSImportAttributes` which both would generate the same name:
		...
		// We push an empty walk first, because we evaluate - and generate -
		...
		let (walk_body, may_inline) = if collection {
			...
			match &*ty.borrow() {
				// TODO: this one is a hot-fix to prevent flattening aliased
				// `Expression`s, Such as `ExpressionArrayElement` and
				// `ClassHeritage`. Shouldn't be an edge case, <https://github.com/oxc-project/oxc/issues/4060>
				RType::Enum(enum_)
			...
			.filter_map(|it| {
				...
				if visitable {
					...
					let body = if args_def.is_empty() {
						...
						// if we have args wrap the result in a block to prevent
						// ident clashes.
						quote! {{
							#(#args_def)*
							#body
