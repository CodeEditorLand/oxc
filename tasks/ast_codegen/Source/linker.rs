...
pub trait Unresolved {
	...
	fn resolved(&self) -> bool { !self.unresolved() }
}

impl Unresolved for Inherit {
	fn unresolved(&self) -> bool { matches!(self, Self::Unlinked(_)) }
}

impl Unresolved for Vec<Inherit> {
	fn unresolved(&self) -> bool { self.iter().any(Unresolved::unresolved) }
}
...
impl<'a> Linker<'a> for CodegenCtx {
	fn link(
		...
		// we sort by `TypeId` so we always have the same ordering as how it is
		// written in the rust.
		...
		while let Some(next) = unresolved.pop_back() {
			...
			if !linker(val, self)? {
				// for now we don't have entangled dependencies so we just add
				// unresolved item back to the list so we revisit it again
				// at the end.
