...
fn main() {
	...
	let rule_test_path = match rule_kind {
		...
		RuleKind::Typescript => {
			format!("{TYPESCRIPT_ESLINT_TEST_PATH}/{kebab_rule_name}.test.ts")
		},
		RuleKind::Unicorn => {
			format!("{UNICORN_TEST_PATH}/{kebab_rule_name}.mjs")
		},
		RuleKind::React => format!("{REACT_TEST_PATH}/{kebab_rule_name}.js"),
		RuleKind::ReactPerf => {
			format!("{REACT_PERF_TEST_PATH}/{kebab_rule_name}.test.ts")
		},
		RuleKind::JSXA11y => {
			format!("{JSX_A11Y_TEST_PATH}/{kebab_rule_name}-test.js")
		},
		RuleKind::NextJS => {
			format!("{NEXT_JS_TEST_PATH}/{kebab_rule_name}.test.ts")
		},
		RuleKind::JSDoc => format!("{JSDOC_TEST_PATH}/{camel_rule_name}.js"),
		RuleKind::Node => format!("{NODE_TEST_PATH}/{kebab_rule_name}.js"),
		RuleKind::TreeShaking => {
			format!("{TREE_SHAKING_PATH}/{kebab_rule_name}.test.ts")
		},
		RuleKind::Promise => {
			format!("{PROMISE_TEST_PATH}/{kebab_rule_name}.js")
		},
		RuleKind::Vitest => {
			format!("{VITEST_TEST_PATH}/{kebab_rule_name}.test.ts")
		},
