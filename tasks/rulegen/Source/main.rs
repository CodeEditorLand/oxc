...
fn format_tagged_template_expression(tag_expr:&TaggedTemplateExpression) -> Option<String> {
	if tag_expr.tag.is_specific_member_access("String", "raw") {
		...
		tag_expr
			...
			.quasis
			.first()
			.map(|quasi| util::dedent(&quasi.value.raw).to_string())
	} else {
...
impl<'a> Visit<'a> for State<'a> {
	...
	fn visit_call_expression(&mut self, expr:&CallExpression<'a>) {
		...
		if let Expression::Identifier(ident) = &expr.callee {
			...
			// e.g. for `describe('valid', () => { ... })`, the group comment will be
			// "valid"
...
/// Adds a module definition for the given rule to the `rules.rs` file, and adds
/// the rule to the `declare_all_lint_rules!` macro block.
fn add_rules_entry(ctx:&Context, rule_kind:RuleKind) -> Result<(), Box<dyn std::error::Error>> {
	...
	// find the rule name (`pub mod xyz;`) that comes alphabetically before the new
	// rule mod def, otherwise just append it to the mod.
	...
	// then, insert `{mod_name}::{rule_name};` in the `declare_all_lint_rules!`
	// macro block in the correct position, alphabetically.
