...
impl<'a> Template<'a> {
	...
	pub fn render(&self, rule_kind: RuleKind) -> Result<(), Error> {
		...
		let path = match rule_kind {
			...
			RuleKind::Typescript => {
				Path::new("crates/oxc_linter/src/rules/typescript")
			},
			RuleKind::Unicorn => {
				Path::new("crates/oxc_linter/src/rules/unicorn")
			},
			RuleKind::React => Path::new("crates/oxc_linter/src/rules/react"),
			RuleKind::ReactPerf => {
				Path::new("crates/oxc_linter/src/rules/react_perf")
			},
			RuleKind::JSXA11y => {
				Path::new("crates/oxc_linter/src/rules/jsx_a11y")
			},
			...
			RuleKind::TreeShaking => {
				Path::new("crates/oxc_linter/src/rules/tree_shaking")
			},
			RuleKind::Promise => {
				Path::new("crates/oxc_linter/src/rules/promise")
			},
			RuleKind::Vitest => Path::new("crates/oxc_linter/src/rules/vitest"),
			RuleKind::Security => {
				Path::new("crates/oxc_linter/src/rules/security")
			},
		};

		std::fs::create_dir_all(path)?;
		let out_path =
			path.join(format!("{}.rs", self.context.snake_rule_name));
