...
impl Parse for ScopeAttribute {
	fn parse(input: ParseStream) -> Result<Self, syn::Error> {
		let parsed =
			input.parse_terminated(CommonAttribute::parse, Token![,])?;
		Ok(parsed.into_iter().fold(
			Self::default(),
			|mut acc, CommonAttribute { ident, args }| {
	...
	attr.map_or_else(
		...
		|attr| {
			...
			let nested = attr.parse_args_with(
				Punctuated::<CommonAttribute, Token![,]>::parse_terminated,
			);
			nested
				.map(|nested| {
					for com in nested {
						if com.ident == "args" {
							...
							visit_as =
								Some(parse2(com.args).expect(
									"Invalid `#[visit[as(...)]]` input!",
								));
...
where
	I: IntoIterator<Item = &'a Attribute>,
{
	fn try_parse_clone_in(
		attr: &Attribute,
	) -> crate::Result<Option<CloneInAttribute>> {
	...
	for attr in attrs {
		if let Some(attr) = try_parse_clone_in(attr)? {
			assert!(
				clone_in.replace(attr).is_none(),
				"Duplicate `#[clone_in(...)]` attribute."
			);
		}
	}
...
pub fn get_scope_attribute<'a, I>(
	attrs: I,
) -> Option<crate::Result<ScopeAttribute>>
where
	I: IntoIterator<Item = &'a Attribute>,
{
