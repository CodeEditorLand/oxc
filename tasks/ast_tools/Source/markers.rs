...
/// A struct representing all the helper attributes that might be used with
/// `#[generate_derive(...)]`
...
/// A enum representing the value passed in `#[clone_in(...)]` derive helper
/// attribute.
#[derive(Debug, Default, Serialize)]
...
/// An enum representing the `#[estree(...)]` attributes that we implement for
/// structs.
...
/// A struct representing the `#[estree(...)]` attributes that we implement for
/// enums.
...
impl Parse for ScopeAttribute {
	fn parse(input:ParseStream) -> Result<Self, syn::Error> {
		...
		Ok(parsed
			.into_iter()
			.fold(Self::default(), |mut acc, CommonAttribute { ident, args }| {
...
where
	...
	fn predicate(it:&&Attribute) -> bool { it.path().is_ident("visit") }
	...
	debug_assert_eq!(
		iter.find(predicate),
		None,
		"For now we only accept one `#[visit]` marker per field/variant, Please merge them \
		 together!"
	);
...
where
	I: IntoIterator<Item = &'a Attribute>, {
	#[expect(clippy::trivially_copy_pass_by_ref)]
	fn predicate(it:&&Attribute) -> bool { it.path().is_ident("scope") }
	...
	debug_assert_eq!(
		iter.find(predicate),
		None,
		"For now we only accept one `#[scope]` marker per field/variant, Please merge them \
		 together!"
	);
