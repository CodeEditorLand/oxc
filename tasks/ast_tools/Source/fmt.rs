...
pub fn pretty_print(input:&TokenStream) -> String {
	...
	// let result = INSERT_REGEX.replace_all(&result,
	// InsertReplacer).to_string();
	let result = COMMENT_REGEX.replace_all(&result, CommentReplacer).to_string();
	result
}

/// Run `cargo fmt`
pub fn cargo_fmt() { Command::new("cargo").arg("fmt").status().unwrap(); }
...
///
/// e.g. `insert!("#![allow(dead_code)]")` is replaced by
/// `#![allow(dead_code)]`.
///
/// We use this when inserting outer attributes (`#![allow(unused)]`) or plain
/// comments (`//` not `///`). `quote!` macro ignores plain comments, so it's
/// not possible to produce them otherwise.
