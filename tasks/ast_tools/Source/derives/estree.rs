...
impl Derive for DeriveESTree {
	fn trait_name() -> &'static str { "ESTree" }

	fn snake_name() -> String { "estree".to_string() }
...
//  2. Type enums, which are not camelCased. These are for example the r#type
//     field of a Function, and are used instead of the struct name as the type
//     field on the JSON.
