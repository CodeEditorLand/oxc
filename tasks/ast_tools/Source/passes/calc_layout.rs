...
impl PlatformLayout {
	...
	const fn zero() -> Self { Self(Layout::zero(), Layout::zero()) }

	const fn ptr() -> Self { Self(Layout::ptr_64(), Layout::ptr_32()) }

	const fn wide_ptr() -> Self { Self(Layout::wide_ptr_64(), Layout::wide_ptr_32()) }

	pub const fn of<T>() -> Self { Self(Layout::of::<T>(), Layout::of::<T>()) }

	/// Return `true` if either of platform layouts is unknown.
	fn is_unknown(&self) -> bool { self.0.is_unknown() || self.1.is_unknown() }
}

impl From<(Layout, Layout)> for PlatformLayout {
	fn from((x64, x32):(Layout, Layout)) -> Self { Self(x64, x32) }
}
...
fn calc_enum_layout(ty:&mut Enum, ctx:&EarlyCtx) -> Result<PlatformLayout> {
	...
	// Largest variant is not necessarily the most highly aligned e.g. `enum {
	// A([u8; 50]), B(u64) }`
	...
	// Add discriminant.
	// All enums are `#[repr(C, u8)]` (fieldful) or `#[repr(u8)]` (fieldless), so
	// disriminant is 1 byte. But padding is inserted to align all payloads to
	// largest alignment of any variant.
	size_align_64.size += size_align_64.align;

	size_align_32.size += size_align_32.align;

	// Variant payloads are not relevant in niche calculation for `#[repr(u8)]` /
	// `#[repr(C, u8)]` enums. The niche optimization for Option-like enums is
	// disabled by `#[repr(u8)]`. https://doc.rust-lang.org/nightly/nomicon/other-reprs.html#repru-repri
	// So number of niches only depends on the number of discriminants.
	// TODO: This isn't quite correct. Number of niches depends only on how many
	// unused discriminant values at *start* or *end* of range.
...
fn calc_type_layout(ty:&TypeAnalysis, ctx:&EarlyCtx) -> Result<PlatformLayout> {
	...
	fn try_fold_option(layout:Layout) -> Layout {
		let Layout::Layout(mut known) = layout else {
			return layout;
		};
		// option needs only one niche, We allow resizing in case there isn't enough
		// space.
