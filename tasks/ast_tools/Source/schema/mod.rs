...
impl<'a> IntoIterator for &'a Schema {
	...
	fn into_iter(self) -> Self::IntoIter { self.defs.iter() }
}
...
fn lower_ast_type(ty:&rust::AstType, ctx:&codegen::EarlyCtx) -> TypeDef {
	match ty {
		...
		rust::AstType::Macro(it) => {
			panic!("{}", unexpanded_macro_err(&it.item))
		},
	}
}
		...
		discriminant:variant.discriminant.as_ref().map_or_else(
			...
			|(_, disc)| {
				match disc {
					...
					_ => {
						panic!("invalid enum discriminant {:?} on {}", disc, enum_dbg_name())
					},
				}
			},
		),
