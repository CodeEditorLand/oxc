...
use crate::{
	...
	layout::KnownLayout,
	markers::{
		get_derive_attributes,
		get_estree_attribute,
		get_scope_attribute,
		get_scope_markers,
		get_visit_markers,
	},
	rust_ast as rust,
	util::{TypeExt, unexpanded_macro_err},
};
...
impl Schema {
	pub fn get(&self, id:TypeId) -> Option<&TypeDef> { self.defs.get(id) }
}
...
fn lower_ast_struct(it @ rust::Struct { item, meta }:&rust::Struct, ctx:&EarlyCtx) -> StructDef {
	// If the struct contains a `span` field, it must be the first field for
	// consistency, and also small performance improvement from byte ordering.
	if item
		.fields
		.iter()
...
fn lower_inherit(inherit:&rust::Inherit, ctx:&EarlyCtx) -> InheritDef {
	match inherit {
		rust::Inherit::Linked { super_, variants } => {
			InheritDef {
				super_:create_type_ref(super_, ctx),
				variants:variants
					.iter()
					.map(|var| {
						lower_variant(var, || super_.get_ident().inner_ident().to_string(), ctx)
					})
					.collect(),
			}
		},
