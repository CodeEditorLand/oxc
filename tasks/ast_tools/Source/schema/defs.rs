...
impl TypeDef {
	pub fn id(&self) -> TypeId { with_either!(self, it => it.id) }

	pub fn name(&self) -> &String { with_either!(self, it => &it.name) }

	pub fn visitable(&self) -> bool { with_either!(self, it => it.visitable) }
	...
	pub fn module_path(&self) -> &str { with_either!(self, it => &it.module_path) }
}
...
impl EnumDef {
	/// Returns an iterator that would first walk all "real" variants and moves
	/// onto inherited ones based on the inheritance order.
	pub fn all_variants(&self) -> impl Iterator<Item = &VariantDef> {
		self.variants
			.iter()
			.chain(self.inherits.iter().flat_map(|it| it.variants.iter()))
	}
	...
	/// }
	/// ```
	pub fn is_unit(&self) -> bool { self.all_variants().all(VariantDef::is_unit) }
}
...
impl VariantDef {
	pub fn ident(&self) -> syn::Ident { self.name.to_ident() }

	pub fn is_unit(&self) -> bool { self.fields.is_empty() }
}
...
impl Visibility {
	pub fn is_pub(&self) -> bool { matches!(self, Self::Pub) }
}
...
impl FieldDef {
	pub fn ident(&self) -> Option<syn::Ident> { self.name.as_ref().map(ToIdent::to_ident) }
}
...
impl TypeRef {
	...
	#[inline]
	pub fn type_id(&self) -> Option<TypeId> { self.id }

	/// Reflects the inner most type id of `Adt1<Adt2<...AdtN<T>>>`
	#[inline]
	pub fn transparent_type_id(&self) -> Option<TypeId> { self.transparent_id }
	...
	#[inline]
	pub fn analysis(&self) -> &TypeAnalysis { &self.analysis }
