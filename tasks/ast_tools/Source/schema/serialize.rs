...
use crate::{Schema, TypeId, markers::ESTreeStructTagMode, schema::GetIdent};
...
pub fn get_type_tag(def:&StructDef) -> Option<String> {
	...
	match tag_mode {
		...
		Some(ESTreeStructTagMode::CustomSerialize) | None => {
			...
			if has_type_field { None } else { Some(def.ident().to_string()) }
		},
	}
}

/// Returns a HashSet of structs that have the #[estree(always_flatten)]
/// attribute.
