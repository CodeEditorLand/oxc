...
impl Layout {
	...
	pub fn layout(self) -> Option<KnownLayout> {
		if let Self::Layout(layout) = self { Some(layout) } else { None }
	}
}

impl From<KnownLayout> for Layout {
	fn from(layout:KnownLayout) -> Self { Self::Layout(layout) }
}
...
impl KnownLayout {
	...
	/// number of available niches
	#[inline]
	pub fn niches(&self) -> u128 { self.niches }

	#[expect(unused)]
	#[inline]
	pub fn offsets(&self) -> Option<&Vec<usize>> { self.offsets.as_ref() }

	pub unsafe fn set_size_unchecked(&mut self, size:usize) { self.size = size; }

	pub unsafe fn set_align_unchecked(&mut self, align:usize) { self.align = align; }

	pub unsafe fn set_niches_unchecked(&mut self, niches:u128) { self.niches = niches; }
...
impl Layout {
	...
	pub const fn ptr_32() -> Self { Layout::known(4, 4, 0) }

	pub const fn ptr_64() -> Self { Layout::known(8, 8, 0) }

	pub const fn wide_ptr_32() -> Self { Layout::known(8, 4, 1) }

	pub const fn wide_ptr_64() -> Self { Layout::of::<&str>() }
