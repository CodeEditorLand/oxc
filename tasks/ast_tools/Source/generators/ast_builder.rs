...
fn fn_ident_name<S:AsRef<str>>(ident:S) -> String { ident.as_ref().to_case(Case::Snake) }
...
fn generate_enum_builder_fn(def:&EnumDef, schema:&Schema) -> TokenStream {
	def.variants
		.iter()
		.map(|it| generate_enum_variant_builder_fn(def, it, schema))
		.collect()
}

/// Create a builder function for an enum variant (e.g. for
/// `Expression::Binary`)
fn generate_enum_variant_builder_fn(
	...
	if does_alloc {
		...
		docs = docs.with_description(format!(
			"This node contains {inner_article} [`{inner_name}`] that will be stored in the \
			 memory arena."
		));
	}
...
///
/// If type has default fields (`scope_id`, `symbol_id`, `reference_id`), also
/// generates functions:
///
/// 3. to create owned object with provided `ScopeId` / `SymbolId` /
///    `ReferenceId`.
/// 4. to create boxed object with provided `ScopeId` / `SymbolId` /
///    `ReferenceId`.
		...
		let fn_docs = DocComment::new(format!("Build {article} [`{ident}`]{extra}."))
			.with_description(format!(
				"If you want the built node to be allocated in the memory arena, use \
				 [`AstBuilder::{alloc_fn_name}`] instead."
			))
			.with_params(params);

		let alloc_docs = DocComment::new(format!(
			"Build {article} [`{ident}`]{extra}, and store it in the memory arena."
		))
		.with_description(format!(
			"Returns a [`Box`] containing the newly-allocated node. If you want a stack-allocated \
			 node, use [`AstBuilder::{fn_name}`] instead."
		))
		.with_params(params);
...
impl Param {
	fn is_default(&self) -> bool { self.is_default }

	fn not_default(&self) -> bool { !self.is_default() }
}
...
///    for them can be extracted from struct fields and enums. This really only
///    applies to functions.
///
/// Each section only appears if there is content for it. Only [`summary`] is
/// required.
///
...
impl<'p> DocComment<'p> {
	...
	/// Add a leading space to a doc comment line if it doesn't already have
	/// one. This makes it easier to read, since the comment won't be directly
	/// next to the `///`.
	fn maybe_add_space(s:Cow<'static, str>) -> Cow<'static, str> {
		if s.is_empty() || s.starts_with(' ') { s } else { Cow::Owned(format!(" {s}")) }
	}
}
