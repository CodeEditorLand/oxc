	...
	fn get_visitor(&mut self, def:&TypeDef, collection:bool) -> Cow<'a, Ident> {
		...
		let ident_snake = {
			...
			let it = if collection {
				...
				// `FormalParameters` which both would generate the same name:
				// `visit_formal_parameters`. and edge case for `Vec<TSImportAttribute>` to
				// avoid conflicts with `TSImportAttributes` which both would generate the
				// same name: `visit_formal_parameters`.
		...
		// We push an empty walk first, because we evaluate - and generate - each walk
		// as we go, This would let us to maintain the order of first visit.
	...
	fn generate_struct_walk(&mut self, struct_:&StructDef) -> (TokenStream, /* inline */ bool) {
		...
		let fields_visits:Vec<TokenStream> = struct_
			...
			.filter_map(|(ix, field)| {
				...
				// This comes first because we would prefer the `enter_node` to be placed on top
				// of `enter_scope`
				if have_enter_scope {
				...
				if have_exit_scope {
					assert!(
						...
						"Scopes cannot be exited more than once. Remove the extra \
						 `#[scope(exit_before)]` attribute(s)."
					);
		...
		let with_scope_events = |body:TokenStream| {
			match (scope_events, enter_scope_at, exit_scope_at) {
				((enter, leave), 0, None) => {
					quote! {
						#enter
						#body
