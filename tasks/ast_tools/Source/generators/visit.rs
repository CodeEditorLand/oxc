	...
	fn generate_struct_walk(
		...
		let fields_visits:Vec<TokenStream> = struct_
			...
			.filter_map(|(ix, field)| {
				...
				// This comes first because we would prefer the `enter_node` to
				// be placed on top of `enter_scope`
				if have_enter_scope {
		...
		let with_node_events = |body:TokenStream| {
			match (node_events, enter_node_at) {
				((enter, leave), 0) => {
					quote! {
						#enter
						#body
		...
		let with_scope_events = |body:TokenStream| {
			match (scope_events, enter_scope_at) {
				((enter, leave), 0) => {
					quote! {
						#enter
						#body
