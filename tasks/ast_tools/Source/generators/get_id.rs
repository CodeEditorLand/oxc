//! Generator for ID getter/setter methods on all types with `scope_id`,
//! `symbol_id`, `reference_id` fields.
//!
//! e.g. Generates `scope_id` and `set_scope_id` methods on all types with a
//! `scope_id` field.
...
fn generate_for_type(def:&TypeDef) -> Option<TokenStream> {
	...
	let methods = def
		...
		.filter_map(|field| {
			...
			let get_doc2 = format!(
				" Only use this method on a post-semantic AST where [`{type_name}`]s are always \
				 defined."
			);
