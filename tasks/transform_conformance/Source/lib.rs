...
impl TestRunner {
	pub fn new(options: TestRunnerOptions) -> Self {
		let snapshot =
			Snapshot::new(&babel_root(), /* show_commit */ true);
	...
	pub fn run(self) {
		for (root, snapshot, exec_snapshot) in &[
			...
			self.generate_snapshot(
				root,
				SnapshotOption::new(transform_paths, snapshot),
			);
			...
			if self.options.exec {
				...
				self.generate_snapshot(
					root,
					SnapshotOption::new(exec_files, exec_snapshot),
				);
	...
	fn glob_files(
		...
		IndexMap<String, Vec<TestCaseKind>>,
		IndexMap<String, Vec<TestCaseKind>>,
	) {
		...
		for case in PLUGINS {
			...
			let (mut transform_paths, mut exec_paths): (
				Vec<TestCaseKind>,
				Vec<TestCaseKind>,
			) = WalkDir::new(root)
				.into_iter()
	...
	fn generate_snapshot(&self, root: &Path, option: SnapshotOption) {
		...
		for (case, test_cases) in paths {
			...
			// Run the test
			let (passed, failed): (Vec<TestCaseKind>, Vec<TestCaseKind>) =
				test_cases
					.into_iter()
					.map(|mut test_case| {
