...
fn test() { TestRunner::new(TestRunnerOptions::default()).run(); }
...
/// The test runner which walks the babel repository and searches for
...
fn babel_root() -> PathBuf { project_root().join("tasks").join("coverage").join("babel") }

fn packages_root() -> PathBuf { babel_root().join("packages") }

fn conformance_root() -> PathBuf { project_root().join("tasks").join("transform_conformance") }

fn snap_root() -> PathBuf { conformance_root().join("snapshots") }

fn oxc_test_root() -> PathBuf { conformance_root().join("tests") }

fn fixture_root() -> PathBuf { conformance_root().join("fixtures") }
...
impl TestRunner {
	...
	fn glob_files(
		...
		// use `IndexMap` to keep the order of the test cases the same in insert
		// order.
...
impl TestRunnerEnv {
	...
	fn run_test(path:&Path) -> bool { Self::get_test_result(path).contains("1 pass") }
}
