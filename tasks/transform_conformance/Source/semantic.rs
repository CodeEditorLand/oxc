use oxc_ast::{
	ast::{
		BindingIdentifier, ExportSpecifier, ImportSpecifier, ModuleExportName,
		Program,
	},
...
impl<'a> Visit<'a> for SemanticTester {
	fn visit_binding_identifier(&mut self, it: &BindingIdentifier<'a>) {
		...
		if let Some(symbol_id) = symbol_id {
			...
			if !self
				.scopes
				.has_binding(self.symbols.get_scope_id(symbol_id), &it.name)
			{
				self.errors.push(format!(
			...
			self.errors.push(format!(
				"Expect BindingIdentifier({}) to have a symbol_id",
				it.name
			));
		}
	}
	fn visit_identifier_reference(
		...
		if let Some(reference_id) = it.reference_id.get() {
			...
			self.errors.push(format!(
				"Expect IdentifierReference({}) to have a reference_id",
				it.name
			));
		}
	}
	...
	fn visit_export_specifier(&mut self, it: &ExportSpecifier<'a>) {
		if let ModuleExportName::IdentifierReference(ident) = &it.local {
			...
			if let Some(symbol_id) = reference_id.and_then(|reference_id| {
				self.symbols.get_reference(reference_id).symbol_id()
			}) {
				if self.symbols.get_flag(symbol_id).is_empty() {
					self.errors.push(format!(
