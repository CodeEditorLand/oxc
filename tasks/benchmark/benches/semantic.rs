		...
		group.bench_with_input(
			...
			|b, source_text| {
				...
				b.iter_with_large_drop(|| {
					// We drop `Semantic` inside this closure as drop time is part of cost of using
					// this API. We return `error`s to be dropped outside of the measured
					// section, as usually code would have no errors. One of our benchmarks
					// `cal.com.tsx` has a lot of errors, but that's atypical, so don't want to
					// include it in benchmark time.
