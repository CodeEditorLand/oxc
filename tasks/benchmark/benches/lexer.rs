...
fn bench_lexer(criterion:&mut Criterion) {
	...
	// Lexer lacks awareness of JS grammar, so it gets confused by a few things
	// without the parser driving it, notably escapes in regexps and template
	// strings. So simplify the input for it, by removing backslashes and
	// converting template strings to normal string literals.
	...
	for file in files {
		...
		group.bench_with_input(
			...
			|b, source_text| {
				...
				// User code would likely reuse the same allocator over and over
				// to parse multiple files, so we do the same here.
