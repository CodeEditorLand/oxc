	...
	for file in TestFiles::complicated().files() {
		...
		group.bench_function(id, |b| {
			b.iter_with_setup_wrapper(|runner| {
				...
				// Clone `trivias` (which is an `Arc`). We keep a 2nd copy, so
				// the value is not dropped when `Transformer` is dropped
				// inside the measured section. We clone `trivias` here
				// rather than in `routine` to avoid the cloning being included
				// in measure.
				let trivias_copy = trivias.clone();

				// `enable_all` enables all transforms except arrow functions
				// transform
				...
				runner.run(|| {
					...
					// Return the `TransformerReturn`, so it's dropped outside
					// of the measured section. `TransformerReturn`
					// contains `ScopeTree` and `SymbolTable` which are costly
					// to drop. That's not central to transformer, so we
					// don't want it included in this measure.
					ret
				});
