		...
		group.bench_function(id, |b| {
			b.iter_with_setup_wrapper(|runner| {
				...
				runner.run(|| {
					...
					// Return the `TransformerReturn`, so it's dropped outside of the measured
					// section. `TransformerReturn` contains `ScopeTree` and `SymbolTable`
					// which are costly to drop. That's not central to transformer, so we don't
					// want it included in this measure.
					ret
				});
			});
		});
