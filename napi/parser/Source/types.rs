...
impl ParseResult {
	...
	pub fn get_program(&mut self) -> String { mem::take(&mut self.program) }

	#[napi(getter)]
	pub fn module(&mut self) -> EcmaScriptModule { mem::take(&mut self.module) }

	#[napi(getter)]
	pub fn comments(&mut self) -> Vec<Comment> { mem::take(&mut self.comments) }

	#[napi(getter)]
	pub fn errors(&mut self) -> Vec<OxcError> { mem::take(&mut self.errors) }
...
pub struct EcmaScriptModule {
	...
	///
	/// Dynamic imports `import('foo')` are ignored since they can be used in
	/// non-ESM files.
