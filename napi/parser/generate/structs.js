...
function generateStructDeserializer(type) {
    const {fields} = type;
    let retCode;
    if (fields.length === 0) {
        retCode = 'null';
        ...
        const fieldCodes = fields.map(
            field => `${field.name}: ${field.code || generateStructFieldCode(field)}`
        );
...
function generateStructFieldCode(field) {
    return `${field.type.deserializerName}(${posWithOffsetAndShift(field.offset)})`;
}

function generateEnumDeserializer(type) {
...
function generateTypedEnumDeserializer(type) {
    const variantCodes = type.variants.map(
        variant => `case ${variant.discriminant}: `
...
function generateBoxDeserializer(type) {
    return `function ${type.deserializerName}(pos) {
        return ${type.type.deserializerName}(uint32[pos >> 2] - ptrOffset);
    }`;
}

function generateVecDeserializer(type) {
    return `function ${type.deserializerName}(pos) {
        const arr = [],
            len = uint32[(pos + 24) >> 2];
...
function generateOptionDeserializer(type) {
    ...
    if (child.size === type.size) {
        ...
        if (niche) {
            if (niche.size === 1) {
                noneCondition = `uint8[${posWithOffsetAndShift(niche.offset, 0)}] === ${niche.value}`;
            ...
            if (type.name === 'OptionHashbang') console.dir(child, {depth: 3});
