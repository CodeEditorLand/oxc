...
pub struct TransformResult {
	...
	/// is provided.
	///
	/// If parsing failed and `declaration` is set, this will be an empty
	/// string.
	///
...
pub struct TransformOptions {
	...
	///
	/// When `true`, the `sourceMap` field of transform result objects will be
	/// populated.
	///
	/// @default false
	///
...
pub struct TypeScriptOptions {
	...
	///
	/// - When set to `rewrite`, it will change `.ts`, `.mts`, `.cts` extensions
	///   to `.js`, `.mjs`, `.cjs` respectively.
	/// - When set to `remove`, it will remove `.ts`/`.mts`/`.cts`/`.tsx`
	///   extension entirely.
	/// - When set to `true`, it's equivalent to `rewrite`.
	/// - When set to `false` or omitted, no changes will be made to the
	///   extensions.
	///
	/// @default false
...
pub struct JsxOptions {
	...
	/// When spreading props, use `Object.assign` directly instead of an extend
	/// helper.
	///
	/// Only used for `classic` {@link runtime}.
	///
...
impl From<JsxOptions> for oxc::transformer::JsxOptions {
	fn from(options:JsxOptions) -> Self {
		...
		oxc::transformer::JsxOptions {
			...
			refresh:options.refresh.and_then(|value| {
				match value {
					Either::A(b) => b.then(oxc::transformer::ReactRefreshOptions::default),
					Either::B(options) => {
						Some(oxc::transformer::ReactRefreshOptions::from(options))
					},
				}
			}),
			..Default::default()
...
#[napi(string_enum)]
pub enum HelperMode {
	/// Runtime mode (default): Helper functions are imported from a runtime
	/// package.
	///
	/// Example:
	///
			...
			.map(|map| {
				map.into_iter()
					.map(|(local, value)| {
						match value {
							Either::A(source) => {
								Ok(InjectImport::default_specifier(&source, &local))
							},
							Either::B(v) => {
								if v.len() != 2 {
...
impl CompilerInterface for Compiler {
	...
	fn enable_sourcemap(&self) -> bool { self.sourcemap }
	...
	fn define_options(&self) -> Option<ReplaceGlobalDefinesConfig> { self.define.clone() }

	fn inject_options(&self) -> Option<InjectGlobalVariablesConfig> { self.inject.clone() }
