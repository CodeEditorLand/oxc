...
use oxc::{
	...
	span::SourceType,
	transformer::{
		InjectGlobalVariables, InjectGlobalVariablesConfig, InjectImport,
		ReplaceGlobalDefines, ReplaceGlobalDefinesConfig, Transformer,
	},
};
...
pub fn transform(
	...
	let declarations_result =
		source_type.is_typescript().then(|| ctx.declarations()).flatten().map(
			|options| isolated_declaration::build_declarations(&ctx, *options),
		);
	...
	let (declaration, declaration_map) = declarations_result
		.map_or((None, None), |d| {
			(Some(d.source_text), d.source_map.map(Into::into))
		});
...
fn transpile(
	...
	let options = options
		.map(oxc::transformer::TransformOptions::from)
		.unwrap_or_default();
