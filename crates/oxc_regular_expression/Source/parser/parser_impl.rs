...
impl<'a> Parser<'a> {
	...
	pub fn parse(mut self) -> Result<ast::Pattern<'a>> {
		...
		//   capturing groups
		// We may re-consider this if we need more performance rather than
		// simplicity.
		...
		// [SS:EE] Pattern :: Disjunction
		// It is a Syntax Error if Pattern contains two or more GroupSpecifiers
		// for which the CapturingGroupName of GroupSpecifier is the same.
	...
	fn parse_term(&mut self) -> Result<Option<ast::Term<'a>>> {
		...
		if self.state.unicode_mode {
			...
			return match (self.parse_atom()?, self.consume_quantifier()?) {
				...
				(None, Some(_)) => {
					Err(diagnostics::lone_quantifier(
	...
	fn parse_atom(&mut self) -> Result<Option<ast::Term<'a>>> {
		...
		// ?UnicodeSetsMode, ?NamedCaptureGroups] ) ( Disjunction[?
	...
	fn parse_atom_escape(&mut self, span_start:usize) -> Result<Option<ast::Term<'a>>> {
		...
		if let Some(index) = self.consume_decimal_escape()? {
			if self.state.unicode_mode {
				...
				// DecimalEscape is strictly greater than
		...
		if self.state.named_capture_groups && self.reader.eat('k') {
			if let Some(name) = self.consume_group_name()? {
				...
				// It is a Syntax Error if GroupSpecifiersThatMatch(GroupName)
				// is empty.
	...
	fn parse_character_class_escape_unicode(
		...
		if self.reader.eat('{') {
			...
			{
				if self.reader.eat('}') {
					// [SS:EE] CharacterClassEscape :: P{
					// UnicodePropertyValueExpression } It is a Syntax
					// Error if MayContainStrings of the
	...
	fn parse_character_escape(&mut self, span_start:usize) -> Result<Option<ast::Character>> {
		...
		if !self.state.unicode_mode {
			if let Some(cp) = self.consume_legacy_octal_escape_sequence() {
				...
				// Otherwise `\0022`(octal \002 + symbol 2) will be `\22`(octal
				// \22)
	...
	fn parse_nonempty_class_ranges(
		...
		loop {
			...
			let Some(class_atom_to) = self.parse_class_atom()? else {
				// ClassAtom[?UnicodeMode]
				// NonemptyClassRangesNoDash[?UnicodeMode] => ClassAtom[?
			...
			// ClassContents[?UnicodeMode, ~UnicodeSetsMode] If both sides
			// are characters, it is a range.
			if let (
				ast::CharacterClassContents::Character(from),
				ast::CharacterClassContents::Character(to),
			) = (&class_atom, &class_atom_to)
			{
				// [SS:EE] NonemptyClassRanges :: ClassAtom - ClassAtom
				// ClassContents [SS:EE] NonemptyClassRangesNoDash ::
				// ClassAtomNoDash - ClassAtom ClassContents It is a Syntax
				// Error if IsCharacterClass of the first ClassAtom is false,
				// IsCharacterClass of the second ClassAtom is false, and the
				// CharacterValue of the first ClassAtom is strictly greater
			...
			// [SS:EE] NonemptyClassRanges :: ClassAtom - ClassAtom
			// ClassContents [SS:EE] NonemptyClassRangesNoDash ::
			// ClassAtomNoDash - ClassAtom ClassContents It is a Syntax Error
			// if IsCharacterClass of the first ClassAtom is true or
			// IsCharacterClass of the second ClassAtom is true and this
			// production has a [UnicodeMode] parameter. (Annex B)
			if self.state.unicode_mode {
		...
		if let Some(class_set_character) = self.parse_class_set_character()? {
			if self.reader.eat('-') {
				if let Some(class_set_character_to) = self.parse_class_set_character()? {
					// [SS:EE] ClassSetRange :: ClassSetCharacter -
					// ClassSetCharacter It is a Syntax Error if the
					// CharacterValue of the first ClassSetCharacter is strictly
					// greater than the CharacterValue of the second
					// ClassSetCharacter.
	...
	fn parse_capturing_group(&mut self) -> Result<Option<ast::CapturingGroup<'a>>> {
		...
		if self.reader.eat('(') {
			...
			// GroupSpecifier is optional, but if it exists, `?` is also
			// required
			if self.reader.eat('?') {
			...
			if let Some(min) = self.consume_decimal_digits()? {
				...
				if self.reader.eat(',') {
					...
					if let Some(max) = self.consume_decimal_digits()? {
						if self.reader.eat('}') {
							if max < min {
								// [SS:EE] QuantifierPrefix :: { DecimalDigits ,
								// DecimalDigits } It is a Syntax Error
								// if the MV of the first DecimalDigits is
								// strictly greater than the MV of the second
								// DecimalDigits.
	...
	fn consume_unicode_property_value_expression(
		...
		if let Some(name) = self.consume_unicode_property_name() {
			if self.reader.eat('=') {
				...
				if let Some(value) = self.consume_unicode_property_value() {
					// [SS:EE] UnicodePropertyValueExpression ::
					// UnicodePropertyName = UnicodePropertyValue
					// It is a Syntax Error if the source text matched by
					// UnicodePropertyName is not a Unicode property name or
					// property alias listed in the “Property name and aliases”
					// column of Table 65.
					// [SS:EE] UnicodePropertyValueExpression ::
					// UnicodePropertyName = UnicodePropertyValue
					// It is a Syntax Error if the source text matched by
					// UnicodePropertyValue is not a property value or property
					// value alias for the Unicode property or property alias
					// given by the source text matched by UnicodePropertyName
		...
		if let Some(name_or_value) = self.consume_unicode_property_value() {
			// [SS:EE] UnicodePropertyValueExpression ::
			// LoneUnicodePropertyNameOrValue It is a Syntax Error if the
			// source text matched by LoneUnicodePropertyNameOrValue is not a
			// Unicode property value or property value alias for the
			// General_Category (gc) property listed in
			// PropertyValueAliases.txt, nor a binary property or binary
			// property alias listed in the “Property name and aliases” column
			// of Table 66, nor a binary property of strings listed in the
			...
			// [SS:EE] UnicodePropertyValueExpression ::
			// LoneUnicodePropertyNameOrValue It is a Syntax Error if the
			// enclosing Pattern does not have a [UnicodeSetsMode] parameter and
			// the source text matched by LoneUnicodePropertyNameOrValue is a
			// binary property of strings listed in the “Property name” column
			// of Table 67.
	...
	fn consume_reg_exp_idenfigier_start(&mut self) -> Result<Option<u32>> {
		...
		if self.reader.eat('\\') {
			if let Some(cp) = self.consume_reg_exp_unicode_escape_sequence(true)? {
				...
				// RegExpUnicodeEscapeSequence It is a Syntax Error if the
				// CharacterValue of RegExpUnicodeEscapeSequence is not the
				// numeric value of some code point matched by the
				// IdentifierStartChar lexical grammar production.
		...
		if !self.state.unicode_mode {
				...
				self.reader.peek().filter(|&cp| surrogate_pair::is_lead_surrogate(cp))
			{
					...
					self.reader.peek2().filter(|&cp| surrogate_pair::is_trail_surrogate(cp))
				{
					...
					// [SS:EE] RegExpIdentifierStart :: UnicodeLeadSurrogate
					// UnicodeTrailSurrogate It is a Syntax Error if the
					// RegExpIdentifierCodePoint of RegExpIdentifierStart is not
	...
	fn consume_reg_exp_idenfigier_part(&mut self) -> Result<Option<u32>> {
		...
		if self.reader.eat('\\') {
			if let Some(cp) = self.consume_reg_exp_unicode_escape_sequence(true)? {
				...
				// RegExpUnicodeEscapeSequence is not the numeric value of some
				// code point matched by the IdentifierPartChar lexical grammar
				// production.
		...
		if !self.state.unicode_mode {
				...
				self.reader.peek().filter(|&cp| surrogate_pair::is_lead_surrogate(cp))
			{
					...
					self.reader.peek2().filter(|&cp| surrogate_pair::is_trail_surrogate(cp))
				{
					...
					let cp =
						surrogate_pair::combine_surrogate_pair(lead_surrogate, trail_surrogate);
					// [SS:EE] RegExpIdentifierPart :: UnicodeLeadSurrogate
					// UnicodeTrailSurrogate It is a Syntax Error if the
					// RegExpIdentifierCodePoint of RegExpIdentifierPart is not
					// matched by the UnicodeIDContinue lexical grammar
					// production.
