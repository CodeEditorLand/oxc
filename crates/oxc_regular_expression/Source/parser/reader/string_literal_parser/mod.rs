...
without continuation""#,
	...
	fn should_fail_early_errors() {
		for source_text in [r#""invalid octal \777""#, r#""invalid non-octal decimal \9""#] {
	...
	fn should_combine_surrogate_pair() {
		...
		let ast = Parser::new(
			source_text,
			Options { combine_surrogate_pair:false, ..Options::default() },
		)
		.parse()
		.unwrap();
		...
		let ast =
			Parser::new(source_text, Options { combine_surrogate_pair:true, ..Options::default() })
				.parse()
				.unwrap();
	...
	fn restore_span() {
		...
		let ast = Parser::new(&source_text[3..13], Options { span_offset:3, ..Options::default() })
			.parse()
			.unwrap();
