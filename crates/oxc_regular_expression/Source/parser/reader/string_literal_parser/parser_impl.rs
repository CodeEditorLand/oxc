...
use crate::parser::reader::string_literal_parser::{
	...
	characters::{
		...
		PS,
		is_line_terminator,
		is_non_escape_character,
		is_single_escape_character,
	},
	diagnostics,
	options::Options,
};
...
impl Parser {
	...
	fn checkpoint(&self) -> (usize, u32) { (self.index, self.offset) }
	...
	fn peek(&mut self) -> Option<char> { self.peek_nth(0) }

	fn peek2(&mut self) -> Option<char> { self.peek_nth(1) }
}
