...
impl<'a> PatternParser<'a> {
		...
		if self.reader.eat('{') {
				...
				self.consume_unicode_property_value_expression()?
			{
				if self.reader.eat('}') {
					...
					// UnicodePropertyValueExpression is true. MayContainStrings is true
	...
	fn parse_nonempty_class_ranges(
		...
		loop {
			...
			) = (&class_atom, &class_atom_to)
			{
				...
				// ClassContents It is a Syntax Error if IsCharacterClass of the first
				// ClassAtom is false, IsCharacterClass of the second ClassAtom is false, and
				// the CharacterValue of the first ClassAtom is strictly greater than the
			...
			// [SS:EE] NonemptyClassRangesNoDash :: ClassAtomNoDash - ClassAtom
			// ClassContents It is a Syntax Error if IsCharacterClass of the first
			// ClassAtom is true or IsCharacterClass of the second ClassAtom is true and
			// this production has a [UnicodeMode] parameter. (Annex B)
			if self.state.unicode_mode {
	...
	fn parse_modifiers(&mut self) -> Result<Option<ast::Modifiers>> {
		...
		// RegularExpressionModifiers contains the same code point more than once.
		// It is a Syntax Error if the source text matched by the second
		// RegularExpressionModifiers contains the same code point more than once.
		// It is a Syntax Error if any code point in the source text matched by the
		// first RegularExpressionModifiers is also contained in the source text matched
		// by the second RegularExpressionModifiers.
		...
		Ok(Some(ast::Modifiers {
			span:self.span_factory.create(span_start, self.reader.offset()),
			enabling:enabling_iter.clone().any(|f| *f == 1).then(|| {
				ast::Modifier {
			...
			disabling:disabling_iter.clone().any(|f| *f == 1).then(|| {
				ast::Modifier {
		...
		if let Some(name) = self.consume_unicode_property_name() {
			if self.reader.eat('=') {
				...
				if let Some(value) = self.consume_unicode_property_value() {
					...
					// UnicodePropertyValue It is a Syntax Error if the source text matched by
					// UnicodePropertyName is not a Unicode property name or property alias listed
					// in the “Property name and aliases” column of Table 65.
					// [SS:EE] UnicodePropertyValueExpression :: UnicodePropertyName =
					// UnicodePropertyValue It is a Syntax Error if the source text matched by
					// UnicodePropertyValue is not a property value or property value alias for the
					// Unicode property or property alias given by the source text matched by
