...
pub fn invalid_indexed_reference(span: Span) -> OxcDiagnostic {
	OxcDiagnostic::error(format!("{PREFIX} Invalid indexed reference"))
		.with_label(span)
}
...
pub fn invalid_named_reference(span: Span) -> OxcDiagnostic {
	OxcDiagnostic::error(format!("{PREFIX} Invalid named reference"))
		.with_label(span)
}

#[cold]
pub fn invalid_unicode_property_name_negative_strings(
	span: Span,
	name: &str,
) -> OxcDiagnostic {
	OxcDiagnostic::error(format!(
...
#[cold]
pub fn character_class_range_out_of_order(
	span: Span,
	kind: &str,
) -> OxcDiagnostic {
	OxcDiagnostic::error(format!(
		"{PREFIX} Character {kind} range out of order"
	))
	.with_label(span)
}
...
pub fn character_class_range_invalid_atom(span: Span) -> OxcDiagnostic {
	OxcDiagnostic::error(format!(
		"{PREFIX} Character class range with invalid atom"
	))
	.with_label(span)
}
...
pub fn empty_class_set_expression(span: Span) -> OxcDiagnostic {
	OxcDiagnostic::error(format!(
		"{PREFIX} Expected nonempty class set expression"
	))
	.with_label(span)
}
...
#[cold]
pub fn class_set_expression_invalid_character(
	span: Span,
	kind: &str,
) -> OxcDiagnostic {
	OxcDiagnostic::error(format!(
		"{PREFIX} Unexpected character inside of {kind}"
	))
	.with_label(span)
}
...
pub fn missing_capturing_group_name(span: Span) -> OxcDiagnostic {
	OxcDiagnostic::error(format!("{PREFIX} Missing capturing group name"))
		.with_label(span)
}
...
pub fn too_large_number_in_braced_quantifier(span: Span) -> OxcDiagnostic {
	OxcDiagnostic::error(format!(
		"{PREFIX} Number is too large in braced quantifier"
	))
	.with_label(span)
}
...
pub fn too_large_number_digits(span: Span, kind: &str) -> OxcDiagnostic {
	OxcDiagnostic::error(format!(
		"{PREFIX} Number is too large in {kind} digits"
	))
	.with_label(span)
}
...
#[cold]
pub fn invalid_unicode_property_of_strings(
	span: Span,
	name: &str,
) -> OxcDiagnostic {
...
pub fn invalid_surrogate_pair(span: Span) -> OxcDiagnostic {
	OxcDiagnostic::error(format!("{PREFIX} Invalid surrogate pair"))
		.with_label(span)
}
