// NOTE: The current implementation switches iteration units depending on the
// mode.
//
// This is all for surrogate pairs in non-unicode mode, but it is required only
// in limited cases:
// - Group names for named `CapturingGroup`: `(?<name>.)`
// - Group names for `NamedReference`: `\k<name>`
// Even if we skip that distinction, it seems the current test262 cases pass due
// to other errors.
//
// Therefore, it is possible to change the implementation to iterate on `char`
// units always, assuming that change some output of AST for
// `Character[kind=Symbol]` to `Character[kind=SurrogatePairs]`.
//
...
pub struct Reader<'a> {
	source:&'a str,
	unicode_mode:bool,
	/// Current index for `u8_units`(unicode mode) or `u16_units`(non-unicode
	/// mode).
	index:usize,
...
impl<'a> Reader<'a> {
	pub fn new(source:&'a str, unicode_mode:bool) -> Self {
		// NOTE: Collecting `Vec` may not be efficient if the source is too
		// large. Implements lookahead cache with `VecDeque` is better...?
	...
	pub fn offset(&mut self) -> usize {
		if self.unicode_mode {
			...
			// NOTE: This does not return valid `Span` offset for surrogate
			// pairs. In the first place, there is no such thing as string
			// slice corresponding to them...
	...
	pub fn checkpoint(&self) -> usize { self.index }
	...
	pub fn peek(&self) -> Option<u32> { self.peek_nth(0) }

	pub fn peek2(&self) -> Option<u32> { self.peek_nth(1) }
