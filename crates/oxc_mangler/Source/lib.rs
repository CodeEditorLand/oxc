...
use oxc_semantic::{
	ReferenceId, ScopeTree, SemanticBuilder, SymbolId, SymbolTable,
};
...
impl Mangler {
	...
	pub fn get_reference_name(
		&self,
		reference_id: ReferenceId,
	) -> Option<&str> {
		let symbol_id =
			self.symbol_table.get_reference(reference_id).symbol_id()?;
	...
	pub fn build<'a>(mut self, program: &'a Program<'a>) -> Mangler {
		...
		let (mut symbol_table, scope_tree) =
		...
		for scope_id in scope_tree.descendants_from_root() {
			...
			let parent_max_slot = scope_tree
				...
				.map_or(0, |parent_scope_id| {
					max_slot_for_scope[parent_scope_id.index()]
				});
		...
		let root_unresolved_references =
			scope_tree.root_unresolved_references();
		...
		let generate_name =
			if self.options.debug { debug_name } else { base54 };
		let mut count = 0;
			...
			&reserved_names.into_iter().chunk_by(CompactStr::len)
		{
			...
			let slice_of_same_len_strings =
				slice_of_same_len_strings_group.collect_vec();
			...
			debug_assert!(
				symbols_renamed_in_this_batch.len()
					== slice_of_same_len_strings.len()
			);
			...
			let symbols_to_rename_with_new_names =
				symbols_renamed_in_this_batch
					.iter()
					.zip(slice_of_same_len_strings.iter());
	...
	fn tally_slot_frequencies(
		...
		let mut frequencies =
			vec![SlotFrequency::default(); total_number_of_slots];
