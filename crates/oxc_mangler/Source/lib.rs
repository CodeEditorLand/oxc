...
impl Mangler {
	...
	pub fn build<'a>(mut self, program:&'a Program<'a>) -> Mangler {
		...
		for scope_id in scope_tree.descendants_from_root() {
			...
			// The current slot number is continued by the maximum slot from the parent
			// scope
			...
			&reserved_names.into_iter().chunk_by(CompactStr::len)
		{
			...
			//    in the source."
			// sorting by slot enables us to sort by the order at which the vars first
			// appear in the source (this is possible because the slots are discovered
			// currently in a DFS method which is the same order  as variables appear in
			// the source code)
			symbols_renamed_in_this_batch.sort_unstable_by_key(|a| a.slot);

			// here we just zip the iterator of symbols to rename with the iterator of new
			// names for the next for loop
...
fn base54(n:usize) -> CompactStr {
	let mut num = n;
	// Base 54 at first because these are the usable first characters in JavaScript
	// identifiers <https://tc39.es/ecma262/#prod-IdentifierStart>
	let base = 54usize;
	let mut ret = String::new();
	ret.push(BASE54_CHARS[num % base] as char);
	num /= base;
	// Base 64 for the rest because after the first character we can also use 0-9
	// too <https://tc39.es/ecma262/#prod-IdentifierPart>
	let base = 64usize;
	while num > 0 {
		num -= 1;
		ret.push(BASE54_CHARS[num % base] as char);
