...
/// ```
///
/// The slot number for a new scope starts after the maximum slot of the parent
/// scope.
///
/// Occurrences of slots and their corresponding newly assigned short
/// identifiers are:
/// - slot2: 4 - a
/// - slot3: 2 - b
/// - slot4: 2 - c
...
impl Mangler {
	...
	pub fn build<'a>(mut self, program:&'a Program<'a>) -> Mangler {
		...
		// A slot is the occurrence index of a binding identifier inside a
		// scope.
		...
		for scope_id in scope_tree.descendants_from_root() {
			...
			// The current slot number is continued by the maximum slot from the
			// parent scope
		...
		// 2) If N number of vars are going to be assigned names of the same length, we
		//    assign the N names based on the order at which the vars first appear in
		//    the source. This makes the output somewhat less random, because symbols
		//    declared close together are assigned names that are quite similar. With
		//    this heuristic, the output is more compressible. For instance, the output
		//    may look like: var da = "..", ea = ".."; function fa() { .. } function
		//    ga() { .. }
			...
			&reserved_names.into_iter().chunk_by(CompactStr::len)
		{
			...
			// 2. "we assign the N names based on the order at which the vars first appear
			//    in the source."
			// sorting by slot enables us to sort by the order at which the vars
			// first appear in the source (this is possible because the slots
			// are discovered currently in a DFS method which is the same order
			//  as variables appear in the source code)
			symbols_renamed_in_this_batch.sort_unstable_by_key(|a| a.slot);

			// here we just zip the iterator of symbols to rename with the
			// iterator of new names for the next for loop
...
fn is_special_name(name:&str) -> bool { matches!(name, "exports" | "arguments") }
...
fn base54(n:usize) -> CompactStr {
	let mut num = n;
	// Base 54 at first because these are the usable first characters in
	// JavaScript identifiers <https://tc39.es/ecma262/#prod-IdentifierStart>
	let base = 54usize;
	let mut ret = String::new();
	ret.push(BASE54_CHARS[num % base] as char);
	num /= base;
	// Base 64 for the rest because after the first character we can also use
	// 0-9 too <https://tc39.es/ecma262/#prod-IdentifierPart>
	let base = 64usize;
	while num > 0 {
		num -= 1;
		ret.push(BASE54_CHARS[num % base] as char);
...
fn debug_name(n:usize) -> CompactStr { CompactStr::from(format!("slot_{n}")) }
