...
impl GraphicalReportHandler {
	...
	fn render_highlight_gutter(
		...
		// created. it's the max width of the gutter, minus how many character-widths
		// we just generated capped at 0 (though this should never go below in
		// reality), and then we add 3 to account for arrowheads when a gutter line
		// ends
...
struct FancySpan {
	...
	/// explicit that there can also be *no* label. If there is a label, it can
	/// have multiple lines which is what the vec is for.
