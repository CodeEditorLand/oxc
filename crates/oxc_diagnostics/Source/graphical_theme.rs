...
///
/// A theme consists of two things: the set of characters to be used for
/// drawing, and the
/// [`owo_colors::Style`](https://docs.rs/owo-colors/latest/owo_colors/struct.Style.html)s to be used to paint various items.
///
/// You can create your own custom graphical theme using this type, or you can
/// use one of the predefined ones using the methods below.
///
/// When created by [`Default::default`], themes are automatically selected
/// based on the `NO_COLOR` environment variable and whether the process is
...
impl ThemeStyles {
	...
	pub fn rgb() -> Self {
		Self {
			error:style().fg_rgb::<225, 80, 80>().bold(), /* CHANGED: <255,
			                                               * 30, 30> */
