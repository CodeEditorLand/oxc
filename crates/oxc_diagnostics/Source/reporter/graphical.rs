...
/// Pretty-prints diagnostics. Primarily meant for human-readable output in a
/// terminal.
///
/// See [`GraphicalReportHandler`] for how to configure colors, context lines,
/// etc.
...
impl Default for GraphicalReporter {
	fn default() -> Self { Self { handler:GraphicalReportHandler::new(), writer:writer() } }
}
...
impl DiagnosticReporter for GraphicalReporter {
	fn finish(&mut self) {
		self.writer
			.flush()
			.or_else(|e| {
				// Do not panic when the process is skill (e.g. piping into
				// `less`).
