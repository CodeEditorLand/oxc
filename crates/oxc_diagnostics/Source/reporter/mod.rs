...
/// Reporters are responsible for rendering diagnostics to some format and
/// writing them to some form of output stream.
///
/// Reporters get used by
/// [`DiagnosticService`](crate::service::DiagnosticService) when they
/// receive diagnostics.
///
/// ## Example
/// ```
...
/// 	fn default() -> Self { Self { writer:BufWriter::new(io::stderr()) } }
/// }
///
/// impl DiagnosticReporter for BufferedReporter {
/// 	// flush all remaining bytes when no more diagnostics will be reported
/// 	fn finish(&mut self) { self.writer.flush().unwrap(); }
///
/// 	// write rendered reports to stderr
/// 	fn render_diagnostics(&mut self, s:&[u8]) { self.writer.write_all(s).unwrap(); }
///
...
pub trait DiagnosticReporter {
	/// Lifecycle hook that gets called when no more diagnostics will be
	/// reported.
	///
	/// Used primarily for flushing output stream buffers, but you don't just
	/// have to use it for that. Some reporters (e.g. [`JSONReporter`]) store
	/// all diagnostics in memory, then write them all at once.
	///
	/// While this method _should_ only ever be called a single time, this is
	/// not a guarantee upheld in Oxc's API. Do not rely on this behavior.
	///
	/// [`JSONReporter`]: crate::reporter::JsonReporter
	fn finish(&mut self);

	/// Write a rendered collection of diagnostics to this reporter's output
	/// stream.
	fn render_diagnostics(&mut self, s:&[u8]);

	/// Render a diagnostic into this reporter's desired format. For example, a
	/// JSONLinesReporter might return a stringified JSON object on a single
	/// line. Returns [`None`] to skip reporting of this diagnostic.
	///
	/// Reporters should not use this method to write diagnostics to their
	/// output stream. That should be done in
	/// [`render_diagnostics`](DiagnosticReporter::render_diagnostics).
	fn render_error(&mut self, error:Error) -> Option<String>;
}
