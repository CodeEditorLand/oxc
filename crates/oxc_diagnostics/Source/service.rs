...
use crate::{
	reporter::{
		CheckstyleReporter, DiagnosticReporter, GithubReporter,
		GraphicalReporter, JsonReporter, UnixReporter,
	},
	...
	pub fn run(&mut self) {
		while let Ok(Some((path, diagnostics))) = self.receiver.recv() {
			...
			for diagnostic in diagnostics {
				...
				let is_error =
					severity == Some(Severity::Error) || severity.is_none();
				...
				if let Some(mut err_str) =
					self.reporter.render_error(diagnostic)
				{
					...
					if err_str.lines().any(|line| line.len() >= 1200) {
						let minified_diagnostic = Error::new(
							...
							.with_help(format!(
								"{path:?} seems like a minified file"
							)),
						);
