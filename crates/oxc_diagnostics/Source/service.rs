...
/// Listens for diagnostics sent over a [channel](DiagnosticSender) by some job,
/// and formats/reports them to the user.
///
/// [`DiagnosticService`] is designed to support multi-threaded jobs that may
/// produce reports. These jobs can send [messages](DiagnosticTuple) to the
/// service over its multi-producer, single-consumer
/// [channel](DiagnosticService::sender).
///
/// # Example
/// ```rust
/// use std::thread;
...
pub struct DiagnosticService {
	...
	/// which can be used to force exit with an error status if there are too
	/// many warning-level rule violations in your project
...
impl Default for DiagnosticService {
	fn default() -> Self { Self::new(GraphicalReporter::default()) }
}

impl DiagnosticService {
	/// Create a new [`DiagnosticService`] that will render and report
	/// diagnostics using the provided [`DiagnosticReporter`].
	///
	/// TODO(@DonIsaac): make `DiagnosticReporter` public so oxc consumers can
	...
	pub fn set_json_reporter(&mut self) { self.reporter = Box::<JsonReporter>::default(); }

	pub fn set_unix_reporter(&mut self) { self.reporter = Box::<UnixReporter>::default(); }
	...
	pub fn set_github_reporter(&mut self) { self.reporter = Box::<GithubReporter>::default(); }

	/// Set to `true` to only report errors and ignore warnings.
	///
	/// Use [`with_silent`](DiagnosticService::with_silent) to disable reporting
	/// entirely.
	///
	/// Default: `false`
	#[must_use]
	...
	///
	/// Use [`with_quiet`](DiagnosticService::with_quiet) to only disable
	/// reporting on warnings.
	///
	/// Default is `false`.
	#[must_use]
	...
	/// Specify a warning threshold, which can be used to force exit with an
	/// error status if there are too many warning-level rule violations in
	/// your project. Errors do not count towards the warning limit.
	///
	/// Use [`max_warnings_exceeded`](DiagnosticService::max_warnings_exceeded)
	/// to check if too many warnings have been received.
	///
	/// Default: [`None`]
	#[must_use]
	...
	///
	/// The service will only start processing diagnostics after
	/// [`run`](DiagnosticService::run) has been called.
	///
	/// [diagnostics]: DiagnosticTuple
	pub fn sender(&self) -> &DiagnosticSender { &self.sender }

	/// Get the number of warning-level diagnostics received.
	pub fn warnings_count(&self) -> usize { self.warnings_count.get() }

	/// Get the number of error-level diagnostics received.
	pub fn errors_count(&self) -> usize { self.errors_count.get() }

	/// Check if the max warning threshold, as set by
	/// [`with_max_warnings`](DiagnosticService::with_max_warnings), has been
	/// exceeded.
	pub fn max_warnings_exceeded(&self) -> bool {
		self.max_warnings
			.map_or(false, |max_warnings| self.warnings_count.get() > max_warnings)
	}

	/// Wrap [diagnostics] with the source code and path, converting them into
	/// [Error]s.
	///
		...
		while let Ok(Some((path, diagnostics))) = self.receiver.recv() {
			...
			for diagnostic in diagnostics {
				...
				if is_warning || is_error {
					...
					// Note that it does not disable ALL diagnostics, only
					// Warning diagnostics
					else if self.quiet {
						continue;
					}
				...
				if let Some(mut err_str) = self.reporter.render_error(diagnostic) {
					...
					// Setting to 1200 because graphical output may contain ansi
					// escape codes and other decorations.
