...
impl OxcDiagnostic {
	...
	#[inline]
	pub fn with_error_code_scope<T: Into<Cow<'static, str>>>(
		mut self,
		code_scope: T,
	) -> Self {
	...
	/// Use [`OxcDiagnostic::with_error_code`] to set both the scope and number at once.
	#[inline]
	pub fn with_error_code_num<T: Into<Cow<'static, str>>>(
		mut self,
		code_num: T,
	) -> Self {
	...
	pub fn with_source_code<T: SourceCode + Send + Sync + 'static>(
		self,
		code: T,
	) -> Error {
