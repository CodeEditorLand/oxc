...
//!
//! The main type in this module is [`OxcDiagnostic`], which is used by all
//! other oxc tools to report problems. It implements [miette]'s [`Diagnostic`]
//! trait, making it compatible with other tooling you may be using.
//!
//! ```rust
...
//! ```
//!
//! See the [miette] documentation for more information on how to interact with
//! diagnostics.
//!
//! ## Reporting
//! If you are writing your own tools that may produce their own errors, you can
//! use [`DiagnosticService`] to format and render them to a string or a stream.
//! It can receive [`Error`]s over a multi-producer, single consumer
//!
//! ```
...
impl OxcCode {
	pub fn is_some(&self) -> bool { self.scope.is_some() || self.number.is_some() }
}
...
impl fmt::Display for OxcDiagnostic {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> std::fmt::Result { write!(f, "{}", &self.message) }
}
...
impl Diagnostic for OxcDiagnostic {
	...
	///
	/// Diagnostics with missing severity levels should be treated as
...
impl OxcDiagnostic {
	...
	/// `with_error_code_scope(scope).with_error_code_num(number)`.
	#[inline]
	...
	///
	/// Use [`OxcDiagnostic::with_error_code`] to set both the scope and number
	/// at once.
	#[inline]
	...
	///
	/// Use [`OxcDiagnostic::error`] or [`OxcDiagnostic::warn`] to create a
	/// diagnostic at the severity you want.
	...
	///
	/// Existing labels will be removed. Use [`OxcDiagnostic::and_label`] append
	/// a label instead.
	///
	...
	///
	/// Note that this pairs nicely with [`oxc_span::Span`], particularly the
	/// [`label`] method.
	///
	...
	///
	/// Existing labels will be removed. Use [`OxcDiagnostic::and_labels`] to
	/// append labels instead.
	///
	/// You need to add some source code using
	/// [`OxcDiagnostic::with_source_code`] for this to actually be useful. If
	/// you only have a single label, consider using
	...
	/// [`oxc_span::Span`]: https://docs.rs/oxc_span/latest/oxc_span/struct.Span.html
	/// [`label`]: https://docs.rs/oxc_span/latest/oxc_span/struct.Span.html#method.label
	pub fn with_labels<L:Into<LabeledSpan>, T:IntoIterator<Item = L>>(mut self, labels:T) -> Self {
	...
	/// Add multiple labels to this diagnostic without clobbering existing
	/// labels.
	pub fn and_labels<L:Into<LabeledSpan>, T:IntoIterator<Item = L>>(mut self, labels:T) -> Self {
	...
	///
	/// You should use a [`NamedSource`] if you have a file name as well as the
	/// source code.
