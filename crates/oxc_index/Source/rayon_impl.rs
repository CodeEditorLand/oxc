...
impl<'data, I:Idx, T:Sync + 'data> IntoParallelIterator for &'data IndexVec<I, T> {
	...
	fn into_par_iter(self) -> Self::Iter { <&[T]>::into_par_iter(&self.raw) }
}
...
impl<'data, I:Idx, T:Send + 'data> IntoParallelIterator for &'data mut IndexVec<I, T> {
	...
	fn into_par_iter(self) -> Self::Iter { <&mut [T]>::into_par_iter(&mut self.raw) }
}
...
impl<I:Idx, T:Send> IntoParallelIterator for IndexVec<I, T> {
	...
	fn into_par_iter(self) -> Self::Iter { IntoIter { vec:self.raw } }
}
...
impl<'data, T:Send> IndexedParallelIterator for Drain<'data, T> {
	...
	where
		CB: ProducerCallback<Self::Item>, {
		unsafe {
			// Make the vector forget about the drained items, and temporarily
			// the tail too.
...
impl<'data, T:Send> Drop for Drain<'data, T> {
	fn drop(&mut self) {
		...
		if self.vec.len() == self.orig_len {
			// We must not have produced, so just call a normal drain to remove
			// the items.
			...
			// Move the tail items to their new place, then set the length to
			// include them.
			unsafe {
...
impl<T:Send> DrainProducer<'_, T> {
	...
	/// released.
	pub(crate) unsafe fn new(slice:&mut [T]) -> DrainProducer<'_, T> { DrainProducer { slice } }
...
impl<'data, T:'data> Iterator for SliceDrain<'data, T> {
	...
	fn count(self) -> usize { self.iter.len() }
}
