...
type SliceMappedMut<Iter, I, T> =
	iter::Map<Iter, fn(&mut [T]) -> &mut IndexSlice<I, [T]>>;
...
impl<I: Idx, T> IndexSlice<I, [T]> {
	...
	pub fn iter_mut_enumerated(
		&mut self,
	) -> Enumerated<slice::IterMut<'_, T>, I, &mut T> {
	...
	#[inline]
	pub fn sort_by<F: FnMut(&T, &T) -> core::cmp::Ordering>(
		&mut self,
		compare: F,
	) {
	...
	#[inline]
	pub fn sort_unstable_by<F: FnMut(&T, &T) -> core::cmp::Ordering>(
		&mut self,
		compare: F,
	) {
	...
	pub fn windows(
		&self,
		size: usize,
	) -> SliceMapped<slice::Windows<'_, T>, I, T> {
	...
	pub fn chunks(
		&self,
		size: usize,
	) -> SliceMapped<slice::Chunks<'_, T>, I, T> {
	...
	pub fn chunks_mut(
		&mut self,
		size: usize,
	) -> SliceMappedMut<slice::ChunksMut<'_, T>, I, T> {
	...
	pub fn chunks_exact(
		&self,
		chunk_size: usize,
	) -> SliceMapped<slice::ChunksExact<'_, T>, I, T> {
	...
	pub fn rchunks(
		&self,
		size: usize,
	) -> SliceMapped<slice::RChunks<'_, T>, I, T> {
	...
	pub fn rchunks_mut(
		&mut self,
		size: usize,
	) -> SliceMappedMut<slice::RChunksMut<'_, T>, I, T> {
	...
	pub fn split<F: FnMut(&T) -> bool>(
		&self,
		f: F,
	) -> SliceMapped<slice::Split<'_, T, F>, I, T> {
	...
	pub fn rsplit<F: FnMut(&T) -> bool>(
		&self,
		f: F,
	) -> SliceMapped<slice::RSplit<'_, T, F>, I, T> {
	...
	pub fn split_first_mut(
		&mut self,
	) -> Option<(&mut T, &mut IndexSlice<I, [T]>)> {
		if self.is_empty() {
			None
		} else {
