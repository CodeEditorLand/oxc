...
impl<I:Idx, T> IndexSlice<I, [T]> {
	...
	#[inline(always)]
	pub fn new<S:AsRef<[T]> + ?Sized>(s:&S) -> &Self { Self::from_slice(s.as_ref()) }
	...
	pub fn into_vec(self: Box<Self>) -> IndexVec<I, T> {
		...
		// thin wrappers around `[T]` and `Vec<T>` with the added marker for the
		// index.
	...
	#[inline]
	pub fn len_idx(&self) -> I { I::from_usize(self.raw.len()) }
	...
	///
	/// See also [`IndexSlice::iter_enumerated`], which gives you indices (of
	/// the correct type) as you iterate.
	#[inline]
	pub fn iter(&self) -> slice::Iter<'_, T> { self.raw.iter() }

	/// Get a iterator over mut reverences to our values.
	///
	/// See also [`IndexSlice::iter_mut_enumerated`], which gives you indices
	/// (of the correct type) as you iterate.
	#[inline]
	pub fn iter_mut(&mut self) -> slice::IterMut<'_, T> { self.raw.iter_mut() }
	...
	#[inline]
	pub fn sort_by_key<F:FnMut(&T) -> K, K:Ord>(&mut self, f:F) { self.raw.sort_by_key(f); }
	...
	#[inline]
	pub fn reverse(&mut self) { self.raw.reverse(); }
	...
	#[inline]
	pub fn swap_with_slice(&mut self, other:&mut Self) { self.raw.swap_with_slice(&mut other.raw); }

	/// Binary searches this sorted vec with a key extraction function,
	...
	#[inline]
	pub fn swap(&mut self, a:I, b:I) { self.raw.swap(a.index(), b.index()); }
	...
	/// the front
	#[inline]
	pub fn rotate_left(&mut self, mid:I) { self.raw.rotate_left(mid.index()); }
	...
	/// front
	#[inline]
	pub fn rotate_right(&mut self, k:I) { self.raw.rotate_right(k.index()); }
	...
	/// Return the the last element, if we are not empty.
	#[inline]
	pub fn last_mut(&mut self) -> Option<&mut T> { self.raw.last_mut() }

	/// Return the the first element, if we are not empty.
	#[inline]
	pub fn first(&self) -> Option<&T> { self.raw.first() }

	/// Return the the first element, if we are not empty.
	#[inline]
	pub fn first_mut(&mut self) -> Option<&mut T> { self.raw.first_mut() }
	...
	#[inline]
	pub fn get<J:IdxSliceIndex<I, T>>(&self, index:J) -> Option<&J::Output> { index.get(self) }
	...
	/// yields `IndexSlice`s with the correct index type.
	#[inline]
	pub fn rchunks_exact(&self, chunk_size:usize) -> SliceMapped<slice::RChunksExact<'_, T>, I, T> {
...
{
	#[inline]
	fn eq(&self, other:&IndexSlice<I, [B]>) -> bool { PartialEq::eq(&self.raw, &other.raw) }

	#[inline]
	fn ne(&self, other:&IndexSlice<I, [B]>) -> bool { PartialEq::ne(&self.raw, &other.raw) }
}
...
{
	#[inline]
	fn eq(&self, other:&[B]) -> bool { PartialEq::eq(&self.raw, other) }

	#[inline]
	fn ne(&self, other:&[B]) -> bool { PartialEq::ne(&self.raw, other) }
}
...
impl<I:Idx, T:core::hash::Hash> core::hash::Hash for IndexSlice<I, [T]> {
	#[inline]
	fn hash<H:core::hash::Hasher>(&self, h:&mut H) { self.raw.hash(h); }
}
	...
	T: Clone,
{
	...
	#[inline]
	fn to_owned(&self) -> Self::Owned { IndexVec::from(self.raw.to_vec()) }
}
...
impl<I:Idx, T> Default for &IndexSlice<I, [T]> {
	#[inline]
	fn default() -> Self { IndexSlice::new(&[]) }
}

impl<I:Idx, T> Default for &mut IndexSlice<I, [T]> {
	#[inline]
	fn default() -> Self { IndexSlice::new_mut(&mut []) }
}

impl<'a, I:Idx, T> From<&'a [T]> for &'a IndexSlice<I, [T]> {
	#[inline]
	fn from(a:&'a [T]) -> Self { IndexSlice::new(a) }
}

impl<'a, I:Idx, T> From<&'a mut [T]> for &'a mut IndexSlice<I, [T]> {
	#[inline]
	fn from(a:&'a mut [T]) -> Self { IndexSlice::new_mut(a) }
}
...
impl<I:Idx, A> Default for Box<IndexSlice<I, [A]>> {
	#[inline(always)]
	fn default() -> Self { index_vec![].into() }
}
