...
impl BasicBlock {
	fn new() -> Self { BasicBlock { instructions:Vec::new(), unreachable:false } }

	pub fn instructions(&self) -> &Vec<Instruction> { &self.instructions }
}
...
impl Instruction {
	pub fn new(kind:InstructionKind, node_id:Option<NodeId>) -> Self { Self { kind, node_id } }
}
...
pub enum EdgeType {
	...
	// misc edges
	Unreachable,
	/// Used to mark the end of a finalizer. It is an experimental approach
	/// might move to it's respective edge kind enum or get removed altogether.
	Join,
}

#[derive(Default, Debug, Clone, Copy)]
pub enum ErrorEdgeKind {
	/// Error kind for edges between a block which can throw, to it's respective
	/// catch block.
	Explicit,
	/// Any block that can throw would have an implicit error block connected
	/// using this kind.
	#[default]
	Implicit,
}
...
impl ControlFlowGraph {
	pub fn graph(&self) -> &Graph<usize, EdgeType> { &self.graph }
	...
	pub fn basic_block(&self, id:BasicBlockId) -> &BasicBlock {
		...
		self.basic_blocks
			.get(ix)
			.expect("expected a valid node id in self.basic_blocks")
	}
	...
	pub fn is_reachable_filtered<F:Fn(BasicBlockId) -> Control<bool>>(
		...
		set_depth_first_search(&self.graph, Some(from), |event| {
			match event {
	...
	/// Returns `None` the given node isn't the cyclic point of an infinite
	/// loop. Otherwise returns `Some(loop_start, loop_end)`.
	...
	where
		...
		// if there are more than one instruction in this block it can't be a
		// valid loop start.
		let Ok(only_instruction) = basic_block.instructions().iter().exactly_one() else {
			return None;
		};

		// if there is exactly one and it is a condition instruction we are in a
		// loop so we check the condition to infer if it is always true.
	...
	pub fn is_cyclic(&self, node:BasicBlockId) -> bool {
		set_depth_first_search(&self.graph, Some(node), |event| {
			match event {
