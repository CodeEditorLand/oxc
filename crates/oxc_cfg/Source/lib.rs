...
impl ControlFlowGraph {
	...
	pub fn basic_block(&self, id: BasicBlockId) -> &BasicBlock {
		let ix = *self
			.graph
			.node_weight(id)
			.expect("expected a valid node id in self.graph");
		self.basic_blocks
			.get(ix)
		...
		set_depth_first_search(&self.graph, Some(from), |event| match event {
			DfsEvent::TreeEdge(a, b) => {
				...
				let unreachable = !graph.edges_connecting(a, b).any(|edge| {
					!matches!(
						edge.weight(),
						EdgeType::NewFunction | EdgeType::Unreachable
					)
				});
		...
		F: Fn(&Instruction) -> EvalConstConditionResult,
	{
		...
		let Ok(only_instruction) =
			basic_block.instructions().iter().exactly_one()
		else {
			return None;
		};
