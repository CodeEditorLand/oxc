...
impl PartialEq<u32> for BasicBlockId {
	#[inline]
	fn eq(&self, other:&u32) -> bool { self.0.get() == *other }
}
...
impl ControlFlowGraph {
	...
	where
		...
		// if there are more than one instruction in this block it can't be a valid loop
		// start.
		let Ok(only_instruction) = basic_block.instructions().iter().exactly_one() else {
			return None;
		};

		// if there is exactly one and it is a condition instruction we are in a loop so
		// we check the condition to infer if it is always true.
