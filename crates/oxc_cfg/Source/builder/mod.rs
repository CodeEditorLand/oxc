...
pub struct ControlFlowGraphBuilder<'a> {
	...
	ctx_stack:Vec<Ctx<'a>>,
	/// Contains the error unwinding path represented as a stack of
	/// `ErrorHarness`es
	error_path:Vec<ErrorHarness>,
	/// Stack of finalizers, the top most element is always the appropriate one
	/// for current node.
...
impl<'a> ControlFlowGraphBuilder<'a> {
	...
	pub fn new_basic_block_function(&mut self) -> BlockNodeId {
		// we might want to differentiate between function blocks and normal blocks down
		// the road.
	...
	pub fn new_basic_block_normal(&mut self) -> BlockNodeId {
		...
		let ErrorHarness(error_edge_kind, error_graph_ix) = self
			.error_path
			.last()
	...
	pub fn push_finalization_stack(&mut self) { self.finalizers.push(None); }
	...
	pub fn enter_statement(&mut self, stmt:NodeId) { self.push_statement(stmt); }
}
