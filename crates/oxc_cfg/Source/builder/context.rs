...
impl<'a> Ctx<'a> {
	...
	fn is(&self, label:&str) -> bool { self.label.as_ref().is_some_and(|it| *it == label) }

	fn r#break(&mut self, entry:BasicBlockId) { self.entries.push((CtxFlags::BREAK, entry)); }

	fn r#continue(&mut self, entry:BasicBlockId) { self.entries.push((CtxFlags::CONTINUE, entry)); }
}
...
pub struct QueryCtx<'a, 'c>(
	&'c mut ControlFlowGraphBuilder<'a>,
	// label
	Option<&'a str>,
);
...
impl<'a, 'c> QueryCtx<'a, 'c> {
	/// Creates a new `Ctx` with the given `CtxFlags` and returns a
	/// `RefCtxCursor` to it.
	#[inline]
	...
	/// Creates a new `Ctx` with empty `CtxFlags` and returns a `RefCtxCursor`
	/// to it.
	pub fn default(self) -> RefCtxCursor<'a, 'c> { self.new(CtxFlags::empty()) }

	/// Creates a new `Ctx` with `CtxFlags::FUNCTION` set as its flags and
	/// returns a `RefCtxCursor` to it.
	pub fn new_function(self) -> RefCtxCursor<'a, 'c> { self.new(CtxFlags::FUNCTION) }

	/// Resolves the current context and adds the required edges to the graph.
	pub fn resolve(mut self) {
		let Some(ctx) = self.0.ctx_stack.pop() else {
			return;
		};
	...
	pub fn resolve_with_upper_label(mut self) {
		...
		// mark the upper label continue jump point the same as ours if it isn't
		// already assigned, NOTE: if it is already assigned there's a
		// resolution before this context.
	...
	fn resolve_ctx(&mut self, ctx:Ctx<'a>) {
		// TODO: This match is here to prevent redundant iterations and/or
		// conditions by handling them before starting the iteration, I don't
		// like the current implementation so it would be nice if we find a
...
impl<'a> ControlFlowGraphBuilder<'a> {
	...
	/// Returns `None` if there is no immediate labeled context before this
	/// call.
