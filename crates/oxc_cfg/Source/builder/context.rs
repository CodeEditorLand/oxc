...
impl<'a> Ctx<'a> {
	...
	fn r#break(&mut self, entry:BlockNodeId) { self.entries.push((CtxFlags::BREAK, entry)); }

	fn r#continue(&mut self, entry:BlockNodeId) { self.entries.push((CtxFlags::CONTINUE, entry)); }
}
...
impl<'a, 'c> QueryCtx<'a, 'c> {
	...
	pub fn resolve_with_upper_label(mut self) {
		...
		// mark the upper label continue jump point the same as ours if it isn't already
		// assigned, NOTE: if it is already assigned there's a resolution before this
		// context.
	...
	fn resolve_ctx(&mut self, ctx:Ctx<'a>) {
		// TODO: This match is here to prevent redundant iterations and/or conditions by
		// handling them before starting the iteration, I don't like the current
		// implementation so it would be nice if we find a better way of doing it.
