...
impl<'a> AstBuilder<'a> {
	...
	#[inline]
	pub fn vec_from_iter<T, I: IntoIterator<Item = T>>(
		self,
		iter: I,
	) -> Vec<'a, T> {
	...
	pub fn move_declaration(
		...
		let empty_decl =
			Declaration::VariableDeclaration(self.alloc(empty_decl));
	...
	pub fn plain_function(
		...
		self.alloc(self.function(
			r#type, span, id, false, false, false, NONE, NONE, params, NONE,
			body,
		))
	}
