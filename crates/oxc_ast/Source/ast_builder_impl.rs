...
/// `IntoIn<'a, Anything<'a>>`. Pass `NONE` instead of `None::<Anything<'a>>`.
#[allow(clippy::upper_case_acronyms)]
pub struct NONE;

impl<'a, T> FromIn<'a, NONE> for Option<Box<'a, T>> {
	fn from_in(_:NONE, _:&'a Allocator) -> Self { None }
}

impl<'a> AstBuilder<'a> {
	/// Create a new AST builder that will allocate nodes in the given
	/// allocator.
	#[inline]
	pub fn new(allocator:&'a Allocator) -> Self { Self { allocator } }

	/// Move a value into the memory arena.
	#[inline]
	pub fn alloc<T>(self, value:T) -> Box<'a, T> { Box::new_in(value, self.allocator) }

	/// Create a new empty [`Vec`] that stores its elements in the memory arena.
	#[inline]
	pub fn vec<T>(self) -> Vec<'a, T> { Vec::new_in(self.allocator) }
	...
	#[inline]
	pub fn vec1<T>(self, value:T) -> Vec<'a, T> { self.vec_from_array([value]) }
	...
	///
	/// This is preferable to `vec_from_iter` where source is an array, as size
	...
	/// Move a string slice into the memory arena, returning a reference to the
	/// slice in the heap.
	#[inline]
	...
	/// Move a formal parameters out by replacing it with an empty
	/// [`FormalParameters`].
	#[inline]
	...
	#[inline]
	pub fn move_vec<T>(self, vec:&mut Vec<'a, T>) -> Vec<'a, T> { mem::replace(vec, self.vec()) }
