...
impl JSXAttribute<'_> {
	...
	/// <Foo bar="value" /> // -> `false`
	/// ```
	pub fn is_key(&self) -> bool { self.is_identifier("key") }
}

impl<'a> JSXAttributeName<'a> {
	/// Try to convert this attribute name into an [identifier](JSXIdentifier).
	/// Returns [`None`] for [namespaced
	/// names](JSXAttributeName::NamespacedName).
...
impl<'a> JSXAttributeItem<'a> {
	...
	/// Get the contained [`JSXSpreadAttribute`] if it is a spread attribute
...
impl JSXChild<'_> {
	/// Returns `true` if this an [expression
	/// container](JSXChild::ExpressionContainer).
