// FIXME: lots of methods are missing docs. If you have time, it would be a huge
// help to add some :)
#![warn(missing_docs)]
...
impl Program<'_> {
	...
	pub fn is_empty(&self) -> bool { self.body.is_empty() && self.directives.is_empty() }
...
impl<'a> Expression<'a> {
	...
	/// `true` if this [`Expression`] is a literal expression for a primitive
	/// value.
	///
	/// Does not include [`TemplateLiteral`]s, [`object literals`], or [`array
	/// literals`].
	///
	...
	/// Returns `true` for [string](StringLiteral) and
	/// [template](TemplateLiteral) literals.
	pub fn is_string_literal(&self) -> bool {
		matches!(self, Self::StringLiteral(_) | Self::TemplateLiteral(_))
	}

	/// Returns `true` for [numeric](NumericLiteral) and [big
	/// int](BigIntLiteral) literals.
	pub fn is_number_literal(&self) -> bool {
		matches!(self, Self::NumericLiteral(_) | Self::BigIntLiteral(_))
	}

	/// Returns `true` for [bigint literals](BigIntLiteral).
	pub fn is_big_int_literal(&self) -> bool { matches!(self, Self::BigIntLiteral(_)) }
	...
	pub fn is_null(&self) -> bool { matches!(self, Expression::NullLiteral(_)) }
	...
	pub fn is_number(&self) -> bool { matches!(self, Self::NumericLiteral(_)) }
	...
	/// Determines whether the given expr is a specific [number](NumericLiteral)
	/// literal.
	...
	pub fn evaluate_to_undefined(&self) -> bool { self.is_undefined() || self.is_void() }
	...
	pub fn is_null_or_undefined(&self) -> bool { self.is_null() || self.evaluate_to_undefined() }

	/// Determines whether the given expr is a `NaN` literal
	pub fn is_nan(&self) -> bool { matches!(self, Self::Identifier(ident) if ident.name == "NaN") }
	...
	/// Returns `true` if this [`Expression`] is an [`IdentifierReference`] with
	/// specified `name`.
	...
	pub fn is_identifier_reference(&self) -> bool { matches!(self, Expression::Identifier(_)) }
	...
	pub fn is_call_expression(&self) -> bool { matches!(self, Expression::CallExpression(_)) }

	/// Returns `true` if this [`Expression`] is a [`Super`].
	pub fn is_super(&self) -> bool { matches!(self, Expression::Super(_)) }

	/// Returns `true` if this [`Expression`] is a [`CallExpression`] with
	/// [`Super`] as callee.
	...
	/// Returns `true` if this [`Expression`] is a [`CallExpression`],
	/// [`NewExpression`], or [`ImportExpression`].
	...
	/// Returns `true` if this [`Expression`] is a [`BinaryExpression`] or
	/// [`LogicalExpression`].
	...
	///
	/// See [`CallExpression::is_require_call`] for details of the exact
	/// patterns that match.
	...
	pub fn is_assignment(&self) -> bool { matches!(self, Expression::AssignmentExpression(_)) }
}

impl fmt::Display for IdentifierName<'_> {
	#[inline]
	fn fmt(&self, f:&mut fmt::Formatter) -> fmt::Result { self.name.fmt(f) }
}
...
impl ArrayExpressionElement<'_> {
	#[allow(missing_docs)]
	pub fn is_elision(&self) -> bool { matches!(self, Self::Elision(_)) }
}
...
impl ObjectPropertyKind<'_> {
	...
	#[inline]
	pub fn is_spread(&self) -> bool { matches!(self, Self::SpreadProperty(_)) }
}
...
impl<'a> PropertyKey<'a> {
	...
	#[allow(missing_docs)]
	pub fn is_private_identifier(&self) -> bool { matches!(self, Self::PrivateIdentifier(_)) }
...
impl PropertyKind {
	...
	pub fn is_accessor(self) -> bool { matches!(self, Self::Get | Self::Set) }
}
...
impl CallExpression<'_> {
	...
	pub fn is_symbol_or_symbol_for_call(&self) -> bool {
		...
		match &self.callee {
			...
			expr => {
				match expr.as_member_expression() {
					Some(member) => {
...
impl Argument<'_> {
	#[allow(missing_docs)]
	pub fn is_spread(&self) -> bool { matches!(self, Self::SpreadElement(_)) }
}

impl<'a> AssignmentTarget<'a> {
	#[allow(missing_docs)]
	pub fn get_identifier(&self) -> Option<&'a str> {
		self.as_simple_assignment_target()
			.and_then(SimpleAssignmentTarget::get_identifier)
	}

	#[allow(missing_docs)]
	pub fn get_expression(&self) -> Option<&Expression<'a>> {
		self.as_simple_assignment_target()
			.and_then(SimpleAssignmentTarget::get_expression)
	}

	#[allow(missing_docs)]
	pub fn get_expression_mut(&mut self) -> Option<&mut Expression<'a>> {
		self.as_simple_assignment_target_mut()
			.and_then(SimpleAssignmentTarget::get_expression_mut)
	}
}
...
impl<'a> ObjectAssignmentTarget<'a> {
	...
	#[allow(missing_docs)]
	pub fn is_empty(&self) -> bool { self.properties.is_empty() && self.rest.is_none() }

	#[allow(missing_docs)]
	pub fn len(&self) -> usize { self.properties.len() + usize::from(self.rest.is_some()) }
}
		...
		match self {
			...
			match_module_declaration!(Self) => {
				self.as_module_declaration()
					.is_some_and(ModuleDeclaration::is_typescript_syntax)
			},
...
impl Directive<'_> {
	...
	/// whose StringLiteral is either of the exact code point sequences "use
	/// strict" or 'use strict'. A Use Strict Directive may not contain an
	/// EscapeSequence or LineContinuation. <https://tc39.es/ecma262/#sec-directive-prologues-and-the-use-strict-directive>
	pub fn is_use_strict(&self) -> bool { self.directive == "use strict" }
}
...
impl VariableDeclaration<'_> {
	#[allow(missing_docs)]
	pub fn is_typescript_syntax(&self) -> bool { self.declare }

	/// Returns `true` if any of this declaration's variables have an
	/// initializer.
	pub fn has_init(&self) -> bool { self.declarations.iter().any(|decl| decl.init.is_some()) }
}

impl VariableDeclarationKind {
	/// `var x`
	pub fn is_var(&self) -> bool { matches!(self, Self::Var) }

	/// `const x`
	pub fn is_const(&self) -> bool { matches!(self, Self::Const) }

	/// `let x` or `const x`
	pub fn is_lexical(&self) -> bool { matches!(self, Self::Const | Self::Let) }

	/// `await using x`
	pub fn is_await(&self) -> bool { matches!(self, Self::AwaitUsing) }
...
impl SwitchCase<'_> {
	...
	pub fn is_default_case(&self) -> bool { self.test.is_none() }
}

impl<'a> BindingPattern<'a> {
	#[allow(missing_docs)]
	pub fn get_identifier(&self) -> Option<Atom<'a>> { self.kind.get_identifier() }
...
impl<'a> BindingPatternKind<'a> {
	...
	#[allow(missing_docs)]
	pub fn is_binding_identifier(&self) -> bool { matches!(self, Self::BindingIdentifier(_)) }

	#[allow(missing_docs)]
	pub fn is_object_pattern(&self) -> bool { matches!(self, Self::ObjectPattern(_)) }

	#[allow(missing_docs)]
	pub fn is_array_pattern(&self) -> bool { matches!(self, Self::ArrayPattern(_)) }

	#[allow(missing_docs)]
	pub fn is_assignment_pattern(&self) -> bool { matches!(self, Self::AssignmentPattern(_)) }
}
...
impl ObjectPattern<'_> {
	...
	/// The number of properties, including rest properties, in this object
	/// pattern.
	pub fn len(&self) -> usize { self.properties.len() + usize::from(self.rest.is_some()) }
}
...
impl ArrayPattern<'_> {
	...
	pub fn is_empty(&self) -> bool { self.elements.is_empty() && self.rest.is_none() }

	/// The number of elements, including rest elements, in this array pattern.
	pub fn len(&self) -> usize { self.elements.len() + usize::from(self.rest.is_some()) }
}

impl<'a> Function<'a> {
	/// Returns this [`Function`]'s name, if it has one.
	#[inline]
	pub fn name(&self) -> Option<Atom<'a>> { self.id.as_ref().map(|id| id.name.clone()) }
	...
	pub fn is_expression(&self) -> bool { self.r#type == FunctionType::FunctionExpression }
...
impl FormalParameter<'_> {
	...
	pub fn is_public(&self) -> bool { matches!(self.accessibility, Some(TSAccessibility::Public)) }
...
impl FormalParameterKind {
	...
	pub fn is_signature(&self) -> bool { matches!(self, Self::Signature) }
}
...
impl FormalParameters<'_> {
	...
	/// `true` if at least one parameter is bound, including [rest
	/// bindings](BindingRestElement).
	pub fn has_parameter(&self) -> bool { !self.is_empty() || self.rest.is_some() }
}

impl FunctionBody<'_> {
	/// `true` if this function body contains no statements or directives.
	pub fn is_empty(&self) -> bool { self.directives.is_empty() && self.statements.is_empty() }
...
impl<'a> ArrowFunctionExpression<'a> {
	/// Get expression part of `ArrowFunctionExpression`: `() =>
	/// expression_part`.
	pub fn get_expression(&self) -> Option<&Expression<'a>> {
		if self.expression {
	...
	/// Returns `true` if this arrow function's body has a `"use strict"`
	/// directive.
	pub fn has_use_strict_directive(&self) -> bool { self.body.has_use_strict_directive() }
}
...
impl Class<'_> {
	...
	/// var Foo = class { /* ... */ }
	/// ```
	pub fn is_expression(&self) -> bool { self.r#type == ClassType::ClassExpression }
	...
	/// }
	/// ```
	pub fn is_declaration(&self) -> bool { self.r#type == ClassType::ClassDeclaration }

	#[allow(missing_docs)]
	pub fn is_typescript_syntax(&self) -> bool { self.declare || self.r#abstract }
}

impl<'a> ClassElement<'a> {
	/// Returns `true` if this is a [`ClassElement::StaticBlock`].
	pub fn is_static_block(&self) -> bool { matches!(self, Self::StaticBlock(_)) }

	/// Returns `true` if this [`ClassElement`] has a static modifier.
	///
	/// Note: Class static blocks do not have a "modifier", as there is no
	/// non-static equivalent. Therefore, returns `false` for static blocks.
	///
...
impl PropertyDefinitionType {
	...
	pub fn is_abstract(&self) -> bool { matches!(self, Self::TSAbstractPropertyDefinition) }
}

impl MethodDefinitionKind {
	/// `true` for constructors.
	pub fn is_constructor(&self) -> bool { matches!(self, Self::Constructor) }

	/// `true` for regular methods.
	pub fn is_method(&self) -> bool { matches!(self, Self::Method) }

	/// `true` for setter methods.
	pub fn is_set(&self) -> bool { matches!(self, Self::Set) }

	/// `true` for getter methods.
	pub fn is_get(&self) -> bool { matches!(self, Self::Get) }
...
impl MethodDefinitionType {
	#[allow(missing_docs)]
	pub fn is_abstract(&self) -> bool { matches!(self, Self::TSAbstractMethodDefinition) }
}
...
impl<'a> ModuleDeclaration<'a> {
	...
	#[allow(missing_docs)]
	pub fn is_import(&self) -> bool { matches!(self, Self::ImportDeclaration(_)) }
	...
	#[allow(missing_docs)]
	pub fn is_default_export(&self) -> bool { matches!(self, Self::ExportDefaultDeclaration(_)) }
...
impl AccessorPropertyType {
	#[allow(missing_docs)]
	pub fn is_abstract(&self) -> bool { matches!(self, Self::TSAbstractAccessorProperty) }
}
...
impl<'a> ImportDeclarationSpecifier<'a> {
	...
	#[allow(missing_docs)]
	pub fn name(&self) -> Cow<'a, str> { Cow::Borrowed(self.local().name.as_str()) }
}
...
impl ExportDefaultDeclaration<'_> {
	#[allow(missing_docs)]
	pub fn is_typescript_syntax(&self) -> bool { self.declaration.is_typescript_syntax() }
}

impl ExportAllDeclaration<'_> {
	#[allow(missing_docs)]
	pub fn is_typescript_syntax(&self) -> bool { self.export_kind.is_type() }
}
