...
impl<'a> Expression<'a> {
	...
	/// returns `true` when node is truthy, `false` when node is falsy, `None`
	/// when it cannot be determined. <https://tc39.es/ecma262/#sec-toboolean>
	/// 1. If argument is a Boolean, return argument.
	/// 2. If argument is one of undefined, null, +0𝔽, -0𝔽, NaN, 0ℤ, or the
...
impl<'a> IdentifierName<'a> {
	pub fn new(span:Span, name:Atom<'a>) -> Self { Self { span, name } }
}
...
impl<'a> IdentifierReference<'a> {
	...
	#[inline]
	pub fn reference_id(&self) -> Option<ReferenceId> { self.reference_id.get() }
}
...
impl<'a> BindingIdentifier<'a> {
	pub fn new(span:Span, name:Atom<'a>) -> Self { Self { span, name, symbol_id:Cell::default() } }
...
impl<'a> ObjectExpression<'a> {
	...
	pub fn has_proto(&self) -> bool {
		...
		self.properties
			.iter()
			.any(|p| p.prop_name().is_some_and(|name| name.0 == "__proto__"))
	}
}
...
impl<'a> Function<'a> {
	...
	///
	/// Returns [`None`] for anonymous functions, or if semantic analysis was
	/// skipped.
	#[inline]
