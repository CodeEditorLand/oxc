...
impl<'a> TSTypeName<'a> {
	...
	pub fn is_identifier(&self) -> bool { matches!(self, Self::IdentifierReference(_)) }

	/// Returns `true` if this is a [qualified name](TSTypeName::QualifiedName)
	/// (i.e. a dot-separated name).
	pub fn is_qualified_name(&self) -> bool { matches!(self, Self::QualifiedName(_)) }
}
...
impl TSAccessibility {
	...
	/// Converts this modifier into a string as it would appear in the source
	/// code.
...
impl From<TSAccessibility> for &'static str {
	fn from(accessibility:TSAccessibility) -> Self { accessibility.as_str() }
}
...
impl TSModuleDeclaration<'_> {
	/// Returns `true` if this module's body exists and has a `"use strict"`
	/// directive.
	pub fn has_use_strict_directive(&self) -> bool {
		self.body
			.as_ref()
			.is_some_and(TSModuleDeclarationBody::has_use_strict_directive)
	}
}

impl TSModuleDeclarationKind {
	/// Returns `true` for `declare global { ... }`
	pub fn is_global(self) -> bool { matches!(self, TSModuleDeclarationKind::Global) }
...
impl<'a> TSModuleDeclarationName<'a> {
	...
	/// namespace bang {}
	/// ```
	pub fn is_string_literal(&self) -> bool { matches!(self, Self::StringLiteral(_)) }
...
impl TSModuleReference<'_> {
	...
	pub fn is_external(&self) -> bool { matches!(self, Self::ExternalModuleReference(_)) }
}
...
impl ImportOrExportKind {
	...
	pub fn is_value(&self) -> bool { matches!(self, Self::Value) }
