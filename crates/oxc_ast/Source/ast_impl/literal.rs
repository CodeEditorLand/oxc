...
impl BooleanLiteral {
	...
	pub fn as_str(&self) -> &'static str { if self.value { "true" } else { "false" } }
}
...
impl ContentHash for NullLiteral {
	#[inline]
	fn content_hash<H:Hasher>(&self, state:&mut H) { Hash::hash(&Option::<bool>::None, state); }
}

impl fmt::Display for NullLiteral {
	#[inline]
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result { "null".fmt(f) }
}
...
impl NumericLiteral<'_> {
	...
	pub fn ecmascript_to_int32(num:f64) -> i32 {
		...
		// NaN, Infinity if not included in our NumericLiteral, so we just serde(skip)
		// step 2.
	...
	/// If `raw` is `None` (node is generated, not parsed from source), fallback
	/// to formatting `value`.
...
impl fmt::Display for NumericLiteral<'_> {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result {
		...
		// For now, we take the 2nd approach, since `NumericLiteral::to_string` is only
		// used in linter, where raw does matter.
		self.raw_str().fmt(f)
	}
}
...
impl BigIntLiteral<'_> {
	...
	pub fn is_zero(&self) -> bool { self.raw == "0n" }
}
...
impl<'a> RegExpPattern<'a> {
	...
	/// Returns the string as this regular expression would appear in source
	/// code.
