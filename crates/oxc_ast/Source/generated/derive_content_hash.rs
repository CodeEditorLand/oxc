...
impl ContentHash for IdentifierName<'_> {
	fn content_hash<H:Hasher>(&self, state:&mut H) { ContentHash::content_hash(&self.name, state); }
}
...
impl ContentHash for BlockStatement<'_> {
	fn content_hash<H:Hasher>(&self, state:&mut H) { ContentHash::content_hash(&self.body, state); }
}
...
impl ContentHash for TSNamespaceExportDeclaration<'_> {
	fn content_hash<H:Hasher>(&self, state:&mut H) { ContentHash::content_hash(&self.id, state); }
}
