...
impl<'a> Prettier<'a> {
	pub(crate) fn wrap_parens(&mut self, doc:Doc<'a>, kind:AstKind<'a>) -> Doc<'a> {
		if self.need_parens(kind) { array![self, ss!("("), doc, ss!(")")] } else { doc }
	}
	...
	fn check_kind(&self, kind:AstKind<'a>) -> bool {
		...
		match kind {
			...
			AstKind::Function(e) if e.is_expression() => {
				match parent_kind {
					AstKind::CallExpression(call_expr) => call_expr.callee.span() == e.span,
					AstKind::NewExpression(new_expr) => new_expr.callee.span() == e.span,
					AstKind::TaggedTemplateExpression(_) => true,
		...
		match self.parent_kind() {
			AstKind::Class(class) => {
				if let Some(h) = &class.super_class {
					match kind {
						AstKind::ArrowFunctionExpression(e) if e.span == h.span() => {
							return true;
						},
						AstKind::AssignmentExpression(e) if e.span == h.span() => {
							return true;
						},
						...
						AstKind::ConditionalExpression(e) if e.span == h.span() => {
							return true;
						},
						...
						AstKind::SequenceExpression(e) if e.span == h.span() => {
							return true;
						},
						AstKind::TaggedTemplateExpression(e) if e.span == h.span() => {
							return true;
						},
						...
						AstKind::TSNonNullExpression(e) if e.span == h.span() => {
							return true;
						},
						AstKind::Class(e)
							if e.is_expression()
	...
	fn is_binary_cast_expression(&self, _span:Span) -> bool { false }
