...
use oxc_ast::{
	ast::{
		match_member_expression, AssignmentTarget, ChainElement,
		ExportDefaultDeclarationKind, Expression, ForStatementInit,
		ForStatementLeft, MemberExpression, ModuleDeclaration,
...
use crate::{
	array, binaryish::BinaryishOperator, doc::Doc, ss, DocBuilder, Prettier,
};

impl<'a> Prettier<'a> {
	pub(crate) fn wrap_parens(
		&mut self,
		doc: Doc<'a>,
		kind: AstKind<'a>,
	) -> Doc<'a> {
	...
	fn check_kind(&self, kind: AstKind<'a>) -> bool {
		...
		match kind {
			...
			AstKind::SequenceExpression(e) => {
				self.check_sequence_expression(e.span)
			},
			AstKind::ObjectExpression(e) => {
				self.check_object_function_class(e.span)
			},
			AstKind::Function(f) if f.is_expression() => {
				...
				match parent_kind {
					AstKind::CallExpression(call_expr) => {
						call_expr.callee.span() == f.span
					},
					AstKind::NewExpression(new_expr) => {
						new_expr.callee.span() == f.span
					},
				...
				AstKind::SequenceExpression(sequence_expr) => {
					match parent_parent_kind {
						Some(AstKind::ForStatement(for_stat))
							if for_stat.init.as_ref().is_some_and(|e| {
								e.span() == sequence_expr.span
							}) || for_stat.update.as_ref().is_some_and(
								|e| e.span() == sequence_expr.span,
							) =>
						{
							false
						},
			...
			AstKind::ConditionalExpression(e) => match parent_kind {
				...
				AstKind::CallExpression(call_expr) => {
					call_expr.callee.span() == e.span
				},
				AstKind::NewExpression(new_expr) => {
					new_expr.callee.span() == e.span
				},
				AstKind::ConditionalExpression(cond_expr) => {
					cond_expr.test.span() == e.span
				},
				AstKind::MemberExpression(member_expr) => {
					member_expr.object().span() == e.span
				},
				_ => false,
			},
			...
			{
				AstKind::NewExpression(new_expr) => {
					new_expr.callee.span() == class.span
				},
				_ => false,
			},
			_ => false,
			...
			AstKind::Class(class) => {
				if let Some(h) = &class.super_class {
					match kind {
						...
						AstKind::AssignmentExpression(e)
							if e.span == h.span() =>
						{
							return true
						},
						AstKind::AwaitExpression(e) if e.span == h.span() => {
							return true
						},
						AstKind::BinaryExpression(e) if e.span == h.span() => {
							return true
						},
						...
						AstKind::LogicalExpression(e) if e.span == h.span() => {
							return true
						},
						AstKind::NewExpression(e) if e.span == h.span() => {
							return true
						},
						AstKind::ObjectExpression(e) if e.span == h.span() => {
							return true
						},
						AstKind::SequenceExpression(e)
							if e.span == h.span() =>
						...
						AstKind::UnaryExpression(e) if e.span == h.span() => {
							return true
						},
						AstKind::UpdateExpression(e) if e.span == h.span() => {
							return true
						},
						AstKind::YieldExpression(e) if e.span == h.span() => {
							return true
						},
						AstKind::TSNonNullExpression(e)
							if e.span == h.span() =>
	...
	fn check_for_of_stmt_head_starts_with_async_or_let(
		&self,
		kind: AstKind<'a>,
	) -> bool {
		...
		let AstKind::ForOfStatement(stmt) = self.parent_kind() else {
			return false;
		};
		if let ForStatementLeft::AssignmentTargetIdentifier(i) = &stmt.left {
			if (i.span == ident.span)
				&& (i.name == "let" || (i.name == "async" && !stmt.r#await))
			{
				return true;
			}
		}
	...
	/// `for ((let.a) of []);`
	fn check_for_of_stmt_head_starts_with_let(
		&self,
		kind: AstKind<'a>,
	) -> bool {
		let AstKind::IdentifierReference(ident) = kind else {
			...
			if let AstKind::ForOfStatement(stmt) = kind {
				if let Some(target) = stmt.left.as_assignment_target() {
					if let Some(e) = target.as_member_expression() {
						return Self::starts_with_no_lookahead_token(
							e.object(),
							ident.span,
						);
					}
	...
	fn check_let_object(&self, kind: AstKind<'a>) -> bool {
		...
		match statement {
			AstKind::ExpressionStatement(stmt) => {
				Self::starts_with_no_lookahead_token(
					&stmt.expression,
					ident.span,
				)
			},
			AstKind::ForStatement(stmt) => stmt
				...
				.map_or(false, |e| {
					Self::starts_with_no_lookahead_token(e, ident.span)
				}),
	...
	fn check_update_unary(&self, span: Span) -> bool {
		match self.parent_kind() {
			AstKind::MemberExpression(member_expr) => {
				member_expr.object().span() == span
			},
			AstKind::TaggedTemplateExpression(_) => true,
			AstKind::CallExpression(call_expr) => {
				call_expr.callee.span() == span
			},
			AstKind::NewExpression(new_expr) => new_expr.callee.span() == span,
			AstKind::BinaryExpression(bin_expr) => {
				bin_expr.left.span() == span
	...
	fn check_binarish(&self, span: Span) -> bool {
		...
		match parent_kind {
			...
			AstKind::MemberExpression(member_expr) => {
				return member_expr.object().span() == span
			},
			AstKind::AssignmentExpression(assign_expr) => {
				return assign_expr.left.span() == span
					&& self.is_binary_cast_expression(span);
			},
			AstKind::AssignmentPattern(assign_pat) => {
				return assign_pat.left.span() == span
					&& self.is_binary_cast_expression(span);
			},
			AstKind::LogicalExpression(parent_logical_expr) => {
				if let AstKind::LogicalExpression(logical_expr) = current_kind {
					return parent_logical_expr.operator
		...
		let operator = match self.current_kind() {
			AstKind::LogicalExpression(e) => {
				BinaryishOperator::from(e.operator)
			},
		...
		if parent_precedence == precedence
			&& !parent_operator.should_flatten(operator)
		{
			return true;
		}
	...
	fn check_member_call_tagged_template_ts_non_null(
		&self,
		span: Span,
	) -> bool {
		match self.parent_kind() {
	...
	fn should_wrap_function_for_export_default(&mut self) -> bool {
		...
		if !Self::has_naked_left_side(kind)
			|| (!b && self.need_parens(self.current_kind()))
		{
			return false;
		}
	...
	fn get_left_side_path_name(kind: AstKind<'a>) -> AstKind<'a> {
		match kind {
			...
			AstKind::ConditionalExpression(e) => {
				AstKind::from_expression(&e.test)
			},
			AstKind::TaggedTemplateExpression(e) => {
				AstKind::from_expression(&e.tag)
			},
			AstKind::AssignmentExpression(e) => {
				AstKind::AssignmentTarget(&e.left)
			},
			AstKind::MemberExpression(e) => {
				AstKind::from_expression(e.object())
			},
	...
	fn starts_with_no_lookahead_token(e: &Expression<'a>, span: Span) -> bool {
		match e {
			Expression::BinaryExpression(e) => {
				Self::starts_with_no_lookahead_token(&e.left, span)
			},
			...
			Expression::SequenceExpression(e) => {
				e.expressions.first().map_or(false, |e| {
					Self::starts_with_no_lookahead_token(e, span)
				})
			},
