...
//! Ref: <https://github.com/prettier/prettier/blob/3.4.1/commands.md>

// NOTE: In addition to those defined here, there are still some that are not
// yet implemented. In terms of macro implementation, there are 2 forms: the
// most frequently used form and the most flexible form.

/// Arrays are used to concatenate a list of `Doc`s to be printed sequentially
/// into a single doc. (In Prettier(.js) this is just an array literal.)
/// ```
/// array!(p, [a, b, c]);
/// array!(p, vec);
/// ```
...
/// However for the algorithm to work properly they shouldn't contain line break
/// characters. (In Prettier(.js) this is just a string literal.)
/// ```
/// text!("const");
/// ```
#[macro_export]
...
/// Groups are usually nested, and the printer will try to fit everything on one
/// line, but if it doesn't fit it will break the outermost group first and try
/// again. It will continue breaking groups until everything fits (or there are
/// no more groups to break).
///
/// A group is forced to break if it's created with the `should_break` option
/// set to true or if it includes `break_parent`. A hard and literal line breaks
/// automatically include this so they always break parent groups. Breaks are
/// propagated to all parent groups, so if a deeply nested expression has a hard
/// break, everything will break. This only matters for "hard" breaks, i.e.
/// newlines that are printed no matter what and can be statically analyzed. <https://github.com/prettier/prettier/blob/3.4.1/commands.md#group>
/// ```
...
/// when nested. This will try to print the first alternative, if it fit use it,
/// otherwise go to the next one and so on. The alternatives is an array of
/// documents going from the least expanded (most flattened) representation
/// first to the most expanded. <https://github.com/prettier/prettier/blob/3.4.1/commands.md#conditionalgroup>
/// ```
...
/// it's going to add a break whenever the next element doesn't fit in the line
/// anymore. The difference with `group` is that it's not going to break all the
...
/// In other words, elements with odd indices must be line breaks (e.g.,
/// `softline`). <https://github.com/prettier/prettier/blob/3.4.1/commands.md#fill>
/// ```
...
/// Print something if the current group or the current element of fill breaks
/// and something else if it doesn't. `group_id` can be used to check another
/// already printed group instead of the current group.
///
/// If a `hardline` or `break_parent` is present within the possible contents,
/// the parent groups will be broken regardless of said content being printed,
/// which might not be desirable. This behaviour is a design limitation.
/// Usually the desired result can be achieved in a different way.
/// <https://github.com/prettier/prettier/blob/3.4.1/commands.md#ifbreak>
/// ```
...
/// more info. <https://github.com/prettier/prettier/blob/3.4.1/commands.md#breakparent>
/// ```
/// break_parent!();
/// ```
#[macro_export]
...
/// <https://github.com/prettier/prettier/blob/3.4.1/commands.md#join>
/// ```
...
/// If an expression fits on one line, the line break will be replaced with a
/// space. Line breaks always indent the next line with the current level of
/// indentation. <https://github.com/prettier/prettier/blob/3.4.1/commands.md#line>
/// ```
/// line!();
/// ```
#[macro_export]
...
/// Specify a line break.
/// The difference from line is that if the expression fits on one line, it will
/// be replaced with nothing. <https://github.com/prettier/prettier/blob/3.4.1/commands.md#softline>
/// ```
/// softline!();
/// ```
#[macro_export]
...
/// expression fits on one line or not. <https://github.com/prettier/prettier/blob/3.4.1/commands.md#hardline>
/// ```
/// hardline!();
/// ```
#[macro_export]
...
/// because "indent if the current group is broken" is the normal behavior of
/// indent. That's why `group_id` is required.
/// <https://github.com/prettier/prettier/blob/3.4.1/commands.md#indentifbreak>
/// ```
