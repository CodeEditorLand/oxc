...
pub trait DocBuilder<'a> {
	...
	fn indent(&self, contents:Vec<'a, Doc<'a>>) -> Doc<'a> { Doc::Indent(contents) }

	fn array(&self, contents:Vec<'a, Doc<'a>>) -> Doc<'a> { Doc::Array(contents) }

	fn fill(&self, contents:Vec<'a, Doc<'a>>) -> Doc<'a> { Doc::Fill(Fill { contents }) }
	...
	fn break_parent(&self) -> Doc<'a> { Doc::BreakParent }
