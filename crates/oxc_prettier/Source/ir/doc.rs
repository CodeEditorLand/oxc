...
impl<'a> Fill<'a> {
	...
	pub fn dequeue(&mut self) -> Option<Doc<'a>> {
		if self.contents.len() > 0 { Some(self.contents.remove(0)) } else { None }
	}

	pub fn enqueue(&mut self, doc:Doc<'a>) { self.contents.insert(0, doc); }

	pub fn parts(&self) -> &[Doc<'a>] { &self.contents }

	pub fn take_parts(self) -> Vec<'a, Doc<'a>> { self.contents }
}
