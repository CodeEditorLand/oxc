...
use crate::{GroupId, array, line, ss};
...
pub enum Doc<'a> {
	...
	/// Groups are usually nested, and the printer will try to fit everything on
	/// one line, but if it doesn't fit it will break the outermost group first
	...
	/// If an expression fits on one line, the line break will be replaced with
	/// a space. Line breaks always indent the next line with the current level
	/// of indentation.
	Line(Line),
	/// This is used to implement trailing comments.
	/// It's not practical to constantly check where the line ends to avoid
	/// accidentally printing some code at the end of a comment. `lineSuffix`
	/// buffers docs passed to it and flushes them before any new line.
	LineSuffix(Vec<'a, Doc<'a>>),
	/// Print something if the current `group` or the current element of `fill`
	/// breaks and something else if it doesn't.
	IfBreak(IfBreak<'a>),
	/// This is an alternative type of group which behaves like text layout:
	/// it's going to add a break whenever the next element doesn't fit in the
	/// line anymore. The difference with `group` is that it's not going to
...
impl Line {
	/// Specify a line break.
	/// The difference from line is that if the expression fits on one line, it
	/// will be replaced with nothing.
	pub fn softline() -> Self { Self { soft:true, ..Self::default() } }

	/// Specify a line break that is **always** included in the output,
	/// no matter if the expression fits on one line or not.
	pub fn hardline() -> Self { Self { hard:true, ..Self::default() } }

	pub fn literal_line() -> Self { Self { literal:true, ..Self::default() } }

	pub fn hardline_without_break_parent() -> Self { Self { hard:true, ..Self::default() } }
...
impl<'a> IndentIfBreak<'a> {
	pub fn new(contents:Vec<'a, Doc<'a>>) -> Self { Self { contents, group_id:None } }
...
impl<'a> Fill<'a> {
	pub fn new(docs:Vec<'a, Doc<'a>>) -> Self { Self { parts:docs } }
	...
	pub fn enqueue(&mut self, doc:Doc<'a>) { self.parts.insert(0, doc); }

	pub fn parts(&self) -> &[Doc<'a>] { &self.parts }

	pub fn take_parts(self) -> Vec<'a, Doc<'a>> { self.parts }
}
...
pub trait DocBuilder<'a> {
	...
	#[inline]
	fn vec<T>(&self) -> Vec<'a, T> { Vec::new_in(self.allocator()) }
	...
	#[inline]
	fn boxed(&self, doc:Doc<'a>) -> Box<'a, Doc<'a>> { Box::new_in(doc, self.allocator()) }
