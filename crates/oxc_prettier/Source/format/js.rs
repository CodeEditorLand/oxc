...
impl<'a> Format<'a> for EmptyStatement {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { text!("") }
}
	...
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> {
		wrap!(p, self, ThrowStatement, {
			array!(
				...
				[
					text!("throw"),
					function::print_return_or_throw_argument(p, Some(&self.argument))
				]
			)
		})
	}
...
impl<'a> Format<'a> for IdentifierName<'a> {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { dynamic_text!(p, self.name.as_str()) }
}
...
impl<'a> Format<'a> for NullLiteral {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { text!("null") }
}
...
impl<'a> Format<'a> for NumericLiteral<'a> {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> {
		wrap!(p, self, NumericLiteral, {
			...
			// Perf: the regexes from prettier code above are ported to manual search for
			// performance reasons.
...
impl<'a> Format<'a> for ThisExpression {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { text!("this") }
}
...
impl<'a> Format<'a> for PropertyKey<'a> {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> {
		...
		wrap!(p, self, PropertyKey, {
			// Perf: Cache the result of `need_quote` to avoid checking it in each
			// PropertyKey
			...
			match self {
				...
				PropertyKey::StringLiteral(literal) => {
					...
					// because prettier uses the function `isEs5IdentifierName` based on unicode
					// version 3, but `is_identifier_name` uses the latest unicode version.
...
impl<'a> Format<'a> for Super {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { text!("super") }
}
...
impl<'a> Format<'a> for ArrayPattern<'a> {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> {
		wrap!(p, self, ArrayPattern, {
			array::print_array(p, &array::Array::ArrayPattern(self))
		})
	}
}
