...
where
	T: Format<'a>,
{
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { (**self).format(p) }
}
...
impl<'a> Format<'a> for EmptyStatement {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("") }
}
...
impl<'a> Format<'a> for TSAnyKeyword {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("any") }
}

impl<'a> Format<'a> for TSBigIntKeyword {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("bigint") }
}

impl<'a> Format<'a> for TSBooleanKeyword {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("boolean") }
}

impl<'a> Format<'a> for TSIntrinsicKeyword {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("intrinsic") }
}

impl<'a> Format<'a> for TSNeverKeyword {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("never") }
}

impl<'a> Format<'a> for TSNullKeyword {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("null") }
}

impl<'a> Format<'a> for TSNumberKeyword {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("number") }
}

impl<'a> Format<'a> for TSObjectKeyword {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("object") }
}

impl<'a> Format<'a> for TSStringKeyword {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("string") }
}

impl<'a> Format<'a> for TSSymbolKeyword {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("symbol") }
}

impl<'a> Format<'a> for TSThisType {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("this") }
}

impl<'a> Format<'a> for TSUndefinedKeyword {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("undefined") }
}

impl<'a> Format<'a> for TSUnknownKeyword {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("unknown") }
}

impl<'a> Format<'a> for TSVoidKeyword {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("void") }
}
...
impl<'a> Format<'a> for JSDocNullableType<'a> {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { line!() }
}
...
impl<'a> Format<'a> for TSEnumMemberName<'a> {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> {
		match self {
			...
			name => {
				array!(p, ss!("["), name.as_expression().unwrap().format(p), ss!("]"))
			},
		}
	}
}
...
impl<'a> Format<'a> for ImportDefaultSpecifier<'a> {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { self.local.format(p) }
}
...
impl<'a> Format<'a> for ExportDefaultDeclaration<'a> {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { self.declaration.format(p) }
}
...
impl<'a> Format<'a> for IdentifierName<'a> {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { p.str(self.name.as_str()) }
}
...
impl<'a> Format<'a> for NumericLiteral<'a> {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> {
		wrap!(p, self, NumericLiteral, {
			...
			// Perf: the regexes from prettier code above are ported to manual
...
impl<'a> Format<'a> for StringLiteral<'a> {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> {
		wrap!(p, self, StringLiteral, {
			...
			// TODO: implement `makeString` from
			// prettier/src/utils/print-string.js
...
impl<'a> Format<'a> for PropertyKey<'a> {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> {
		...
		wrap!(p, self, PropertyKey, {
			// Perf: Cache the result of `need_quote` to avoid checking it in
			// each PropertyKey
			let need_quote = p.options.quote_props.consistent()
				&& match p.parent_parent_kind() {
					Some(AstKind::ObjectExpression(a)) => {
						a.properties.iter().any(|x| {
							match x {
			...
			match self {
				...
				PropertyKey::StringLiteral(literal) => {
					...
					// because prettier uses the function `isEs5IdentifierName`
					// based on unicode version 3, but `is_identifier_name`
					// uses the latest unicode version.
					if is_identifier_name(literal.value.as_str())
						&& (p.options.quote_props.as_needed()
							|| (
								p.options.quote_props.consistent()
...
impl<'a> Format<'a> for ArrowFunctionExpression<'a> {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> {
		wrap!(p, self, ArrowFunctionExpression, {
...
impl<'a> Format<'a> for Super {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { Doc::Str("super") }
}
...
impl<'a> Format<'a> for JSXIdentifier<'a> {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { ss!(self.name.as_str()) }
}
...
impl<'a> Format<'a> for JSXEmptyExpression {
	fn format(&self, _:&mut Prettier<'a>) -> Doc<'a> { ss!("") }
}
...
impl<'a> Format<'a> for JSXOpeningFragment {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { ss!("<>") }
}
...
impl<'a> Format<'a> for JSXText<'a> {
	fn format(&self, p:&mut Prettier<'a>) -> Doc<'a> { ss!(self.value.as_str()) }
}
