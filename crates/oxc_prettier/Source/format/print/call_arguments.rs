...
use crate::{
	...
	conditional_group,
	format::print::{
		array::{Array, is_concisely_printed_array},
		call_expression::{CallExpressionLike, is_commons_js_or_amd_call},
		misc,
	},
	group,
	hardline,
...
fn should_expand_last_arg(args:&Vec<'_, Argument<'_>>) -> bool {
	...
	let Some(last_arg) = last_arg.as_expression() else {
		return false;
	};
...
fn is_simple_call_argument(node:&Expression, depth:usize) -> bool {
	...
	if let Expression::ArrayExpression(expr) = node {
		return expr
			.elements
			.iter()
			.all(|elem| elem.as_expression().is_some_and(is_child_simple));
	}
