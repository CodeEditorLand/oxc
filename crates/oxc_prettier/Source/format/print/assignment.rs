...
pub fn print_assignment<'a>(
	...
	// TODO: set the layout in options so that when we print the right-hand side, we
	// can refer to it.
...
fn should_break_after_operator<'a>(
	...
	while let Expression::UnaryExpression(_) | Expression::TSNonNullExpression(_) = current_expr {
		current_expr = match current_expr {
			...
			// SAFETY: the `while` loop above ensures that `current_expr` is either a
			// `UnaryExpression` or a `TSNonNullExpression`.
			_ => unreachable!(),
		};
	}
