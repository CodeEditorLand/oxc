...
pub(super) fn print_assignment<'a>(
	...
	// TODO: set the layout in options so that when we print the right-hand
	// side, we can refer to it.
...
fn choose_layout<'a>(
	...
	let Some(right_expr) = right_expr else {
	...
	// wrapping object properties with very short keys usually doesn't add much
	// value
...
fn is_assignment(expr:&Expression) -> bool { matches!(expr, Expression::AssignmentExpression(_)) }
...
fn is_complex_type_alias_params(expr:&AssignmentLikeNode) -> bool { false }

fn has_complex_type_annotation(expr:&AssignmentLikeNode) -> bool { false }
...
fn is_object_property_with_short_key<'a>(
	...
	let AssignmentLikeNode::ObjectProperty(object_prop) = expr else {
		return false;
	};
...
fn should_break_after_operator<'a>(
	...
	while let Expression::UnaryExpression(_) | Expression::TSNonNullExpression(_) = current_expr {
		current_expr = match current_expr {
			...
			// SAFETY: the `while` loop above ensures that `current_expr` is
			// either a `UnaryExpression` or a `TSNonNullExpression`.
			_ => unreachable!(),
		};
	}
...
fn is_lone_short_argument(arg:&Argument) -> bool { false }

fn is_complex_type_arguments(type_parameters:&TSTypeParameterInstantiation) -> bool { false }
