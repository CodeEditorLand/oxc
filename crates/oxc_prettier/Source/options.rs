...
pub struct PrettierOptions {
	...
	/// trailing commas.) Default: [TrailingComma::All]
	...
	/// Put the `>` of a multi-line HTML (HTML, JSX) element at the end of the
	/// last line instead of being alone on the next line (does not apply to
	/// self closing elements). Default: false
...
impl QuoteProps {
	pub fn as_needed(self) -> bool { matches!(self, Self::AsNeeded) }

	pub fn preserve(self) -> bool { matches!(self, Self::Preserve) }

	pub fn consistent(self) -> bool { matches!(self, Self::Consistent) }
}
...
impl TrailingComma {
	pub fn is_all(self) -> bool { self == Self::All }

	pub fn is_es5(self) -> bool { self == Self::ES5 }
