...
impl SymbolTable {
	...
	pub fn get_symbol_id_from_span(&self, span: Span) -> Option<SymbolId> {
		self.spans.iter_enumerated().find_map(|(symbol, &inner_span)| {
			if inner_span == span {
				Some(symbol)
			} else {
				None
	...
	pub fn get_scope_id_from_span(&self, span: Span) -> Option<ScopeId> {
		self.get_symbol_id_from_span(span)
			.map(|symbol_id| self.get_scope_id(symbol_id))
	}
	...
	#[inline]
	pub fn get_resolved_reference_ids(
		&self,
		symbol_id: SymbolId,
	) -> &Vec<ReferenceId> {
	...
	pub fn delete_resolved_reference(
		&mut self,
		symbol_id: SymbolId,
		reference_id: ReferenceId,
	) {
...
impl<'a> IsGlobalReference for IdentifierReference<'a> {
	fn is_global_reference(&self, symbols: &SymbolTable) -> bool {
		self.reference_id.get().is_some_and(|reference_id| {
			reference_id.is_global_reference(symbols)
		})
	}
