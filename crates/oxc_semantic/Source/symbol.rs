...
///
/// Most symbols won't have redeclarations, so instead of storing `Vec<Span>`
/// directly in `redeclare_variables` (32 bytes per symbol), store
/// `Option<RedeclarationId>` (4 bytes). That ID indexes into `redeclarations`
/// where the actual `Vec<Span>` is stored.
...
impl SymbolTable {
	...
	///
	/// Use [`ScopeTree::iter_bindings_in`] to only iterate over symbols
	/// declared in a specific scope.
	///
	...
	pub fn get_symbol_id_from_span(&self, span:Span) -> Option<SymbolId> {
		self.spans.iter_enumerated().find_map(
			|(symbol, &inner_span)| {
				if inner_span == span { Some(symbol) } else { None }
			},
		)
	}
	...
	/// Get the [`SymbolFlags`] for a symbol, which describe how the symbol is
	/// declared.
	///
	/// To find how a symbol is used, use
	/// [`SymbolTable::get_resolved_references`].
	#[inline]
	pub fn get_flags(&self, symbol_id:SymbolId) -> SymbolFlags { self.flags[symbol_id] }
	...
	#[inline]
	pub fn get_scope_id(&self, symbol_id:SymbolId) -> ScopeId { self.scope_ids[symbol_id] }
	...
	///
	/// This node will be a [`VariableDeclaration`], [`Function`], or some other
	/// AST node that _has_ a [`BindingIdentifier`] or a [`BindingPattern`]. It
	/// will not point to the binding pattern or identifier node itself.
	///
	...
	#[inline]
	pub fn get_declaration(&self, symbol_id:SymbolId) -> NodeId { self.declarations[symbol_id] }
	...
	///
	/// [`ReferenceId`]s can be found in [`IdentifierReference`] and similar
	/// nodes.
	#[inline]
	...
	/// Returns `true` if the corresponding [`Reference`] is resolved to a
	/// symbol.
	///
	/// When `false`, this could either be a reference to a global value or an
	...
	///
	/// # Panics
	/// Panics if provided `reference_id` is not a resolved reference for
	/// `symbol_id`.
