...
#[derive(Debug, Clone)]
pub struct UnaryExpressioneAssignmentValue(
	pub AstNodeId,
	pub UnaryOperator,
	pub Register,
);
...
impl ControlFlowGraph {
	...
	/// # Panics
	pub fn basic_block_by_index(
		&self,
		index: NodeIndex,
	) -> &Vec<BasicBlockElement> {
		let idx = *self
			.graph
			.node_weight(index)
			.expect("expected a valid node index in self.graph");
		self.basic_blocks
			.get(idx)
			.expect("expected a valid node index in self.basic_blocks")
	}

	/// # Panics
	pub fn basic_block_by_index_mut(
		&mut self,
		index: NodeIndex,
	) -> &mut Vec<BasicBlockElement> {
		let idx = *self
			.graph
			.node_weight(index)
			.expect("expected a valid node index in self.graph");
	...
	pub fn put_x_in_register(&mut self, asmt_value: AssignmentValue) {
		let register = self
			.use_this_register
			.take()
			.unwrap_or_else(|| self.new_register());

		let basic_block_element =
			BasicBlockElement::Assignment(register, asmt_value);
		...
		if let Some(arr) =
			self.store_final_assignments_into_this_array.last_mut()
		{
			arr.push(register);
		}
	}
	...
	pub fn preserve_expression_state(&mut self) -> PreservedExpressionState {
		...
		PreservedExpressionState {
			use_this_register,
			store_final_assignments_into_this_array,
		}
	}

	pub fn restore_expression_state(
		&mut self,
		mut preserved_state: PreservedExpressionState,
	) {
		...
		match basic_block {
			...
			BasicBlockElement::Assignment(to, with) => {
				...
				match with {
					...
					AssignmentValue::NotImplicitUndefined => {
