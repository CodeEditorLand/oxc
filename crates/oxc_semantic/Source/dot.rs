...
use oxc_cfg::{
	...
	BasicBlock, ControlFlowGraph, EdgeType, ErrorEdgeKind, Instruction,
		...
		format!(
			...
			Dot::with_attr_getters(
				...
				&|_graph, node| {
					...
					let basic_block_debug_str =
						self.basic_blocks[*node.1].debug_dot(ctx);
...
impl DebugDot for Instruction {
	fn debug_dot(&self, ctx: DebugDotContext) -> String {
		match self.kind {
			InstructionKind::Statement => self
				.node_id
				.map_or("None".to_string(), |id| ctx.debug_ast_kind(id)),
			...
			InstructionKind::Condition => {
				self.node_id.map_or("None".to_string(), |id| {
					format!(
						"Condition({})",
						ctx.try_eval_literal(id)
							.unwrap_or_else(|| ctx.debug_ast_kind(id))
					)
				})
			},
			InstructionKind::Iteration(ref kind) => {
				format!(
					"Iteration({} {} {})",
					self.node_id.map_or("None".to_string(), |id| ctx
						.debug_ast_kind(id)),
					if matches!(kind, IterationInstructionKind::Of) {
						"of"
					} else {
						"in"
					},
			...
			InstructionKind::Break(LabeledInstruction::Labeled) => {
				let Some(AstKind::BreakStatement(BreakStatement {
					label: Some(label),
					..
				})) = self
					.node_id
			...
			InstructionKind::Break(LabeledInstruction::Unlabeled) => {
				"break".to_string()
			},
