...
impl<'a, 'b> DebugDotContext<'a, 'b> {
	fn debug_ast_kind(self, id:NodeId) -> String { self.nodes.kind(id).debug_name().into_owned() }
...
impl<'a, 'b> From<&'b AstNodes<'a>> for DebugDotContext<'a, 'b> {
	fn from(value:&'b AstNodes<'a>) -> Self { Self::new(value, true) }
}
		...
		match self.kind {
			...
			InstructionKind::Iteration(ref kind) => {
				format!(
					...
					// TODO: at this point we can't evaluate this node. needs
