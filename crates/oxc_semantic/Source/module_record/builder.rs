...
impl ModuleRecordBuilder {
	...
	pub fn visit(&mut self, program:&Program) {
		...
		// The `ParseModule` algorithm requires `importedBoundNames` (import
		// entries) to be resolved before resolving export entries.
		self.resolve_export_entries();
	}

	pub fn build(self) -> ModuleRecord { self.module_record }
	...
	fn add_export_entry(&mut self, entry:ExportEntry) { self.export_entries.push(entry); }
		...
		for ee in export_entries {
			...
			if ee.module_request.is_none() {
				...
				match found_import_entry {
					// i. If ee.[[LocalName]] is not an element of
					...
					Some(ie) => {
						match &ie.import_name {
							...
							ImportImportName::NamespaceObject => {
								// a. NOTE: This is a re-export of an imported
								// module namespace object. b. Append ee
								// to localExportEntries.
								self.append_local_export_entry(ee);
							},
							// 3. Else,
							// a. NOTE: This is a re-export of a single name.
							// Append the ExportEntry Record {
							// [[ModuleRequest]]: ie.[[ModuleRequest]],
							// [[ImportName]]: ie.[[ImportName]], [[LocalName]]:
							// null, [[ExportName]]: ee.[[ExportName]] }
