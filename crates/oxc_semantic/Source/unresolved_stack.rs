...
/// The difference with Scope's `UnresolvedReferences` is that this type uses
/// Atom as the key. its clone is very cheap!
...
// Indexed by scope depth. We recycle `UnresolvedReferences` instances during
// traversal to reduce allocations, so the stack grows to maximum scope depth,
// but never shrinks. See: <https://github.com/oxc-project/oxc/issues/4169>
// This stack abstraction uses the invariant that stack only grows to avoid
// bounds checks.
...
impl<'a> UnresolvedReferencesStack<'a> {
	...
	// so initialize `stack` with this length, to reduce reallocations as it
	// grows. This is just an estimate of a good initial size, but certainly
	// better than `Vec`'s default initial capacity of 4.
	...
	pub(crate) fn increment_scope_depth(&mut self) {
		...
		// Grow stack if required to ensure
		// `self.stack[self.current_scope_depth]` is in bounds
	...
	pub(crate) fn decrement_scope_depth(&mut self) {
		...
		// If it did, would cause UB in `current_and_parent_mut`, which relies
		// on `current_scope_depth - 1` always being a valid index into
		// `self.stack`.
		assert!(self.current_scope_depth > 0);
	}

	#[inline]
	pub(crate) fn scope_depth(&self) -> usize { self.current_scope_depth }
	...
	pub(crate) fn current_mut(&mut self) -> &mut TempUnresolvedReferences<'a> {
		...
		// and it grows `stack` to ensure `stack.len()` always exceeds
		// `current_scope_depth`. So this read is always guaranteed to be in
		// bounds.
