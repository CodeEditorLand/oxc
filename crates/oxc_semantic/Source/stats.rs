...
impl Stats {
	...
	/// Gather [`Stats`] by visiting AST and counting nodes, scopes, symbols,
	/// and references.
	///
	/// Nodes, scopes and references counts will be exactly accurate.
	/// Symbols count may be an over-estimate if there are multiple declarations
	/// for a single symbol. e.g. `var x; var x;` will produce a count of 2
	/// symbols, but this is actually only 1 symbol.
	///
	/// If semantic analysis has already been run on AST, prefer getting counts
	/// with [`Semantic::stats`]. They will be 100% accurate, and very cheap to
	/// obtain, whereas this method performs a complete AST traversal.
	///
	/// [`Semantic::stats`]: super::Semantic::stats
	...
	pub fn assert_accurate(self, actual:Self) {
		...
		// `Counter` may overestimate number of symbols, because multiple
...
impl<'a> Visit<'a> for Counter {
	#[inline]
	fn enter_node(&mut self, _:AstKind<'a>) { self.stats.nodes += 1; }

	#[inline]
	fn enter_scope(&mut self, _:ScopeFlags, _:&Cell<Option<ScopeId>>) { self.stats.scopes += 1; }
