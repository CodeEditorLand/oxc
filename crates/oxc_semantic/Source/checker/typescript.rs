...
/// '?' at the end of a type is not valid TypeScript syntax. Did you mean to
/// write 'number | null | undefined'?(17019)
...
pub fn check_variable_declarator(decl:&VariableDeclarator, ctx:&SemanticBuilder<'_>) {
	...
	if decl.id.optional {
		// NOTE: BindingPattern spans cover the identifier _and_ the type
		// annotation.
		let ty = decl
			.id
			.type_annotation
	...
	if decl.definite {
		...
		//                 ^
		let Some(span) = find_char(decl.span, ctx.source_text, '!') else {
			return;
		};
...
fn abstract_element_cannot_have_initializer(
	...
	ts_error(
		code,
		format!(
			"{elem_name} '{prop_name}' cannot have an {init_or_impl} because it is marked \
			 abstract."
		),
	)
	.with_label(span)
...
/// TS(1245): Method 'foo' cannot have an implementation because it is marked
/// abstract.
...
/// TS(1267): Property 'foo' cannot have an initializer because it is marked
/// abstract.
...
/// TS(1318): Accessor 'foo' cannot have an implementation because it is marked
/// abstract.
///
...
/// A parameter property is only allowed in a constructor
/// implementation.ts(2369)
	...
	if is_abstract {
		...
		if method.kind.is_constructor() {
			...
			match method.kind {
				...
				// abstract classes can have concrete methods. Constructors
				// cannot have abstract modifiers, but this gets checked
				// during parsing
...
fn type_annotation_in_for_left(span:Span, is_for_in:bool) -> OxcDiagnostic {
	...
	ts_error(
		"2483",
		format!("The left-hand side of a '{for_of_or_in}' statement cannot use a type annotation.",),
	)
	.with_label(span)
	.with_help(
		"This iterator's type will be inferred from the iterable. You can safely remove the type \
		 annotation.",
	)
}
