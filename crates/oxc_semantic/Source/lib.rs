...
pub struct Semantic<'a> {
	...
	/// Symbol table containing all symbols in the program and their
	/// references.
	symbols:SymbolTable,
...
impl<'a> Semantic<'a> {
	...
	/// Source code of the JavaScript/TypeScript program being analyzed.
	pub fn source_text(&self) -> &'a str { self.source_text }

	/// What kind of source code is being analyzed. Comes from the parser.
	pub fn source_type(&self) -> &SourceType { &self.source_type }

	/// Nodes in the Abstract Syntax Tree (AST)
	pub fn nodes(&self) -> &AstNodes<'a> { &self.nodes }

	/// The [`ScopeTree`] containing scopes and what identifier names are bound
	/// in each one.
...
mod tests {
	...
	/// Create a [`Semantic`] from source code, assuming there are no
	...
	fn test_symbols() {
		...
		match decl.kind() {
			...
			kind => {
				panic!("Expected VariableDeclarator for 'let', got {kind:?}")
			},
		}
	...
	fn test_reference_resolutions_simple_read_write() {
		...
		let sources = [
			...
			// note: we consider a to be written, and the read of `1`
			// propagates upwards
			...
			// identifiers not in last read_write are also considered a read
			// (at least, or now)
			...
			// a = 1 writes, but the CallExpression reads the rhs (1) so a
			// isn't read
			(
				SourceType::default(),
		...
		for (source_type, source, flags) in sources {
			...
			assert!(
				num_refs == 1,
				"expected to find 1 reference to '{target_symbol_name}' but {num_refs} were \
				 found\n\nsource:\n{source}"
			);
			...
			if flags.is_write() {
				...
				assert!(
					!ref_type.is_write(),
					"expected reference to '{target_symbol_name}' not to have been written to, \
					 but it is\n\nsource:\n{source}"
				);
			}
			if flags.is_read() {
				...
				assert!(
					!ref_type.is_read(),
					"expected reference to '{target_symbol_name}' not to be read, but it \
					 is\n\nsource:\n{source}"
				);
			}
		}
