...
impl<'a> Semantic<'a> {
	...
	pub fn is_reference_to_global_variable(
		&self,
...
mod tests {
	...
	fn test_symbols() {
		...
		let top_level_a = semantic
			.scopes()
			.get_binding(semantic.scopes().root_scope_id(), "a")
			.unwrap();
	...
	fn test_reference_resolutions_simple_read_write() {
		...
		let sources = [
			...
			(
				SourceType::default(),
				"console.log;let a=0;a++",
				ReferenceFlags::write(),
			),
			// typescript
			(
				typescript,
				"let a: number = 1; (a as any) = true",
				ReferenceFlags::write(),
			),
			(
				typescript,
				"let a: number = 1; a = true as any",
				ReferenceFlags::write(),
			),
			(
				typescript,
				"let a: number = 1; a = 2 as const",
				ReferenceFlags::write(),
			),
			(
				typescript,
				"let a: number = 1; a = 2 satisfies number",
				ReferenceFlags::write(),
			),
			(
				typescript,
				"let a: number; (a as any) = 1;",
				ReferenceFlags::write(),
			),
		];
