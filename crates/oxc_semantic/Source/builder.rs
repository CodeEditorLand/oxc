...
impl<'a> SemanticBuilder<'a> {
	...
	pub fn build(mut self, program:&Program<'a>) -> SemanticBuilderReturn<'a> {
		...
		if self.source_type.is_typescript_definition() {
			...
			// Use counts of nodes, scopes, symbols, and references to pre-allocate
			// sufficient capacity in `AstNodes`, `ScopeTree` and `SymbolTable`.
			//
			// This means that as we traverse the AST and fill up these structures with
			// data, they never need to grow and reallocate - which is an expensive
			// operation as it involves copying all the memory from the old allocation to
			// the new one. For large source files, these structures are very large, so
			// growth is very costly as it involves copying massive chunks of memory.
	...
	fn record_ast_node(&mut self) {
		...
		// `ast_node_records` is empty if CFG is disabled. But benchmarks showed
		// removing the extra check is a perf regression. <https://github.com/oxc-project/oxc/pull/4273>
		if self.cfg.is_some() {
				...
				references.retain(|&reference_id| {
					...
					if symbol_flags.is_value() && flags.is_value() {
						...
						*flags -= ReferenceFlags::Type;
						...
						//                                      ^^^^^ We have confirmed the symbol
						// is                                            not a value symbol,
						// so we need to                                            make sure
						// the reference is a type only.
						*flags = ReferenceFlags::Type;
					}
...
impl<'a> Visit<'a> for SemanticBuilder<'a> {
	...
	fn leave_scope(&mut self) {
		...
		// `get_parent_id` always returns `Some` because this method is not called for
		// `Program`. So we could `.unwrap()` here. But that seems to produce a small
		// perf impact, probably because `leave_scope` then doesn't get inlined
		// because of its larger size due to the panic code.
	...
	fn visit_program(&mut self, program:&Program<'a>) {
		...
		// Don't call `enter_node` here as `Program` is a special case - node has no
		// `parent_id`. Inline the specific logic for `Program` here instead.
		...
		// and `self.jsdoc.retrieve_attached_jsdoc`, as they are all no-ops for
		// `Program`.
		...
		// Don't call `enter_scope` here as `Program` is a special case - scope has no
		// `parent_id`. Inline the specific logic for `Program` here instead.
		// This simplifies logic in `enter_scope`, as it doesn't have to handle the
		// special case.
		...
		// Don't call `leave_scope` here as `Program` is a special case - scope has no
		// `parent_id`. This simplifies `leave_scope`.
		self.resolve_references_for_current_scope();
	...
	fn visit_assignment_expression(&mut self, expr:&AssignmentExpression<'a>) {
		...
		if !expr.operator.is_assign() {
			// Only when the operator is not an `=` operator, the left-hand side is both
			// read and write. <https://tc39.es/ecma262/#sec-assignment-operators-runtime-semantics-evaluation>
	...
	fn visit_switch_statement(&mut self, stmt:&SwitchStatement<'a>) {
		...
		control_flow!(self, |cfg| {
			...
			// if we don't have a default case there should be an edge from discriminant to
	...
	fn visit_function(&mut self, func:&Function<'a>, flags:ScopeFlags) {
		...
		if func.params.has_parameter() || func.return_type.is_some() {
			...
			//                            ^^^^^^^^   ^^^^^^^^
			// Parameter initializers must be resolved after all parameters have been
			// declared. Param types and return type must be resolved after type
			// parameters have been declared. In both cases, need to avoid binding to
			// variables/types declared inside the function body.
			self.resolve_references_for_current_scope();
		}
		...
		control_flow!(self, |cfg| {
			...
			// If the last is an unreachable instruction, it means there is already a
			// explicit return or throw statement at the end of function body, we don't
			// need to insert an implicit return.
			if !matches!(
	...
	fn visit_member_expression(&mut self, it:&MemberExpression<'a>) {
		...
		// A.B = 1;
		// ^^^ Can't treat A as a Write reference since it's A's property(B) that
		// changes.
	...
	fn visit_export_named_declaration(&mut self, it:&ExportNamedDeclaration<'a>) {
		...
		for specifier in &it.specifiers {
			...
			if it.export_kind.is_type() || specifier.export_kind.is_type() {
				...
				// If the export specifier is not a explicit type export, we consider it as a
				// potential type and value reference. If it references to a value in the
				// end, we would delete the `ReferenceFlags::Type` flag in `fn
				// resolve_references_for_current_scope`.
				self.current_reference_flags = ReferenceFlags::Read | ReferenceFlags::Type;
			}
