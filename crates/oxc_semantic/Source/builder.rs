...
pub struct SemanticBuilder<'a> {
	...
	pub(crate) hoisting_variables:
		FxHashMap<ScopeId, FxHashMap<Atom<'a>, SymbolId>>,
...
impl<'a> SemanticBuilder<'a> {
	...
	pub fn with_cfg(mut self, cfg: bool) -> Self {
		self.cfg =
			if cfg { Some(ControlFlowGraphBuilder::default()) } else { None };
		self
	}
	...
	pub fn build(mut self, program: &Program<'a>) -> SemanticBuilderReturn<'a> {
		...
		if self.source_type.is_typescript_definition() {
			let scope_id =
				self.scope.add_scope(None, NodeId::DUMMY, ScopeFlags::Top);
			...
			self.symbols
				.reserve(stats.symbols as usize, stats.references as usize);
	...
	fn retrieve_recorded_ast_node(&mut self) -> Option<NodeId> {
		if self.cfg.is_some() {
			Some(
				self.ast_node_records
					.pop()
					.expect("there is no ast node record to stop."),
			)
		} else {
			None
		}
	...
	pub(crate) fn check_redeclaration(
		...
		let symbol_id =
			self.scope.get_binding(scope_id, name).or_else(|| {
				self.hoisting_variables
					.get(&scope_id)
					.and_then(|symbols| symbols.get(name).copied())
			})?;
		if report_error
			&& self.symbols.get_flags(symbol_id).intersects(excludes)
		{
	...
	fn resolve_references_for_current_scope(&mut self) {
		let (current_refs, parent_refs) =
			self.unresolved_references.current_and_parent_mut();
	...
	fn add_export_flag_to_identifier(&mut self, name: &str) {
		if let Some(symbol_id) =
			self.scope.get_binding(self.current_scope_id, name)
		{
...
impl<'a> Visit<'a> for SemanticBuilder<'a> {
	...
	fn enter_scope(
		...
		self.current_scope_id = self.scope.add_scope(
	...
	fn visit_program(&mut self, program: &Program<'a>) {
		...
		self.current_scope_id =
			self.scope.add_scope(None, self.current_node_id, flags);
	...
	fn visit_break_statement(&mut self, stmt: &BreakStatement<'a>) {
		...
		/* cfg */
		control_flow!(self, |cfg| cfg.append_break(
			node_id,
			stmt.label.as_ref().map(|it| it.name.as_str())
		));
		/* cfg */
	...
	fn visit_block_statement(&mut self, it: &BlockStatement<'a>) {
		...
		if self.scope.get_flags(parent_scope_id).is_catch_clause() {
			...
			if !parent_bindings.is_empty() {
				let parent_bindings =
					parent_bindings.drain(..).collect::<Bindings>();
	...
	fn visit_continue_statement(&mut self, stmt: &ContinueStatement<'a>) {
		...
		/* cfg */
		control_flow!(self, |cfg| cfg.append_continue(
			node_id,
			stmt.label.as_ref().map(|it| it.name.as_str())
		));
		/* cfg */
	...
	fn visit_logical_expression(&mut self, expr: &LogicalExpression<'a>) {
		...
		/* cfg  */
		let (left_expr_end_ix, right_expr_start_ix) =
			control_flow!(self, |cfg| {
	...
	fn visit_for_statement(&mut self, stmt: &ForStatement<'a>) {
		...
		/* cfg */
		let (after_test_graph_ix, update_graph_ix) = control_flow!(
			self,
	...
	fn visit_for_in_statement(&mut self, stmt: &ForInStatement<'a>) {
		...
		let (end_of_prepare_cond_graph_ix, iteration_graph_ix, body_graph_ix) =
			control_flow!(self, |cfg| {
				...
				cfg.append_iteration(
					right_node_id,
					IterationInstructionKind::In,
				);
		...
		control_flow!(self, |cfg| {
			...
			cfg.add_edge(
				before_for_stmt_graph_ix,
				start_prepare_cond_graph_ix,
				EdgeType::Normal,
			);
	...
	fn visit_if_statement(&mut self, stmt: &IfStatement<'a>) {
		...
		let else_graph_ix = if let Some(alternate) = &stmt.alternate {
			/* cfg */
			let else_graph_ix =
				control_flow!(self, |cfg| cfg.new_basic_block_normal());
			/* cfg */

			self.visit_statement(alternate);

			control_flow!(self, |cfg| Some((
				else_graph_ix,
				cfg.current_node_ix
			)))
		} else {
		...
		control_flow!(self, |cfg| {
			...
			cfg.add_edge(
				after_consequent_stmt_graph_ix,
				after_if_graph_ix,
				EdgeType::Normal,
			);
	...
	fn visit_labeled_statement(&mut self, stmt: &LabeledStatement<'a>) {
		...
		control_flow!(self, |cfg| {
			...
			cfg.add_edge(
				after_body_graph_ix,
				after_labeled_stmt_graph_ix,
				EdgeType::Normal,
			);

			cfg.ctx(Some(label.as_str()))
				.mark_break(after_labeled_stmt_graph_ix)
				.resolve();
		});
		/* cfg */
		...
		control_flow!(self, |cfg| {
			for i in 0..switch_case_graph_spans.len() {
				...
				if switch_case_graph_spans.len() > i + 1 {
					...
					let (next_switch_statement_condition, _) =
						switch_case_graph_spans[i + 1];
	...
	fn visit_try_statement(&mut self, stmt: &TryStatement<'a>) {
		...
		let (
			...
			let error_harness = stmt
				.handler
				.as_ref()
				.map(|_| cfg.attach_error_harness(ErrorEdgeKind::Explicit));
			let before_finalizer_graph_ix =
				stmt.finalizer.as_ref().map(|_| cfg.attach_finalizer());
		...
		let finally_block_end_ix = if let Some(finalizer) = &stmt.finalizer {
			/* cfg */
			control_flow!(self, |cfg| {
				let Some(before_finalizer_graph_ix) = before_finalizer_graph_ix
				else {
	...
	fn make_all_namespaces_valuelike(&mut self) {
		for symbol_id in &self.namespace_stack {
			...
			if self
				.symbols
				.get_flags(symbol_id)
				.intersects(SymbolFlags::Ambient)
			{
				continue;
			}
			...
			return match node.kind() {
				...
				AstKind::ExpressionStatement(_) => {
					if self.current_scope_flags().is_arrow() {
						if let Some(node) =
							self.nodes.iter_parents(node.id()).nth(2)
						{
							// (x) => x++
