...
macro_rules! control_flow {
	($self:ident, | $cfg:tt | $body:expr) => {
		if let Some(ref mut $cfg) = $self.cfg { $body } else { Default::default() }
	};
}
...
pub struct SemanticBuilder<'a> {
	...
	/// Stores current `AstKind::Function` and
	/// `AstKind::ArrowFunctionExpression` during AST visit
...
impl<'a> SemanticBuilder<'a> {
	...
	/// Provide statistics about AST to optimize memory usage of semantic
	/// analysis.
	///
	/// Accurate statistics can greatly improve performance, especially for
	/// large ASTs. If no stats are provided, [`SemanticBuilder::build`] will
	/// compile stats by performing a complete AST traversal.
	/// If semantic analysis has already been performed on this AST, get the
	/// existing stats with [`Semantic::stats`], and pass them in with this
	/// method, to avoid the stats collection AST pass.
	#[must_use]
	...
	/// Request `SemanticBuilder` to allocate excess capacity for scopes,
	/// symbols, and references.
	///
	/// `excess_capacity` is provided as a fraction.
	/// e.g. to over-allocate by 20%, pass `0.2` as `excess_capacity`.
	///
	/// Has no effect if a `Stats` object is provided with
	/// [`SemanticBuilder::with_stats`], only if `SemanticBuilder` is
	/// calculating stats itself.
	///
	/// This is useful when you intend to modify `Semantic`, adding more
	/// `nodes`, `scopes`, `symbols`, or `references`. Allocating excess
	/// capacity for these additions at the outset prevents `Semantic`'s data
	/// structures needing to grow later on which involves memory copying.
	/// For large ASTs with a lot of semantic data, re-allocation can be very
	/// costly.
	#[must_use]
	...
	pub fn module_record(&self) -> Arc<ModuleRecord> { Arc::clone(&self.module_record) }
	...
	pub fn build(mut self, program:&Program<'a>) -> SemanticBuilderReturn<'a> {
		...
		if self.source_type.is_typescript_definition() {
			...
			// Use counts of nodes, scopes, symbols, and references to
			// pre-allocate sufficient capacity in `AstNodes`, `ScopeTree`
			// and `SymbolTable`.
			//
			// This means that as we traverse the AST and fill up these
			// structures with data, they never need to grow and reallocate -
			// which is an expensive operation as it involves copying all the
			// memory from the old allocation to the new one.
			// For large source files, these structures are very large, so
			// growth is very costly as it involves copying massive chunks of
			// memory. Avoiding this growth produces up to 30% perf boost on
			// our benchmarks.
			//
			// If user did not provide existing `Stats`, calculate them by
			// visiting AST.
			...
			// Checking syntax error on module record requires scope information
			// from the previous AST pass
	...
	/// Push a Syntax Error
	pub(crate) fn error(&self, error:OxcDiagnostic) { self.errors.borrow_mut().push(error); }
	...
	fn record_ast_node(&mut self) {
		// The `self.cfg.is_some()` check here could be removed, since
		// `ast_node_records` is empty if CFG is disabled. But benchmarks
		// showed removing the extra check is a perf regression. <https://github.com/oxc-project/oxc/pull/4273>
		if self.cfg.is_some() {
	...
	pub(crate) fn strict_mode(&self) -> bool { self.current_scope_flags().is_strict_mode() }
	...
	/// Declares a `Symbol` for the node, adds it to symbol table, and binds it
	/// to the scope.
	///
	/// includes: the `SymbolFlags` that node has in addition to its declaration
	/// type (eg: export, ambient, etc.) excludes: the flags which node cannot
	/// be declared alongside in a symbol table. Used to report forbidden
	/// declarations.
	///
	...
	/// current scope. Returns the symbol ID if it exists and is not excluded by
	/// `excludes`.
	///
	...
	///
	/// # Panics
	pub(crate) fn declare_reference(&mut self, name:Atom<'a>, reference:Reference) -> ReferenceId {
	...
	/// Declares a `Symbol` for the node, shadowing previous declarations in the
	/// same scope.
	...
	fn resolve_references_for_current_scope(&mut self) {
		...
		for (name, mut references) in current_refs.drain() {
			...
			// If unresolved, transfer it to parent scope's unresolved
			// references.
				...
				references.retain(|&reference_id| {
								...
								|| symbol_flags.is_type_import())
					{
						if flags.is_value_as_type() {
							// Resolve pending type references (e.g., from
							// `typeof` expressions) to proper type references.
							*reference.flags_mut() = ReferenceFlags::Type;
						} else if symbol_flags.is_value() && !flags.is_type_only() {
							// The non type-only ExportSpecifier can reference a
							// type/value symbol, If the symbol is a
							// value symbol and reference flag is not type-only,
							// remove the type flag.
							*reference.flags_mut() -= ReferenceFlags::Type;
						} else {
							// If the symbol is a type symbol and reference flag
							// is not type-only, remove the value flag.
...
impl<'a> Visit<'a> for SemanticBuilder<'a> {
	...
	fn leave_scope(&mut self) {
		...
		// `get_parent_id` always returns `Some` because this method is not
		// called for `Program`. So we could `.unwrap()` here. But that seems
		// to produce a small perf impact, probably because `leave_scope` then
		// doesn't get inlined because of its larger size due to the panic code.
	...
	fn visit_program(&mut self, program:&Program<'a>) {
		...
		// Don't call `enter_node` here as `Program` is a special case - node
		// has no `parent_id`. Inline the specific logic for `Program` here
		// instead. This avoids `Nodes::add_node` having to handle the special
		// case. We can also skip calling `self.enter_kind`,
		// `self.record_ast_node` and `self.jsdoc.retrieve_attached_jsdoc`, as
		// they are all no-ops for `Program`.
		...
		// Don't call `enter_scope` here as `Program` is a special case - scope
		// has no `parent_id`. Inline the specific logic for `Program` here
		// instead. This simplifies logic in `enter_scope`, as it doesn't have
		...
		// as scope depth is initialized as 1 already (the scope depth for
		// `Program`).
		...
		// Don't call `leave_scope` here as `Program` is a special case - scope
		// has no `parent_id`. This simplifies `leave_scope`.
		self.resolve_references_for_current_scope();
	...
	fn visit_break_statement(&mut self, stmt:&BreakStatement<'a>) {
		...
		control_flow!(self, |cfg| {
			cfg.append_break(node_id, stmt.label.as_ref().map(|it| it.name.as_str()))
		});
		// cfg
	...
	fn visit_block_statement(&mut self, it:&BlockStatement<'a>) {
		...
		// Move all bindings from catch clause param scope to catch clause body
		// scope to make it easier to resolve references and check redeclare
		// errors
	...
	fn visit_continue_statement(&mut self, stmt:&ContinueStatement<'a>) {
		...
		// cfg
		control_flow!(self, |cfg| {
			cfg.append_continue(node_id, stmt.label.as_ref().map(|it| it.name.as_str()))
		});
		// cfg
	...
	fn visit_for_in_statement(&mut self, stmt:&ForInStatement<'a>) {
		...
		control_flow!(self, |cfg| {
			...
			// connect the end of the iterable expression to the basic block
			// with back edge
			...
			// connect the basic block with back edge to the basic block after
			// the for loop for when there are no more iterations left in the
			// iterable
	...
	fn visit_switch_statement(&mut self, stmt:&SwitchStatement<'a>) {
		...
		for case in &stmt.cases {
			...
			control_flow!(self, |cfg| {
				switch_case_graph_spans.push((before_case_graph_ix, cfg.current_node_ix))
			});
		}
		...
		control_flow!(self, |cfg| {
			for i in 0..switch_case_graph_spans.len() {
				...
				// so there's a possible jump from it to the next switch case
				// condition
			...
			// if we don't have a default case there should be an edge from
			// discriminant to the end of the statement.
	...
	fn visit_function(&mut self, func:&Function<'a>, flags:ScopeFlags) {
		...
		// cfg
		control_flow!(self, |cfg| {
			cfg.add_edge(before_function_graph_ix, function_graph_ix, EdgeType::NewFunction)
		});
		// cfg
	...
	fn visit_arrow_function_expression(&mut self, expr:&ArrowFunctionExpression<'a>) {
		...
		// cfg
		control_flow!(self, |cfg| {
			cfg.add_edge(current_node_ix, function_graph_ix, EdgeType::NewFunction)
		});
		// cfg
		...
		match kind {
			...
			AstKind::TSPropertySignature(signature) => {
				if signature.key.is_expression() {
					...
					//               ^^^^^ The property can reference value or
					// [`SymbolFlags::TypeImport`] symbol
			...
			AstKind::TSTypeName(_) => {
				match self.nodes.parent_kind(self.current_node_id) {
					...
					_ => {
						// Handled in `AstKind::PropertySignature` or
						// `AstKind::TSTypeQuery`
			...
			AstKind::MemberExpression(_) => {
				// A.B = 1;
				// ^^^ we can't treat A as Write reference, because it's the
				// property(B) of A that change
				self.current_reference_flags -= ReferenceFlags::Write;
			},
				...
				AstKind::ExpressionStatement(_) => {
					if self.current_scope_flags().is_arrow() {
						if let Some(node) = self.nodes.iter_parents(node.id()).nth(2) {
							// (x) => x++
							//        ^^^ implicit return, we need to treat `x`
							// as a read reference
