...
impl ScopeTree {
	...
	pub fn delete_root_unresolved_reference(&mut self, name:&str, reference_id:ReferenceId) {
		// It would be better to use `Entry` API to avoid 2 hash table lookups
		// when deleting, but `map.entry` requires an owned key to be
		// provided. Currently we use `CompactStr`s as keys which are not
		// cheap to construct, so this is best we can do at present.
