...
impl ScopeTree {
	...
	/// program scope.
	#[inline]
	pub fn len(&self) -> usize { self.parent_ids.len() }
	...
	#[inline]
	pub fn is_empty(&self) -> bool { self.parent_ids.is_empty() }
	...
	#[inline]
	pub const fn root_scope_id(&self) -> ScopeId { Self::ROOT_SCOPE_ID }

	/// Get the flags for the root scope.
	///
	/// This is a shorthand for `scope.get_flags(scope.root_scope_id())`.
	#[inline]
	pub fn root_flags(&self) -> ScopeFlags { self.flags[self.root_scope_id()] }
	...
	///
	/// If the `ReferenceId` provided is only reference remaining for this
	/// unresolved reference (i.e. this `x` was last `x` in the AST), deletes
	/// the key from `root_unresolved_references`.
	///
	/// # Panics
	/// Panics if there is no unresolved reference for provided `name` and
	/// `reference_id`.
	#[inline]
	pub fn delete_root_unresolved_reference(&mut self, name:&str, reference_id:ReferenceId) {
		// It would be better to use `Entry` API to avoid 2 hash table lookups when
		// deleting, but `map.entry` requires an owned key to be provided. Currently
		// we use `CompactStr`s as keys which are not cheap to construct, so this is
		// best we can do at present. TODO: Switch to `Entry` API once we use `&str`s
		// or `Atom`s as keys.
	...
	#[inline]
	pub fn get_flags(&self, scope_id:ScopeId) -> ScopeFlags { self.flags[scope_id] }
	...
	#[inline]
	pub fn get_parent_id(&self, scope_id:ScopeId) -> Option<ScopeId> { self.parent_ids[scope_id] }
	...
	///
	/// This will also remove the scope from the child list of the old parent
	...
	#[inline]
	pub fn get_bindings(&self, scope_id:ScopeId) -> &Bindings { &self.bindings[scope_id] }

	/// Get the ID of the [`AstNode`] that created a scope.
	///
	/// [`AstNode`]: crate::AstNode
	#[inline]
	pub fn get_node_id(&self, scope_id:ScopeId) -> NodeId { self.node_ids[scope_id] }
	...
	#[inline]
	pub fn has_child_ids(&self) -> bool { self.build_child_ids }

	/// Get the child scopes of a scope
	#[inline]
	pub fn get_child_ids(&self, scope_id:ScopeId) -> &[ScopeId] { &self.child_ids[scope_id] }
	...
	pub fn rename_binding(
		...
		// Remove old entry. `swap_remove` swaps the last entry into the place of
		// removed entry. We just inserted the new entry as last, so the new entry
		// takes the place of the old. Order of entries is same as before, only the
		// key has changed.
