...
impl ScopeTree {
	...
	pub fn ancestors(
		...
		std::iter::successors(Some(scope_id), |scope_id| {
	...
	#[inline]
	pub fn delete_root_unresolved_reference(
		&mut self,
		name: &str,
	...
	pub fn iter_bindings(
		&self,
	) -> impl Iterator<Item = (ScopeId, SymbolId, &'_ CompactStr)> + '_ {
		self.bindings.iter_enumerated().flat_map(|(scope_id, bindings)| {
			bindings
				.iter()
				.map(move |(name, symbol_id)| (scope_id, *symbol_id, name))
		})
	}
