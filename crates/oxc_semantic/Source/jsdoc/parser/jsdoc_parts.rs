...
impl<'a> JSDocCommentPart<'a> {
	pub fn new(part_content:&'a str, span:Span) -> Self { Self { raw:part_content, span } }
	...
	// And it also includes leading `*` prefixes in every line, even in a single
	// line tag. (comment `Span` for `kind2` is ` C2\n * `)
	//
	// Since these are trimmed by `parsed()` output, this raw `Span` may not be
	// suitable for linter diagnostics.
	//
	...
	//     * @kind2 C2
	// ```
	// It may not be perfect for multiline, but for single line, which is probably
	// the majority, it is enough.
	...
	/// Returns the content of the comment part without leading `*` in each
	/// line.
...
impl<'a> JSDocTagKindPart<'a> {
	...
	/// Returns `kind`, it can be any string like `param`, `type`, `whatever`,
	/// ...etc.
