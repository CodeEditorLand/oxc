...
pub fn parse_jsdoc(source_text:&str, jsdoc_span_start:u32) -> (JSDocCommentPart, Vec<JSDocTag>) {
	...
	// But `@` can be found inside of `{}` (e.g. `{@see link}`), it should be
	// distinguished.
	...
	while let Some(ch) = chars.next() {
		...
		match ch {
			...
			// Not so common but technically, major markdown parser can handle 3
			// or more backticks as code fence. (for nested code blocks)
			// But for now, 4, 6, ... backticks are not handled here to keep
			// things simple...
			'`' => {
...
fn parse_jsdoc_tag(tag_content:&str, jsdoc_tag_span:Span) -> JSDocTag {
	...
	// Splitter whitespace should be included to distinguish these cases for
	// comment parser. ```
	// /**
	//  * @k * <- should not omit
	//  */
	...
	//  */
	// ```
	// If not included, both body content will start with `* <- ...` and fails
	// to parse(trim). This is only for comment parser, it will be ignored for
	// type and type name parser.
