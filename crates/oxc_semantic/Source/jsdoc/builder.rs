...
impl<'a> JSDocBuilder<'a> {
	...
	//
	// - 1) At semantic build time, visit each node and flag it if 1 or more JSDoc
	//   comments found
	// - 2) At runtime (usecases like oxlint), reference that flag from the visited
	//   node
	//
	...
	// The benchmark says that perf actually drops by -3~4% if we check every
	// kind.
	//
	// This means that some JSDoc comments may not be parsed as originally
	// written. (In the first place, comments can be written anywhere,
	//  although some may already be inconsistent when converted from Token to
	// AST nodes).
	//
	// Check the `should_attach_jsdoc()` function below to see which nodes are
	// listed.
	//
	// ## Usecase matters
	//
	// "Where to write comments and what meaning you want them to have" depends
	// entirely on the usecase.
	//
	...
	// ```
	//
	// In the current implementation, this JSDoc is attached to the
	// `FunctionDeclaration'.
	//
	// - How to validate parameter `x` should have `@param` JSDoc?
	//
	// In this plugin-jsdoc usecase,
	//  visit `FunctionDeclaration`, find `params.items`, get attached JSDoc,
	// and ... OK.
	//
	// Then how about this?
	//
	...
	// ```
	//
	// So we need extra work to find+ask parent (or sibling?) node until desired
	// JSDoc is found.
	//
	// - How to get type information when visiting `FormalParameter`(or its
	//   `Identifier`)?
	//
	...
	//
	// Therefore, the `should_attach_jsdoc()` function and its candidates should
	// be carefully listed.
	//
	// As many reasonable types as possible should be listed, as long as it does
...
//
// It's a bit hard to explain, but theoretically the more outer ones should be
// listed.
//
