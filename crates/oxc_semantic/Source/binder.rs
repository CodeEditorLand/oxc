		...
		self.id.bound_names(&mut |ident| {
			...
			for scope_id in &var_scope_ids {
				if let Some(symbol_id) = builder
					.check_redeclaration(*scope_id, span, name, excludes, true)
				{
...
fn is_function_part_of_if_statement(
	function: &Function,
	...
	let Some(AstKind::IfStatement(stmt)) =
...
impl<'a> Binder<'a> for BindingRestElement<'a> {
	...
	fn bind(&self, builder: &mut SemanticBuilder) {
		let parent_kind =
			builder.nodes.parent_kind(builder.current_node_id).unwrap();
		...
		let excludes = SymbolFlags::FunctionScopedVariable
			| SymbolFlags::FunctionScopedVariableExcludes;
...
impl<'a> Binder<'a> for TSEnumMember<'a> {
	fn bind(&self, builder: &mut SemanticBuilder) {
		...
		let name = match &self.id {
			TSEnumMemberName::StaticIdentifier(id) => {
				Cow::Borrowed(id.name.as_str())
			},
			TSEnumMemberName::StaticStringLiteral(s) => {
				Cow::Borrowed(s.value.as_str())
			},
			TSEnumMemberName::StaticTemplateLiteral(s) => Cow::Borrowed(
				s.quasi()
					.expect("Template enum members must have no substitutions.")
					.as_str(),
			),
			TSEnumMemberName::StaticNumericLiteral(n) => {
				Cow::Owned(n.value.to_string())
			},
...
impl<'a> Binder<'a> for TSModuleDeclaration<'a> {
	fn bind(&self, builder: &mut SemanticBuilder) {
		...
		let ambient =
			if self.declare { SymbolFlags::Ambient } else { SymbolFlags::None };
