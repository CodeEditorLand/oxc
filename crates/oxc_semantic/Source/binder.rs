...
impl<'a> Binder<'a> for VariableDeclarator<'a> {
	fn bind(&self, builder:&mut SemanticBuilder<'a>) {
		...
		self.id.bound_names(&mut |ident| {
			...
			// to support redeclaration checks when declaring variables with the
			// same name later.
			for scope_id in &var_scope_ids {
				builder
					.hoisting_variables
...
// FunctionDeclarations. https://tc39.es/ecma262/#sec-block-level-function-declarations-web-legacy-compatibility-semantics
	...
	fn bind(&self, builder:&mut SemanticBuilder) {
		...
		if let Some(ident) = &self.id {
			if is_function_part_of_if_statement(self, builder) {
				...
				let (includes, excludes) = if (scope_flags.is_strict_mode()
...
impl<'a> Binder<'a> for TSEnumMember<'a> {
	fn bind(&self, builder:&mut SemanticBuilder) {
		...
		let name = match &self.id {
			...
			TSEnumMemberName::StaticTemplateLiteral(s) => {
				Cow::Borrowed(
...
impl<'a> Binder<'a> for TSModuleDeclaration<'a> {
	fn bind(&self, builder:&mut SemanticBuilder) {
		...
		// At declaration time a module has no value declaration it is only when
		// a value declaration is made inside a the scope of a module that the
		// symbol is modified
