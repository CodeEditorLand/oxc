...
impl<'a> Binder<'a> for VariableDeclarator<'a> {
	fn bind(&self, builder:&mut SemanticBuilder<'a>) {
		...
		self.id.bound_names(&mut |ident| {
			...
			// to support redeclaration checks when declaring variables with the same name
			// later.
			for &scope_id in &var_scope_ids {
				builder
					.hoisting_variables
...
impl<'a> Binder<'a> for TSModuleDeclaration<'a> {
	fn bind(&self, builder:&mut SemanticBuilder) {
		...
		// At declaration time a module has no value declaration it is only when a value
		// declaration is made inside a the scope of a module that the symbol is
		// modified
