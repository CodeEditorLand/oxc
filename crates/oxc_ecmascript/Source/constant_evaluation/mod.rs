...
use crate::{ToBigInt, ToBoolean, ToInt32, ToJsString, ToNumber, side_effects::MayHaveSideEffects};
...
pub trait ConstantEvaluation<'a> {
	...
	fn get_side_free_number_value(&self, expr:&Expression<'a>) -> Option<f64> {
		...
		// Calculating the number value, if any, is likely to be faster than calculating
		// side effects, and there are only a very few cases where we can compute a
		// number value, but there could also be side effects. e.g. `void
		// doSomething()` has value NaN, regardless of the behavior of `doSomething()`
	...
	fn eval_binary_operation(
		...
		match operator {
			...
			BinaryOperator::LessThan => {
				self.is_less_than(left, right, true).map(|value| {
					match value {
			...
			BinaryOperator::GreaterThan => {
				self.is_less_than(right, left, false).map(|value| {
					match value {
						ConstantValue::Undefined => ConstantValue::Boolean(false),
						_ => value,
					}
		...
		match expr.operator {
			...
			UnaryOperator::UnaryNegation => {
				...
				match ty {
					...
					ValueType::Number => {
						self.eval_to_number(&expr.argument)
