...
impl<'a> BoundNames<'a> for BindingIdentifier<'a> {
	fn bound_names<F:FnMut(&Self)>(&self, f:&mut F) { f(self); }
}
...
impl<'a> BoundNames<'a> for AssignmentPattern<'a> {
	fn bound_names<F:FnMut(&BindingIdentifier<'a>)>(&self, f:&mut F) { self.left.bound_names(f); }
}
...
impl<'a> BoundNames<'a> for Function<'a> {
	fn bound_names<F:FnMut(&BindingIdentifier<'a>)>(&self, f:&mut F) { self.bound_name(f); }
}
