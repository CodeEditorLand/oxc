...
use crate::{ToBoolean, array_join::ArrayJoin};
...
impl<'a> ToJsString<'a> for StringLiteral<'a> {
	fn to_js_string(&self) -> Option<Cow<'a, str>> { Some(Cow::Borrowed(self.value.as_str())) }
}
...
impl<'a> ToJsString<'a> for NullLiteral {
	fn to_js_string(&self) -> Option<Cow<'a, str>> { Some(Cow::Borrowed("null")) }
}
...
impl<'a> ToJsString<'a> for ObjectExpression<'a> {
	fn to_js_string(&self) -> Option<Cow<'a, str>> { Some(Cow::Borrowed("[object Object]")) }
}
