...
impl SourceMap {
	...
	pub fn set_debug_id(&mut self, debug_id:&str) { self.debug_id = Some(debug_id.into()); }

	pub fn get_debug_id(&self) -> Option<&str> { self.debug_id.as_deref() }
...
fn greatest_lower_bound<'a, T, K:Ord, F:Fn(&'a T) -> K>(
	...
	let mut idx = match slice.binary_search_by_key(key, &map) {
		...
		Err(index) => {
			...
			// should insert a new token, and that the token directly before is the
	...
	// exact matches is generally very small, and almost certainly smaller than the
	// number of tokens before the index.
	for i in (0..idx).rev() {
		if map(&slice[i]) == *key {
			idx = i;
		} else {
