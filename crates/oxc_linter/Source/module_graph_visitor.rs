...
/// Returning value of `visit_fold` closures, It will stop on returning a `Stop`
/// variant, Otherwise it would continue with the iteration.
...
impl<T> VisitFoldWhile<T> {
	pub fn is_done(&self) -> bool { matches!(self, Self::Stop(_)) }
...
impl<'a, T> ModuleGraphVisitorBuilder<'a, T> {
	...
	// #[must_use]
	// pub fn enter<F: FnMut(ModulePair, &ModuleRecord) + 'a>(mut self, enter: F) ->
	// Self { self.enter = Some(Box::new(enter));
	// self
	// }

	// /// Sets the leave module event closure.
	// #[must_use]
	// pub fn leave<F: FnMut(ModulePair, &ModuleRecord) + 'a>(mut self, leave: F) ->
	// Self { self.leave = Some(Box::new(leave));
	// self
	// }
