...
impl<'a> LintContext<'a> {
	...
	pub fn new(file_path:Box<Path>, semantic:Rc<Semantic<'a>>) -> Self {
		...
		// We should always check for `semantic.cfg()` being `Some` since we
		// depend on it and it is unwrapped without any runtime checks after
		// construction.
		assert!(
	...
	pub fn semantic(&self) -> &Rc<Semantic<'a>> { &self.semantic }
	...
	pub fn disable_directives(&self) -> &DisableDirectives<'a> { &self.disable_directives }
	...
	/// Plugin settings
	pub fn settings(&self) -> &OxlintSettings { &self.config.settings }

	pub fn globals(&self) -> &OxlintGlobals { &self.config.globals }

	/// Runtime environments turned on/off by the user.
	///
	/// Examples of environments are `builtin`, `browser`, `node`, etc.
	pub fn env(&self) -> &OxlintEnv { &self.config.env }
	...
	pub fn frameworks(&self) -> FrameworkFlags { self.frameworks }
	...
	// #[inline]
	// fn plugin_name_to_prefix(&self, plugin_name: &'static str) -> &'static
	// str {     let plugin_name = if self. plugin_name == "jest" &&
	// self.frameworks.contains(FrameworkFlags::Vitest) {         "vitest"
	//     } else {
	//         plugin_name
	//     };
