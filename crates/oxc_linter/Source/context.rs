...
impl<'a> LintContext<'a> {
	const WEBSITE_BASE_URL: &'static str =
	...
	pub fn new(file_path: Box<Path>, semantic: Rc<Semantic<'a>>) -> Self {
		...
		let disable_directives = DisableDirectivesBuilder::new(
			...
			semantic.trivias().clone(),
		)
		.build();
		Self {
			semantic,
			diagnostics: RefCell::new(Vec::with_capacity(
				DIAGNOSTICS_INITIAL_CAPACITY,
			)),
	...
	#[cfg(debug_assertions)]
	pub fn with_rule_fix_capabilities(
		mut self,
		capabilities: RuleFixMeta,
	) -> Self {
	...
	fn add_diagnostic(&self, mut message: Message<'a>) {
		if self
			.disable_directives
			.contains(self.current_rule_name, message.span())
		{
			return;
		}
	...
	/// [closure]: <https://doc.rust-lang.org/book/ch13-01-closures.html>
	#[inline]
	pub fn diagnostic_with_suggestion<C, F>(
		&self,
	...
	///
	#[inline]
	pub fn diagnostic_with_dangerous_fix<C, F>(
		&self,
		diagnostic: OxcDiagnostic,
		fix: F,
	) where
		...
		F: FnOnce(RuleFixer<'_, 'a>) -> C,
	{
		self.diagnostic_with_fix_of_kind(
			diagnostic,
