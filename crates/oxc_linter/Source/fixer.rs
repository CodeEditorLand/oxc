...
impl<'c, 'a:'c> RuleFixer<'c, 'a> {
	...
	// NOTE(@DonIsaac): Internal methods shouldn't use `T: Into<Foo>` generics
	// to optimize binary size. Only use such generics in public APIs.
	...
	#[inline]
	pub fn source_text(&self) -> &'a str { self.ctx.source_text() }

	/// Get a snippet of source text covered by the given [`Span`]. For details,
	/// see [`Span::source_text`].
	#[inline]
	pub fn source_range(&self, span:Span) -> &'a str { self.ctx.source_range(span) }

	/// Create a [`RuleFix`] that deletes the text covered by the given [`Span`]
	/// or AST node.
	#[inline]
	pub fn delete<S:GetSpan>(&self, spanned:&S) -> RuleFix<'a> { self.delete_range(spanned.span()) }
	...
	/// Replace a `target` AST node with the source code of a `replacement`
	/// node..
	...
	/// Creates a fix command that inserts text before the specified range in
	/// the source text.
	#[inline]
	pub fn insert_text_before_range<S:Into<Cow<'a, str>>>(&self, span:Span, text:S) -> RuleFix<'a> {
	...
	/// Creates a fix command that inserts text after the specified range in the
	/// source text.
	#[inline]
	pub fn insert_text_after_range<S:Into<Cow<'a, str>>>(&self, span:Span, text:S) -> RuleFix<'a> {
		self.insert_text_at(span.end, text)
	}

	/// Creates a fix command that inserts text at the specified index in the
	/// source text.
	#[allow(clippy::unused_self)]
	...
	#[allow(clippy::unused_self)]
	#[inline]
	pub fn noop(&self) -> RuleFix<'a> { self.new_fix(CompositeFix::None, None) }
	...
	where
		...
		if snippet.len() > Self::MAX_SNIPPET_LEN {
			let substring = match snippet.char_indices().nth(Self::MAX_SNIPPET_LEN) {
				...
				// slow path when MAX_SNIPPET_LEN is on a UTF-8 character
				// boundary
...
impl From<Message<'_>> for OxcDiagnostic {
	#[inline]
	fn from(message:Message) -> Self { message.error }
}
...
/// Note that our parser has handled the BOM, so we don't need to port the BOM
...
impl<'a> Fixer<'a> {
	...
	pub fn fix(mut self) -> FixResult<'a> {
		...
		self.messages
			.sort_unstable_by_key(|m| m.fix.as_ref().unwrap_or(&Fix::default()).span);
...
mod test {
	...
	fn insert_at_end() -> OxcDiagnostic { OxcDiagnostic::warn("End") }

	fn insert_at_start() -> OxcDiagnostic { OxcDiagnostic::warn("Start") }

	fn insert_at_middle() -> OxcDiagnostic { OxcDiagnostic::warn("Multiply") }

	fn replace_id() -> OxcDiagnostic { OxcDiagnostic::warn("foo") }

	fn replace_var() -> OxcDiagnostic { OxcDiagnostic::warn("let") }

	fn replace_num() -> OxcDiagnostic { OxcDiagnostic::warn("5") }

	fn remove_start() -> OxcDiagnostic { OxcDiagnostic::warn("removestart") }

	fn remove_middle(span:Span) -> OxcDiagnostic {
		OxcDiagnostic::warn("removemiddle").with_label(span)
	}

	fn remove_end() -> OxcDiagnostic { OxcDiagnostic::warn("removeend") }

	fn reverse_range() -> OxcDiagnostic { OxcDiagnostic::warn("reversed range") }

	fn no_fix(span:Span) -> OxcDiagnostic { OxcDiagnostic::warn("nofix").with_label(span) }

	fn no_fix_1(span:Span) -> OxcDiagnostic { OxcDiagnostic::warn("nofix1").with_label(span) }

	fn no_fix_2(span:Span) -> OxcDiagnostic { OxcDiagnostic::warn("nofix2").with_label(span) }
	...
	fn get_fix_result(messages:Vec<Message>) -> FixResult { Fixer::new(TEST_CODE, messages).fix() }

	fn create_message(error:OxcDiagnostic, fix:Option<Fix>) -> Message { Message::new(error, fix) }
