...
impl<'c, 'a: 'c> RuleFixer<'c, 'a> {
	...
	pub fn new_fix_with_capacity(&self, capacity: usize) -> RuleFix<'a> {
		RuleFix::new(
			self.kind,
			None,
			CompositeFix::Multiple(Vec::with_capacity(capacity)),
		)
	}
	...
	pub fn replace_with<T: GetSpan, S: GetSpan>(
		&self,
		target: &T,
		replacement: &S,
	) -> RuleFix<'a> {
		...
		let message = self.auto_message.then(|| {
			...
			let replacement_text =
				self.possibly_truncate_snippet(&borrowed_replacement);
			Cow::Owned(format!(
				"Replace `{target_text}` with `{replacement_text}`."
			))
		});
	...
	#[allow(clippy::unused_self)]
	pub fn replace<S: Into<Cow<'a, str>>>(
		&self,
		target: Span,
		replacement: S,
	) -> RuleFix<'a> {
		...
		let message = self.auto_message.then(|| {
			Cow::Owned(format!("Replace `{target_text}` with `{content}`."))
		});
	...
	#[allow(clippy::unused_self)]
	fn insert_text_at<S: Into<Cow<'a, str>>>(
		&self,
		index: u32,
		text: S,
	) -> RuleFix<'a> {
		...
		let message = self
			.auto_message
			.then(|| Cow::Owned(format!("Insert `{content}`")));
		self.new_fix(CompositeFix::Single(fix), message)
	}

	#[allow(clippy::unused_self)]
	pub fn codegen(self) -> CodeGenerator<'a> {
		CodeGenerator::new().with_source_text(self.source_text()).with_options(
		...
		'a: 's,
	{
		if snippet.len() > Self::MAX_SNIPPET_LEN {
			let substring =
				match snippet.char_indices().nth(Self::MAX_SNIPPET_LEN) {
					Some((pos, _)) => Cow::Borrowed(&snippet[..pos]),
					// slow path when MAX_SNIPPET_LEN is on a UTF-8 character boundary
					None => Cow::Owned(
						snippet.chars().take(Self::MAX_SNIPPET_LEN).collect(),
					),
				};
			substring + "..."
		} else {
...
impl<'a> Fixer<'a> {
	...
	pub fn fix(mut self) -> FixResult<'a> {
		...
		self.messages.sort_unstable_by_key(|m| {
			m.fix.as_ref().unwrap_or(&Fix::default()).span
		});
		let mut fixed = false;
...
mod test {
	...
	const INSERT_AT_END: Fix =
		Fix { span: Span::new(19, 19), content: Cow::Borrowed("// end") };
	const INSERT_AT_START: Fix =
		Fix { span: Span::new(0, 0), content: Cow::Borrowed("// start") };
	const INSERT_AT_MIDDLE: Fix =
		Fix { span: Span::new(13, 13), content: Cow::Borrowed("5 *") };
	const REPLACE_ID: Fix =
		Fix { span: Span::new(4, 10), content: Cow::Borrowed("foo") };
	const REPLACE_VAR: Fix =
		Fix { span: Span::new(0, 3), content: Cow::Borrowed("let") };
	const REPLACE_NUM: Fix =
		Fix { span: Span::new(13, 14), content: Cow::Borrowed("5") };
	...
	const REVERSE_RANGE: Fix =
		Fix { span: Span::new(3, 0), content: Cow::Borrowed(" ") };
	...
	fn insert_at_the_end() {
		...
		assert_eq!(
			result.fixed_code,
			TEST_CODE.to_string() + INSERT_AT_END.content.as_ref()
		);
	...
	fn insert_at_the_start() {
		let result = get_fix_result(vec![create_message(
			insert_at_start(),
			Some(INSERT_AT_START),
		)]);
		assert_eq!(
			result.fixed_code,
			INSERT_AT_START.content.to_string() + TEST_CODE
		);
		assert_eq!(result.messages.len(), 0);
	}
	...
	fn insert_at_the_middle() {
		let result = get_fix_result(vec![create_message(
			insert_at_middle(),
			Some(INSERT_AT_MIDDLE),
		)]);
		assert_eq!(
			result.fixed_code,
			TEST_CODE.replace(
				"6 *",
				&format!("{}{}", INSERT_AT_MIDDLE.content, "6 *")
			)
		);
		assert_eq!(result.messages.len(), 0);
	}
	...
	fn should_not_apply_fix_with_one_no_fix() {
		let result =
			get_fix_result(vec![create_message(no_fix(Span::default()), None)]);
		assert_eq!(result.fixed_code, TEST_CODE);
		assert_eq!(result.messages.len(), 1);
		assert_eq!(result.messages[0].error.to_string(), "nofix");
		assert!(!result.fixed);
	...
	fn assert_fixed_corrected(
		source_text: &str,
		expected: &str,
		composite_fix: CompositeFix,
	) {
	...
	fn merge_fixes_in_composite_fix() {
		...
		let fixes = vec![
			Fix::new("quux", Span::new(0, 3)),
			Fix::new("qux", Span::new(4, 7)),
		];
	...
	#[test]
	#[should_panic(
		expected = "Fix must not be overlapped, last_pos: 3, span.start: 2"
	)]
	fn overlapping_ranges_in_composite_fix() {
		let source_text = "foo bar baz";
		let fixes = vec![
			Fix::new("quux", Span::new(0, 3)),
			Fix::new("qux", Span::new(2, 5)),
		];
		let composite_fix = CompositeFix::Multiple(fixes);
		assert_fixed_corrected(source_text, source_text, composite_fix);
	}
	...
	#[test]
	#[should_panic(
		expected = "Negative range is invalid: Span { start: 5, end: 2 }"
	)]
	fn negative_ranges_in_composite_fix() {
		let source_text = "foo bar baz";
		let fixes = vec![
			Fix::new("quux", Span::new(0, 3)),
			Fix::new("qux", Span::new(5, 2)),
		];
		let composite_fix = CompositeFix::Multiple(fixes);
		assert_fixed_corrected(source_text, source_text, composite_fix);
	}
	...
	fn merge_fixes_into_one() {
		let source_text = "foo\nbar";
		let fixes = vec![
			Fix::new("foo", Span::new(1, 2)),
			Fix::new("bar", Span::new(4, 5)),
		];
		assert_fixes_merged(
			fixes,
			&Fix::new("fooo\nbar", Span::new(1, 5)),
	...
	fn throw_error_when_ranges_overlap() {
		let source_text = "foo\nbar";
		let fixes = vec![
			Fix::new("foo", Span::new(0, 3)),
			Fix::new("x", Span::new(2, 5)),
		];
