...
/// ESM Module Record
///
/// All data inside this data structure are for ESM, no commonjs data is
/// allowed.
///
/// See
...
pub struct ModuleRecord {
	...
	/// by this record to request the importation of a module. The List is in
	/// source text occurrence order.
	///
	...
	/// `[[Specifier]]`.
	///
	/// Note that Oxc does not support cross-file analysis, so this map will be
	/// empty after [`ModuleRecord`] is created. You must link the module
	/// records yourself.
...
impl fmt::Debug for ModuleRecord {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> std::fmt::Result {
		// recursively formatting loaded modules can crash when the module graph is
		// cyclic
...
impl NameSpan {
	pub fn new(name:CompactStr, span:Span) -> Self { Self { name, span } }
...
pub struct ImportEntry {
	...
	/// Whether this binding is for a TypeScript type-only import. This is a
	/// non-standard field. When creating a [`ModuleRecord`] for a JavaScript
	/// file, this will always be false.
	///
	/// ## Examples
	///
...
impl ImportImportName {
	pub fn is_default(&self) -> bool { matches!(self, Self::Default(_)) }

	pub fn is_namespace_object(&self) -> bool { matches!(self, Self::NamespaceObject) }
}
...
///
/// Describes a single exported binding from a module. Named export statements
/// that contain more than one binding produce multiple ExportEntry records.
///
/// ## Examples
///
/// ```ts
...
pub struct ExportEntry {
	...
	/// not have a ModuleSpecifier. "all" is used for `export * as ns from
	/// "mod"`` declarations. "all-but-default" is used for `export * from
	/// "mod" declarations`.
...
impl ExportImportName {
	...
	pub fn is_all_but_default(&self) -> bool { matches!(self, Self::AllButDefault) }
}
...
impl ExportLocalName {
	...
	/// Get the bound name of this export. [`None`] for
...
impl ModuleRecord {
	pub fn new(
		...
		Self {
			...
			local_export_entries:other.local_export_entries.iter().map(ExportEntry::from).collect(),
