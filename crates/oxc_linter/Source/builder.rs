...
impl LinterBuilder {
	...
	/// # Errors
	///
	/// Will return a [`LinterBuilderError::UnknownRules`] if there are unknown
	/// rules in the config. This can happen if the plugin for a rule is not
	/// enabled, or the rule name doesn't match any recognized rules.
	...
	#[inline]
	pub fn plugins(&self) -> LintPlugins { self.config.plugins }
	...
	where
		...
		// NOTE: we may want to warn users if they're configuring a rule that does not
		// exist.
	...
	pub fn build(self) -> Linter {
		// When a plugin gets disabled before build(), rules for that plugin aren't
		// removed until with_filters() gets called. If the user never calls it, those
		// now-undesired rules need to be taken out.
		let plugins = self.plugins();

		let mut rules = if self.cache.is_stale() {
			self.rules
				.into_iter()
				.filter(|r| plugins.contains(r.plugin_name().into()))
				.collect()
		} else {
...
pub enum LinterBuilderError {
	/// There were unknown rules that could not be matched to any known
	/// plugins/rules.
...
impl RulesCache {
	...
	pub fn is_stale(&self) -> bool {
		// NOTE: After all_rules cache has been initialized _at least once_ (e.g. its
		// borrowed, or initialize() is called), all_rules will be some if and only if
		// last_fresh_plugins == plugins. Right before creation, (::new()) and before
		// initialize() is called, these two fields will be equal _but all_rules will
		// be none_. This is OK for this function, but is a possible future foot-gun.
		// LinterBuilder uses this to re-build its rules list in ::build(). If cache
		// is created but never made stale (by changing plugins), LinterBuilder's rule
		// list won't need updating anyways, meaning its sound for this to
		// return `false`.
		self.last_fresh_plugins != self.plugins
	}
	...
	fn initialize(&self) {
		...
		let mut all_rules:Vec<_> = if self.plugins.is_all() {
			...
			// we need to include some jest rules when vitest is enabled, see
...
mod test {
	...
	fn test_filter_deny_on_default() {
		...
		// By default, all correctness rules are set to warn. the above filter should
		// only re-configure those rules, and not add/remove any others.
		assert!(!builder.rules.is_empty());
	...
	fn test_plugin_configuration() {
		...
		// ==========================================================================================
		// Test LinterBuilder::and_plugins, which deltas the plugin list instead of
		// overriding it
		// ==========================================================================================
