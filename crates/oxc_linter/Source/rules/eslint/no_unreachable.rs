...
impl Rule for NoUnreachable {
	fn run_once(&self, ctx:&LintContext) {
		...
		// A pre-allocated vector containing the reachability status of all the basic
		// blocks. We initialize this vector with all nodes set to `unreachable` since
		// if we don't visit a node in our paths then it should be unreachable by
		// definition.
		...
		// In our first path we first check if each block is definitely unreachable, If
		// it is then we set it as such, If we encounter an infinite loop we keep its
		// end block since it can prevent other reachable blocks from ever getting
		// executed.
		...
		// In the second path we go for each infinite loop end block and follow it
		// marking all edges as unreachable unless they have a reachable jump (eg.
		// break).
		for loop_ in infinite_loops {
			// A loop end block usually is also its condition and start point but what is
			// common in all cases is that it may have `Jump` or `Backedge` edges so we
			// only want to follow the `Normal` edges as these are the exiting edges.
