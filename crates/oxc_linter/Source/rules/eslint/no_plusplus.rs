...
/// Determines whether the given node is considered to be a for loop
/// "afterthought" by the logic of this rule. In particular, it returns `true`
/// if the given node is either:
///   - The update node of a `ForStatement`: for (;; i++) {}
///   - An operand of a sequence expression that is the update node: for (;;
///     foo(), i++) {}
///   - An operand of a sequence expression that is child of another sequence
///     expression, etc., up to the sequence expression that is the update node:
///     for (;; foo(), (bar(), (baz(), i++))) {}
