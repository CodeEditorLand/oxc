...
fn no_restricted_imports_diagnostic(
	...
	ctx.diagnostic(
		OxcDiagnostic::warn(msg)
			.with_help("Remove the import statement.")
			.with_label(span),
	);
}
...
fn test() {
	...
	let pass = vec![
		...
		(
			...
			Some(serde_json::json!(["crypto", "stream", "os"])),
		),
		...
		//         } from "foo";"#,
		//     Some(
		//         serde_json::json!([{ "paths": [{ "name": "foo", "importNames":
		// ["DisallowedObject"] }] }]),     ),
		// ),
		...
		//             "message": r#"Please import anything except "AllowedObject" from /bar/
		// instead."#         }]
		//     }])),
		// ),
		// (
	...
	let fail = vec![
		...
		(
			r#"import os from "os";"#,
			Some(serde_json::json!(["fs", "crypto ", "stream", "os"])),
		),
		...
		//     Some(
		//         serde_json::json!([{ "patterns": [{ "group": ["foo/*", "!foo/bar"], "message":
		// "foo is forbidden, use foo/bar instead" }] }]),     ),
		// ),
		// (
		//     r#"import withPatterns from "foo/baz";"#,
		//     Some(
		//         serde_json::json!([{ "patterns": [{ "group": ["foo/bar", "foo/baz"], "message":
		// "some foo subimports are restricted" }] }]),     ),
		// ),
		// (
		//     r#"import withPatterns from "foo/bar";"#,
		...
		//             "name": "foo",
		//             "importNames": ["default"],
		//             "message": r#"Please import the default import of "foo" from /bar/
		// instead."#         }]
		//     }])),
		// ),
		// (
		...
		//             "name": "foo",
		//             "importNames": ["DisallowedObject", "DisallowedObjectTwo"],
		//             "message": r#"Please import "DisallowedObject" and "DisallowedObjectTwo"
		// from /bar/ instead."#         }]
		//     }])),
		// ),
		// (
		...
		//             "group": ["foo"],
		//             "allowImportNamePattern": "^Foo",
		//             "message": r#"Only imports that match the pattern "/^Foo/u" are allowed to
		// be imported from "foo"."#         }]
		//     }])),
		// ),
		// (
		//     r#"import { AllowedObject, DisallowedObject } from "foo";"#,
		...
		//             "group": ["foo/*"],
		//             "allowImportNamePattern": "^Allow",
		//             "message": r#"Only import names starting with "Allow" are allowed to be
		// imported from "foo"."#         }]
		//     }])),
		// ),
		// (
		//     r#"import withPatterns from "foo/baz";"#,
		//     Some(
		//         serde_json::json!([{ "patterns": [{ "regex": "foo/(?!bar)", "message": "foo is
		// forbidden, use bar instead" }] }]),     ),
		// ),
		// (
		//     "import withPatternsCaseSensitive from 'FOO';",
		//     Some(serde_json::json!([{
