			...
			match node.kind() {
				...
				AstKind::CallExpression(expression) => {
					if let Expression::Identifier(identifier) = &expression.callee {
						...
						let ast_span = ctx.nodes().ancestors(node.id()).skip(1).find_map(|p| {
							match p.kind() {
								AstKind::Function(func) => {
									let func_name = func.name()?;
				...
				ctx.diagnostic_with_fix(
					...
					|fixer| {
						guess_function_name(ctx, parent_node.id()).map_or_else(
							|| fixer.noop(),
							|name| {
								// if this name shadows a variable in the outer scope **and** that
								// name is referenced inside the function body, it is unsafe
								// to add a name to this function
...
fn test() {
	...
	let pass = vec![
		...
		("export default function foo() {}", never.clone()), /* { "sourceType": "module",
		                                                      * "ecmaVersion": 6 }, */
		("export default function() {}", never.clone()), /* { "sourceType": "module",
		                                                  * "ecmaVersion": 6 }, */
	...
	let fail = vec![
		...
		("export default function() {}", always.clone()), /* { "sourceType": "module",
		                                                   * "ecmaVersion": 6 }, */
