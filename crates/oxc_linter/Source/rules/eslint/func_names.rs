				...
				match v {
					...
					_ => {
						Err(OxcDiagnostic::warn(
							"Expecting always, as-needed or never for eslint/func-names \
							 configuration",
						))
					},
				}
...
/// Determines whether the current FunctionExpression node has a name that would
/// be inferred from context in a conforming ES6 environment.
...
impl Rule for FuncNames {
	...
	fn run_once(&self, ctx:&LintContext<'_>) {
		...
		for node in ctx.nodes() {
			match node.kind() {
				// check function if it invalid, do not report it because maybe
				// later the function is calling itself
				...
				// when yes remove it from invalid_funcs because recursion are
				// always named
				AstKind::CallExpression(expression) => {
					if let Expression::Identifier(identifier) = &expression.callee {
						// check at first if the callee calls an invalid
						// function
						if !invalid_funcs
							.iter()
						...
						// a function which is calling itself inside is always
						// valid
						...
						// we found a recursive function, remove it from the
						// invalid list
				...
				ctx.diagnostic_with_fix(
					...
					|fixer| {
						guess_function_name(ctx, parent_node.id()).map_or_else(
							|| fixer.noop(),
							|name| {
								// if this name shadows a variable in the outer
								// scope **and** that name is referenced
								// inside the function body, it is unsafe to add
								// a name to this function
								if ctx
									.scopes()
...
fn test() {
	...
	let pass = vec![
		...
		("export default function foo() {}", always.clone()), /* { "sourceType": "module",
		                                                       * "ecmaVersion": 6 }, */
		("export default function foo() {}", as_needed.clone()), /* { "sourceType": "module",
		                                                          * "ecmaVersion": 6 }, */
		("export default function foo() {}", never.clone()), /* { "sourceType": "module",
		                                                      * "ecmaVersion": 6 }, */
		("export default function() {}", never.clone()), /* { "sourceType":
		                                                  * "module", "
		                                                  * ecmaVersion": 6
		                                                  * }, */
		...
		(
			...
			Some(json!(["always", { "generators": "always" }])),
		), // { "ecmaVersion": 6 },
		(
			"var foo = bar(function *baz() {});",
		...
		("var foo = function*() {};", as_needed.clone()),       /* { "ecmaVersion"
		                                                         * : 6 }, */
		(
			"var foo = bar(function *baz() {});",
		...
		(
			"var foo = bar(function *baz() {});",
			Some(json!(["never", { "generators": "always" }])),
		), // { "ecmaVersion": 6 },
		(
			"var foo = bar(function *baz() {});",
		...
		("class C { foo = function() {}; }", as_needed.clone()),                       /* { "ecmaVersion":
		                                                                                * 2022 }, */
	...
	let fail = vec![
		...
		("export default function() {}", always.clone()), /* { "sourceType":
		                                                   * "module", "
		                                                   * ecmaVersion": 6
		                                                   * }, */
		("export default function() {}", as_needed.clone()), /* { "sourceType": "module",
		                                                      * "ecmaVersion": 6 }, */
		("export default (function(){});", as_needed.clone()), /* { "sourceType": "module",
		                                                        * "ecmaVersion": 6 }, */
