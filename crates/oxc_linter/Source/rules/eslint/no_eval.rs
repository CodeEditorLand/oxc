...
pub struct NoEval {
	...
	/// not called. For example, the following code is valid if this property
	/// is true:
	///
	/// ```javascript
	/// const foo = eval;
...
fn test() {
	...
	let pass = vec![
		...
		// }]))), ("(0, window.eval)('foo')", Some(serde_json::json!([{
		// "allowIndirect": true }]))), ("(0, window['eval'])('foo')",
		// Some(serde_json::json!([{ "allowIndirect": true }]))), ("var EVAL
		// = eval; EVAL('foo')", Some(serde_json::json!([{ "allowIndirect":
		// true }]))), ("var EVAL = this.eval; EVAL('foo')",
		// Some(serde_json::json!([{ "allowIndirect": true }]))), (
		//     "(function(exe){ exe('foo') })(eval);",
		//     Some(serde_json::json!([{ "allowIndirect": true }])),
		// ),
		// ("window.eval('foo')", Some(serde_json::json!([{ "allowIndirect":
		// true }]))), ("window.window.eval('foo')",
		// Some(serde_json::json!([{ "allowIndirect": true }]))),
		// ("window.window['eval']('foo')", Some(serde_json::json!([{
		// "allowIndirect": true }]))), ("global.eval('foo')",
		// Some(serde_json::json!([{ "allowIndirect": true }]))),
		// ("global.global.eval('foo')", Some(serde_json::json!([{
		// "allowIndirect": true }]))), ("this.eval('foo')",
		// Some(serde_json::json!([{ "allowIndirect": true }]))), (
		//     "function foo() { this.eval('foo') }",
		//     Some(serde_json::json!([{ "allowIndirect": true }])),
		// ),
		// ("(0, globalThis.eval)('foo')", Some(serde_json::json!([{
		// "allowIndirect": true }]))), ("(0, globalThis['eval'])('foo')",
		// Some(serde_json::json!([{ "allowIndirect": true }]))), (
		...
		// }]))), ("window?.eval('foo')", Some(serde_json::json!([{
		// "allowIndirect": true }]))), ("(window?.eval)('foo')",
		// Some(serde_json::json!([{ "allowIndirect": true }]))),
	];
