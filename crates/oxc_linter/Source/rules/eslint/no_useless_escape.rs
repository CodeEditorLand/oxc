...
fn check_character(
	...
	if let Some(class) = character_class {
		if escape_char == '^' {
			// The '^' character is also a special case; it must always be escaped outside
			// of character classes, but it only needs to be escaped in character classes
			// if it's at the beginning of the character class. To account for this,
			// consider it to be a valid escape character outside of character classes, and
			// filter out '^' characters that appear at the start of a character class.
		...
		if unicode_sets {
			if REGEX_CLASS_SET_RESERVED_DOUBLE_PUNCTUATOR.contains(escape_char) {
				...
				if let Some(prev_prev_char) = source_text.chars().nth(span.start as usize - 1) {
					if prev_prev_char == escape_char {
						...
						// Escaping caret is unnecessary if the previous character is a `negate`
						// caret(`^`).
						if !class.negative {
							return None;
						}
			...
			// The '-' character is a special case, because it's only valid to escape it if
			// it's in a character class, and is not at either edge of the character
			// class. To account for this, don't consider '-' characters to be valid in
			// general, and filter out '-' characters that appear in the middle of a
			// character class.
			// (From ESLint source: https://github.com/eslint/eslint/blob/v9.9.1/lib/rules/no-useless-escape.js)
