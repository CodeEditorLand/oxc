...
fn is_parent_arrow_function_expression<'a>(node:&AstNode<'a>, ctx:&LintContext<'a>) -> bool {
	...
	let AstKind::FunctionBody(_) = parent.kind() else {
		return false;
	};
...
fn test() {
	...
	let pass = vec![
		...
		("<div />", None),                     /* { "parserOptions": { "ecmaFeatures": { "jsx":
		                                        * true } } }, */
	...
	let fail = vec![
		...
		("obj?.foo().bar", None),                                       // { "ecmaVersion": 2020 },
		("<div />", Some(serde_json::json!([{ "enforceForJSX": true }]))), /* { "parserOptions": {
		                                                                 * "ecmaFeatures": {
		                                                                 * "jsx": true } } }, */
		("<></>", Some(serde_json::json!([{ "enforceForJSX": true }]))), /* { "parserOptions": {
		                                                                  * "ecmaFeatures": {
		                                                                  * "jsx": true } } }, */
