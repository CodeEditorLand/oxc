...
/// its exit blocks.
///
/// For example, an "if" statement is terminated by explicit return if and only
/// if either:
/// 1. the test is always true and the consequent is terminated by explicit
///    return
/// 2. the test is always false and the alternate is terminated by explicit
///    return
...
impl StatementReturnStatus {
	...
	/// Union the status of two sequential statements. Similar to a logical *or*
	/// operation.
	///
	/// E.g.,
	/// {
...
/// Return checkers runs a Control Flow-like Analysis on a statement to see if
/// it always returns on all paths of execution.
	...
	match statement {
		...
		Statement::IfStatement(stmt) => {
			...
			let right = stmt
				.alternate
				.as_ref()
				.map_or(StatementReturnStatus::NotReturn, check_statement);
		...
		Statement::SwitchStatement(stmt) => {
			...
			case_statuses
				.iter()
				.fold(default_case_status, |accum, &lattice| accum.join(lattice))
		},
...
pub fn check_switch_case(
	case:&SwitchCase,
	accum:&mut StatementReturnStatus, /* Lattice accumulated from previous
	                                   * branches */
) -> bool {
...
pub fn check_block_statement(block:&BlockStatement) -> StatementReturnStatus {
	...
	for s in &block.body {
		// The only case where we can see break is if the block is inside a
		// loop, which means the loop does not return
		if let Statement::BreakStatement(_) = s {
			break;
		}
...
mod tests {
  ...
  "#;
	...
	#[test]
	fn test_block() {
		// The block statement could: return a, return, or does not return in
		// the end
		let source = r"
        function foo() {
          {
