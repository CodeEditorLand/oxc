...
pub struct ArrayCallbackReturn {
	/// When set to true, rule will also report forEach callbacks that return a
	/// value.
	check_for_each:bool,
	/// When set to true, allows callbacks of methods that require a return
	/// value to implicitly return undefined with a return statement containing
	/// no expression.
...
/// We're currently on a `Function` or `ArrowFunctionExpression`, findout if it
/// is an argument to the target array methods we're interested in.
pub fn get_array_method_name<'a>(node:&AstNode<'a>, ctx:&LintContext<'a>) -> Option<&'static str> {
	...
	while let Some(parent) = ctx.nodes().parent_node(current_node.id()) {
		match parent.kind() {
			...
			AstKind::ReturnStatement(_) => {
				let Some(func_node) = get_enclosing_function(parent, ctx) else {
					break;
				};
