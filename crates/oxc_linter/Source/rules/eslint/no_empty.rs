...
impl Rule for NoEmpty {
	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		match node.kind() {
			...
			// The visitor does not visit the `BlockStatement` inside the
			// `FinallyClause`. See `Visit::visit_finally_clause`.
			AstKind::FinallyClause(finally_clause) if finally_clause.body.is_empty() => {
				...
				ctx.diagnostic_with_suggestion(
					...
					|fixer| {
						...
						// if there's no `catch`, we can't remove the `finally`
						// block
...
fn test() {
	...
	let fix = vec![
		...
		// we can't fix this because removing the `catch` block would change
