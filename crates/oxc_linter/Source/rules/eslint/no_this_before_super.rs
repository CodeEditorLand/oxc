...
impl NoThisBeforeSuper {
		...
		neighbors_filtered_by_edge_weight(
			...
			&mut |basic_block_id, _| {
				...
				if basic_blocks_with_local_violations.contains_key(basic_block_id) {
					// super was not called before this in the current code
					// path:
					return (DefinitelyCallsThisBeforeSuper::Yes, false);
				}

				if super_called {
					// If super is called but we are in a try-catch(-finally)
					// block mark it as a maybe, since we might throw on
					// super call and still call this in `catch`/`finally`
					// block(s).
					...
					// Otherwise we know for sure that super is called in this
					// branch before reaching a this expression.
					} else {
						(DefinitelyCallsThisBeforeSuper::No, false)
					}
				// If we haven't visited a super call and we have a
				// non-error/finalize path forward, continue visiting this
				// branch.
				} else if cfg
					.graph
				...
				// Otherwise we mark it as a `Maybe` so we can analyze
				// error/finalize paths separately.
				} else {
					(DefinitelyCallsThisBeforeSuper::Maybe(*basic_block_id), false)
				}
			},
	...
	fn check_for_violation(
		...
		output.into_iter().any(|y| {
			match y {
				DefinitelyCallsThisBeforeSuper::Yes => true,
				DefinitelyCallsThisBeforeSuper::No => false,
				DefinitelyCallsThisBeforeSuper::Maybe(id) => {
					cfg.graph.edges(id).any(|edge| {
						let weight = edge.weight();
						let is_explicit_error =
							matches!(weight, EdgeType::Error(ErrorEdgeKind::Explicit));
...
fn test() {
	...
	let pass = vec![
		...
		("class A extends B { constructor() { foo |= super().a; this.c(); } }", None),
		("class A extends B { constructor() { foo &= super().a; this.c(); } }", None),
		// allows `this`/`super` in nested executable scopes, even if before
		// `super()`.
		(
			"class A extends B { constructor() { class B extends C { constructor() { super(); \
			 this.d = 0; } } super(); } }",
			None,
		),
		(
			"class A extends B { constructor() { var B = class extends C { constructor() { \
			 super(); this.d = 0; } }; super(); } }",
			None,
		),
		(
		...
		// multi code path.
		(
			"class A extends B { constructor() { if (a) { super(); this.a(); } else { super(); \
			 this.b(); } } }",
			None,
		),
		(
		...
		(
			"class A extends B { constructor(a) { super(); this.a = a && function(){} && \
			 this.foo; } }",
			None,
		),
	...
	let fail = vec![
		...
		(
			"class A extends B { constructor() { class C extends D { constructor() { this.e(); \
			 super(); } } super(); this.f(); } }",
			None,
		),
		// multi code path.
