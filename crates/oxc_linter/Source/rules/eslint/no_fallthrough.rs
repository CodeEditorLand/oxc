...
impl Rule for NoFallthrough {
	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		let Some(default_or_exit) = default.or(exit) else {
			// TODO: our `get_switch_semantic_cases` can't evaluate cfg_ids for switch
			// statements with conditional discriminant. If we can access the IDs
			// correctly it should never be `None`.
			return;
		};
...
impl NoFallthrough {
	...
	fn is_comment_fall_through(&self, comment:&str) -> bool {
		...
		if let Some(custom_pattern) = &self.0.comment_pattern {
			...
			// We are doing a quick check here to see if it starts with the expected "falls"
			// comment, so that we don't need to initialize the pattern matcher if we
			// don't need it.
