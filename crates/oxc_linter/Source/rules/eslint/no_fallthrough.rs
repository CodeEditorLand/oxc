...
impl Default for NoFallthrough {
	fn default() -> Self { Self::new(None, None, None) }
}
...
impl Rule for NoFallthrough {
	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		let Some(default_or_exit) = default.or(exit) else {
			// TODO: our `get_switch_semantic_cases` can't evaluate cfg_ids for
			// switch statements with conditional discriminant. If we can
			// access the IDs correctly it should never be `None`.
			return;
		};
		...
		while let Some((case, _)) = iter.next() {
			let Some((next_case, next_cfg_id)) = iter.peek() else {
				continue;
			};
...
impl NoFallthrough {
	fn has_blanks_between(ctx:&LintContext, range:Range<u32>) -> bool {
		...
		// check for at least 2 new lines, we allow the first new line for
		// formatting.
	...
	fn is_comment_fall_through(&self, comment:&str) -> bool {
		...
		if let Some(custom_pattern) = &self.0.comment_pattern {
			...
			// We are doing a quick check here to see if it starts with the
			// expected "falls" comment, so that we don't need to initialize
			// the pattern matcher if we don't need it.
...
// REPEATING PATTERN IN THE PROJECT, ONE TIME HACK TO GET IT DONE  >>  TODO: it
// is a hack to get our cases `cfg_id`s. please replace me with semantic API
// when          one became available. This code is highly volitile and has a
// lot of assumptions about          the current shape of the CFG, It is just a
// slow and dirty workaround!
// ----------------------------------------------------------------------------------------------
// TREAT LIKE BLACK MAGIC, IT BREAKS WITH SMALLEST CHANGES TO THE SWITCH CASE
// CFG! NOTE: DO NOT COPY -- DO NOT REUSE -- DO NOT EXTEND
// NOTE: DO NOT COPY -- DO NOT REUSE -- DO NOT EXTEND
// NOTE: DO NOT COPY -- DO NOT REUSE -- DO NOT EXTEND
// NOTE: DO NOT COPY -- DO NOT REUSE -- DO NOT EXTEND
// NOTE: DO NOT COPY -- DO NOT REUSE -- DO NOT EXTEND
// IF U NEED THIS AS AN API COMMENT ON THE ISSUE OR CREATE A DUP IF IT IS
// CLOSED! TAKE IT AS A MAGICAL BLACK BOX, NO DOCUMENTATION TO PREVENT REUSE!
...
fn get_switch_semantic_cases(
	...
	let (tests, exit) = graph.edges_directed(node.cfg_id(), Direction::Outgoing).fold(
		(Vec::new(), None),
		|(mut conds, exit), it| {
			let target = it.target();
				...
				.any(|it| matches!(it.kind, InstructionKind::Condition))
			{
				let is_empty = graph
					...
					.is_some_and(|it| {
						it.consequent.is_empty()
							|| it.consequent.iter().exactly_one().is_ok_and(
								|it| matches!(it, Statement::BlockStatement(b) if b.body.is_empty()),
							)
					});
...
fn test() {
	...
	let pass = vec![
		...
		("switch(foo) { }", None),
		(
			"switch(foo) { case 0: switch(bar) { case 2: break; } /* falls through */ case 1: \
			 break; }",
			None,
		),
		...
		(
			"switch(foo) { case 0: a(); /* caution: break is omitted intentionally */ case 1: \
			 b(); /* break omitted */ default: c(); }",
			Some(serde_json::json!([{
	...
	let fail = vec![
		...
		// TODO: it should fail but doesn't, we ignore conditional
		// discriminants for now. ("switch (a === b ? c : d) { case 1: ; case
		// 2: ; case 3: ; }", None)
	];
