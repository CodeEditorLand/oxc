...
impl Rule for NoShadowRestrictedNames {
	fn run_on_symbol(&self, symbol_id:SymbolId, ctx:&LintContext<'_>) {
		...
		if name == "undefined" {
			// Allow to declare `undefined` variable but not allow to assign
			// value to it.
...
fn test() {
	...
	let fail = vec![
		...
		(
			"function undefined(undefined) { !function undefined(undefined) { try {} \
			 catch(undefined) {} }; }",
			None,
		),
		(
			"function Infinity(Infinity) { var Infinity; !function Infinity(Infinity) { try {} \
			 catch(Infinity) {} }; }",
			None,
		),
		(
			"function arguments(arguments) { var arguments; !function arguments(arguments) { try \
			 {} catch(arguments) {} }; }",
			None,
		),
		(
		...
		(
			"var {undefined} = obj; var {a: undefined} = obj; var {a: {b: {undefined}}} = obj; \
			 var {a, ...undefined} = obj;",
			Some(json!({
				"parserOptions": {
					"ecmaVersion": 9
