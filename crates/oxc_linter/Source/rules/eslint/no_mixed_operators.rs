...
impl Default for NoMixedOperatorsConfig {
	fn default() -> Self { Self { groups:default_groups(), allow_same_precedence:true } }
}
...
impl Rule for NoMixedOperators {
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		// Now we know that node is a BinaryExpression or LogicalExpression, and
		// parent is a BinaryExpression or LogicalExpression or
...
impl NoMixedOperators {
	pub fn try_from_configuration(value:&serde_json::Value) -> Option<Self> {
		...
		if let Some(groups_config) = config.get("groups") {
			if let Some(groups_config) = groups_config.as_array() {
				'outer: for group_config in groups_config {
					// Parse current group configuration. On failure fall
		...
		let allow_same_precedence = config
			.get("allowSamePrecedence")
			.map_or(true, |val| val.as_bool().unwrap_or_default());
	...
	fn is_mixed_with_parent(node:AstKind, parent:AstKind) -> bool {
		...
		// Note that there is not need to check for parenthesis explicitly
		// because if an expression is parenthesized, its parent node is a
		// ParenthesizedExpression and will never enter the code path.
	}

	/// Report mixed operator pare between node and parent corresponding to
	/// configuration.
	fn report(&self, node:AstKind, parent:AstKind, ctx:&LintContext<'_>) {
		...
		// Since we don't store the exact span of the operators, approximate
		// that span to be between the lhs and rhs of the expression.
...
fn operator_and_precedence(operator:&str) -> Option<(&'static str, u8)> {
	OPERATORS
		...
		.map(|idx| (OPERATORS[idx], PRECEDENCES[idx]))
}
