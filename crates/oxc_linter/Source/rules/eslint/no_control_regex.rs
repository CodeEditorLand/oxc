...
fn no_control_regex_diagnostic(count:usize, regex:&str, span:Span) -> OxcDiagnostic {
	...
	let (message, help) = if count == 1 {
		(
			...
			format!("'{regex}' is not a valid control character."),
		)
	} else {
		(
			"Unexpected control characters",
			format!("'{regex}' are not valid control characters."),
		)
	};
	...
	fn run<'a>(&self, node:&AstNode<'a>, context:&LintContext<'a>) {
		match node.kind() {
			...
			AstKind::NewExpression(expr) if expr.callee.is_specific_id("RegExp") => {
				...
				// Missing or non-string arguments will be runtime errors, but are not covered
				// by this rule.
...
impl<'a> Visit<'a> for ControlCharacterFinder {
	fn visit_pattern(&mut self, it:&Pattern<'a>) {
		...
		// 3. Capture groups may be referenced before they are defined. This is why we
		//    need to do this check here, instead of filtering inside of
		//    visit_character.
