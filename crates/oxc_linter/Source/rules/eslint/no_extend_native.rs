	...
	fn run_once(&self, ctx:&LintContext) {
		...
		for reference_id_list in ctx.scopes().root_unresolved_references_ids() {
			for reference_id in reference_id_list {
				...
				// If the referenced name does not appear to be a global object,
				// skip it.
				...
				// If the first letter is capital, like `Object`, we will assume
				...
				// Check if being used like
				// `Object.defineProperty(String.prototype, 'xyz', 0)`
...
/// If this usage of `*.prototype` is a `Object.defineProperty` or
/// `Object.defineProperties` call, then this function returns the
/// `CallExpression` node.
fn get_define_property_call<'a>(ctx:&'a LintContext, node:&AstNode<'a>) -> Option<&'a AstNode<'a>> {
...
/// Checks if a given `CallExpression` is a call to `Object.defineProperty` or
...
fn get_property_assignment<'a>(ctx:&'a LintContext, node:&AstNode<'a>) -> Option<&'a AstNode<'a>> {
	for parent in ctx.nodes().iter_parents(node.id()).skip(1) {
		match parent.kind() {
			...
			AstKind::MemberExpression(MemberExpression::ComputedMemberExpression(computed)) => {
				if let AstKind::MemberExpression(node_expr) = node.kind() {
					// Ignore computed member expressions like
					// `obj[Object.prototype] = 0` (i.e., the given node is
					// the `expression` of the computed member expression)
					if computed
						.expression
...
fn test() {
	...
	let pass = vec![
		...
		(
			...
			Some(serde_json::json!([{ "exceptions": ["Object"] }])),
		),
