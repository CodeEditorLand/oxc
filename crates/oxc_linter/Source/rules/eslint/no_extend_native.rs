...
use oxc_span::{CompactStr, GetSpan, cmp::ContentEq};
	...
	fn run_once(&self, ctx:&LintContext) {
		...
		for reference_id_list in ctx.scopes().root_unresolved_references_ids() {
			for reference_id in reference_id_list {
				...
				// If the first letter is capital, like `Object`, we will assume it is a native
				// object
		...
		match parent.kind() {
			...
			AstKind::MemberExpression(MemberExpression::ComputedMemberExpression(computed)) => {
				if let AstKind::MemberExpression(node_expr) = node.kind() {
					// Ignore computed member expressions like `obj[Object.prototype] = 0` (i.e.,
					// the given node is the `expression` of the computed member expression)
					if computed
						.expression
