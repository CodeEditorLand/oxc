...
// Check if a node is in a context where its value would be coerced to a boolean
// at runtime
...
// Checks whether the node is a logical expression and that the option is
// enabled
...
fn is_first_arg(node:&AstNode, parent:&AstNode) -> bool {
	match parent.kind() {
		AstKind::CallExpression(expr) => {
			expr.arguments.first().map_or(false, |arg| {
	...
	get_real_parent(node, ctx).map_or(false, |parent| {
		match parent.kind() {
			...
			AstKind::ForStatement(stmt) => {
				stmt.test.as_ref().map_or(false, |expr| {
...
fn get_real_parent<'a, 'b>(node:&AstNode, ctx:&'a LintContext<'b>) -> Option<&'a AstNode<'b>> {
	for (_, parent) in ctx
		...
		.tuple_windows::<(&AstNode<'b>, &AstNode<'b>)>()
	{
...
fn test() {
	...
	let pass = vec![
		...
		(
			"if (!!foo || bar) {}",
			Some(serde_json::json!([{ "enforceForLogicalOperands": false }])),
		),
		(
			"if ((!!foo || bar) === baz) {}",
			Some(serde_json::json!([{ "enforceForLogicalOperands": true }])),
