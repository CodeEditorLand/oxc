	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		if !matches!(binary_expr.operator, BinaryOperator::Equality | BinaryOperator::Inequality) {
			if enforce_inverse_rule_for_null && is_null {
				...
				// In ESlint, `null >= null` will be auto fixed to `null > null`
				// which is also wrong. So I just report it.
		...
		// The "smart" option enforces the use of `===` and `!==` except for
		// these cases:
		...
		// If the comparison is a `typeof` comparison or both sides are literals
		// with the same type, then it's safe to fix.
