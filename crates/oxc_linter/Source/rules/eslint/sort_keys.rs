...
impl Rule for SortKeys {
	fn from_configuration(value:serde_json::Value) -> Self {
		...
		let sort_order = if config_array.is_empty() {
			...
			config_array[0].as_str().map_or(SortOrder::Asc, |s| {
				match s {
					"desc" => SortOrder::Desc,
					_ => SortOrder::Asc,
				}
...
fn alphanumeric_sort(arr:&mut [String]) { arr.sort_unstable(); }
...
fn test() {
	...
	let pass = vec![
		...
		// Some(serde_json::json!([]))), // { "ecmaVersion": 6 },
		("var obj = {'':1, [f()]:2, a:3}", Some(serde_json::json!([]))), /* { "ecmaVersion": 6
		                                                                  * }, */
		// ("var obj = {a:1, [b++]:2, '':3}",
		// Some(serde_json::json!(["desc"]))), // { "ecmaVersion": 6 },
		...
		("var obj = {b:1, ...c, ...d, e:2}", Some(serde_json::json!([]))), /* { "ecmaVersion":
		                                                                    * 2018 }, */
		("var obj = {a:1, ...z, '':2}", Some(serde_json::json!([]))), /* { "ecmaVersion": 2018
		                                                               * }, */
		("var obj = {'':1, ...z, 'a':2}", Some(serde_json::json!(["desc"]))), /* { "ecmaVersion"
		                                                                       * : 2018 }, */
		...
		("let {a, b} = {}", Some(serde_json::json!([]))), /* { "ecmaVersion"
		                                                   * : 6 }, */
	...
	let fail = vec![
		...
		("var obj = {...z, '':1, a:2}", Some(serde_json::json!(["desc"]))), /* { "ecmaVersion":
		                                                                     * 2018 }, */
