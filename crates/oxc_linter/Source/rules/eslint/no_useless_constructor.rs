...
fn no_redundant_super_call(constructor_span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Redundant super call in constructor")
		.with_label(constructor_span)
		.with_help(
			"Constructors of subclasses invoke super() automatically if they are empty. Remove \
			 this constructor or add code to it.",
		)
}
...
impl Rule for NoUselessConstructor {
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		// constructor() {}` is the same as `constructor() {}` and so is not
		// allowed.
...
/// Check if a function body only contains a single super call. Ignores
/// directives.
///
/// Returns the call expression if the body contains a single super call,
/// otherwise [`None`].
fn is_single_super_call<'f, 'a:'f>(body:&'f FunctionBody<'a>) -> Option<&'f CallExpression<'a>> {
	if body.statements.len() != 1 {
		return None;
	}
	let Statement::ExpressionStatement(expr) = &body.statements[0] else {
		return None;
	};
...
fn is_passing_through<'a>(
	...
	if let Some(rest) = &constructor_params.rest {
		...
		let Some(last_arg) = super_args.iter().next_back() else {
			return false;
		};
		constructor_params
			.items
...
fn test() {
	...
	let pass = vec![
		...
		// ts
		"declare class A { constructor(options: any); }", /* {                "parser":
		                                                   * require("../../fixtures/parsers/
		                                                   * typescript-parsers/declare-class")
		                                                   * } */
...
class A extends B {  foo() { bar(); } }",
	...
	Tester::new(NoUselessConstructor::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
