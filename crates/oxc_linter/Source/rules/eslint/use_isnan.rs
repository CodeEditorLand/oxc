...
impl Default for UseIsnan {
	fn default() -> Self { Self { enforce_for_switch_case:true, enforce_for_index_of:false } }
}
	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		match node.kind() {
			...
			AstKind::CallExpression(call) if self.enforce_for_index_of => {
				...
				// NaN
				let Some(method) = is_target_callee(&call.callee) else {
					return;
				};
...
fn test() {
	...
	let pass = vec![
		...
		(
			"foo.lastIndexOf(NaN)",
			Some(serde_json::json!([{ "enforceForIndexOf": false }])),
		),
	...
	let fail = vec![
		...
		(
			"switch(foo) { case bar: break; case NaN: break; case baz: break; case Number.NaN: \
			 break; }",
			Some(serde_json::json!([{ "enforceForSwitchCase": true }])),
		),
		(
	...
	Tester::new(UseIsnan::NAME, UseIsnan::CATEGORY, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
