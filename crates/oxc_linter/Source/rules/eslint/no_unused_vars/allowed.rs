...
impl<'s, 'a> Symbol<'s, 'a> {
	...
	/// 1. passed as a callback to another [`CallExpression`] or
	///    [`NewExpression`]
...
impl NoUnusedVars {
	...
	pub(super) fn is_allowed_argument<'a>(
		...
		// `args` is `all`, it may be ignored using `ignoreRestSiblings` or
		...
		// This is the last parameter, so need to check for usages on following
		// parameters
	...
	pub(super) fn is_allowed_binding_rest_element(symbol:&Symbol) -> bool {
		for parent in symbol.iter_parents() {
			// If this is a binding rest element that is part of a TS function
			// parameter, for example: `function foo(...messages: string[])
