...
impl<'s, 'a> Symbol<'s, 'a> {
	...
	/// - `true` if `pattern` is a destructuring pattern and only contains one
	///   symbol
	/// - `false` if `pattern` is a destructuring pattern and contains more than
	///   one symbol
			...
			BindingPatternKind::ArrayPattern(arr) => {
				match arr.elements.len() {
					0 => {
						...
						BindingInfo::multi_or_single(
							arr.rest.as_ref().map(|r| (r.span, true)),
							true,
						)
					},
					1 => {
			...
			BindingPatternKind::ObjectPattern(obj) => {
				match obj.properties.len() {
					0 => {
						...
						BindingInfo::multi_or_single(
							obj.rest.as_ref().map(|r| (r.span, true)),
							true,
						)
					},
					1 => {
						...
						if let Some(rest) = obj.rest.as_ref() {
							if rest.span.contains_inclusive(self.span()) {
								// assume rest spreading in objects are at the
								// end
...
pub(super) enum BindingInfo {
	...
	SingleDestructure,
	/// Notes:
	/// 1. Symbol declaration span will cover the entire pattern, so we need to
	///    extract the symbol's _exact_ span
