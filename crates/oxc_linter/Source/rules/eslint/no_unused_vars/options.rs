...
pub struct NoUnusedVarsOptions {
	...
	/// Specifies exceptions to this rule for errors caught within a `catch`
	/// block. Variables declared within a `catch` block whose names match
	...
	/// This option specifies exceptions within destructuring patterns that
	/// will not be checked for usage. Variables declared within array
	/// destructuring whose names match this pattern will be ignored.
	///
	...
	/// "ignoreClassWithStaticInitBlock": true }` option
	///
	/// ```javascript
	...
	/// ## Example
	///
	/// Examples of **incorrect** code for the `{ "reportUsedIgnorePattern":
	/// true }` option:
	///
	/// ```javascript
	...
	/// ```
	///
	/// Examples of **correct** code for the `{ "reportUsedIgnorePattern": true
	/// }` option:
	///
	/// ```javascript
	/// /*eslint no-unused-vars: ["error", { "reportUsedIgnorePattern": true, "varsIgnorePattern": "[iI]gnored" }]*/
...
pub enum VarsOption {
	/// All variables are checked for usage, including those in the global
	/// scope.
	#[default]
	All,
...
impl VarsOption {
	pub const fn is_local(&self) -> bool { matches!(self, Self::Local) }
}
...
impl ArgsOption {
	#[inline]
	pub const fn is_after_used(&self) -> bool { matches!(self, Self::AfterUsed) }
...
impl CaughtErrors {
	pub const fn all() -> Self { Self(true) }

	pub const fn none() -> Self { Self(false) }
}
...
fn invalid_option_error<M:Into<Cow<'static, str>>>(option_name:&str, message:M) -> OxcDiagnostic {
		...
		match value {
			Value::String(s) => {
				match s.as_str() {
					...
					"none" => Ok(Self::None),
					s => {
						Err(invalid_option_mismatch_error("args", ["after-used", "all", "none"], s))
					},
				}
			},
...
impl TryFrom<Value> for NoUnusedVarsOptions {
	...
	fn try_from(value:Value) -> Result<Self, Self::Error> {
		...
		match config {
			...
			Value::Object(config) => {
				let vars =
					config.get("vars").map(|vars| vars.try_into()).transpose()?.unwrap_or_default();
				...
				let args:ArgsOption =
					config.get("args").map(|args| args.try_into()).transpose()?.unwrap_or_default();
				...
				let caught_errors:CaughtErrors = config
					.get("caughtErrors")
					.map(|caught_errors| caught_errors.try_into())
					.transpose()?
					.unwrap_or_default();
			...
			_ => {
				Err(OxcDiagnostic::error(
					"Invalid 'vars' option for no-unused-vars: Expected a string or an object, \
					 got {config}",
				))
			},
		}
