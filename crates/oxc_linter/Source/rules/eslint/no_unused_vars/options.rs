...
pub struct NoUnusedVarsOptions {
	...
	/// block. Variables declared within a `catch` block whose names match this
...
impl<R> IgnorePattern<R> {
	...
	#[inline]
	pub fn is_default(&self) -> bool { matches!(self, Self::Default) }

	/// Returns `true` if the pattern is [`IgnorePattern::None`].
	#[inline]
	pub fn is_none(&self) -> bool { matches!(self, Self::None) }

	/// Returns `true` if the pattern is [`IgnorePattern::Some`] or
	/// [`IgnorePattern::Default`].
	#[inline]
	pub fn is_some(&self) -> bool { matches!(self, Self::Some(_) | Self::Default) }
	...
	pub fn expect(self, msg:&str) -> R {
		if let Self::Some(r) = self { r } else { panic!("{}", msg) }
	}
