...
impl FoundStatus {
	#[inline]
	pub const fn is_found(self) -> bool { matches!(self, Self::Ignored | Self::NotIgnored) }

	#[inline]
	pub const fn is_ignored(self) -> bool { matches!(self, Self::Ignored) }
...
impl NoUnusedVars {
	...
	///
	/// Does not handle ignore checks for re-assignments to array/object
	/// destructures.
	pub(super) fn is_ignored(&self, symbol:&Symbol<'_, '_>) -> bool {
		...
		match symbol.declaration().kind() {
			...
			s => {
				// panic when running test cases so we can find unsupported node
				// kinds
				debug_assert!(
					false,
	...
	fn search_binding_pattern<'a>(
		...
		match &binding.kind {
			// if found, not ignored. Ignoring only happens in destructuring
			// patterns.
			...
			BindingPatternKind::ArrayPattern(arr) => {
				for el in arr.elements.iter().flatten() {
					...
					match el.get_binding_identifier() {
						...
						// a destructuring pattern that doesn't contain the
						// target. Keep looking
						Some(_) | None => {
							continue;
						},
	...
	fn search_assignment_target<'a>(
		...
		match assignment {
			...
			// expression. Those will never have destructures and can be safely
			// ignored.
