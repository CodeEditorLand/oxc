...
impl<'s, 'a> Symbol<'s, 'a> {
	...
	/// 2. Catch variables are always parameter-like and will therefore never
	...
	#[inline]
	const fn is_type_alias(&self) -> bool { self.flags().contains(SymbolFlags::TypeAlias) }

	/// Check if this [`Symbol`] has an [`Reference`]s that are considered a
	/// usage.
	pub fn has_usages(&self, options:&NoUnusedVars) -> bool {
		...
		for reference in self.references() {
			...
			// e.g. reference on `a` in expression `let a = 0; let b = (a++,
			// 0);`
	...
	/// - When we encounter an AST node that updates the value of the symbol
	///   this
	...
	fn is_self_reassignment(&self, reference:&Reference) -> bool {
		...
		for node in self.nodes().iter_parents(reference.node_id()).skip(1) {
			match node.kind() {
				// references used in declaration of another variable are
				// definitely used by others
				AstKind::VariableDeclarator(_)
				...
				// When symbol is being assigned a new value, we flag the
				// reference as only affecting itself until proven
				// otherwise.
				...
				// RHS usage when LHS != reference's symbol is definitely used
				// by others
				AstKind::AssignmentExpression(AssignmentExpression { left, .. }) => {
					match left {
						...
						// variable is being used to index another variable,
						// this is always a usage
				...
				AstKind::ReturnStatement(_) => {
					match self.get_nearest_function(node.id()) {
						...
						None => return false,
						// Is this a return within the same function being
						// declared?
				...
				// function* foo() {
				//    let a = 1;
				//    a = yield a // <- still considered used b/c it's
	...
	/// expressions within [`SequenceExpression`]s that are not in the last
	/// position.
	///
	/// ```ts
	...
	fn is_discarded_read(&self, reference:&Reference) -> bool {
			...
			self.iter_relevant_parent_and_grandparent_kinds(reference.node_id())
		{
			...
			match (parent, grandparent) {
				...
				// expression check.
				(AstKind::AssignmentExpression(assignment), _) if self != &assignment.left => {
					break;
				},
				...
				(parent, AstKind::SequenceExpression(seq)) => {
					...
					// skipped. This means an equality check cannot be used
					// here.
	...
	/// [`NewExpression`] for a method/function/class within its own
	/// declaration. These do not count as a usage.
	///
	/// ## Examples
	///
	/// ```ts
	...
	fn is_self_call(&self, reference:&Reference) -> bool {
		...
		// check for assignment/declaration of a function expression to a
		// variable
	...
	fn is_self_call_simple(&self, reference:&Reference) -> bool {
		...
		let Some(container_id) = self.declaration().kind().get_container_scope_id() else {
			debug_assert!(
				false,
				"Found a function call or or new expr reference on a node flagged as a function \
				 or class, but the symbol's declaration node has no scope id. It should always be \
				 a container."
			);
			return false;
		};
	...
	/// 2. "relevant" nodes are non "transparent". For example, parenthesis are
	///    "transparent".
	#[inline]
