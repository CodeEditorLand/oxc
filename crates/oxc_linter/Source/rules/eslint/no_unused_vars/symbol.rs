...
impl<'s, 'a> Symbol<'s, 'a> {
	...
	#[inline]
	pub fn id(&self) -> SymbolId { self.id }

	#[inline]
	pub fn name(&self) -> &str { self.symbols().get_name(self.id) }

	#[inline]
	pub const fn flags(&self) -> SymbolFlags { self.flags }

	#[inline]
	pub fn scope_id(&self) -> ScopeId { self.symbols().get_scope_id(self.id) }

	#[inline]
	pub fn declaration(&self) -> &AstNode<'a> { self.nodes().get_node(self.declaration_id()) }
	...
	#[inline]
	fn declaration_id(&self) -> NodeId { self.symbols().get_declaration(self.id) }
		...
		for kind in self.iter_self_and_parents().map(AstNode::kind) {
			match kind {
				...
				AstKind::VariableDeclarator(decl) => {
					return self.clean_binding_id(&decl.id);
				},
				AstKind::FormalParameter(param) => {
					return self.clean_binding_id(&param.pattern);
				},
				_ => break,
			}
		}
	...
	fn clean_binding_id(&self, binding:&BindingPattern) -> Span {
		...
		binding
			.type_annotation
			.as_ref()
			.map_or(own, |ann| Span::new(own.start, ann.span.start))
	}
}
...
impl<'s, 'a> Symbol<'s, 'a> {
	...
	#[inline]
	pub fn is_in_jsx(&self) -> bool { self.semantic.source_type().is_jsx() }

	#[inline]
	pub fn is_in_ts(&self) -> bool { self.semantic.source_type().is_typescript() }
...
impl<'a> PartialEq<VariableDeclarator<'a>> for Symbol<'_, 'a> {
	fn eq(&self, decl:&VariableDeclarator<'a>) -> bool { self == &decl.id }
}
...
impl<'s, 'a> PartialEq<ImportDeclarationSpecifier<'a>> for Symbol<'s, 'a> {
	fn eq(&self, import:&ImportDeclarationSpecifier<'a>) -> bool { self == import.local() }
}
