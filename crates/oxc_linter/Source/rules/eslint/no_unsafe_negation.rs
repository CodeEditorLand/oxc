...
fn no_unsafe_negation_diagnostic(operator:&str, span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!(
		"Unexpected logical not in the left hand side of '{operator}' operator"
	))
	.with_help(format!(
		"use parenthesis to express the negation of the whole boolean expression, as '!' binds \
		 more closely than '{operator}'"
	))
	.with_label(span)
}
...
pub struct NoUnsafeNegation {
	/// true: disallow negation of the left-hand side of ordering relational
	/// operators false: allow negation of the left-hand side of ordering
...
fn test() {
	...
	let pass = vec![
		...
		(
			"foo = ! a >= b;",
			Some(serde_json::json!([{ "enforceForOrderingRelations": false }])),
		),
		(
			"foo = (!a) >= b;",
