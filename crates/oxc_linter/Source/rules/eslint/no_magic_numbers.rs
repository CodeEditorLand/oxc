...
/// Value must coerce to a valid array index name: "0", "1", "2" ...
/// "4294967294".
///
/// All other values, like "-1", "2.5", or "4294967295", are just "normal"
/// object properties, which can be created and accessed on an array in addition
/// to the array index properties, but they don't affect array's length and are
/// not considered by methods such as .map(), .forEach() etc.
///
/// The maximum array length by the specification is 2 ** 32 - 1 = 4294967295,
/// thus the maximum valid index is 2 ** 32 - 2 = 4294967294.
///
/// All notations are allowed, as long as the value coerces to one of "0", "1",
/// "2" ... "4294967294".
///
/// This current implementation does not check for BigInt Numbers, they will
/// always accept them as array_index Valid examples:
/// ```javascript
...
fn is_ts_enum(parent_kind:&AstKind<'_>) -> bool { matches!(parent_kind, AstKind::TSEnumMember(_)) }
...
fn test() {
	...
	let pass = vec![
		...
		("foo[0b110]", ignore_array_indexes.clone()), /* { "ecmaVersion":
		                                               * 2015 }, */
		("foo[0o71]", ignore_array_indexes.clone()), /* { "ecmaVersion":
		                                              * 2015 }, */
		("foo[0xABC]", ignore_array_indexes.clone()),
		("foo[0123]", ignore_array_indexes.clone()), /* {
		                                              * "sourceType":
		                                              * "script"
		                                              * }, */
		...
		("var a = <input maxLength={10} />;", None), /* {
		                                            ...
		                                            * }
		                                            * }, */
		("var a = <div objectProp={{ test: 1}}></div>;", None), /* {
		                                                         * "parserOptions"
		                                                         * : {
		                                                         * "ecmaFeatures"
		                                                         * : {
	...
	let fail = vec![
		...
		("var foo = 0 + 1;", None),
		("var foo = 42n", enforce_const.clone()), /* {
		                                           * "ecmaVersion": 2020
		                                           * }, */
		("var foo = 0n + 1n;", None), /* {                "ecmaVersion":
		                               * 2020            }, */
		("a = a + 5;", None),
		("a += 5;", None),
		...
		("console.log(0x1A + 0x02); console.log(071);", None), /* {
		                                                        * "sourceType"
		                                                        * : "script"
		                                                        * }, */
		(
		...
		("foo[-012]", ignore_array_indexes.clone()), /* { "sourceType":
		                                              * "script" }, */
		("foo[0.1]", ignore_array_indexes.clone()),
		...
		("foo[-1e310]", ignore_array_indexes.clone()),
		// ("foo[-1n]", ignore_array_indexes.clone()), // { "ecmaVersion": 2020
		// }, ("foo[-100n]", ignore_array_indexes.clone()), // {
		// "ecmaVersion": 2020 }, ("foo[-0x12n]",
		// ignore_array_indexes.clone()), // { "ecmaVersion": 2020 },
		// ("foo[4294967295n]", ignore_array_indexes.clone()), // {
		// "ecmaVersion": 2020 },
		("foo[+0]", ignore_array_indexes.clone()),
		("foo[+1]", ignore_array_indexes.clone()),
		("foo[-(-1)]", ignore_array_indexes.clone()),
		// ("foo[+0n]", ignore_array_indexes.clone()), // { "ecmaVersion": 2020
		// }, ("foo[+1n]", ignore_array_indexes.clone()), // { "ecmaVersion":
		// 2020 }, ("foo[- -1n]", ignore_array_indexes.clone()), // {
		// "ecmaVersion": 2020 },
		("100 .toString()", ignore_array_indexes.clone()),
		("200[100]", ignore_array_indexes.clone()),
		("var a = <div arrayProp={[1,2,3]}></div>;", None), /* {
		                                                     * "parserOptions"
		                                                     * : {
		                                                     * "ecmaFeatures"
		                                                     * : {
		...
		("const { param = 123 } = sourceObject;", None),                          /* { "ecmaVersion":
		                                                                           * 6 }, */
		(
		...
		// ("type Foo = Bar[10n];", Some(serde_json::json!([{
