...
impl Rule for NoDuplicateImports {
	fn from_configuration(value:serde_json::Value) -> Self {
		let Some(value) = value.get(0) else {
			return Self { include_exports:false };
		};
	...
	fn run_once(&self, ctx:&LintContext) {
		...
		for entry in &module_record.import_entries {
			...
			import_map
				.entry(source)
				.or_default()
				.push((import_type, span, ModuleType::Import));
			...
			for entry in &module_record.star_export_entries {
				if let Some(module_request) = &entry.module_request {
					...
					if entry.import_name.is_all_but_default() {
						if let Some(existing) = import_map.get(source) {
							if existing.iter().any(|(t, ..)| matches!(t, ImportType::AllButDefault))
							{
					...
					if let Some(existing) = import_map.get(source) {
						if existing
							.iter()
							.any(|(t, ..)| matches!(t, ImportType::Named | ImportType::SideEffect))
						{
							ctx.diagnostic(no_duplicate_exports_diagnostic(
								source,
								span,
...
fn can_merge_imports(
	...
	let namespace = existing.iter().find(|(t, ..)| matches!(t, ImportType::Namespace));

	let named = existing.iter().find(|(t, ..)| matches!(t, ImportType::Named));

	let default = existing.iter().find(|(t, ..)| matches!(t, ImportType::Default));
