...
fn no_unexpected_multiline_diagnostic(kind:&DiagnosticKind) -> OxcDiagnostic {
	match kind {
		DiagnosticKind::FunctionCall { open_paren_span } => {
			OxcDiagnostic::warn(
				"Unexpected newline between function name and open parenthesis of function call",
			)
			.with_label(
				open_paren_span
					.label("this is parsed as a function call, which may be unintentional"),
			)
			.with_help(
				"If you did not intend to make a function call, insert ';' before the parenthesis",
			)
		},
		DiagnosticKind::PropertyAccess { open_bracket_span } => {
			OxcDiagnostic::warn(
		...
		DiagnosticKind::TaggedTemplate { backtick_span } => {
			OxcDiagnostic::warn("Unexpected newline between template tag and template literal")
				.with_label(
					backtick_span
						.label("this is parsed as a tagged template, which may be unintentional"),
				)
				.with_help(
					"If you did not intend for this to be a tagged template, insert ';' before \
					 the backtick",
				)
		},
		DiagnosticKind::Division { slash_span } => {
			OxcDiagnostic::warn("Unexpected newline between numerator and division operator")
				.with_label(
	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		match node.kind() {
			...
			AstKind::BinaryExpression(binary_expr) => {
				...
				// the "identifier" will be the characters immediately following
				// the second slash until we reach a non-identifier
				// character
				let ident_name = src
                    .chars()
                    .skip(second_slash + 1)
...
/// Check if there is a newline proceeding a target character within a snippet
/// of source text. Returns `None` if the character is not found at all or has
/// no proceeding newline. Otherwise, returns the byte offset of the target
/// character with respect to the start of the span.
///
/// Newlines do not have to be directly before the target character, but can be
/// anywhere before it.
...
fn test() {
	...
	let pass = vec![
		...
		"
			...
			`;
		", /* {                "parser":
		    * require("../../fixtures/parsers/typescript-parsers/
		    * tagged-template-with-generic/tagged-template-with-generic-1")
		    * }, */
		"
			tag<
				generic
		...
		", /* {                "parser":
		    * require("../../fixtures/parsers/typescript-parsers/
		    * tagged-template-with-generic/tagged-template-with-generic-2")
		    * }, */
		"
			tag<
				generic
		...
		", /* {                "parser":
		    * require("../../fixtures/parsers/typescript-parsers/
		    * tagged-template-with-generic/tagged-template-with-generic-3")
		    * }, */
	...
	let fail = vec![
		...
		"const x = aaaa<\n  test\n>/*\ntest\n*/`foo`", /* {                "parser":
		                                                ...
		                                                * typescript-parsers/
		                                                * tagged-template-with-generic/
		                                                * tagged-template-with-generic-and-comment"
		                                                * )            }, */
		"class C { field1 = obj\n[field2]; }", /* { "ecmaVersion":
		                                        * 2022 }, */
