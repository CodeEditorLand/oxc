	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		match node.kind() {
			...
			AstKind::BinaryExpression(binary_expr) => {
				...
				// the "identifier" will be the characters immediately following the second
				// slash until we reach a non-identifier character
				let ident_name = src
                    .chars()
                    .skip(second_slash + 1)
...
fn test() {
	...
	let pass = vec![
		...
		"
			...
			`;
		", /* {                "parser":
		    * require("../../fixtures/parsers/typescript-parsers/tagged-template-with-generic/
		    * tagged-template-with-generic-1")            }, */
		"
			tag<
				generic
			>`
		...
		", /* {                "parser":
		    * require("../../fixtures/parsers/typescript-parsers/tagged-template-with-generic/
		    * tagged-template-with-generic-2")            }, */
		"
			tag<
				generic
			>`multiline`;
		", /* {                "parser":
		    * require("../../fixtures/parsers/typescript-parsers/tagged-template-with-generic/
		    * tagged-template-with-generic-3")            }, */
