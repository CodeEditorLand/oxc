...
fn test() {
	...
	let pass = vec![
		// While this _would_ be a constant condition in React, ESLint has a policy of not
		// attributing any specific behavior to JSX.
		("<p /> && foo", None),
		("<></> && foo", None),
		("<p /> ?? foo", None),
		("<></> ?? foo", None),
	...
	Tester::new(
		NoConstantBinaryExpression::NAME,
		NoConstantBinaryExpression::CATEGORY,
		pass,
		fail,
	)
	.test_and_snapshot();
