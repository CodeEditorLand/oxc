			...
			AstKind::LogicalExpression(expr) => {
				match expr.operator {
					LogicalOperator::Or | LogicalOperator::And
						if expr.left.is_constant(true, ctx) =>
					{
...
impl NoConstantBinaryExpression {
	///  Test if an AST node has a statically knowable constant nullishness.
	/// Meaning, it will always resolve to a constant value of either: `null`,
	/// `undefined` or not `null` _or_ `undefined`. An expression that can vary
	/// between those three states at runtime would return `false`.
	...
	/// Test if an AST node will always give the same result when _strictly_
	/// compared to a boolean value. This can happen if the expression can
	/// never be boolean, or if it is always the same boolean value.
...
fn test() {
	...
	let pass = vec![
		// While this _would_ be a constant condition in React, ESLint has a
		// policy of not attributing any specific behavior to JSX.
		("<p /> && foo", None),
		("<></> && foo", None),
		("<p /> ?? foo", None),
		("<></> ?? foo", None),
		...
		("true ? foo : bar", None), /* We leave ConditionalExpression for
		...
		// None), ("function Boolean(n) { return Math.random(); }; Boolean(1)
		// == true", None),
		("new Foo() === x", None),
	...
	let fail = vec![
		...
		("x === (y ? {} : [])", None),
		("x === /[a-z]/", None),
		// It's not obvious what this does, but it compares the old value of
		// `x` to the new object.
		("x === (x = {})", None),
