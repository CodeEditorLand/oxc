...
impl Rule for SortImports {
	fn from_configuration(value:serde_json::Value) -> Self {
		...
		let ignore_member_sort = config
			.get("ignoreMemberSort")
			.and_then(serde_json::Value::as_bool)
			.unwrap_or_default();
...
impl SortImports {
	...
	fn check_member_sort_with_fix(&self, current:&ImportDeclaration, ctx:&LintContext) {
		...
		// ESLint check if there are comments in the ImportSpecifier list, and don't
		// rearrange the specifiers. We don't have a direct way to check if there are
		// comments in the specifiers(may need lookahead/lookbehind source text).
		// ```js
		// // this is not fixable in ESLint
		// import { /* comment */ a, b, c, d } from 'foo.js'
		// ```
		// I use ImportStatement's span to check if there are comments between the
		// specifiers.
...
// Calculates number of lines between two nodes. It is assumed that the given
// `left` span appears before the given `right` span in the source code. Lines
// are counted from the end of the `left` span till the start of the `right`
// span. If the given span are on the same line, or `right` span is appears
// before `left` span, it returns `0`.
fn get_number_of_lines_between(left:Span, right:Span, ctx:&LintContext) -> usize {
	...
	// In different lines, need to subtract 2 because the count includes the first
	// and last line.
	count - 2
}
