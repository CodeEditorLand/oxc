...
impl SortImports {
	...
	fn check_member_sort_with_fix(&self, current:&ImportDeclaration, ctx:&LintContext) {
		...
		// ESLint check if there are comments in the ImportSpecifier list, and
		// don't rearrange the specifiers. We don't have a direct way to check
		// if there are comments in the specifiers(may need lookahead/lookbehind
		// source text). ```js
		// // this is not fixable in ESLint
		// import { /* comment */ a, b, c, d } from 'foo.js'
		// ```
		// I use ImportStatement's span to check if there are comments between
		// the specifiers.
		let is_fixable = !ctx.semantic().trivias().has_comments_between(current.span);

		if is_fixable {
			// Safe to index because we know that `specifiers` is at least 2
			// element long
