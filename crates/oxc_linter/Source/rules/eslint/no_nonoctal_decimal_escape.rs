...
fn replacement(escape_sequence:&str, replacement:&str, span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!("Don't use '{escape_sequence}' escape sequence."))
		.with_help(format!(
			"Replace '{escape_sequence}' with '{replacement}'. This maintains the current \
			 functionality."
		))
		.with_label(span)
}
...
fn escape_backslash(escape_sequence:&str, replacement:&str, span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!("Don't use '{escape_sequence}' escape sequence."))
		.with_help(format!(
			"Replace '{escape_sequence}' with '{replacement}' to include the actual backslash \
			 character."
		))
		.with_label(span)
}
...
trait StickyRegex {
	fn sticky_captures<'h>(&self, haystack:&'h str, start:usize) -> (Option<Captures<'h>>, usize);
}
