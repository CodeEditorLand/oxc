...
impl GetterReturn {
	...
	fn run_diagnostic<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>, span:Span) {
		...
		let definitely_returns_in_all_codepaths = 'returns: {
			...
			// Therefore, if a function is an expression, it always returns its
			// value.
			//
			// Example expression:
			// ```js
			...
			if let AstKind::ArrowFunctionExpression(arrow_expr) = node.kind() {
				...
				// If the signature of function supports the return of the
				// `undefined` value, you do not need to check this rule
			...
			let output = set_depth_first_search(graph, Some(node.cfg_id()), |event| {
				match event {
					...
					DfsEvent::TreeEdge(a, b) => {
						...
						if edges.iter().any(|e| {
							matches!(
								e.weight(),
								EdgeType::Normal
									| EdgeType::Jump | EdgeType::Error(ErrorEdgeKind::Explicit)
							)
						}) {
							Control::Continue
						} else {
					...
					DfsEvent::Discover(basic_block_id, _) => {
						...
						// Return true as the second argument to signify we
						// should continue walking this branch, as we
						// haven't seen anything that will signify to
						// us that this path of the program will
						// definitely return or throw.
						if graph.edges_directed(basic_block_id, Direction::Outgoing).any(|e| {
							matches!(
								e.weight(),
								EdgeType::Jump
									| EdgeType::Normal | EdgeType::Backedge
...
fn test() {
	...
	let pass = vec![
		...
		(
			"Reflect.defineProperty(foo, \"bar\", { get: function () { ~function (){ return true; \
			 }();return true;}});",
			None,
		),
        ...
        Object.defineProperty(targetWindow, 'vscodeOriginalClearTimeout', { get: () => \
