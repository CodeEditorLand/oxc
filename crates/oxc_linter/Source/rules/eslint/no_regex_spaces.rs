...
impl NoRegexSpaces {
	...
	// For skipping if there aren't any consecutive spaces in the source, to avoid
	// reporting cases where the space is explicitly escaped, like: `RegExp(' \
	// ')``.
	fn has_double_space(input:&str) -> bool { input.contains("  ") }
}
