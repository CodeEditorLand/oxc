...
impl NoRegexSpaces {
	...
	// For skipping if there aren't any consecutive spaces in the source, to
	// avoid reporting cases where the space is explicitly escaped, like:
	// `RegExp(' \ ')``.
	fn has_double_space(input:&str) -> bool { DOUBLE_SPACE.is_match(input) }
}
...
impl<'a> Visit<'a> for ConsecutiveSpaceFinder {
	...
	fn visit_character(&mut self, ch:&Character) {
		...
		if let Some(ref mut space_span) = self.last_space_span {
			...
			// If it is not consecutive, and the last space is only one space,
			// move it up
