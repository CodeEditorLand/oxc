	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		match node.kind() {
			...
			AstKind::AssignmentExpression(expr) if self.config == NoCondAssignConfig::Always => {
				...
				// Only report the diagnostic if the assignment is in a span where it should not
				// be. For example, report `if (a = b) { ... }`, not `if (...) { a = b }`
...
impl NoCondAssign {
	...
	fn emit_diagnostic(ctx:&LintContext<'_>, expr:&AssignmentExpression<'_>) {
		...
		let start = operator_span
			.source_text(ctx.source_text())
			.find(expr.operator.as_str())
			.unwrap_or(0) as u32;
...
fn test() {
	...
	let pass = vec![
		...
		(
			"const nums = [1,2,3]; for(let i = 0; i < nums.length; i += 1) { dosomething(); }",
			None,
		),
		(
