...
fn test() {
	...
	let pass = vec![
		...
		// None), ("function* foo() { for (let x = yield; x < 10; x++)
		// {yield;}yield;}", None), ("function* foo() { for (let x = yield; ;
		// x++) { yield; }}", None),
	];
	...
	let fail = vec![
		...
		// re-assign, but that's an obscure enough edge case to not worry
		// about)
		("if (undefined) {}", None),
		...
		// ("function foo() {while (true) {function* bar() {while (true)
		// {yield;}}}}", None), ("function foo() {while (true) {const bar =
		// function*() {while (true) {yield;}}}}", None), ("function* foo() {
		// for (let foo = 1 + 2 + 3 + (yield); true; baz) {}}", None),
	];
