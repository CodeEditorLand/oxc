...
impl Rule for NoRestrictedGlobals {
	fn from_configuration(value:serde_json::Value) -> Self {
		let list = match value {
			Value::Array(arr) => {
				arr.iter().fold(FxHashMap::default(), |mut acc, v| {
					match v {
						// "no-restricted-globals": ["error", "event"]
						Value::String(name) => {
						...
						// "no-restricted-globals": ["error", { "name": "event", "message": "Use
						// local parameter instead." }]
						Value::Object(obj) => {
...
fn test() {
	...
	let pass = vec![
		...
		(
			"foo",
			Some(serde_json::json!([{ "name": "bar", "message": "Use baz instead." }])),
		),
	];
