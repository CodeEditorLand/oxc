...
use secrets::{ALL_RULES, CustomSecret, SecretsEnum};
...
pub struct ApiKeysInner {
	...
	/// This is a performance optimization to avoid checking each rule for every
	/// string.
	min_len:NonZeroU32,
...
impl Rule for ApiKeys {
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		for rule in &self.rules {
			...
			// This clone allocs no memory and so is relatively cheap. rustc should optimize
			// it away anyways.
