...
pub struct ApiKeysInner {
	...
	/// This is a performance optimization to avoid checking each rule for
	/// every string.
	min_len:NonZeroU32,
...
impl Default for ApiKeysInner {
	fn default() -> Self { Self::new(ALL_RULES.clone()) }
}
...
impl Rule for ApiKeys {
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		// skip strings that are below the length/entropy threshold of _all_
		// rules. Perf optimization, avoid O(n) len/entropy checks (for n
		// rules)
		...
		for rule in &self.rules {
			// order here is important: they're in order of cheapest to most
			// expensive
			...
			// This clone allocs no memory and so is relatively cheap. rustc
			// should optimize it away anyways.
