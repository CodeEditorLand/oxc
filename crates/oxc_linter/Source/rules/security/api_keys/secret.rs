...
/// Metadata trait separated out of [`SecretScanner`]. The easiest way to
/// implement this is with the [`oxc_macros::declare_oxc_secret!`] macro.
pub trait SecretScannerMeta {
	/// Human-readable unique identifier describing what service this rule finds
	/// api keys for. Must be kebab-case.
	fn rule_name(&self) -> Cow<'static, str>;

	fn message(&self) -> Cow<'static, str>;

	/// Min str length a key candidate must have to be considered a violation.
	/// Must be >= 1.
	#[inline]
	fn min_len(&self) -> NonZeroU32 { DEFAULT_MIN_LEN }

	/// Secret candidates above this length will not be considered.
	///
	/// By default, no maximum length is enforced.
	#[inline]
	fn max_len(&self) -> Option<NonZeroU32> { None }

	/// Min entropy a key must have to be considered a violation. Must be >= 0.
	///
	/// Defaults to 0.5
	#[inline]
	fn min_entropy(&self) -> f32 { DEFAULT_MIN_ENTROPY }
}

/// Detects hard-coded API keys and other credentials of a single kind or for a
/// single SaaS service.
pub trait SecretScanner: SecretScannerMeta {
	...
	/// `verify` lets secret rules modify diagnostic messages and/or perform
	/// additional verification checks on secrets before they are reported. You
	/// may mutate state such as the diagnostic message, but you _must not_
	/// modify the secret itself as it is shared between all rules.
	///
	/// Returns `true` to report the violation, or `false` to ignore it.
	#[inline]
	fn verify(&self, violation:&mut SecretViolation<'_>) -> bool { true }
}
...
impl Entropy for Secret<'_> {
	#[inline]
	fn entropy(&self) -> f32 { self.entropy }
}
...
impl<'a> SecretViolation<'a> {
	...
	pub fn rule_name(&self) -> &str { &self.rule_name }
}

impl GetSpan for SecretViolation<'_> {
	#[inline]
	fn span(&self) -> Span { self.secret.span() }
}
