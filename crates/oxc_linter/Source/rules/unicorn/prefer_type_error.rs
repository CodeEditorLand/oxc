...
const TYPE_CHECKING_GLOBAL_IDENTIFIERS:phf::Set<&'static str> = phf::phf_set!("isFinite", "isNaN",);
...
fn test() {
	...
	let fail = vec![
        ...
        ",
		// disabled for now - not sure how do differentiate between `throw new
		// Error` and `throw new Error()` r#"                if
		// (wrapper._.isElement(foo)) {             throw new TypeError;
		//         }
		//         "#,
		r#"
	...
	Tester::new(PreferTypeError::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
