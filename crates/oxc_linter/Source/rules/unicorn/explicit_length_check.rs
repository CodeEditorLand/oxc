...
fn test() {
	...
	let pass = vec![
		...
		(
			"if (foo.length > 0) {}",
			Some(serde_json::json!([{"non-zero": "greater-than"}])),
		),
		...
		// (r#"const foo = { size: "small" }; if (foo.size) {}"#, None), // Not
		// a number ("const foo = { length: -1 }; if (foo.length) {}", None),
		// // Array lengths cannot be negative ("const foo = { length: 1.5 };
		// if (foo.length) {}", None), // Array lengths must be integers
		// ("const foo = { length: NaN }; if (foo.length) {}", None), // Array
		// lengths cannot be NaN ("const foo = { length: Infinity }; if
		// (foo.length) {}", None), // Array lengths cannot be Infinity
		// Logical OR
		...
		(
			"const totalCount = tests.reduce((count, test) => count + (test.enabled ? \
			 test.maxSize : test.size), 0)",
			None,
		),
	];
	...
	let fail = vec![
		...
		// need getStaticValue
		// (r#"const NON_NUMBER = "2"; const x = foo.length || NON_NUMBER"#,
		// None),
		(
...
!( foo.length || !!foo.length || foo.length !== 0 || foo.length != 0 || foo.length >= 1 || 0 !== foo.length || 0 != foo.length || 0 < foo.length || 1 <= foo.length )
			...
			"if ( !!!( foo.length === 0 && foo.length === 0 && foo.length === 0 && foo.length === \
			 0 && foo.length === 0 && foo.length === 0 ) ||
!( foo.length > 0 || foo.length > 0 || foo.length > 0 || foo.length > 0 || foo.length > 0 || \
			 foo.length > 0 || foo.length > 0 || foo.length > 0 || foo.length > 0 )
            ) {}",
			None,
		),
		(
			"if ( foo.length || !!foo.length || foo.length != 0 || foo.length > 0 || foo.length \
			 >= 1 || 0 !== foo.length || 0 != foo.length || 0 < foo.length || 1 <= foo.length ) {}",
			"if ( foo.length !== 0 || foo.length !== 0 || foo.length !== 0 || foo.length !== 0 || \
			 foo.length !== 0 || foo.length !== 0 || foo.length !== 0 || foo.length !== 0 || \
			 foo.length !== 0 ) {}",
			Some(serde_json::json!([{"non-zero": "not-equal"}])),
		),
		...
		// ("const bar = void !foo.length;", "const bar = void (foo.length ===
		// 0);", None),
		(
		...
		(
			...
			"function foo() {return foo.length === 0}",
			None,
		),
		(
			"function foo() {throw!foo.length}",
			"function foo() {throw foo.length === 0}",
			None,
		),
		(
		...
		(
			...
			"function * foo() {yield foo.length === 0}",
			None,
		),
		(
			"function * foo() {yield*!foo.length}",
	...
	Tester::new(ExplicitLengthCheck::NAME, pass, fail)
		.expect_fix(fixes)
		.test_and_snapshot();
}
