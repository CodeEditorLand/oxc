...
fn test() {
	...
	let pass = vec![
		...
		// ("const foo = { length: -1 }; if (foo.length) {}", None), // Array lengths cannot be
		// negative ("const foo = { length: 1.5 }; if (foo.length) {}", None), // Array lengths
		// must be integers ("const foo = { length: NaN }; if (foo.length) {}", None), // Array
		// lengths cannot be NaN ("const foo = { length: Infinity }; if (foo.length) {}", None),
		// // Array lengths cannot be Infinity Logical OR
