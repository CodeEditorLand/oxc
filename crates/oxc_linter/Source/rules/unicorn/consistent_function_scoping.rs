...
impl Default for ConsistentFunctionScopingConfig {
	fn default() -> Self { Self { check_arrow_functions:true } }
}
...
impl Rule for ConsistentFunctionScoping {
	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
				...
				AstKind::Function(function) => {
					...
					if let Some(func_scope_id) = function.scope_id.get() {
						if let Some(parent_scope_id) = ctx.scopes().get_parent_id(func_scope_id) {
							...
							// The bar function scope id is 1. In order to
							// ignore this rule, its parent's scope id
							// (in this case `foo`'s scope id is 0 and is equal
							// to root scope id) should be considered.
		...
		// if the function is declared at the root scope, we don't need to check
		// anything
...
impl<'a> Visit<'a> for ReferencesFinder {
	...
	fn visit_jsx_element_name(&mut self, _it:&oxc_ast::ast::JSXElementName<'a>) {
		...
		// No need to walk children as only references they may contain are also
		// JSX identifiers.
	}
...
fn test() {
	...
	let pass = vec![
		...
		("foo => foo;", None),
		(
			"function doFoo(foo) { function doBar(bar) { return foo + bar; } return foo; }",
			None,
		),
		(
		...
		(
			"for (let foo = 0; foo < 1; foo++) { function doBar(bar) { return bar + foo; } }",
			None,
		),
		(
			"let foo = 0;
		...
		(
			"function doFoo(Foo) { function doBar() { return new Foo(); } return doBar; };",
			None,
		),
		...
		("function doFoo(Foo) { const doBar = () => () => this; return doBar(); };", None),
		(
			"function doFoo(Foo) { const doBar = () => () => () => this; return doBar(); };",
			None,
		),
	...
	let fail = vec![
		...
		("function doFoo() { const doBar = function(bar) { return bar; }; }", None),
		(
			"function doFoo() { const doBar = function(bar) { return bar; }; doBar(); }",
			None,
		),
		("const doFoo = () => { const doBar = bar => { return bar; } }", None),
		(
			"function doFoo(Foo) { function doBar() { return this; } return doBar(); };",
			None,
		),
		(
			"function doFoo(Foo) { const doBar = () => (function() {return this})(); return \
			 doBar(); };",
			None,
		),
		(
		...
		(
			"for (let foo = 0; foo < 1; foo++) { function doBar(bar) { return bar; } }",
			None,
		),
