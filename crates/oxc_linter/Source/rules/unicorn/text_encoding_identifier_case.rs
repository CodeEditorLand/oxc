...
fn is_jsx_meta_elem_with_charset_attr(id:NodeId, ctx:&LintContext) -> bool {
	...
	let Some(tag_name) = opening_elem.name.get_identifier_name() else {
		return false;
	};
...
fn test() {
	...
	let fix = vec![
		...
		(
			r#"fs.promises.readFile(file, "UTF-8",)"#,
			r#"fs.promises.readFile(file, "utf8",)"#,
		),
	...
	Tester::new(
		TextEncodingIdentifierCase::NAME,
		TextEncodingIdentifierCase::CATEGORY,
		pass,
		fail,
	)
	.expect_fix(fix)
