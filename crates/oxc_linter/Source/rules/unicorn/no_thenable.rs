...
fn object(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Do not add `then` to an object.")
		.with_help(
			"If an object is defined as 'thenable', once it's accidentally used in an await \
			 expression, it may cause problems",
		)
		.with_label(span)
}
...
fn check_expression(expr:&Expression, ctx:&LintContext<'_>) -> Option<oxc_span::Span> {
	match expr {
		...
		Expression::TemplateLiteral(lit) => {
			lit.quasi()
				.and_then(|quasi| if quasi == "then" { Some(lit.span) } else { None })
		},
