	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		match parent.kind() {
			AstKind::ForStatement(for_stmt) => {
				...
				// Check if the `test` of the for statement is the same node as
				// the call expression.
			...
			AstKind::ConditionalExpression(conditional_expr) => {
				...
				// Check if the `test` of the conditional expression is the same
				// node as the call expression.
				if !std::ptr::eq(call_expr2.as_ref(), call_expr) {
					return;
				}
			},
...
fn test() {
	...
	let fix = vec![
		(
			...
			"const re = /a/; const bar = !foo.test(re)",
		),
		(
		...
		(
			...
			"const re = /a/; const bar = !re.test(foo)",
		),
		(
	...
	Tester::new(PreferRegexpTest::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
