...
fn spread_in_arguments(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Using a spread operator here creates a new array unnecessarily.")
		.with_help(
			"This function accepts a rest parameter, it's unnecessary to create a new array and \
			 then spread it. Instead, supply the arguments directly.\nFor example, replace \
			 `foo(...[1, 2, 3])` with `foo(1, 2, 3)`.",
		)
		.with_label(span)
}
...
fn iterable_to_array(span:Span, ctor_name:&str) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!(
		"`{ctor_name}` accepts an iterable, so it's unnecessary to convert the iterable to an \
		 array."
	))
	.with_help("Consider removing the spread operator.")
	.with_label(span)
...
fn iterable_to_array_in_yield_star(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Using a spread operator here creates a new array unnecessarily.")
		.with_help(
			"`yield*` can delegate to another iterable, so it's unnecessary to convert the \
...
fn clone(span:Span, is_array:bool, method_name:Option<&str>) -> OxcDiagnostic {
	...
	OxcDiagnostic::warn(format!("Using a spread operator here creates a new {noun} unnecessarily."))
		.with_help(if let Some(method_name) = method_name {
			format!(
				"`{method_name}` returns a new {noun}. Spreading it into an {noun} expression to \
				 create a new {noun} is redundant."
			)
		} else {
			format!(
				"This expression returns a new {noun}. Spreading it into an {noun} expression to \
				 create a new {noun} is redundant."
			)
		})
		.with_label(span)
...
fn test() {
	...
	let pass = vec![
		...
		r"[...not.array()]",
		r"[...array.unknown()]",
		r"const arr = [1, 2, 3]; const unique = [...arr];", /* valid method
		                                                     * to shallow-clone
		                                                     * an array */
	...
	Tester::new(NoUselessSpread::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
