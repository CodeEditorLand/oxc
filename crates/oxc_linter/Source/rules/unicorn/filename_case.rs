...
impl Rule for FilenameCase {
	fn from_configuration(value:serde_json::Value) -> Self {
		...
		let off = Self { kebab_case:false, camel_case:false, snake_case:false, pascal_case:false };
	...
	fn run_once<'a>(&self, ctx:&LintContext<'_>) {
		...
		let mut enabled_cases = cases.iter().filter(|(enabled, ..)| *enabled);
	...
	fn test_cases<const N: usize>(
		...
		(
			...
			if casings.is_empty() {
				...
				// turn ["camelCase", "snakeCase"] into [{ "cases": { "camelCase": true,
				// "snakeCase": true } }]
