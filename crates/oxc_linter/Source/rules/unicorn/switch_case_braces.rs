			...
			for case_consequent in &case.consequent {
				match case_consequent {
					...
					_ => {
						...
						// After first incorrect consequent we have to break to
						// not repeat the work
						break;
					},
				}
...
fn test() {
	...
	let fail = vec![
		...
		"switch(foo) { case 1: { /* fallthrough */ } default: {}/* fallthrough */ case 3: { \
		 doSomething(); break; } }",
		"switch(foo) { default: doSomething(); }",
		"switch(foo) { case 1: { doSomething(); } break; /* <-- This should be between braces */ }",
		"switch(foo) { default: label: {} }",
		"switch(something) { case 1: case 2: { console.log('something'); break; } case 3: \
	...
	Tester::new(SwitchCaseBraces::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
