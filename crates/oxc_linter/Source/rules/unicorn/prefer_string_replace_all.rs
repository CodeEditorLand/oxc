...
fn use_replace_all(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(
		"Prefer `String#replaceAll()` over `String#replace()` when using a regex with the global \
		 flag.",
	)
	.with_label(span)
}
			...
			"replaceAll" => {
				if let Some(k) = get_pattern_replacement(pattern, ctx) {
					ctx.diagnostic_with_fix(string_literal(pattern.span(), &k), |fixer| {
						// foo.replaceAll(/hello world/g, bar) =>
						// foo.replaceAll("hello world", bar)
...
fn get_pattern_replacement<'a>(expr:&'a Argument<'a>, ctx:&LintContext<'a>) -> Option<CompactStr> {
...
fn test() {
	...
	let fail = vec![
		...
		// r#"const pattern = new RegExp("foo", "g"); foo.replace(pattern,
		// bar)"#,
	...
	Tester::new(PreferStringReplaceAll::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
