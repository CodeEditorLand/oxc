...
fn no_useless_fallback_in_spread_diagnostic(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Empty fallbacks in spreads are unnecessary")
		.with_help(
			"Spreading falsy values in object literals won't add any unexpected properties, so \
			 it's unnecessary to add an empty object as fallback.",
		)
		.with_label(span)
}
...
fn test() {
	...
	Tester::new(NoUselessFallbackInSpread::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
