...
fn no_instanceof_array_diagnostic(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Use `Array.isArray()` instead of `instanceof Array`.")
		.with_help(
			"The instanceof Array check doesn't work across realms/contexts, for example, \
			 frames/windows in browsers or the vm module in Node.js.",
		)
		.with_label(span)
}
...
fn test() {
	...
	Tester::new(NoInstanceofArray::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
