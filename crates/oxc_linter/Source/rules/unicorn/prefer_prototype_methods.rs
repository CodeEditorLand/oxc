...
impl Rule for PreferPrototypeMethods {
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		// TODO: Replace `static_property_name` with a function similar to
		// `getPropertyName` in @eslint-community/eslint-utils to generate
...
fn test() {
	...
	let fail = vec![
		...
		r#"const method = "realMethodName";const foo = [][method].call(foo)"#, /* TODO: Improve
		                                                                        * error message
		                                                                        * for this case. */
		...
		"Reflect.apply([][Symbol()], baz, [])",
		r#"Reflect.apply({}[Symbol("symbol description")], baz, [])"#,
		"[][Symbol.iterator].call(foo)", /* TODO: Improve error message for
		                                  * this case. */
	...
	let fix = vec![
		...
		(
			"const foo = [][method].call(foo)",
			"const foo = Array.prototype[method].call(foo)",
			None,
		),
		(
		...
		(
			"Reflect.apply([].bar, baz, [])",
			"Reflect.apply(Array.prototype.bar, baz, [])",
			None,
		),
		(
			"const foo = ({}).toString.call(bar);",
		...
		(
			...
			"Array.prototype[Symbol.iterator].call(foo)",
			None,
		),
	...
	Tester::new(PreferPrototypeMethods::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
