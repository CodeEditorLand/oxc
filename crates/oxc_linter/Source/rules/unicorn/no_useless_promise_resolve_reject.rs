...
fn resolve(span:Span, preferred:&str) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!(
		"Prefer `{preferred} value` over `{preferred} Promise.resolve(value)`."
	))
	.with_help(
		"Wrapping the return value in `Promise.Resolve` is needlessly verbose. All return values \
		 in async functions are already wrapped in a `Promise`.",
	)
	.with_label(span)
}
...
fn reject(span:Span, preferred:&str) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!("Prefer `throw error` over `{preferred} Promise.reject(error)`."))
		.with_help(
			"Wrapping the error in `Promise.reject` is needlessly verbose. All errors thrown in \
			 async functions are already wrapped in a `Promise`.",
		)
		.with_label(span)
}
	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		match parent.kind() {
			...
			AstKind::ExpressionStatement(_) => {
				...
				let AstKind::FunctionBody(function_body) = grand_parent.kind() else {
					return;
				};
...
fn match_arrow_function_body<'a>(ctx:&LintContext<'a>, parent:&AstNode<'a>) -> bool {
	match ctx.nodes().parent_node(parent.id()) {
		Some(arrow_function_body) => {
			match arrow_function_body.kind() {
				AstKind::FunctionBody(_) => {
					match ctx.nodes().parent_node(arrow_function_body.id()) {
						Some(arrow_function) => {
...
fn test() {
	...
	let fix = vec![
		...
		// Promise#then/catch/finally callbacks returning
