		...
		ctx.diagnostic_with_fix(
			...
			|fixer| {
				...
				if is_member_expression && is_decimal_integer {
					...
					// If tokenBefore is a Punctuator (e.g., a symbol like ;, ],
					// or )), it determines whether a semicolon is necessary
					// based on the context (e.g., the type of the last block
					// node). If the token type is in
					// tokenTypesNeedsSemicolon, it returns true (semicolon
					// needed). Special cases like Template strings,
					// ObjectExpression blocks, and certain Identifier cases are
					// handled explicitly. https://github.com/sindresorhus/eslint-plugin-unicorn/blob/77f32e5a6b2df542cf50dfbd371054f2cd8ce2d6/rules/no-zero-fractions.js#L56
				}

				// Handle special cases where a space is needed after certain
				// keywords to prevent the number from being interpreted as
				// a property access
...
fn format_raw(raw:&str) -> Option<(String, bool)> {
	...
	let formatted = format!(
		"{}{}{}{}",
		if before.is_empty() && fixed_dot_and_fractions.is_empty() {
			"0"
		} else {
			before
		},
...
fn test() {
	...
	Tester::new(NoZeroFractions::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
