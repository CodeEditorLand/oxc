	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		let problem_node =
			match node.kind() {
						...
						ExportDefaultDeclarationKind::ParenthesizedExpression(expr) => {
							...
							match expr {
								Expression::ClassExpression(class_expr) => {
									class_expr.id.as_ref().map_or(
										Some((class_expr.span, ErrorNodeKind::Class)),
										|_| None,
									)
								},
								Expression::FunctionExpression(func_expr) => {
									func_expr.id.as_ref().map_or(
										Some((func_expr.span, ErrorNodeKind::Function)),
										|_| None,
									)
								},
								_ => None,
				...
				AstKind::AssignmentExpression(expr) if is_common_js_export(expr) => {
					match &expr.right {
						Expression::ClassExpression(class_expr) => {
							class_expr
								.id
								.as_ref()
								.map_or(Some((expr.span, ErrorNodeKind::Class)), |_| None)
						},
						Expression::FunctionExpression(function_expr) => {
							function_expr
								.id
								.as_ref()
