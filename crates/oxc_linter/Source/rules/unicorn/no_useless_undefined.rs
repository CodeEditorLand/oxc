...
fn no_useless_undefined_diagnostic(span:Span) -> OxcDiagnostic { warn().with_label(span) }
...
impl Default for NoUselessUndefined {
	fn default() -> Self { Self { check_arguments:true, check_arrow_function_body:true } }
}
	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		match node.kind() {
				...
				if undefined_literal.name == "undefined" =>
			{
				...
				let Some(parent_node) = ctx.nodes().parent_node(parent_node.id()) else {
					return;
				};
			...
			AstKind::CallExpression(call_expr) => {
				...
				// If all arguments removed, and there is trailing comma, we
				// need remove it.
				if remaining_count == 0 {
...
fn test() {
	...
	let pass = vec![
		...
		(r"function foo(bar) {}", None),
		// I guess nobody uses this, but `yield* undefined;` is valid code, and
		// `yield*;` is not
	...
	Tester::new(NoUselessUndefined::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
