...
impl Rule for PreferNegativeIndex {
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		let Some(name) = call_expr.callee_name() else {
			return;
		};
...
fn test() {
	...
	let fail = vec![
		...
		"
        							...
        							ArrayBuffer.prototype.slice.call(foo, foo.length - 1, foo.length - 2, foo.length - \
		 3);

        							ArrayBuffer.prototype.splice.call(foo, foo.length - 1, foo.length - 2, foo.length - \
		 3);

        							Int8Array.prototype.slice.call(foo, foo.length - 1, foo.length - 2, foo.length - 3);

        							Int8Array.prototype.splice.call(foo, foo.length - 1, foo.length - 2, foo.length - \
		 3);

        							Uint8Array.prototype.slice.call(foo, foo.length - 1, foo.length - 2, foo.length - \
		 3);

        							Uint8Array.prototype.splice.call(foo, foo.length - 1, foo.length - 2, foo.length - \
		 3);

        							Uint8ClampedArray.prototype.slice.call(foo, foo.length - 1, foo.length - 2, \
		 foo.length - 3);

        							Uint8ClampedArray.prototype.splice.call(foo, foo.length - 1, foo.length - 2, \
		 foo.length - 3);

        							Int16Array.prototype.slice.call(foo, foo.length - 1, foo.length - 2, foo.length - \
		 3);

        							Int16Array.prototype.splice.call(foo, foo.length - 1, foo.length - 2, foo.length - \
		 3);

        							Uint16Array.prototype.slice.call(foo, foo.length - 1, foo.length - 2, foo.length - \
		 3);

        							Uint16Array.prototype.splice.call(foo, foo.length - 1, foo.length - 2, foo.length - \
		 3);

        							Int32Array.prototype.slice.call(foo, foo.length - 1, foo.length - 2, foo.length - \
		 3);

        							Int32Array.prototype.splice.call(foo, foo.length - 1, foo.length - 2, foo.length - \
		 3);

        							Uint32Array.prototype.slice.call(foo, foo.length - 1, foo.length - 2, foo.length - \
		 3);

        							Uint32Array.prototype.splice.call(foo, foo.length - 1, foo.length - 2, foo.length - \
		 3);

        							Float32Array.prototype.slice.call(foo, foo.length - 1, foo.length - 2, foo.length - \
		 3);

        							Float32Array.prototype.splice.call(foo, foo.length - 1, foo.length - 2, foo.length \
		 - 3);

        							Float64Array.prototype.slice.call(foo, foo.length - 1, foo.length - 2, foo.length - \
		 3);

        							Float64Array.prototype.splice.call(foo, foo.length - 1, foo.length - 2, foo.length \
		 - 3);

        							BigInt64Array.prototype.slice.call(foo, foo.length - 1, foo.length - 2, foo.length \
		 - 3);

        							BigInt64Array.prototype.splice.call(foo, foo.length - 1, foo.length - 2, foo.length \
		 - 3);

        							BigUint64Array.prototype.slice.call(foo, foo.length - 1, foo.length - 2, foo.length \
		 - 3);

        							BigUint64Array.prototype.splice.call(foo, foo.length - 1, foo.length - 2, \
		 foo.length - 3);

        							NOT_SUPPORTED.prototype.slice.call(foo, foo.length - 1, foo.length - 2, foo.length \
		 - 3);

        							NOT_SUPPORTED.prototype.splice.call(foo, foo.length - 1, foo.length - 2, foo.length \
		 - 3);
        						",
		"
        							...
        							String.prototype.splice.apply(foo, [foo.length - 1, foo.length - 2, foo.length - \
		 3]);

        							ArrayBuffer.prototype.slice.apply(foo, [foo.length - 1, foo.length - 2, foo.length \
		 - 3]);

        							ArrayBuffer.prototype.splice.apply(foo, [foo.length - 1, foo.length - 2, foo.length \
		 - 3]);

        							Int8Array.prototype.slice.apply(foo, [foo.length - 1, foo.length - 2, foo.length - \
		 3]);

        							Int8Array.prototype.splice.apply(foo, [foo.length - 1, foo.length - 2, foo.length - \
		 3]);

        							Uint8Array.prototype.slice.apply(foo, [foo.length - 1, foo.length - 2, foo.length - \
		 3]);

        							Uint8Array.prototype.splice.apply(foo, [foo.length - 1, foo.length - 2, foo.length \
		 - 3]);

        							Uint8ClampedArray.prototype.slice.apply(foo, [foo.length - 1, foo.length - 2, \
		 foo.length - 3]);

        							Uint8ClampedArray.prototype.splice.apply(foo, [foo.length - 1, foo.length - 2, \
		 foo.length - 3]);

        							Int16Array.prototype.slice.apply(foo, [foo.length - 1, foo.length - 2, foo.length - \
		 3]);

        							Int16Array.prototype.splice.apply(foo, [foo.length - 1, foo.length - 2, foo.length \
		 - 3]);

        							Uint16Array.prototype.slice.apply(foo, [foo.length - 1, foo.length - 2, foo.length \
		 - 3]);

        							Uint16Array.prototype.splice.apply(foo, [foo.length - 1, foo.length - 2, foo.length \
		 - 3]);

        							Int32Array.prototype.slice.apply(foo, [foo.length - 1, foo.length - 2, foo.length - \
		 3]);

        							Int32Array.prototype.splice.apply(foo, [foo.length - 1, foo.length - 2, foo.length \
		 - 3]);

        							Uint32Array.prototype.slice.apply(foo, [foo.length - 1, foo.length - 2, foo.length \
		 - 3]);

        							Uint32Array.prototype.splice.apply(foo, [foo.length - 1, foo.length - 2, foo.length \
		 - 3]);

        							Float32Array.prototype.slice.apply(foo, [foo.length - 1, foo.length - 2, foo.length \
		 - 3]);

        							Float32Array.prototype.splice.apply(foo, [foo.length - 1, foo.length - 2, \
		 foo.length - 3]);

        							Float64Array.prototype.slice.apply(foo, [foo.length - 1, foo.length - 2, foo.length \
		 - 3]);

        							Float64Array.prototype.splice.apply(foo, [foo.length - 1, foo.length - 2, \
		 foo.length - 3]);

        							BigInt64Array.prototype.slice.apply(foo, [foo.length - 1, foo.length - 2, \
		 foo.length - 3]);

        							BigInt64Array.prototype.splice.apply(foo, [foo.length - 1, foo.length - 2, \
		 foo.length - 3]);

        							BigUint64Array.prototype.slice.apply(foo, [foo.length - 1, foo.length - 2, \
		 foo.length - 3]);

        							BigUint64Array.prototype.splice.apply(foo, [foo.length - 1, foo.length - 2, \
		 foo.length - 3]);

        							NOT_SUPPORTED.prototype.slice.apply(foo, [foo.length - 1, foo.length - 2, \
		 foo.length - 3]);

        							NOT_SUPPORTED.prototype.splice.apply(foo, [foo.length - 1, foo.length - 2, \
		 foo.length - 3]);
        						",
		"/**/foo.slice(foo.length - 2, foo.length - 1)",
		"/**/foo.splice(foo.length - 1, 1)",
