		...
		for (i, arg) in call_expr.arguments.iter().enumerate() {
			if let Some(expression_arg) = arg.as_expression() {
				...
				if check_literal_leading(i, literal_raw) {
					report_diagnostic(
						"leading",
						// SAFETY: `is_method_call` ensures that `call_expr`'s `callee` is a
						// `MemberExpression` with a `MemberExpression` as its `object`.
...
fn test() {
	...
	let fail = vec![
		...
		// Note: This behavior differs to `eslint-plugin-unicorn(no-console-spaces)` as it "passes"
		// there.
		("console[\"log\"](\" a \", \" b \");", None),
		("console[\"debug\"](\" a \", \" b \");", None),
		("console[\"info\"](\" a \", \" b \");", None),
