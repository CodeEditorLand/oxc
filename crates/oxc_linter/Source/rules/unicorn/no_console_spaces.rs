...
fn no_console_spaces_diagnostic(
	...
	.with_help(
		"The `console.log()` method and similar methods join the parameters with a space so \
		 adding a leading/trailing space to a parameter, results in two spaces being added.",
	)
	.with_label(span)
}
		...
		for (i, arg) in call_expr.arguments.iter().enumerate() {
			if let Some(expression_arg) = arg.as_expression() {
				...
				if check_literal_leading(i, literal_raw) {
					report_diagnostic(
						"leading",
						// SAFETY: `is_method_call` ensures that `call_expr`'s
						// `callee` is a `MemberExpression` with a
						// `MemberExpression` as its `object`.
...
fn check_literal_leading(i:usize, literal:&str) -> bool { i != 0 && literal.starts_with(' ') }
...
fn test() {
	...
	let fail = vec![
		...
		// Note: This behavior differs to
		// `eslint-plugin-unicorn(no-console-spaces)` as it "passes" there.
		("console[\"log\"](\" a \", \" b \");", None),
		("console[\"debug\"](\" a \", \" b \");", None),
		("console[\"info\"](\" a \", \" b \");", None),
		("console[\"warn\"](\" a \", \" b \");", None),
	...
	Tester::new(NoConsoleSpaces::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
