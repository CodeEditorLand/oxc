...
impl ValueHint {
	...
	#[inline]
	pub fn is_object(&self) -> bool { matches!(self, Self::NewObject) }

	#[inline]
	pub fn is_array(&self) -> bool { matches!(self, Self::NewArray) }
}
...
impl<'a> ConstEval for ConditionalExpression<'a> {
	fn const_eval(&self) -> ValueHint { self.consequent.const_eval() & self.alternate.const_eval() }
}
...
/// Matches `<expr>.split(...)`, which usually is
/// `String.prototype.split(pattern)`
