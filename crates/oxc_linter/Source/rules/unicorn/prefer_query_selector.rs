...
fn prefer_query_selector_diagnostic(good_method:&str, bad_method:&str, span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!("Prefer `.{good_method}()` over `.{bad_method}()`."))
		.with_help(
			"It's better to use the same method to query DOM elements. This helps keep \
			 consistency and it lends itself to future improvements (e.g. more specific \
			 selectors).",
		)
		.with_label(span)
}
...
fn test() {
	...
	Tester::new(PreferQuerySelector::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
