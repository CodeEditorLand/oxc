...
fn catch_error_name_diagnostic(caught_ident:&str, expected_name:&str, span:Span) -> OxcDiagnostic {
...
impl Default for CatchErrorNameConfig {
	fn default() -> Self { Self { ignore:vec![], name:CompactStr::new_const("error") } }
}
...
impl CatchErrorName {
	...
	fn check_function_arguments(&self, arg0:&Argument, ctx:&LintContext) -> Option<OxcDiagnostic> {
...
fn test() {
	...
	let pass = vec![
		...
		("try { } catch (error) { }", None),
		(
			"promise.catch(unicorn => { })",
			Some(serde_json::json!([{"ignored": ["unicorn"]}])),
		),
		(
			"try { } catch (exception) { }",
