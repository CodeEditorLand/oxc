...
/// Represents a number literal, broken down into its integer, fractional, and
/// exponent parts.
#[derive(Debug)]
...
fn parse_number_literal(num:&str) -> ParsedNumberLiteral {
	...
	// Integer part is everything until the decimal separator or the exponent
	// (exclusive).
	...
	if let Some(ch) = num[offset..].chars().next() {
		if ch == '.' {
			...
			let decimal_part = num[offset..]
				.split_once(['e', 'E'])
				.map_or(&num[offset..], |(decimal, _)| decimal);
