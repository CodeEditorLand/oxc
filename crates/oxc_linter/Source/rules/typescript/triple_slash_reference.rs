...
fn triple_slash_reference_diagnostic(ref_kind:&str, span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!(
		"Do not use a triple slash reference for {ref_kind}, use `import` style instead."
	))
	.with_help(
		"Use of triple-slash reference type directives is generally discouraged in favor of \
...
impl Rule for TripleSlashReference {
	fn from_configuration(value:serde_json::Value) -> Self {
		...
		Self(Box::new(TripleSlashReferenceConfig {
			lib:options
				...
				.map_or_else(LibOption::default, |value| {
					match value {
			...
			path:options
				...
				.and_then(serde_json::Value::as_str)
				.map_or_else(PathOption::default, |value| {
					match value {
			...
			types:options
				...
				.and_then(serde_json::Value::as_str)
				.map_or_else(TypesOption::default, |value| {
					match value {
	...
	fn run_once(&self, ctx:&LintContext) {
		...
		// We don't need to iterate over all comments since Triple-slash
		// directives are only valid at the top of their containing file.
		// We are trying to get the first statement start potioin, falling back
		// to the program end if statement does not exist
