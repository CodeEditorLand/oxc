...
impl Rule for NoVarRequires {
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		if is_global_require_call(expr, ctx) {
			// If the parent is an expression statement => this is a top level
			// require() Or, if the parent is a chain expression
			// (require?.()) and the grandparent is an expression statement
			...
			// If this is an expression statement, it means the `require()`'s
			// return value is unused. If the return value is unused, this
			// isn't a problem.
...
fn test() {
	...
	let fail = vec![
		...
		// Because of TypeScript disallows angle bracket type assertions in
		// .tsx files, comment out this below case all tests parsing as tsx.
