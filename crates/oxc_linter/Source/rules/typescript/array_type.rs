...
fn generic(readonly_prefix:&str, name:&str, type_name:&str, span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!(
		"Array type using '{readonly_prefix}{type_name}[]' is forbidden. Use \
		 '{name}<{type_name}>' instead."
	))
	.with_label(span)
}
...
fn generic_simple(readonly_prefix:&str, name:&str, type_name:&str, span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!(
		"Array type using '{readonly_prefix}{type_name}[]' is forbidden for non-simple types. Use \
		 '{name}<{type_name}>' instead."
	))
	.with_label(span)
}
...
fn array(readonly_prefix:&str, type_name:&str, generic_name:&str, span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!(
		"Array type using '{type_name}<{generic_name}>' is forbidden. Use \
		 '{readonly_prefix}{generic_name}[]' instead."
	))
	.with_label(span)
}
...
fn array_simple(
	...
	OxcDiagnostic::warn(format!(
		"Array type using '{type_name}<{generic_name}>' is forbidden for simple types. Use \
		 '{readonly_prefix}{generic_name}[]' instead."
	))
	.with_label(span)
}
...
pub struct ArrayTypeConfig {
	...
	default:ArrayOption,
	// The array type expected for readonly cases. If omitted, the value for `default` will be
	// used.
...
fn check_and_report_error_array(
	...
	// TODO: support example: type Foo = ReadonlyArray<object>[]; -> type Foo =
	// (readonly object[])[]; let mut parent_parens: bool = readonly_prefix != "";
...
// Get the type name from the type node. for example: `Array<string>` ->
// `string`
...
fn test() {
	...
	let fail = vec![
		...
		// Some(serde_json::json!([{"default":"array-simple"}]))), ("let w: fooName.
		// BazType<string>[] = [['baz']];", Some(serde_json::json!([{"default":"array-simple"}]))),
		(
			"let x: Array<undefined> = [undefined] as undefined[];",
			Some(serde_json::json!([{"default":"array"}])),
		),
		...
		// ("type Unwrap<T> = T extends Array<infer E> ? E : T;",
		// Some(serde_json::json!([{"default":"array"}]))), ("type Unwrap<T> = T extends (infer
		// E)[] ? E : T;", Some(serde_json::json!([{"default":"generic"}]))), ("type Foo =
		// ReadonlyArray<object>[];", Some(serde_json::json!([{"default":"array"}]))),
		(
	...
	let fix:Vec<(&str, &str, Option<serde_json::Value>)> = vec![
		...
		// ("let a: Array<{ foo: Foo | Bar[] }> = [];", "let a: Array<{ foo: Foo | Array<Bar> }> =
		// [];", Some(serde_json::json!([{"default":"generic"}]))),
		(
			"function foo(a: Array<Bar>): Array<Bar> {}",
		...
		// ("let y: string[] = <Array<string>>['2'];", "let y: string[] = <string[]>['2'];",
		// Some(serde_json::json!([{"default":"array-simple"}]))),
		(
			"let z: Array = [3, '4'];",
			"let z: any[] = [3, '4'];",
		...
		// ("let v: Array<fooName.BarType> = [{ bar: 'bar' }];", "let v: fooName.BarType[] = [{
		// bar: 'bar' }];", Some(serde_json::json!([{"default":"array-simple"}]))),
		// ("let w: fooName.BazType<string>[] = [['baz']];", "let w: Array<fooName.BazType<string>>
		// = [['baz']];", Some(serde_json::json!([{"default":"array-simple"}]))),
		(
			"let x: Array<undefined> = [undefined] as undefined[];",
			"let x: undefined[] = [undefined] as undefined[];",
			Some(serde_json::json!([{"default":"array"}])),
		...
		// ("let y: string[] = <Array<string>>['2'];", "let y: Array<string> =
		// <Array<string>>['2'];", Some(serde_json::json!([{"default":"generic"}]))),
		(
			"let ya = [[1, '2']] as [number, string][];",
			"let ya = [[1, '2']] as Array<[number, string]>;",
			Some(serde_json::json!([{"default":"generic"}])),
		...
		// ("type Unwrap<T> = T extends Array<infer E> ? E : T;", "type Unwrap<T> = T extends
		// (infer E)[] ? E : T;", Some(serde_json::json!([{"default":"array"}]))),
		// ("type Unwrap<T> = T extends (infer E)[] ? E : T;", "type Unwrap<T> = T extends
		// Array<infer E> ? E : T;", Some(serde_json::json!([{"default":"generic"}]))),
		// ("type Foo = ReadonlyArray<object>[];", "type Foo = (readonly object[])[];",
		// Some(serde_json::json!([{"default":"array"}]))),
		(
			"const foo: Array<new (...args: any[]) => void> = [];",
