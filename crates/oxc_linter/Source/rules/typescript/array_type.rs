...
fn test() {
	...
	let fail = vec![
		...
		// ("type Unwrap<T> = T extends Array<infer E> ? E : T;",
		// Some(serde_json::json!([{"default":"array"}]))), ("type Unwrap<T>
		// = T extends (infer E)[] ? E : T;",
		// Some(serde_json::json!([{"default":"generic"}]))), ("type Foo =
	...
	let fix:Vec<(&str, &str, Option<serde_json::Value>)> = vec![
		...
		// ("let a: Array<{ foo: Foo | Bar[] }> = [];", "let a: Array<{ foo:
		// Foo | Array<Bar> }> = [];",
		// Some(serde_json::json!([{"default":"generic"}]))),
		(
			"function foo(a: Array<Bar>): Array<Bar> {}",
		...
		// ("let y: string[] = <Array<string>>['2'];", "let y: string[] =
		// <string[]>['2'];",
		// Some(serde_json::json!([{"default":"array-simple"}]))),
		(
			"let z: Array = [3, '4'];",
		...
		// ("let v: Array<fooName.BarType> = [{ bar: 'bar' }];", "let v:
		// fooName.BarType[] = [{ bar: 'bar' }];",
		// Some(serde_json::json!([{"default":"array-simple"}]))),
		// ("let w: fooName.BazType<string>[] = [['baz']];", "let w:
		// Array<fooName.BazType<string>> = [['baz']];",
		// Some(serde_json::json!([{"default":"array-simple"}]))),
		(
			"let x: Array<undefined> = [undefined] as undefined[];",
			"let x: undefined[] = [undefined] as undefined[];",
		...
		// ("let y: string[] = <Array<string>>['2'];", "let y: string[] =
		// <string[]>['2'];", Some(serde_json::json!([{"default":"array"}]))),
		(
			"let z: Array = [3, '4'];",
			"let z: any[] = [3, '4'];",
			Some(serde_json::json!([{"default":"array"}])),
		...
		// ("type Unwrap<T> = T extends Array<infer E> ? E : T;", "type
		// Unwrap<T> = T extends (infer E)[] ? E : T;",
		// Some(serde_json::json!([{"default":"array"}]))), ("type Unwrap<T>
		// = T extends (infer E)[] ? E : T;", "type Unwrap<T> = T extends
		// Array<infer E> ? E : T;",
		// Some(serde_json::json!([{"default":"generic"}]))), ("type Foo =
		// ReadonlyArray<object>[];", "type Foo = (readonly object[])[];",
		// Some(serde_json::json!([{"default":"array"}]))),
		(
			"const foo: Array<new (...args: any[]) => void> = [];",
