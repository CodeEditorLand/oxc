...
fn no_import_type_side_effects_diagnostic(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(
		"TypeScript will only remove the inline type specifiers which will leave behind a side \
		 effect import at runtime.",
	)
...
impl Rule for NoImportTypeSideEffects {
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		// Can report and fix only if all specifiers are inline `type`
		// qualifier: `import { type A, type B } from 'foo.js'`
...
fn test() {
	...
	Tester::new(NoImportTypeSideEffects::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
