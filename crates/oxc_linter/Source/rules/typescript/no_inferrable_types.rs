		...
		TSType::TSSymbolKeyword(_) => {
			...
			if let Expression::CallExpression(call_expr) = init.get_inner_expression() {
				call_expr
					...
					.map_or(false, |id| id.name == "Symbol")
			} else {
				false
			}
		},
	...
	match init {
		Expression::CallExpression(call_expr) => {
			call_expr
				.callee
				.get_identifier_reference()
				.map_or(false, |id| id.name == "BigInt")
		},
	...
	match init.get_inner_expression() {
		...
		Expression::CallExpression(call_expr) => {
			call_expr
				.callee
				.get_identifier_reference()
				.map_or(false, |id| id.name == "Boolean")
		},
	...
	match init {
		...
		Expression::CallExpression(call_expr) => {
			call_expr
				.callee
				.get_identifier_reference()
				.map_or(false, |id| id.name == "Number")
		},
	...
	match init {
		Expression::CallExpression(call_expr) => {
			call_expr
				.callee
				.get_identifier_reference()
				.map_or(false, |id| id.name == "String")
		},
