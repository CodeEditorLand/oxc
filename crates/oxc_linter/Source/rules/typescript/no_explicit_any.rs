...
fn no_explicit_any_diagnostic(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Unexpected any. Specify a different type.")
		.with_help(
			"Use `unknown` instead, this will force you to explicitly, and safely, assert the \
			 type is correct.",
		)
		.with_label(span)
}
...
pub struct NoExplicitAny {
	/// Whether to enable auto-fixing in which the `any` type is converted to
	/// the `unknown` type.
	///
	/// `false` by default.
	fix_to_unknown:bool,
...
mod tests {
	...
	fn test() {
		...
		let fixes = vec![
			...
			// NOTE: no current way to check that fixes don't occur when
			// `ignoreRestArgs` is `true`, since no fix technically occurs
			// and `expect_fix()` panics without a fix.
		];
		Tester::new(NoExplicitAny::NAME, pass, fail)
			.expect_fix(fixes)
			.test_and_snapshot();
	}
}
