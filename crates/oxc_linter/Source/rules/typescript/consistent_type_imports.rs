...
fn fix_to_type_import_declaration<'a>(options:&FixOptions<'a, '_>) -> FixerResult<RuleFix<'a>> {
        ...
        && import_decl.with_clause.is_none()
	{
		...
		{
			// if there is a default specifier but it isn't a type specifier, then just add
			// the inline type modifier to the named specifiers import AValue, {BValue,
			// Type1, Type2} from 'foo'
	...
	if !type_names_specifiers.is_empty() {
			...
			get_type_only_named_import(ctx, import_decl.source.value.as_str())
		{
			...
			// The import is both default and named.  Insert named on new line because can't
			// mix default type import and named type imports
			if matches!(fix_style, FixStyle::InlineTypeImports) {
				let text = format!(
					"import {{{}}} from {};\n",
...
fn get_named_specifier_ranges(
	...
	// It will never empty, in `get_fixes_named_specifiers`, we have already checked
	// every group is not empty.
...
fn test() {
	...
	// And the insert text didn't follow the indent of last statement, it just
