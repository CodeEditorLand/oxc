	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		if import_decl.import_kind.is_value() && !type_references_without_type_qualifier.is_empty()
		{
			...
			if is_only_type_references && import_decl.import_kind.is_value() {
				// `import type {} from 'foo' assert { type: 'json' }` is
				// invalid Import assertions cannot be used with type-only
				// imports or exports.
...
// Given an array of words, returns an English-friendly concatenation, separated
// with commas, with the `and` clause inserted before the last item.
//
...
// Returns `true` if the symbol is only used as a type reference, and `false`
// otherwise. Specifically, return `false` if the symbol does not have any
// references.
...
fn fix_to_type_import_declaration<'a>(options:&FixOptions<'a, '_>) -> FixerResult<RuleFix<'a>> {
        ...
        && import_decl.with_clause.is_none()
	{
		...
		{
			// if there is a default specifier but it isn't a type specifier,
			// then just add the inline type modifier to the named specifiers
		...
		{
			// import { Type1, Type2 } from 'foo' => import type { Type1, Type2
			// } from 'foo'
			return fix_insert_type_specifier_for_import_declaration(
				options, // is_default_import
				false,
	...
	if !type_names_specifiers.is_empty() {
			...
			get_type_only_named_import(ctx, import_decl.source.value.as_str())
		{
			...
			// The import is both default and named.  Insert named on new line
			// because can't mix default type import and named type imports
			if matches!(fix_style, FixStyle::InlineTypeImports) {
				let text = format!(
					"import {{{}}} from {};\n",
...
fn get_named_specifier_ranges(
	...
	// It will never empty, in `get_fixes_named_specifiers`, we have already
	// checked every group is not empty.
...
// e.g. I know "{" is in the text when call this in ImportDeclaration which
// contains named specifiers.
...
fn fix_inline_type_import_declaration<'a>(options:&FixOptions<'a, '_>) -> FixerResult<RuleFix<'a>> {
...
fn test() {
	...
	// 'foo';`` And the insert text didn't follow the indent of last statement,
	// it just insert before `text + '\n'`, so let's remove the prefix space.
	let fix = fix
		.into_iter()
		.map(|(a, b, c)| (remove_common_prefix_space(a), remove_common_prefix_space(b), c))
		.collect::<Vec<_>>();

	Tester::new(ConsistentTypeImports::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
