...
fn no_non_null_asserted_nullish_coalescing_diagnostic(span:Span) -> OxcDiagnostic {
	...
	.with_help(
		"The nullish coalescing operator is designed to handle undefined and null - using a \
		 non-null assertion is not needed.",
	)
	.with_label(span)
}
...
impl Rule for NoNonNullAssertedNullishCoalescing {
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		let Expression::TSNonNullExpression(ts_non_null_expr) = &expr.left else {
			return;
		};
...
fn has_assignment_before_node(symbol_id:SymbolId, ctx:&LintContext, parent_span_end:u32) -> bool {
