		...
		if self.is_record_mode {
			match node.kind() {
				AstKind::TSInterfaceDeclaration(inf) => {
					...
					let TSSignature::TSIndexSignature(sig) = member else {
						return;
					};
			...
			if let TSTypeName::IdentifierReference(ide) = &tref.type_name {
				...
				let Some(params) = &tref.type_parameters else {
					return;
				};
...
fn test() {
	...
	let fail = vec![
		...
		(
			"funcction foo(): Record<string, any> {}",
			Some(serde_json::json!(["index-signature"])),
		),
	];

	Tester::new(ConsistentIndexedObjectStyle::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
