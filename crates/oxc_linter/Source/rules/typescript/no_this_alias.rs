...
fn no_this_alias_diagnostic(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Unexpected aliasing of 'this' to local variable.")
		.with_help(
			"Assigning a variable to this instead of properly using arrow lambdas may be a \
			 symptom of pre-ES6 practices or not managing scope well.",
		)
		.with_label(span)
}
...
impl Default for NoThisAliasConfig {
	fn default() -> Self { Self { allow_destructuring:true, allow_names:FxHashSet::default() } }
}
impl NoThisAlias {
	fn is_allowed(&self, name:&str) -> bool { self.allow_names.contains(name) }
}
