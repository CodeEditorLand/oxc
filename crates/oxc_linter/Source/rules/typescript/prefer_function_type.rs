...
fn prefer_function_type_diagnostic(suggestion:&str, span:Span) -> OxcDiagnostic {
	...
	OxcDiagnostic::warn("Enforce using function types instead of interfaces with call signatures.")
		.with_help(format!(
			"The function type form `{suggestion}` is generally preferred when possible for being \
			 more succinct."
		))
		.with_label(span)
}
				...
				match node.kind() {
					...
					AstKind::TSTypeAnnotation(ts_type_annotation) => {
						match &ts_type_annotation.type_annotation {
							...
							_ => {
								ctx.diagnostic(prefer_function_type_diagnostic(
									&suggestion,
									decl.span,
								));
							},
...
type X = {} & (() => void);
	...
	Tester::new(PreferFunctionType::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
