...
impl ExplicitFunctionReturnType {
	fn check_arrow_function_with_void(&self, func:&ArrowFunctionExpression) -> bool {
		...
		let Some(expr) = func.get_expression() else {
			return false;
		};
	...
	fn check_parent_for_is_allowed_function<'a>(
		...
		match parent.kind() {
			...
			AstKind::MethodDefinition(def) => {
				let Some(name) = def.key.name() else {
					return false;
				};
	...
	fn returns_const_assertion_directly(&self, node:&AstNode) -> bool {
		...
		let AstKind::ArrowFunctionExpression(func) = node.kind() else {
			return false;
		};
...
fn is_constructor(node:&AstNode) -> bool {
	let AstKind::MethodDefinition(method_def) = node.kind() else {
		return false;
	};
...
fn is_function_argument(parent:&AstNode, callee:Option<&AstNode>) -> bool {
	...
	match call_expr.callee.without_parentheses() {
		Expression::FunctionExpression(func_expr) => {
			let AstKind::Function(callee_func_expr) = callee.unwrap().kind() else {
				return false;
			};
...
fn is_default_function_parameter_with_type_annotation(node:&AstNode) -> bool {
	let AstKind::AssignmentPattern(assign) = node.kind() else {
		return false;
	};
...
fn is_typed_jsx(node:&AstNode) -> bool {
	...
	let AstKind::JSXAttributeItem(jsx_attr_item) = node.kind() else {
		return false;
	};
...
fn test() {
	...
	Tester::new(
		ExplicitFunctionReturnType::NAME,
		ExplicitFunctionReturnType::CATEGORY,
		pass,
		fail,
	)
	.test_and_snapshot();
