	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		let span = match declaration.kind {
			...
			TSModuleDeclarationKind::Module => {
				declaration_code.find("module").map(|i| {
					Span::new(
						declaration.span.start + i as u32,
						declaration.span.start + i as u32 + 6,
					)
				})
			},
			TSModuleDeclarationKind::Namespace => {
				declaration_code.find("namespace").map(|i| {
					Span::new(
						declaration.span.start + i as u32,
						declaration.span.start + i as u32 + 9,
					)
				})
			},
		};
