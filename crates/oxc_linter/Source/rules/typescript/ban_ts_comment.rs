...
fn ignore_instead_of_expect_error(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(
		"Use \"@ts-expect-error\" instead of @ts-ignore, as \"@ts-ignore\" will do nothing if the \
		 following line is error-free.",
	)
...
fn comment_requires_description(ts_comment_name:&str, min_len:u64, span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!(
		"Include a description after the @ts-{ts_comment_name} directive to explain why the \
		 @ts-{ts_comment_name} is necessary. The description must be {min_len} characters or \
		 longer."
	))
	.with_label(span)
}
	...
	fn run_once(&self, ctx:&LintContext) {
		...
		for comm in comments {
			...
			if let Some(captures) = find_ts_comment_directive(raw, comm.is_line()) {
				// safe to unwrap, if capture success, it can always capture one of the four
				// directives
...
fn test() {
	...
	let pass = vec![
		...
		(
			"// @ts-nocheck no doubt, people will put nonsense here from time to time just to get \
			 the rule to stop reporting, perhaps even long messages with other nonsense in them \
			 like other // @ts-nocheck or // @ts-ignore things",
...
if (false) {
    ...
    console.log('hello');
		...
		(
			"// @ts-check",
			Some(serde_json::json!([{ "ts-check": "allow-with-description" }])),
		),
		(
			"// @ts-check: TS1234 because xyz",
			Some(serde_json::json!([
	...
	let fix = vec![
		...
		(
			"// @ts-ignore    : TS1234 because xyz",
			r"// @ts-expect-error    : TS1234 because xyz",
		),
	];
