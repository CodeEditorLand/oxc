...
fn not_need_no_confusing_non_null_assertion_diagnostic(op_str:&str, span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!(
		"Confusing combinations of non-null assertion and equal test like \"a! {op_str} b\", \
		 which looks very similar to not equal \"a !{op_str} b\"."
	))
...
fn wrap_up_no_confusing_non_null_assertion_diagnostic(op_str:&str, span:Span) -> OxcDiagnostic {
	...
	.with_help(
		"Wrap left-hand side in parentheses to avoid putting non-null assertion \"!\" and \"=\" \
		 together.",
	)
	.with_label(span)
}
...
fn confusing_non_null_assignment_assertion_diagnostic(op_str:&str, span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!(
		"Confusing combinations of non-null assertion and assignment like \"a! {op_str} b\", \
		 which looks very similar to not equal \"a !{op_str} b\"."
	))
	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		match node.kind() {
				...
				if assignment_expr.operator == AssignmentOperator::Assign =>
			{
				...
				let SimpleAssignmentTarget::TSNonNullExpression(_) = simple_target else {
					return;
				};
...
fn test() {
	...
	//     // ("f = 1 + d! == 2", "f = (1 + d!) == 2", None), TODO: Add suggest
	// or the weird ;() fix     // ("f =  d! == 2", "f = d == 2", None), TODO:
	// Add suggest remove bang ];
