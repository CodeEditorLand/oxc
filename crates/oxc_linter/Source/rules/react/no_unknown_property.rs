...
/// - Name starts with "data-" and has alphanumeric words (browsers require
///   lowercase, but React and TS lowercase them),
/// - Does not start with any casing of "xml"
/// - Words are separated by hyphens (-) (which is also called "kebab case" or
///   "dash case")
...
fn matches_html_tag_conventions(tag:&str) -> bool {
	tag.char_indices()
		.all(|(i, c)| if i == 0 { c.is_ascii_lowercase() } else { c != '-' })
}
