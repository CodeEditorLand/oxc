...
/// if the name starts with "data-" and has alphanumeric words (browsers require
/// lowercase, but React and TS lowercase them), not start with any casing of
/// "xml", and separated by hyphens (-) (which is also called "kebab case" or
/// "dash case"), then the attribute is a valid data attribute.
...
fn has_uppercase(name:&str) -> bool { name.contains(char::is_uppercase) }
...
fn test() {
	...
	let pass = vec![
		...
		(
			r#"<img src="cat_keyboard.jpeg" alt="A cat sleeping on a keyboard" align="top" />"#,
			None,
		),
		...
		(
			r#"<div class="bar"></div>;"#,
			Some(serde_json::json!([{ "ignore": ["class"] }])),
		),
		(
			r#"<div someProp="bar"></div>;"#,
			Some(serde_json::json!([{ "ignore": ["someProp"] }])),
		),
		(
			r"<div css={{flex: 1}}></div>;",
			Some(serde_json::json!([{ "ignore": ["css"] }])),
		),
