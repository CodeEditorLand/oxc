...
fn invalid_type_prop(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("`button` elements must have a valid `type` attribute.")
		.with_help(
			"Change the `type` attribute to one of the allowed values: `button`, `submit`, or \
			 `reset`.",
		)
		.with_label(span)
}
...
impl Default for ButtonHasType {
	fn default() -> Self { Self { button:true, submit:true, reset:true } }
}
...
fn test() {
	...
	let pass = vec![
		...
		(r#"React.createElement("button", {type: `reset`})"#, None),
		(
			r#"React.createElement("button", {type: condition ? "button" : "submit"})"#,
			None,
		),
		(
			r#"React.createElement("button", {type: condition ? 'button' : 'submit'})"#,
