...
use crate::{
	AstNode,
	ast_util::{
		get_declaration_from_reference_id,
...
fn dependency_array_not_array_literal_diagnostic(hook_name:&str, span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!(
		"React Hook {hook_name} was passed a dependency list that is not an array literal. This \
		 means we can't statically verify whether you've passed the correct dependencies."
	))
	.with_label(span)
...
fn unnecessary_outer_scope_dependency_diagnostic(
	...
	.with_label(span)
	.with_help(
		"Consider removing it from the dependency array. Outer scope values like aren't valid \
...
fn infinite_rerender_call_to_set_state_diagnostic(hook_name:&str, span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!(
		"React Hook {hook_name} contains a call to setState. Without a list of dependencies, this \
		 can lead to an infinite chain of updates."
	))
	.with_label(span)
	.with_help(
		"Consider adding an empty list of dependencies to make it clear which values are intended \
		 to be stable.",
	)
	.with_error_code_scope(SCOPE)
}
...
fn ref_accessed_directly_in_effect_cleanup_diagnostic(span:Span) -> OxcDiagnostic {
	...
	.with_label(span)
	.with_help(
		"The ref value will likely have changed by the time this effect cleanup function runs. If \
		 this ref points to a node rendered by react, copy it to a variable inside the effect and \
	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		let component_scope_id = {
			match get_enclosing_function(node, ctx.semantic()).map(oxc_semantic::AstNode::kind) {
				...
				// If we hit here, it means that the hook is called at the top level which isn't
				// allowed, so lets bail out. Reporting of this error should've been handled by
				// rules-of-hooks
				_ => return,
			}
		};

		let Some(callback_index) = get_reactive_hook_callback_index(call_expr) else {
			return;
		};
		...
		let declared_dependencies_iter = dependencies_node.elements.iter().filter_map(|elem| {
			match elem {
		...
		if !is_effect {
			...
			// for example if `props.foo`, AND `props.foo.bar.baz` was declared in the deps
			// array `props.foo.bar.baz` is unnecessary (already covered by `props.foo`)
...
impl PartialEq for Dependency<'_> {
	fn eq(&self, other:&Self) -> bool { self.name == other.name && self.chain == other.chain }
}
...
fn is_identifier_a_dependency<'a>(
	...
	if ctx
		...
		.contains_key(ident_name.as_str())
	{
		return false;
	}
	...
	if scopes
		.iter_all_child_ids(component_scope_id)
		.any(|id| id == declaration.scope_id())
	{
		return false;
	}

	// if the value is stable (for example the setter returned by useState), then
...
fn is_stable_value<'a, 'b>(
	...
	match node.kind() {
		...
		AstKind::VariableDeclarator(declaration) => {
			...
			// if the variables is a constant, and the initializer is a literal, then it's a
			// stable value. (excluding regex literals)
			if declaration.kind == VariableDeclarationKind::Const
				&& (matches!(
					init,
...
fn func_call_without_react_namespace<'a>(call_expr:&'a CallExpression<'a>) -> Option<&'a Atom<'a>> {
	...
	let Some(reference) = &member.object.get_identifier_reference() else {
		return None;
	};
...
fn test() {
  ...
  return useMemo(() => {
	...
	let fail = vec![
		...
		// actual: Outer scope values aren't considered dependencies as they don't re-render the
		// component r"function Thing() {
		//   useEffect(() => {
