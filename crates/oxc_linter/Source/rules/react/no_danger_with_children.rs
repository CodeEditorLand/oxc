	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		match node.kind() {
			AstKind::JSXElement(jsx) => {
				// Either children are passed in as a prop like `children={}` or they are nested
				// between the tags.
			...
			AstKind::CallExpression(call_expr) => {
				// Calls with zero or one arguments are safe since they are not proper
				// createElement calls.
				...
				// If it's just two arguments, it only has children if the props object has a
				// children property.
