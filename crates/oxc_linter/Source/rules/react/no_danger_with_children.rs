...
fn no_danger_with_children_diagnostic(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Only set one of `children` or `props.dangerouslySetInnerHTML`")
		.with_help(
			"`dangerouslySetInnerHTML` is not compatible with also passing children and React \
			 will throw a warning at runtime.",
		)
		.with_label(span)
}
	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		match node.kind() {
			AstKind::JSXElement(jsx) => {
				// Either children are passed in as a prop like `children={}` or
				// they are nested between the tags.
			...
			AstKind::CallExpression(call_expr) => {
				// Calls with zero or one arguments are safe since they are not
				// proper createElement calls.
				...
				// If there are three arguments, then it is a JSX element with
				// children. If it's just two arguments, it only has
				// children if the props object has a children property.
...
/// Given a variable name, finds the variable and checks if it is an object that
/// has a property by the given name, either by directly being set or by being
	...
	obj_expr.properties.iter().any(|prop| {
		match prop {
			...
			ObjectPropertyKind::SpreadProperty(spread_prop) => {
				...
				// If the next symbol is the same as the current symbol, then
				// there is a cycle, for example: `const props =
				// {...props}`, so we will stop searching.
...
/// Given the name of an identifier, find the variable declaration in the
/// current scope.
fn find_var_in_scope<'c>(ctx:&'c LintContext, node:&AstNode, name:&str) -> Option<&'c AstNode<'c>> {
	ctx.scopes()
