...
impl Allowed {
	pub fn is_never(self) -> bool { matches!(self, Self::Never) }

	#[inline]
	pub fn is_always(self) -> bool { matches!(self, Self::Always) }
}
...
fn contains_multiline_comment(s:&str) -> bool { s.contains("/*") || s.contains("*/") }

fn contains_quote_characters(s:&str) -> bool { s.chars().any(|c| matches!(c, '"' | '\'')) }
...
fn has_adjacent_jsx_expression_containers<'a>(
	...
	let Some(parent) = ctx.semantic().nodes().parent_kind(node_id) else {
		return false;
	};
...
fn test() {
	...
	let pass = vec![
		...
		(
			r#"<MyComponent>{<App/>}{"123"}</MyComponent>"#,
			Some(json!([{ "children": "never" }])),
		),
		(
			r#"<App>{"foo 'bar' \"foo\" bar"}</App>"#,
			Some(json!([{ "children": "never" }])),
		),
		(
		...
		(
			r#"<MyComponent>{"div { margin-top: 0; }"}</MyComponent>"#,
			Some(json!(["never"])),
		),
		(r#"<MyComponent>{"<Foo />"}</MyComponent>"#, Some(json!(["never"]))),
		(
			r#"<MyComponent prop={"Hello \u1026 world"}>bar</MyComponent>"#,
			Some(json!(["never"])),
		),
		(r#"<MyComponent>{"Hello \u1026 world"}</MyComponent>"#, Some(json!(["never"]))),
		(
			r#"<MyComponent prop={"Hello &middot; world"}>bar</MyComponent>"#,
			Some(json!(["never"])),
		),
	...
	let fail = vec![
		...
		// ("<MyComponent>foo bar <App/></MyComponent>", Some(json!([{
		// "children": "always" }]))),
		("<MyComponent>foo \n bar</MyComponent>", Some(json!([{ "children": "always" }]))),
		(
			"<MyComponent>foo \\u1234 bar</MyComponent>",
			Some(json!([{ "children": "always" }])),
		),
