...
impl Rule for JsxBooleanValue {
	fn from_configuration(value:serde_json::Value) -> Self {
		let enforce_boolean_attribute = value
			...
			.map_or_else(EnforceBooleanAttribute::default, |value| {
				match value {
		...
		// The exceptions are the inverse of the default, specifying both always
		// and never in the rule configuration is not allowed and ignored.
...
fn test() {
	...
	let pass = vec![
		...
		(
			"<App foo bar={true} />;",
			Some(serde_json::json!(["always", { "never": ["foo"] }])),
		),
		("<App foo />;", None),
		("<App foo={true} />;", Some(serde_json::json!(["always"]))),
		(
			"<App foo={true} bar />;",
			Some(serde_json::json!(["never", { "always": ["foo"] }])),
		),
		(
			"<App />;",
	...
	let fail = vec![
		...
		(
			"<App foo bar baz />;",
			Some(serde_json::json!(["never", { "always": ["foo", "bar"] }])),
		),
		(
	...
	Tester::new(JsxBooleanValue::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
