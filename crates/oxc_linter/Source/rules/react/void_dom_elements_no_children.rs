...
pub fn is_void_dom_element(element_name:&str) -> bool { VOID_DOM_ELEMENTS.contains(element_name) }
		...
		match node.kind() {
			AstKind::JSXElement(jsx_el) => {
				...
				let has_children_attribute_or_danger =
					jsx_opening_el.attributes.iter().any(|attribute| {
						match attribute {
			...
			AstKind::CallExpression(call_expr) => {
				...
				let has_children_prop_or_danger = obj_expr.properties.iter().any(|property| {
					match property {
						ObjectPropertyKind::ObjectProperty(prop) => {
							match &prop.key {
								PropertyKey::StaticIdentifier(iden) => {
									iden.name == "children"
										|| iden.name == "dangerouslySetInnerHTML"
								},
								_ => false,
							}
						},
...
fn test() {
	...
	let pass = vec![
		...
		(
			r"React.createElement('div', { dangerouslySetInnerHTML: { __html: 'Foo' } });",
			None,
		),
	...
	let fail = vec![
		...
		(
			r"React.createElement('br', { dangerouslySetInnerHTML: { __html: 'Foo' } });",
			None,
		),
		(
			r"
