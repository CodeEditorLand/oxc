	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		if let AstKind::JSXOpeningElement(jsx_ele) = node.kind() {
			...
			if self.check_is_link(tag_name, ctx) || self.check_is_forms(tag_name, ctx) {
				...
				jsx_ele.attributes.iter().for_each(|attribute| {
					match attribute {
...
fn check_is_external_link(link:&str) -> bool { link.contains("//") }

fn match_href_expression(expr:&Expression, is_external_link:&mut bool, is_dynamic_link:&mut bool) {
	match expr {
...
fn test() {
	...
	let pass = vec![
		...
		(
			r#"<a href="foobar" target="_blank" rel={"noopener noreferrer"}></a>"#,
			None,
			None,
		),
		(r#"<a href="foobar" target="_blank" rel={"noreferrer"}></a>"#, None, None),
		...
		(r"<a href={`foobar`} target={`_blank`} rel={`noreferrer`}></a>", None, None),
		(
			r#"<a target="_blank" {...spreadProps} rel="noopener noreferrer"></a>"#,
			None,
			None,
		),
		...
		(
			r#"<a target="_blank" rel="noopener noreferrer" {...spreadProps}></a>"#,
			None,
			None,
		),
