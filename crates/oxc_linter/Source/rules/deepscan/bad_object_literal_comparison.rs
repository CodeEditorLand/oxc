...
enum BadObjectLiteralComparisonDiagnostic {
	...
	#[diagnostic(
		severity(warning),
		help(
			"This comparison will always return {1:?} as object literals are never equal to each \
			 other. Consider using `Object.entries()` of `Object.keys()` and comparing their \
			 lengths."
		)
	)]
	...
	#[diagnostic(
		severity(warning),
		help(
			"This comparison will always return {1:?} as array literals are never equal to each \
			 other. Consider using `Array.length` if empty checking was intended."
		)
	)]
