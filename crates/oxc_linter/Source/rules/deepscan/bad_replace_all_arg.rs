...
#[error(
	"deepscan(bad-replace-all-arg): Global flag (g) is missing in the regular expression supplied \
	 to the `replaceAll` method."
)]
#[diagnostic(
	severity(warning),
	help(
		"To replace all occurrences of a string, use the `replaceAll` method with the global flag \
		 (g) in the regular expression."
	)
)]
...
fn resolve_flags<'a>(expr:&'a Expression<'a>, ctx:&LintContext<'a>) -> Option<(RegExpFlags, Span)> {
