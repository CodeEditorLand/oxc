...
#[diagnostic(
	severity(warning),
	help(
		"The 'arguments' object does not have '{0}()' method. If an array method was intended, \
		 consider converting the 'arguments' object to an array or using ES6 rest parameter \
		 instead."
	)
)]
			...
			MemberExpression::ComputedMemberExpression(expr) => {
				match &expr.expression {
					...
					Expression::TemplateLiteral(template) => {
						// only check template string like
						// "arguments[`METHOD_NAME`]" for Deepscan compatible
