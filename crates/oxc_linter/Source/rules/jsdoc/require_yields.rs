...
impl Rule for RequireYields {
	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		match node.kind() {
			...
			// A. Rule behavior is different whether `yield` value is present or
			// not.
			//
			// `oxc_semantic` add node flag to detect `yield` is used or NOT.
			// But existence of value is still unknown.
			//
			// Find `YieldExpression` inside of `(Generator)Function` requires
			// more complex logic. Use bottom-up approach to find the nearest
			...
			AstKind::YieldExpression(yield_expr) => {
				...
				while let Some(parent_node) = ctx.nodes().parent_node(current_node.id()) {
					// If syntax is valid, `yield` should be inside a generator
					// function
