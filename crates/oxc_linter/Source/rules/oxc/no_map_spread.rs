...
pub struct NoMapSpreadConfig {
	...
	/// In these cases, `Object.assign` is not really more performant than
	/// spreads.
	///
	/// Default: `true`
	...
	/// ### Example
	///
	/// Examples of **incorrect** code for this rule when `ignoreArgs` is
	/// `true`: ```ts
...
impl From<NoMapSpreadConfig> for NoMapSpread {
	fn from(config:NoMapSpreadConfig) -> Self { Self(config) }
}

impl Default for NoMapSpreadConfig {
	fn default() -> Self { Self { ignore_rereads:true, ignore_args:true } }
}
...
fn test() {
	...
	let fail = vec![
		...
		(
			"let a = b.map(x => someCond ? { ...x, foo: true } : { ...x, foo: false })",
			None,
		),
