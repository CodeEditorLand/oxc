						...
						.is_export()
				{
					ctx.diagnostic_with_dangerous_fix(
						...
						|fixer| {
							...
							// search for references to the function and remove
							// the argument
							for reference in ctx.semantic().symbol_references(
								...
								{
									// check if the number of arguments is the
									// same
...
fn is_argument_only_used_in_recursion<'a>(
		...
		.symbol_references(arg.symbol_id.get().expect("`symbol_id` should be set"))
	{
		...
		if let Some(AstKind::Argument(argument)) = ctx.nodes().parent_kind(reference.node_id()) {
			if let Some(AstKind::CallExpression(call_expr)) = ctx
				.nodes()
...
function writeChunks(a,callac){writeChunks(m,callac)}writeChunks(i,{})",
	...
	Tester::new(OnlyUsedInRecursion::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
