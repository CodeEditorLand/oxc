...
use crate::{
	AstNode,
	ast_util::get_function_like_declaration,
	context::LintContext,
	fixer::Fix,
	rule::Rule,
};
...
fn only_used_in_recursion_diagnostic(span:Span, param_name:&str) -> OxcDiagnostic {
	OxcDiagnostic::warn(format!("Parameter `{param_name}` is only used in recursive calls"))
		.with_help(
			"Remove the argument and its usage. Alternatively, use the argument in the function \
			 body.",
		)
		.with_label(span)
}
...
fn create_diagnostic_jsx(
	...
	let Some(property_ident) = property.value.get_binding_identifier() else {
		return;
	};
	...
	ctx.diagnostic_with_dangerous_fix(
		...
		|fixer| {
			...
			for reference in ctx.semantic().symbol_references(property_symbol_id) {
				...
				let Some(attr) = ancestor_ids.find_map(|node| {
					match ctx.nodes().get_node(node).kind() {
...
fn used_with_spread_attribute(node_id:NodeId, ctx:&LintContext) -> bool {
	ctx.nodes().ancestor_kinds(node_id).any(|kind| {
		match kind {
...
fn is_argument_only_used_in_recursion<'a>(
	...
	for reference in references {
		...
		let Some(AstKind::CallExpression(call_expr)) = ctx
			.nodes()
			.parent_kind(ctx.nodes().parent_node(reference.node_id()).unwrap().id())
		else {
			return false;
		};
...
fn is_property_only_used_in_recursion_jsx(
	...
	for reference in references {
		...
		// 3. The reference is in a JSXAttribute, and the attribute name has the same
		...
		let AstKind::JSXExpressionContainer(_) = may_jsx_expr_container.kind() else {
			// In this case, we simply ignore the references inside JSXExpressionContainer
			// that are not single-node expression.   e.g. <Increment count={count+1} />
			//
