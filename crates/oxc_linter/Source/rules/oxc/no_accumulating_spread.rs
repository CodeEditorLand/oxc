...
fn reduce_likely_array_spread_diagnostic(spread_span:Span, reduce_span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Do not spread accumulators in Array.prototype.reduce()")
		.with_help(
			"It looks like you're spreading an `Array`. Consider using the `Array.push` or \
			 `Array.concat` methods to mutate the accumulator instead.\nUsing spreads within \
			 accumulators leads to `O(n^2)` time complexity.",
		)
		.with_labels([spread_span.label("From this spread"), reduce_span.label("For this reduce")])
}

fn reduce_likely_object_spread_diagnostic(spread_span:Span, reduce_span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Do not spread accumulators in Array.prototype.reduce()")
		.with_help(
			"It looks like you're spreading an `Object`. Consider using the `Object.assign` or \
			 assignment operators to mutate the accumulator instead.\nUsing spreads within \
			 accumulators leads to `O(n^2)` time complexity.",
		)
		.with_labels([spread_span.label("From this spread"), reduce_span.label("For this reduce")])
}
...
fn reduce_unknown(spread_span:Span, reduce_span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Do not spread accumulators in Array.prototype.reduce()")
		.with_help(
			"Consider using `Object.assign()` or `Array.prototype.push()` to mutate the \
			 accumulator instead.\nUsing spreads within accumulators leads to `O(n^2)` time \
			 complexity.",
		)
		.with_labels([spread_span.label("From this spread"), reduce_span.label("For this reduce")])
}
...
fn loop_spread_likely_object_diagnostic(
	...
	OxcDiagnostic::warn("Do not spread accumulators in loops")
		.with_help(
			"Consider using `Object.assign()` to mutate the accumulator instead.\nUsing spreads \
...
fn loop_spread_likely_array_diagnostic(
	...
	OxcDiagnostic::warn("Do not spread accumulators in loops")
		.with_help(
			"Consider using `Array.prototype.push()` to mutate the accumulator instead.\nUsing \
			 spreads within accumulators leads to `O(n^2)` time complexity.",
		)
		.with_labels([
			accumulator_decl_span.label("From this accumulator"),
			spread_span.label("From this spread"),
...
fn check_reduce_usage<'a>(
	...
	let first_param_symbol_id = params
		.items
		.first()
		.and_then(|item| get_identifier_symbol_id(&item.pattern.kind));
...
fn check_loop_usage<'a>(
	...
	let AstKind::SimpleAssignmentTarget(_) = assignment_target.kind() else {
		return;
	};

	let Some(assignment_expr) = ctx.nodes().parent_node(assignment_target.id()) else {
		return;
	};
	if !matches!(assignment_expr.kind(), AstKind::AssignmentTarget(_)) {
		return;
	}
	let Some(assignment) = ctx.nodes().parent_node(assignment_expr.id()) else {
		return;
	};
		...
		if let Some(loop_span) = get_loop_span(parent.kind()) {
				...
				&& parent.kind().span().contains_inclusive(spread_span)
			{
				match assignment_expression_right_inner_expr {
					...
					// we check above that the expression is either an array or
					// object expression
					_ => unreachable!(),
				}
			}
...
fn get_reduce_diagnostic<'a>(call_expr:&'a CallExpression<'a>, spread_span:Span) -> OxcDiagnostic {
...
fn test() {
	...
	let pass = vec![
		...
		"foo.reduce((acc) => [...acc], [])",
		// Wrong number of arguments to known method (reduce can have 1 or 2
		// args, but not more)
		...
		"let foo = {}; while (Object.keys(foo).length < 10) { foo[Object.keys(foo).length] = \
	...
	let fail = vec![
		...
		"let foo = {}; while (Object.keys(foo).length < 10) { foo = { ...foo, \
		 [Object.keys(foo).length]: Object.keys(foo).length }; }",
	];
