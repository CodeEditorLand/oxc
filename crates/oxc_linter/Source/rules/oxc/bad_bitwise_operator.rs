...
fn bad_bitwise_operator_diagnostic(
	...
	OxcDiagnostic::warn("Bad bitwise operator")
		.with_help(format!(
			"Bitwise operator '{bad_operator}' seems unintended. Did you mean logical operator \
			 '{suggestion}'?"
		))
		.with_label(span2)
}
...
fn bad_bitwise_or_operator_diagnostic(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Bad bitwise operator")
		.with_help(
			"Bitwise operator '|=' seems unintended. Consider using non-compound assignment and \
