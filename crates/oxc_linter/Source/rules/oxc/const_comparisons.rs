...
fn identical_expressions_logical_operator(left_span:Span, right_span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Both sides of the logical operator are the same")
		.with_help(
			"This logical expression will always evaluate to the same value as the expression \
			 itself.",
		)
		.with_labels([
			left_span.label("If this expression evaluates to true"),
			right_span.label("This expression will always evaluate to true"),
		])
}
...
impl ConstComparisons {
	...
	fn check_logical_expression_const_literal_comparison<'a>(
			...
			all_and_comparison_to_const(logical_expr.left.get_inner_expression())
		{
			...
			// Mostly to simplify the implementation, but it is also covered by
			...
			if left_cmp_op.direction() == right_cmp_op.direction() {
				...
				if left_side_is_useless(left_cmp_op, ordering) {
					ctx.diagnostic(redundant_left_hand_side(
						left_span,
						logical_expr.right.span(),
						format!(
							"if `{rhs_str}` evaluates to true, `{lhs_str}` will always evaluate \
							 to true as well"
						),
					));
				} else {
					ctx.diagnostic(redundant_right_hand_side(
						logical_expr.right.span(),
						left_span,
						format!(
							"if `{lhs_str}` evaluates to true, `{rhs_str}` will always evaluate \
							 to true as well"
						),
					));
				}
				...
				let diagnostic_note = match ordering {
					Ordering::Less => {
						format!(
							"since `{lhs_str}` < `{rhs_str}`, the expression evaluates to false \
							 for any value of `{expr_str}`"
						)
					},
					Ordering::Equal => {
						format!(
							"`{expr_str}` cannot simultaneously be greater than and less than \
							 `{lhs_str}`"
						)
					},
