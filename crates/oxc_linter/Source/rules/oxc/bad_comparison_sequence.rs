...
fn bad_comparison_sequence_diagnostic(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Bad comparison sequence")
		.with_help(
			"Comparison result should not be used directly as an operand of another comparison. \
			 If you need to compare three or more operands, you should connect each comparison \
			 operation with logical AND operator (`&&`)",
		)
		.with_label(span)
}
...
fn has_no_bad_comparison_in_parents<'a, 'b>(node:&'b AstNode<'a>, ctx:&'b LintContext<'a>) -> bool {
	for node_id in ctx.nodes().ancestor_ids(node.id()).skip(1) {
		let kind = ctx.nodes().kind(node_id);

		// `a === b === c === d === e` only produce one error, since `(a === b === c)
		// === d === e` will produce two errors. So we should treat Parenthesized
		// Expression as a boundary.
