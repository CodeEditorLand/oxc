...
fn double_comparisons_diagnostic(span:Span, operator:&str) -> OxcDiagnostic {
	OxcDiagnostic::warn("Unexpected double comparisons.")
		.with_help(format!(
			"This logical expression can be simplified. Try using the `{operator}` operator \
			 instead."
		))
		.with_label(span)
}
...
impl Rule for DoubleComparisons {
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		// check that (LLHS === RLHS && LRHS === RRHS) || (LLHS === RRHS && LRHS
		// === RLHS)
...
fn test() {
	...
	Tester::new(DoubleComparisons::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
