...
fn bad_array_method_on_arguments_diagnostic(method_name:&str, span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Bad array method on arguments")
		.with_help(format!(
			"The 'arguments' object does not have '{method_name}()' method. If an array method \
			 was intended, consider converting the 'arguments' object to an array or using ES6 \
			...
			MemberExpression::ComputedMemberExpression(expr) => {
				match &expr.expression {
					...
					Expression::TemplateLiteral(template) => {
						...
						// "arguments[`METHOD_NAME`]" for Oxc compatible
