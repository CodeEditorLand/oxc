...
fn object_comparison(span:Span, const_result:bool) -> OxcDiagnostic {
	OxcDiagnostic::warn("Unexpected object literal comparison.")
		.with_help(format!(
			"This comparison will always return {const_result:?} as object literals are never \
			 equal to each other. Consider using `Object.entries()` of `Object.keys()` and \
...
fn array_comparison(span:Span, const_result:bool) -> OxcDiagnostic {
	OxcDiagnostic::warn("Unexpected array literal comparison.")
		.with_help(format!(
			"This comparison will always return {const_result:?} as array literals are never \
			 equal to each other. Consider using `Array.length` if empty checking was intended."
		))
		.with_label(span)
}
...
fn test() {
	...
	Tester::new(
		BadObjectLiteralComparison::NAME,
		BadObjectLiteralComparison::CATEGORY,
		pass,
		fail,
	)
	.test_and_snapshot();
