...
use crate::{
	...
	utils::{
		...
		calculate_logical_operation,
		calculate_unary_operation,
		get_write_expr,
		has_comment_about_side_effect_check,
		has_pure_notation,
		is_function_side_effect_free,
		is_local_variable_a_whitelisted_module,
		is_pure_function,
		no_effects,
		FunctionName,
		NodeListenerOptions,
...
// it's same as `reportSideEffectsInDefinitionWhenCalled` in
// eslint-plugin-tree-shaking <https://github.com/lukastaegert/eslint-plugin-tree-shaking/blob/463fa1f0bef7caa2b231a38b9c3557051f506c92/src/rules/no-side-effects-in-initialization.ts#L1070-L1080>
...
impl<'a> ListenerMap for BindingIdentifier<'a> {
	fn report_effects(&self, _options:&NodeListenerOptions) { no_effects(); }
...
impl<'a> ListenerMap for FormalParameter<'a> {
	fn report_effects(&self, options:&NodeListenerOptions) { self.pattern.report_effects(options); }
}
...
impl<'a> ListenerMap for StaticMemberExpression<'a> {
	fn report_effects(&self, options:&NodeListenerOptions) { self.object.report_effects(options); }
