	...
	fn from_configuration(value:serde_json::Value) -> Self {
		...
		if let Value::Array(arr) = value {
			for obj in arr {
				...
				// { "module": "react", "functions": ["createContext",
				// "createRef"] } { "module": "react", "functions": "*" }
...
fn test() {
	...
	let pass = vec![
		...
		// r#"import /* tree-shaking  no-side-effects-when-called*/ /* test */
		// x from "import-default-no-effects"; x()"#,
	...
	let fail = vec![
		...
		// CatchClause
		"try {} catch (error) {ext()}",
		// TODO: check global function `ext` call when called `x()` in no
		// strict mode "var x=()=>{}; try {} catch (error) {var x=ext}; x()",
		// ClassBody
		"class x {[ext()](){}}",
		// ClassBody when called
		...
		"class y {constructor(){ext()}}; class x extends y {constructor(){super()}}; const z = \
		 new x()",
	...
	let pass = pass
		.into_iter()
		.map(|case| (case, None))
		.chain(pass_with_options)
		.collect::<Vec<_>>();

	let fail = fail
		.into_iter()
		.map(|case| (case, None))
		.chain(fail_with_options)
		.collect::<Vec<_>>();
