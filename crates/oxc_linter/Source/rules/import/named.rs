...
impl Rule for Named {
	fn run_once(&self, ctx:&LintContext<'_>) {
		...
		for export_entry in &module_record.indirect_export_entries {
			...
			// `export { default as foo } from './source'` <> `export default
			// xxx`
...
fn test() {
	...
	let fail = vec![
		...
		// "const { baz } = require('./bar')",
		// "let { baz } = require('./bar')",
		// "const { baz: bar, bop } = require('./bar'), { a } =
		// require('./re-export-names')", "const { default: defExport } =
		// require('./named-exports')", flow
