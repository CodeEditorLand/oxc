...
impl Rule for NoDuplicates {
	...
	fn run_once(&self, ctx:&LintContext<'_>) {
		...
		for (_path, group) in &groups {
			...
			// When prefer_inline is false, 0 is value, 1 is type named, 2 is type namespace
			// and 3 is type default When prefer_inline is true, 0 is value and type
			// named, 2 is type // namespace and 3 is type default
			let mut import_entries_maps:FxHashMap<u8, Vec<&RequestedModule>> = FxHashMap::default();
...
fn test() {
	...
	let pass = vec![
		...
		(
			r"import type * as something from './foo'; import type { y } from './foo';",
			None,
		),
		(r"import type y from './foo'; import type * as something from './foo';", None),
		(
			r"import type { y } from './foo'; import type * as something from './foo';",
			None,
		),
		// type + import
	...
	let fail = vec![
		...
		// TODO:   settings: { 'import/resolve': { paths: [path.join(process.cwd(), 'tests',
		// 'files')], }, }, r#"import { x } from './bar'; import { y } from 'bar';"#,
