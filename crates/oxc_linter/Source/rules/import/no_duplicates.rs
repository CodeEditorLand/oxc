...
impl Rule for NoDuplicates {
	...
	fn run_once(&self, ctx:&LintContext<'_>) {
		...
		for (_path, group) in &groups {
			...
			// When prefer_inline is false, 0 is value, 1 is type named, 2 is
			// type default or type namespace When prefer_inline is true, 0
			// is value and type named, 2 is type default or type namespace
...
fn test() {
	...
	let fail = vec![
		...
		// TODO:   settings: { 'import/resolve': { paths:
		// [path.join(process.cwd(), 'tests', 'files')], }, }, r#"import { x
		// } from './bar'; import { y } from 'bar';"#,
