...
fn test() {
	...
	let pass = vec![
		...
		(r#"var bar = require("./bar")"#, None),
		// TODO: settings 'import/external-module-folders':
		// ['cycles/external'], (r#"import { foo } from
		// "./external-depth-two""#, Some(json!([[{"ignoreExternal":true}]))),
		// (
		...
		(
			r#"import("./es6/depth-two").then(function({ foo }) {})"#,
			Some(json!([{"maxDepth":1}])),
		),
		// parser: parsers.BABEL_OLD
		// (r#"import type { FooType } from "./es6/depth-one""#, None),
		// (r#"import type { FooType, BarType } from "./es6/depth-one""#,
		// None),
		(
		...
		// (r#"import { bar } from
		// "./flow-types-only-importing-multiple-types""#, None), (r#"import
		// { bar } from "./flow-typeof""#, None),
	];

	let fail = vec![
		// (r#"import { bar } from "./flow-types-some-type-imports""#, None),
		// TODO: settings  'import/resolver': 'webpack',
		// 'import/external-module-folders': ['cycles/external'], (r#"import
		// { foo } from "cycles/external/depth-one""#, None), TODO: settings
		// 'import/external-module-folders': ['cycles/external'], (r#"import
		// { foo } from "./external-depth-two""#, None), (r#"import { foo }
		// from "./es6/depth-one""#, None),
		(r#"import { foo } from "./es6/depth-one""#, Some(json!([{"maxDepth":1}]))),
		// (r#"const { foo } = require("./es6/depth-one")"#,
		// Some(json!([{"commonjs":true}]))), TODO: amd
		// (r#"require(["./es6/depth-one"], d1 => {})"#,
		// Some(json!([{"amd":true}]))), (r#"define(["./es6/depth-one"], d1
		// => {})"#, Some(json!([{"amd":true}]))),
		...
		// (
		// r#"const { foo } = require("./es6/depth-one")"#,
		// Some(json!([{"allowUnsafeDynamicCyclicDependency":true,"commonjs":
		// true}])), ),
		// TODO: amd
		// (
		// r#"require(["./es6/depth-one"], d1 => {})"#,
		// Some(json!([{"allowUnsafeDynamicCyclicDependency":true,"amd":
		// true}])), ),
		// (
		...
		(r#"import { foo } from "./es6/depth-two""#, Some(json!([{"maxDepth":"∞"}]))),
		// TODO: dynamic import
		// (r#"function bar(){ return import("./es6/depth-one"); } // #2265
		// 5"#, None), (r#"import { foo } from "./es6/depth-one-dynamic"; //
		// #2265 6"#, None), (r#"function bar(){ return
		// import("./es6/depth-one"); } // #2265 7"#, None), (r#"import { foo
		// } from "./es6/depth-one-dynamic"; // #2265 8"#, None), // Flow not
		// supported (r#"import { bar } from "./flow-types-depth-one""#,
		// None),
