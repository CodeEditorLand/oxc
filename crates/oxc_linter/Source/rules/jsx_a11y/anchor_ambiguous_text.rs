...
fn normalize_str(text:&str) -> CompactStr {
	// `to_lowercase` is disallowed. however we need to remove certain chars later
	// which requires converting to a String the overhead of going &str -> cow
	// string -> string is greater than just using to_lowercase
	...
	if normalized_str.contains(char::is_whitespace) {
		let parts:Vec<String> = normalized_str
			.split_whitespace()
