...
fn click_events_have_key_events_diagnostic(span:Span) -> OxcDiagnostic {
	...
	.with_help(
		"Visible, non-interactive elements with click handlers must have one of keyup, keydown, \
		 or keypress listener.",
	)
	.with_label(span)
}
...
fn test() {
	...
	let pass = vec![
		...
		(
			r"<div onClick={() => void 0} onKeyDown={foo} onKeyUp={bar} />;",
			None,
			None,
			None,
		),
		...
		(
			r"<div onClick={() => void 0} aria-hidden={false} onKeyDown={foo} />;",
			None,
			None,
			None,
		),
		...
		(
			r#"<a onClick={() => void 0} href="http://x.y.z" tabIndex="0" />"#,
			None,
			None,
			None,
		),
