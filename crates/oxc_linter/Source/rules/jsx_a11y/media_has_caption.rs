...
impl Rule for MediaHasCaption {
	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		// Bail out if the element is muted as captions are not required for
		// muted media. (e.g <video muted />)
		if muted {
			return;
		}
			...
			parent.children.iter().any(|child| {
				match child {
					JSXChild::Element(child_el) => {
						let Some(child_name) = get_element_type(ctx, &child_el.opening_element)
						else {
							return false;
						};
						self.0.track.contains(&child_name)
							&& child_el.opening_element.attributes.iter().any(|attr| {
								if let JSXAttributeItem::Attribute(attr) = attr {
									if let JSXAttributeName::Identifier(iden) = &attr.name {
										if let Some(JSXAttributeValue::StringLiteral(s)) =
											&attr.value
										{
...
fn test() {
	...
	let pass = vec![
		...
		(r"<audio><track kind='Captions' /></audio>", None, None, None),
		(
			r"<audio><track kind='Captions' /><track kind='subtitles' /></audio>",
			None,
			None,
			None,
		),
		...
		(r"<video><track kind='Captions' /></video>", None, None, None),
		(
			r"<video><track kind='Captions' /><track kind='subtitles' /></video>",
			None,
			None,
			None,
		),
	...
	let fail = vec![
		...
		(
			r"<Box as='audio'><Track kind='subtitles' /></Box>",
			None,
			Some(settings()),
			None,
		),
