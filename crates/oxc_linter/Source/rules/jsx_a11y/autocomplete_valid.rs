...
fn is_valid_autocomplete_value(value:&str) -> bool {
	...
	match parts.len() {
		...
		2 => {
			VALID_AUTOCOMPLETE_COMBINATIONS
				.get(parts[0])
...
fn test() {
	...
	let pass = vec![
		...
		// ("<input type='text' autocomplete='billing family-name' />;", None,
		// None), ("<input type='text' autocomplete='section-blue shipping
		// street-address' />;", None, None), ("<input type='text'
		// autocomplete='section-somewhere shipping work email' />;", None,
		// None),
		("<input type='text' autocomplete />;", None, None),
