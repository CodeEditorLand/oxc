...
fn no_aria_hidden_on_focusable_diagnostic(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("`aria-hidden` must not be true on focusable elements.")
		.with_help(
			"Remove `aria-hidden=\"true\"` from focusable elements or modify the element to be \
			 not focusable.",
		)
		.with_label(span)
}
...
///
/// This function checks the passed `JSXOpeningElement` against these criteria
/// to determine if it is focusable.
///
/// # Arguments
///
...
fn test() {
	...
	let fix = vec![
		...
		(
			r#"<p tabIndex="0" aria-hidden="true">text</p>;"#,
			r#"<p tabIndex="0" >text</p>;"#,
		),
	];

	Tester::new(NoAriaHiddenOnFocusable::NAME, pass, fail)
		.expect_fix(fix)
		.test_and_snapshot();
}
