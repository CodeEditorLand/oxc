...
fn no_access_key_diagnostic(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("No access key attribute allowed.")
		.with_help(
			"Remove the accessKey attribute. Inconsistencies between keyboard shortcuts and \
			 keyboard commands used by screenreaders and keyboard-only users create a11y \
			 complications.",
		)
		.with_label(span)
}
