...
fn img_redundant_alt_diagnostic(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Redundant alt attribute.")
		.with_help(
			"Provide no redundant alt text for image. Screen-readers already announce `img` tags \
			 as an image. You don’t need to use the words `image`, `photo,` or `picture` (or any \
...
impl Rule for ImgRedundantAlt {
	fn from_configuration(value:Value) -> Self {
		...
		let words = config
			...
			.map_or(Vec::from(REDUNDANT_WORDS), |v| {
...
fn test() {
	...
	let pass = vec![
		...
		(
			r"<img alt='picture of me taking a photo of an image' aria-hidden />",
			None,
			None,
		),
	...
	Tester::new(ImgRedundantAlt::NAME, pass, fail)
