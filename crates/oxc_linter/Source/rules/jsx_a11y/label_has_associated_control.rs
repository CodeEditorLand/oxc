...
fn label_has_associated_control_diagnostic(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("A form label must be associated with a control.")
		.with_help(
			"Either give the label a `htmlFor` attribute with the id of the associated control, \
			 or wrap the label around the control.",
		)
		.with_label(span)
}
...
impl LabelHasAssociatedControl {
	fn has_accessible_label<'a>(&self, root:&JSXElement<'a>, ctx:&LintContext<'a>) -> bool {
		if root.opening_element.attributes.iter().any(|attribute| {
			match attribute {
	...
	fn search_for_accessible_label<'a>(
		...
		match node {
			...
			JSXChild::Element(element) => {
				let has_labelling_prop = element.opening_element.attributes.iter().any(|attr| {
					match attr {
