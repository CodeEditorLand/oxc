...
fn run<'a>(possible_jest_node:&PossibleJestNode<'a, '_>, ctx:&LintContext<'a>) {
	...
	if let AstKind::CallExpression(call_expr) = node.kind() {
		if let Some(jest_fn_call) = parse_general_jest_fn_call(call_expr, possible_jest_node, ctx) {
			...
			{
				// isJestEach but not a TaggedTemplateExpression, so this must
				// be the `jest.each([])()` syntax which this rule doesn't
				// support due to its complexity (see
				// jest-community/eslint-plugin-jest#710)
				return;
			}
...
fn test() {
	...
	Tester::new(NoDoneCallback::NAME, pass, fail)
