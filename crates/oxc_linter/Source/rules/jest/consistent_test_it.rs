...
impl Default for ConsistentTestItConfig {
	fn default() -> Self { Self { within_describe:TestCaseName::IT, within_fn:TestCaseName::Test } }
}
	...
	fn get_prefer_test_name_and_span<'s>(
		...
		match expr {
			...
			Expression::CallExpression(call_expr) => {
				Self::get_prefer_test_name_and_span(
					call_expr.callee.get_inner_expression(),
					test_name,
					fix_jest_name,
				)
...
fn test() {
	...
	let fix_vitest = vec![
		// Note: couldn't  fixed, because the fixer doesn't support to set the
		// options for the fix cases. Todo: this need to fixer support option
		// configuration. ("test(\"shows error\", () => {});", "it(\"shows
		// error\", () => {});"), ("test.skip(\"shows error\");",
		// "it.skip(\"shows error\");"), ("test.only('shows error');",
		// "it.only('shows error');"), (
		//     "describe('foo', () => { it('bar', () => {}); });",
		...
		// {});",     "import { it } from \"vitest\"\nit(\"shows error\", ()
		// => {});", ),
		// ("describe(\"suite\", () => { it(\"foo\") })", "describe(\"suite\",
		// () => { test(\"foo\") })"), ("test(\"foo\")", "it(\"foo\")"),
		// ("describe(\"suite\", () => { it(\"foo\") })", "describe(\"suite\",
		// () => { test(\"foo\") })"),
