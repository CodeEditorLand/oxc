...
fn test() {
	...
	let pass = vec![
		...
		(
			"describe('a test', () => { it('an it', () => { const func = () => { \
			 expect(1).toBe(1); }; }); });",
			None,
		),
		(
		...
		(
			"it.each([1, true])('trues', value => { expect(value).toBe(true); }); it('an it', () \
			 => { expect(1).toBe(1) });",
			None,
		),
		(
		...
		(
			"describe.each([1, true])('trues', value => { it('an it', () => \
			 expect(value).toBe(true) ); });",
			None,
		),
		(
	...
	Tester::new(NoStandaloneExpect::NAME, pass, fail)
