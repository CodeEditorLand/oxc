...
// To flag we encountered a conditional block/catch block when traversing the
// parents.
...
fn run<'a>(possible_jest_node:&PossibleJestNode<'a, '_>, ctx:&LintContext<'a>) {
	...
	if let AstKind::CallExpression(call_expr) = node.kind() {
		...
		// When first visiting the node, we assume it's not in a conditional
		// block.
...
fn check_parents<'a>(
	...
	// if the node is already visited, we should return `false` to avoid
	// infinite loop.
	...
	match parent_node.kind() {
		...
		AstKind::Function(function) => {
			...
			// }
			// ````
			// To avoid infinite loop, we need to check if the function is
			// already visited when call `check_parents`.
