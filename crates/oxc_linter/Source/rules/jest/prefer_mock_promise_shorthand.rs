...
fn test() {
	...
	let fix = vec![
		...
		// Todo: Fixed
		// (
		//     "jest.spyOn(fs, \"readFile\").mockReturnValue(Promise.reject(new Error(\"oh
		// noes!\")))",     "jest.spyOn(fs, \"readFile\").mockRejectedValue(new Error(\"oh
		// noes!\"))",     None,
		// ),
	];

	Tester::new(
		PreferMockPromiseShorthand::NAME,
		PreferMockPromiseShorthand::CATEGORY,
		pass,
		fail,
	)
