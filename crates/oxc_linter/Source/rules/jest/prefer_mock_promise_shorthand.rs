...
impl PreferMockPromiseShorthand {
	fn report<'a>(
		...
		let prefer_name:&'static str = if arg_name.ends_with("reject") {
			mock_promise_reject
		} else {
...
fn test() {
	...
	let fix = vec![
		...
		(
			"jest.fn().mockReturnValue(Promise.resolve(42))",
			"jest.fn().mockResolvedValue(42)",
			None,
		),
		(
			"jest.fn().mockReturnValue(Promise.reject(42))",
			"jest.fn().mockRejectedValue(42)",
			None,
		),
		(
			"aVariable.mockReturnValue(Promise.resolve(42))",
			"aVariable.mockResolvedValue(42)",
			None,
		),
		(
			"aVariable.mockReturnValue(Promise.reject(42))",
			"aVariable.mockRejectedValue(42)",
			None,
		),
		(
		...
		// Todo: Fixed
		// (
		//     "aVariable.mockReturnValue(Promise.resolve({ target: 'world',
		// message: 'hello' }))",     "aVariable.mockResolvedValue({ target:
		// 'world', message: 'hello' })",     None,
		// ),
		(
			"
                aVariable
		...
		// Todo: Fixed
		// (
		//     "jest.spyOn(fs, \"readFile\").mockReturnValue(Promise.reject(new
		// Error(\"oh noes!\")))",     "jest.spyOn(fs,
		// \"readFile\").mockRejectedValue(new Error(\"oh noes!\"))",
		//     None,
		// ),
	];
