...
fn tests() {
	...
	let fail = vec![
		...
		(
			"expect((meta.get('pages') as \
			 YArray<unknown>).length).toBe((originalMeta.get('pages') as YArray<unknown>).length);",
			None,
		),
		(
	...
	let fix = vec![
		...
		(
			"expect(files.length).not.toStrictEqual(1);",
			"expect(files).not.toHaveLength(1);",
			None,
		),
		(
			"expect((meta.get('pages') as \
			 YArray<unknown>).length).toBe((originalMeta.get('pages') as YArray<unknown>).length);",
			"expect((meta.get('pages') as \
			 YArray<unknown>)).toHaveLength((originalMeta.get('pages') as \
			 YArray<unknown>).length);",
			None,
		),
		(
			"expect(assetTypeContainer.getElementsByTagName('time').length).toEqual(
