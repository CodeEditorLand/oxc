...
fn no_export_diagnostic(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::warn("Do not export from a test file.")
		.with_help(
			"If you want to share code between tests, move it into a separate file and import it \
			 from there.",
		)
		.with_label(span)
}
...
fn test() {
	...
	let fail = vec![
		...
		// ("module.exports['invalid'] = function() {};  test('a test', () => {
		// expect(1).toBe(1);});", None), ("module.exports = function() {}; ;  test('a test', ()
		// => { expect(1).toBe(1);});", None), ("module.export.invalid = function() {}; ;
		// test('a test', () => { expect(1).toBe(1);});", None)
	];
