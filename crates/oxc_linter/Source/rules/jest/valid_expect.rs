...
fn get_parent_with_ignore<'a, 'b>(
	...
	loop {
		...
		if !matches!(parent.kind(), AstKind::Argument(_) | AstKind::ArrayExpressionElement(_)) {
			// we don't want to report `Promise.all([invalidExpectCall_1,
			// invalidExpectCall_2])` twice. so we need mark whether the node is the
			// first item of an array. if it not the first item, we ignore it in
			// `find_promise_call_expression_node`.
...
fn get_parent_if_thenable<'a, 'b>(
	...
	let grandparent = ctx
		.nodes()
		.parent_node(node.id())
		.and_then(|node| ctx.nodes().parent_node(node.id()));
...
fn test_1() {
	...
	let pass = vec![(
		"test('valid-expect', async () => { await \
		 Promise.race([expect(Promise.reject(2)).rejects.not.toBeDefined(), \
		 expect(Promise.reject(2)).rejects.not.toBeDefined()]); })",
		None,
	)];
...
fn test() {
	...
	let mut pass = vec![
		...
		(
			"test('valid-expect', () => { return \
			 expect(Promise.resolve(2)).rejects.not.toBeDefined(); });",
			None,
		),
		(
			"test('valid-expect', function () { return \
			 expect(Promise.resolve(2)).resolves.not.toBeDefined(); });",
			None,
		),
		(
		...
		(
			"test('valid-expect', function () { return \
			 Promise.resolve(expect(Promise.resolve(2)).rejects.not.toBeDefined()); });",
			None,
		),
		(
		...
		(
			"test('valid-expect', async () => { await \
			 expect(Promise.reject(2)).resolves.not.toBeDefined(); });",
			None,
		),
		(
			"test('valid-expect', async () => { await \
		...
		(
			"test('valid-expect', async function () { await \
			 expect(Promise.reject(2)).resolves.not.toBeDefined(); });",
			None,
		),
		(
		...
		(
			"test('valid-expect', async () => { await \
			 Promise.resolve(expect(Promise.reject(2)).rejects.not.toBeDefined()); });",
			None,
		),
		(
			"test('valid-expect', async () => { await \
			 Promise.reject(expect(Promise.reject(2)).rejects.not.toBeDefined()); });",
			None,
		),
		(
			"test('valid-expect', async () => { await \
			 Promise.all([expect(Promise.reject(2)).rejects.not.toBeDefined(), \
			 expect(Promise.reject(2)).rejects.not.toBeDefined()]); });",
			None,
		),
		(
		...
		(
			"test('valid-expect', async () => { await \
			 Promise.allSettled([expect(Promise.reject(2)).rejects.not.toBeDefined(), \
			 expect(Promise.reject(2)).rejects.not.toBeDefined()]); });",
			None,
		),
		(
			"test('valid-expect', async () => { await \
			 Promise.any([expect(Promise.reject(2)).rejects.not.toBeDefined(), \
			 expect(Promise.reject(2)).rejects.not.toBeDefined()]); });",
			None,
		),
		(
			"test('valid-expect', async () => { return \
			 expect(Promise.reject(2)).resolves.not.toBeDefined().then(() => \
		...
		(
			"test('valid-expect', async () => { return \
			 expect(Promise.reject(2)).resolves.not.toBeDefined().then(() => \
			 console.log('valid-case')).then(() => console.log('another valid case')); });",
			None,
		),
		(
			"test('valid-expect', async () => { return \
			 expect(Promise.reject(2)).resolves.not.toBeDefined().catch(() => \
			 console.log('valid-case')); });",
			None,
		),
		(
			"test('valid-expect', async () => { return \
			 expect(Promise.reject(2)).resolves.not.toBeDefined().then(() => \
			 console.log('valid-case')).catch(() => console.log('another valid case')); });",
			None,
		),
		(
			"test('valid-expect', async () => { return \
		...
		(
			"expect(1, '1 !== 2').toBe(2);",
			Some(serde_json::json!([{ "maxArgs": 2, "minArgs": 2 }])),
		),
		(
	...
	let mut fail = vec![
		...
		(
			"expect('something', 'else', 'entirely').toEqual('something');",
			Some(serde_json::json!([{ "maxArgs": 2 }])),
		),
		(
			"expect('something', 'else', 'entirely').toEqual('something');",
			Some(serde_json::json!([{ "maxArgs": 2, "minArgs": 2 }])),
		...
		(
			"expect('something', 'else', 'entirely').toEqual('something');",
			Some(serde_json::json!([{ "maxArgs": 2, "minArgs": 1 }])),
		),
		(
			"expect('something').toEqual('something');",
			Some(serde_json::json!([{ "minArgs": 2 }])),
		),
		(
			"expect('something', 'else').toEqual('something');",
			Some(serde_json::json!([{ "maxArgs": 1, "minArgs": 3 }])),
		),
		...
		(
			"test('valid-expect', () => { expect(Promise.resolve(2)).not.toReject(); });",
			None,
		),
		(
		...
		(
			"test('valid-expect', () => { expect(Promise.reject(2)).rejects.toBe(2); });",
			Some(serde_json::json!([{ "asyncMatchers": ["toRejectWith"] }])),
		),
		(
			"
	...
	let fail_vitest = vec![
		(
			"expect().toBe(2);",
			Some(serde_json::json!([{ "minArgs": "undefined", "maxArgs": "undefined" }])),
		),
		("expect().toBe(true);", None),
		("expect().toEqual(\"something\");", None),
		("expect(\"something\", \"else\").toEqual(\"something\");", None),
		...
		(
			"test(\"valid-expect\", () => { expect(Promise.resolve(2)).toResolve(); });",
			None,
		),
		(
			"test(\"valid-expect\", () => { expect(Promise.resolve(2)).toResolve(); });",
		...
		(
			"test(\"valid-expect\", () => { expect(Promise.resolve(2)).toReject(); });",
			None,
		),
		(
			"test(\"valid-expect\", () => { expect(Promise.resolve(2)).not.toReject(); });",
