...
fn get_parent_with_ignore<'a, 'b>(
	...
	loop {
		...
		if !matches!(
			...
			// invalidExpectCall_2])` twice. so we need mark whether the node
			// is the first item of an array. if it not the first item, we
			// ignore it in `find_promise_call_expression_node`.
