...
fn check_assert_function_used<'a>(
	...
	// If we have visited this node before and didn't find any assert function, we
	// can return `false` to avoid infinite loop.
	//
	// ```javascript
	// test("should fail", () => {
	//    function foo() {
	...
	match expr {
		...
		Expression::CallExpression(call_expr) => {
			...
			// If CallExpression is not an assert function, we need to check its arguments,
			// it may trigger another assert function.
			// ```javascript
...
fn test() {
	...
	let mut fail = vec![
		...
		// TODO: is this case usual? not support this now, which need visit all call expression and
		// get it's node name (
		//     "
		// 	theoretically('the number {input} is correctly translated to string', theories, theory
		// => { 	const output = NumberToLongString(theory.input);
	...
	let fail_vitest = vec![
		...
		//     "
		//         theoretically('the number {input} is correctly translated to string', theories,
		// theory => {             const output = NumberToLongString(theory.input);
		//         })
		//     ",
		//     Some(serde_json::json!([{ "additionalTestBlockFunctions": ["theoretically"] }])),
		// ),
