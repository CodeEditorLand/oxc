	...
	if !valid_title.disallowed_words.is_empty() {
		...
		if let Some(matched) = disallowed_words_reg.find(title) {
			...
			ctx.diagnostic(valid_title_diagnostic(
				&error,
				"It is included in the `disallowedWords` of your config file, try to remove it \
				 from your title",
				span,
			));
		}
		...
		match self {
			...
			Self::DuplicatePrefix => {
				(
					...
					"The function name has already contains the prefix, try remove the duplicate \
					 prefix",
				)
			},
...
fn test() {
	...
	let fail = vec![
		...
		// TODO: The regex `(?:#(?!unit|e2e))\w+` in those test cases is not
		// valid in Rust (
		//     "
		//         describe('things to test', () => {
		//             describe('unit tests #unit', () => {
		...
		// (
		//     "
		//         import { describe, describe as context, it as thisTest }
		// from '@jest/globals';
		...
		//         ",
		//     Some(
		//         serde_json::json!([ { "mustNotMatch":
		// r#"(?:#(?!unit|e2e))\w+"#, "mustMatch": "^[^#]+$|(?:#(?:unit|e2e))",
		// }, ]),     ),
		// ),
		// (
		//     "
