...
fn test() {
	...
	let pass = vec![
		...
		(
			...
			Some(serde_json::json!([{ "not.toBe": null }])),
		),
		(
	...
	let fail = vec![
		...
		(
			"expect(a).resolves.not.toBe(b)",
			Some(serde_json::json!([{ "resolves": null }])),
		),
		(
			"expect(a).resolves.not.toBe(b)",
			Some(serde_json::json!([{ "resolves.not": null }])),
		),
	...
	Tester::new(NoRestrictedMatchers::NAME, pass, fail)
