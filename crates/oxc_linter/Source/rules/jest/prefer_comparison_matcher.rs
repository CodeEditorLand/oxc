...
impl PreferComparisonMatcher {
	fn run<'a>(possible_jest_node:&PossibleJestNode<'a, '_>, ctx:&LintContext<'a>) {
		...
		ctx.diagnostic_with_fix(use_to_be_comparison(prefer_matcher_name, matcher.span), |fixer| {
			// This is to handle the case can be transform into the
			// following case: expect(value > 1,).toEqual(true,) =>
			// expect(value,).toBeGreaterThan(1,)                 ^
			// ^ Therefore the range starting after ',' and before '.'
			// is called as call_span_end, and the same as
			// `arg_span_end`.
			let call_span_end =
