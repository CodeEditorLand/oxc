...
impl Rule for CatchOrReturn {
	fn from_configuration(value:serde_json::Value) -> Self {
		...
		if let Some(allow_finally_config) = value
			.get(0)
			.and_then(|v| v.get("allowFinally"))
			.and_then(serde_json::Value::as_bool)
		{
			config.allow_finally = allow_finally_config;
		}

		if let Some(allow_then_config) = value
			.get(0)
			.and_then(|v| v.get("allowThen"))
			.and_then(serde_json::Value::as_bool)
		{
	...
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		// Check for a promise or a method call at the end of a promise for
		// example: foo().catch().randomFunc()
...
fn test() {
	...
	let pass = vec![
		...
		(
			"frank().then(goo).done()",
			Some(serde_json::json!([{ "terminationMethod": "done" }])),
		),
		(
			"frank().then(go).catch()",
