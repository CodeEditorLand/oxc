...
impl LintPlugins {
	...
	#[inline]
	pub fn has_vitest(self) -> bool { self.contains(LintPlugins::VITEST) }

	/// Returns `true` if the Jest plugin is enabled.
	#[inline]
	pub fn has_jest(self) -> bool { self.contains(LintPlugins::JEST) }

	/// Returns `true` if Jest or Vitest plugins are enabled.
	#[inline]
	pub fn has_test(self) -> bool { self.intersects(LintPlugins::JEST.union(LintPlugins::VITEST)) }

	/// Returns `true` if the import plugin is enabled.
	#[inline]
	pub fn has_import(self) -> bool { self.contains(LintPlugins::IMPORT) }
}
...
impl From<&str> for LintPlugins {
	fn from(value:&str) -> Self {
		match value {
			...
			// deepscan for backwards compatibility. Those rules have been moved
			// into oxc
			...
			// "eslint" is not really a plugin, so it's 'empty'. This has the
			// added benefit of making it the default value.
			_ => LintPlugins::empty(),
		}
	}
}
...
impl JsonSchema for LintPlugins {
	fn schema_name() -> String { "LintPlugins".to_string() }

	fn schema_id() -> std::borrow::Cow<'static, str> { std::borrow::Cow::Borrowed("LintPlugins") }

	fn json_schema(gen:&mut SchemaGenerator) -> Schema { gen.subschema_for::<Vec<&str>>() }
}
