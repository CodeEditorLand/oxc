...
impl AllowWarnDeny {
	pub fn is_warn_deny(self) -> bool { self != Self::Allow }

	pub fn is_allow(self) -> bool { self == Self::Allow }
...
impl TryFrom<&Value> for AllowWarnDeny {
	...
	fn try_from(value:&Value) -> Result<Self, OxcDiagnostic> {
		match value {
			...
			_ => {
				Err(OxcDiagnostic::error(format!(
					"Failed to parse rule severity, expected a string or a number, but got \
					 {value:?}"
				)))
			},
		}
...
impl JsonSchema for AllowWarnDeny {
	fn schema_name() -> String { "AllowWarnDeny".to_string() }

	fn schema_id() -> std::borrow::Cow<'static, str> { "AllowWarnDeny".into() }
