...
/// 1. Filter by rule name and/or plugin: `no-const-assign`,
...
impl LintFilter {
	/// # Errors
	///
	/// If `kind` is an empty string, or is a `<plugin>/<rule>` filter but is
	/// missing either the plugin or the rule.
	...
	#[inline]
	pub fn severity(&self) -> AllowWarnDeny { self.severity }

	#[inline]
	pub fn kind(&self) -> &LintFilterKind { &self.kind }
}
...
impl From<LintFilter> for (AllowWarnDeny, LintFilterKind) {
	fn from(val:LintFilter) -> Self { (val.severity, val.kind) }
}

impl<'a> From<&'a LintFilter> for (AllowWarnDeny, &'a LintFilterKind) {
	fn from(val:&'a LintFilter) -> Self { (val.severity, &val.kind) }
}
...
impl LintFilterKind {
	/// # Errors
	///
	/// If `filter` is an empty string, or is a `<plugin>/<rule>` filter but is
	/// missing either the plugin or the rule.
	pub fn parse(filter:Cow<'static, str>) -> Result<Self, InvalidFilterKind> {
		...
		if filter.contains('/') {
			// this is an unfortunate amount of code duplication, but it needs
			// to be done for `filter` to live long enough to avoid a String
			// allocation for &'static str
...
impl TryFrom<String> for LintFilterKind {
	...
	#[inline]
	fn try_from(filter:String) -> Result<Self, Self::Error> { Self::parse(Cow::Owned(filter)) }
}
...
impl TryFrom<Cow<'static, str>> for LintFilterKind {
	...
	#[inline]
	fn try_from(filter:Cow<'static, str>) -> Result<Self, Self::Error> { Self::parse(filter) }
}

impl From<RuleCategory> for LintFilterKind {
	#[inline]
	fn from(category:RuleCategory) -> Self { LintFilterKind::Category(category) }
}
