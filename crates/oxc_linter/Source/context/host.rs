...
pub(crate) struct ContextHost<'a> {
	/// Shared semantic information about the file being linted, which includes
	/// scopes, symbols and AST nodes. See [`Semantic`].
	pub(super) semantic:Rc<Semantic<'a>>,
	/// Cross module information.
	pub(super) module_record:Arc<ModuleRecord>,
	/// Information about specific rules that should be disabled or enabled, via
	/// comment directives like `eslint-disable` or `eslint-disable-next-line`.
...
impl<'a> ContextHost<'a> {
	...
	pub fn new<P:AsRef<Path>>(
		...
		// We should always check for `semantic.cfg()` being `Some` since we depend on
		// it and it is unwrapped without any runtime checks after construction.
		assert!(
	...
	#[inline]
	pub fn semantic(&self) -> &Semantic<'a> { &self.semantic }

	/// Shared reference to the [`ModuleRecord`] of the file.
	#[inline]
	pub fn module_record(&self) -> &ModuleRecord { &self.module_record }
	...
	#[inline]
	pub fn plugins(&self) -> LintPlugins { self.plugins }

	/// Add a diagnostic message to the end of the list of diagnostics. Can be
	/// used by any rule to report issues.
	#[inline]
	...
	/// Maps Jest rule names and maps to Vitest rules when possible, returning
	/// the original plugin otherwise.
	///
	/// Many Vitest rules are essentially ports of the Jest plugin rules with
	/// minor modifications. For these rules, we use the corresponding jest
	/// rules with some adjustments for compatibility.
...
impl<'a> From<ContextHost<'a>> for Vec<Message<'a>> {
	fn from(ctx_host:ContextHost<'a>) -> Self { ctx_host.diagnostics.into_inner() }
}
