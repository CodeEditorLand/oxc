...
#[derive(Clone)]
#[must_use]
/// Contains all of the state and context specific to this lint rule. Includes
/// information like the rule name, plugin name, and severity of the rule. It
/// also has a reference to the shared linting data [`ContextHost`], which is
/// the same for all rules.
pub struct LintContext<'a> {
	/// Shared context independent of the rule being linted.
	parent:Rc<ContextHost<'a>>,
	/// Name of the plugin this rule belongs to. Example: `eslint`, `unicorn`,
	/// `react`
	...
	/// Kebab-cased name of the current rule being linted. Example:
...
impl<'a> LintContext<'a> {
	/// Base URL for the documentation, used to generate rule documentation URLs
	...
	/// Set the current rule name. Name should be kebab-cased like:
	/// `no-unused-vars` or `no-undef`.
	pub fn with_rule_name(mut self, name:&'static str) -> Self {
		self.current_rule_name = name;
		self
	}

	/// Set the current rule fix capabilities. See [`RuleFixMeta`] for more
	/// information.
	#[cfg(debug_assertions)]
	...
	#[inline]
	pub fn semantic(&self) -> &Rc<Semantic<'a>> { &self.parent.semantic }

	/// Get the control flow graph for the current program.
	#[inline]
	pub fn cfg(&self) -> &ControlFlowGraph {
		// SAFETY: `LintContext::new` is the only way to construct a
		// `LintContext` and we always assert the existence of control flow so
	...
	#[inline]
	pub fn disable_directives(&self) -> &DisableDirectives<'a> { &self.parent.disable_directives }

	/// Source code of the file being linted.
	#[inline]
	pub fn source_text(&self) -> &'a str { self.semantic().source_text() }
	...
	#[inline]
	pub fn source_type(&self) -> &SourceType { self.semantic().source_type() }

	/// Path to the file currently being linted.
	#[inline]
	pub fn file_path(&self) -> &Path { &self.parent.file_path }

	/// Plugin settings
	#[inline]
	pub fn settings(&self) -> &OxlintSettings { &self.parent.config.settings }

	/// Sets of global variables that have been enabled or disabled.
	#[inline]
	pub fn globals(&self) -> &OxlintGlobals { &self.parent.config.globals }

	/// Runtime environments turned on/off by the user.
	///
	/// Examples of environments are `builtin`, `browser`, `node`, etc.
	#[inline]
	pub fn env(&self) -> &OxlintEnv { &self.parent.config.env }

	/// Checks if a given variable named is defined as a global variable in the
	/// current environment.
	///
	/// Example:
	/// - `env_contains_var("Date")` returns `true` because it is a global
	///   builtin in all environments.
	/// - `env_contains_var("HTMLElement")` returns `true` only if the `browser`
	///   environment is enabled.
	/// - `env_contains_var("globalThis")` returns `true` only if the `es2020`
	///   environment or higher is enabled.
	...
	/// Add a diagnostic message to the list of diagnostics. Outputs a
	/// diagnostic with the current rule name, severity, and a link to the
	...
	/// and definitely does not change semantics, use
	/// [`LintContext::diagnostic_with_fix`]. If your fix has the potential to
	/// create parse errors, use
	/// [`LintContext::diagnostic_with_dangerous_fix`].
	///
	/// [closure]: <https://doc.rust-lang.org/book/ch13-01-closures.html>
	#[inline]
	...
	/// - Your fix is extremely aggressive and risky, but you want to provide it
	...
	/// Report a lint rule violation and provide an automatic fix of a specific
	/// kind.
	///
	/// The second argument is a [closure] that takes a [`RuleFixer`] and
	...
	pub fn frameworks(&self) -> FrameworkFlags { self.parent.frameworks }

	/// AST nodes
	///
	/// Shorthand for `self.semantic().nodes()`.
	pub fn nodes(&self) -> &AstNodes<'a> { self.semantic().nodes() }

	/// Scope tree
	///
	/// Shorthand for `ctx.semantic().scopes()`.
	pub fn scopes(&self) -> &ScopeTree { self.semantic().scopes() }

	/// Symbol table
	///
	/// Shorthand for `ctx.semantic().symbols()`.
	pub fn symbols(&self) -> &SymbolTable { self.semantic().symbols() }

	/// Imported modules and exported symbols
	///
	/// Shorthand for `ctx.semantic().module_record()`.
	pub fn module_record(&self) -> &ModuleRecord { self.semantic().module_record() }

	/// JSDoc comments
	///
	/// Shorthand for `ctx.semantic().jsdoc()`.
	pub fn jsdoc(&self) -> &JSDocFinder<'a> { self.semantic().jsdoc() }
}
