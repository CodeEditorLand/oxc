...
impl<'a> Deref for LintContext<'a> {
	...
	#[inline]
	fn deref(&self) -> &Self::Target { self.parent.semantic() }
}
...
impl<'a> LintContext<'a> {
	...
	#[inline]
	pub fn module_record(&self) -> &ModuleRecord { self.parent.module_record() }

	/// Get the control flow graph for the current program.
	#[inline]
	pub fn cfg(&self) -> &ControlFlowGraph {
		// SAFETY: `LintContext::new` is the only way to construct a `LintContext` and
		// we always assert the existence of control flow so it should always be
		// `Some`.
