...
impl Default for Linter {
	fn default() -> Self { LinterBuilder::default().build() }
}

impl Linter {
	pub(crate) fn new(options:LintOptions, config:ConfigStore) -> Self { Self { options, config } }
	...
	pub(crate) fn options(&self) -> &LintOptions { &self.options }

	pub fn number_of_rules(&self) -> usize { self.config.number_of_rules() }

	pub(crate) fn rules(&self) -> &Arc<[RuleWithSeverity]> { self.config.rules() }
	...
	pub fn run<'a>(
		...
		// Get config + rules for this file. Takes base rules and applies glob-based
		// overrides.
	...
	pub fn print_rules_json<W:Write>(writer:&mut W) {
		...
		let rules_info = RULES.iter().map(|rule| {
			RuleInfoJson { scope:rule.plugin_name(), value:rule.name(), category:rule.category() }
		});
