...
pub use crate::{
	...
	options::{
		AllowWarnDeny, InvalidFilterKind, LintFilter, LintFilterKind,
		OxlintOptions,
	},
...
impl Linter {
	...
	pub fn run<'a>(
		...
		let ctx_host = Rc::new(
			ContextHost::new(path, semantic, self.options)
	...
	pub fn print_rules<W: Write>(writer: &mut W) {
		...
		for section in table.sections {
			writeln!(writer, "{}", section.render_markdown_table(None))
				.unwrap();
		}
		writeln!(writer, "Default: {}", table.turned_on_by_default_count)
			.unwrap();
