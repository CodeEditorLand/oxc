...
impl Value {
	...
	/// If the value is a boolean, return the negation of the boolean, otherwise
	/// return `None`.
...
pub fn is_pure_function(function_name:&FunctionName, options:&NodeListenerOptions) -> bool {
	...
	if options
		.whitelist_functions
		.is_some_and(|whitelist| whitelist.contains(&name.to_string()))
	{
		return true;
	}
...
pub fn get_leading_tree_shaking_comment<'a>(span:Span, ctx:&LintContext<'a>) -> Option<&'a str> {
	...
	// Next step, we need make sure it's not the trailing comment of the previous
	// line.
		...
		BinaryOperator::ShiftRightZeroFill => {
			match (left, right) {
				...
				(Value::Number(a), Value::Number(b)) => {
					...
					// 4. Return the result of performing a zero-filling right shift of lNum by
					//    shiftCount bits.
		...
		BinaryOperator::ShiftRight => {
			match (left, right) {
				...
				(Value::Number(a), Value::Number(b)) => {
					...
					// 3. Let shiftCount be ℝ(rNum) modulo 32.
					let shift_count = r_num % 32;
					// 4. Return the result of performing a sign-extending right shift of lNum by
					//    shiftCount bits. The most significant bit is propagated. The mathematical
...
fn test_calculate_binary_operation() {
	...
	assert_eq!(
		fun(op, Value::Number(1.0), Value::Number(31.0),),
