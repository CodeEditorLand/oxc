...
pub fn has_jsx_prop_ignore_case<'a, 'b>(
	...
	node.attributes.iter().find(|attr| {
		attr.as_attribute()
			.is_some_and(|attr| attr.is_identifier_ignore_case(target_prop))
	})
}
...
pub fn is_hidden_from_screen_reader<'a>(ctx:&LintContext<'a>, node:&JSXOpeningElement<'a>) -> bool {
	...
	has_jsx_prop_ignore_case(node, "aria-hidden").map_or(false, |v| {
...
pub fn is_react_hook(expr:&Expression) -> bool {
	match expr {
		match_member_expression!(Expression) => {
			// SAFETY: We already have checked that `expr` is a member
			// expression using the `match_member_expression` macro.
...
pub fn is_react_function_call(call:&CallExpression, expected_call:&str) -> bool {
	let Some(subject) = call.callee_name() else {
		return false;
	};
