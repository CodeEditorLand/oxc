...
pub fn get_function_nearest_jsdoc_node<'a, 'b>(
	...
	// Whether the node has attached JSDoc or not is determined by
	// `JSDocBuilder`
...
pub fn collect_params(params:&FormalParameters) -> Vec<ParamKind> {
	...
	fn get_param_name(pattern:&BindingPattern, is_rest:bool) -> ParamKind {
		match &pattern.kind {
				...
				for prop in &obj_pat.properties {
					...
					match get_param_name(&prop.value, false) {
						...
						ParamKind::Nested(params) => {
							...
							for param in params {
								collected
									.push(Param { name:format!("{name}.{}", param.name), ..param });
							}
						},
					}
				}
			...
			BindingPatternKind::AssignmentPattern(assign_pat) => {
				match &assign_pat.right {
					Expression::Identifier(_) => get_param_name(&assign_pat.left, false),
					_ => {
						// TODO: If `config.useDefaultObjectProperties` = true,
						// collect default parameters from `assign_pat.right`
						// like: { prop = { a: 1, b: 2 }} => [prop,
						// prop.a, prop.b]     get_param_name(&assign_pat.
						// left, false) }
	...
	let mut collected = params
		.items
		.iter()
		.map(|param| get_param_name(&param.pattern, false))
		.collect::<Vec<_>>();
