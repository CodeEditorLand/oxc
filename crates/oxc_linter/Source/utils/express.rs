...
fn is_req_param(param:&FormalParameter) -> bool {
	param
		.pattern
		.get_identifier()
		.map_or(false, |id| COMMON_REQUEST_NAMES.contains(id.as_str()))
}
...
fn is_res_param(param:&FormalParameter) -> bool {
	param
		.pattern
		.get_identifier()
		.map_or(false, |id| COMMON_RESPONSE_NAMES.contains(id.as_str()))
}
...
fn is_next_param(param:&FormalParameter) -> bool {
	param
		.pattern
		.get_identifier()
		.map_or(false, |id| COMMON_NEXT_NAMES.contains(id.as_str()))
}
...
fn is_error_param(param:&FormalParameter) -> bool {
	param
		.pattern
		.get_identifier()
		.map_or(false, |id| COMMON_ERROR_NAMES.contains(id.as_str()))
}
