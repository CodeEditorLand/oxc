...
pub use crate::utils::jest::parse_jest_fn::{
	ExpectError,
	KnownMemberExpressionParentKind,
...
pub fn is_jest_file(ctx:&LintContext) -> bool {
	if ctx.file_path().components().any(|c| {
		match c {
...
pub struct PossibleJestNode<'a, 'b> {
	...
	pub original:Option<&'a str>, /* if this node is imported from 'jest/globals', this field
	                               * will be Some(original_name), otherwise None */
}

/// Collect all possible Jest fn Call Expression,
/// for `expect(1).toBe(1)`, the result will be a collection of node `expect(1)`
/// and node `expect(1).toBe(1)`.
...
/// for `expect(1).toBe(1)`, the result will be an iter over node `expect(1)`
/// and node `expect(1).toBe(1)`.
pub fn iter_possible_jest_call_node<'a, 'c>(
	semantic:&'c Semantic<'a>,
) -> impl Iterator<Item = PossibleJestNode<'a, 'c>> + 'c {
	// Some people may write codes like below, we need lookup imported test function
	// and global test function. ```
...
/// Find name in the Import Declaration, not use name because of lifetime not
/// long enough.
fn find_original_name<'a>(import_decl:&'a ImportDeclaration<'a>, name:&str) -> Option<&'a str> {
	import_decl.specifiers.iter().flatten().find_map(|specifier| {
		match specifier {
