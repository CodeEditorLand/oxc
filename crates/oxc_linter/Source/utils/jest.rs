...
pub struct PossibleJestNode<'a, 'b> {
	...
	pub original:Option<&'a str>, /* if this node is imported from
	                               * 'jest/globals', this field will be
	                               * Some(original_name), otherwise None */
}
...
pub fn collect_possible_jest_call_node<'a, 'b>(
	...
	// Some people may write codes like below, we need lookup imported test
	// function and global test function. ```
	...
	reference_id_with_original_list
		.iter()
		.fold(vec![], |mut acc, id_with_original| {
