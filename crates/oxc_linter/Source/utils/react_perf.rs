...
pub(crate) trait ReactPerfRule: Sized + Default + fmt::Debug {
	...
	/// Check if a node of some [`AstKind`] violates a react perf rule. If it
	/// does, report the [`OxcDiagnostic`] and return `true`.
	///
	/// [`OxcDiagnostic`]: oxc_diagnostics::OxcDiagnostic
	...
	R: ReactPerfRule,
{
	fn run<'a>(&self, node:&AstNode<'a>, ctx:&LintContext<'a>) {
		...
		// look for JSX attributes whose values are expressions (foo={bar}) (as
		// opposed to spreads ({...foo}) or just boolean attributes) (<div foo
		// />)
