...
pub fn parse_jest_fn_call<'a>(
	...
	let params = NodeChainParams {
		expr:callee,
		parent:None, /* TODO: not really know how to convert type of
		              * call_expr to Expression, set to `None` temporarily. */
	...
	if let Some(last) = chain.last() {
		// If we're an `each()`, ensure we're the outer CallExpression (i.e
		// `.each()()`)
		...
		// Ensure that we're at the "top" of the function call chain otherwise
		// when parsing e.g. x().y.z(), we'll incorrectly find & parse "x()"
		// even though the full chain is not a valid jest function call chain
...
fn parse_jest_jest_fn_call<'a>(
	...
	let kind = if lowercase_name == "jest" {
		JestGeneralFnKind::Jest
	} else {
		JestGeneralFnKind::Vitest
	};
...
impl<'a> KnownMemberExpressionProperty<'a> {
	pub fn name(&self) -> Option<Cow<'a, str>> {
		match &self.element {
			MemberExpressionElement::Expression(expr) => {
				match expr {
	...
	pub fn is_name_equal(&self, name:&str) -> bool { self.name().map_or(false, |n| n == name) }

	pub fn is_name_unequal(&self, name:&str) -> bool { !self.is_name_equal(name) }
