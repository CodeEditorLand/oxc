...
impl RuleTable {
	pub fn new() -> Self {
		...
		.filter_map(|category| {
			...
			let plugin_column_width =
				rows.iter().map(|r| r.plugin.len()).max()?;
...
impl RuleTableSection {
	...
	pub fn render_markdown_table(&self, link_prefix: Option<&str>) -> String {
		...
		writeln!(
			s,
			"| {x:-<rule_width$} | {x:-<plugin_width$} | {x:-<7} | {x:-<8} |"
		)
		.unwrap();
		...
		for row in rows {
			...
			let (default, default_width) = if row.turned_on_by_default {
				("✅", DEFAULT - 1)
			} else {
				("", DEFAULT)
			};
			...
			let (fix_emoji, fix_emoji_width) =
				row.autofix.emoji().map_or(("", FIX), |emoji| {
					let len = emoji.len();
					if len > FIX {
						(emoji, 0)
					} else {
