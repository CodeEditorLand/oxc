...
use crate::{Linter, RuleCategory, RuleFixMeta, rules::RULES};
...
impl RuleTableSection {
	...
	pub fn render_markdown_table(&self, link_prefix:Option<&str>) -> String {
		...
		for row in rows {
			...
			writeln!(
				s,
				"| {rendered_name:<rule_width$} | {plugin_name:<plugin_width$} | \
				 {default:<default_width$} | {fix_emoji:<fix_emoji_width$} |"
			)
			.unwrap();
		}
...
mod test {
	...
	fn table() -> &'static RuleTable { TABLE.get_or_init(RuleTable::new) }
