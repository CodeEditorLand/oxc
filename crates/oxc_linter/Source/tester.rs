...
pub struct Tester {
	...
	/// Providing `Some(vec![])` allows for intentional disabling of this
	/// behavior.
	///
	/// Note that disabling this check should be done as little as possible, and
	/// never in bad faith (e.g. no `#[test]` functions have fixer cases at
	/// all).
...
impl Tester {
	...
	fn test_fix(&mut self) {
		...
		let Some(fix_test_cases) = self.expect_fix.clone() else {
			assert!(
				!rule.fix().has_fix(),
				"'{}/{}' reports that it can auto-fix violations, but no fix cases were provided. \
				 Please add fixer test cases with `tester.expect_fix()`",
				rule.plugin_name(),
				rule.name()
			);
