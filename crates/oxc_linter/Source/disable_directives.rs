	...
	fn build_impl(&mut self, source_text:&'a str, comments:&[Comment]) {
		...
		for comment in comments {
			...
			if let Some(text) = text
				...
				.or_else(|| text.strip_prefix("oxlint-disable"))
			{
				// `eslint-disable`
				...
				// Remaining text should start with a space, else it's probably a typo of the
				// correct syntax. Like `eslint-disable-lext-nine` where `text` is `-lext-nine`,
			...
			if let Some(text) = text
				...
				.or_else(|| text.strip_prefix("oxlint-enable"))
			{
				// `eslint-enable`
				if text.trim().is_empty() {
...
fn test() {
	...
	for prefix in ["eslint", "oxlint"] {
		...
		let pass = vec![
			// To disable rule warnings in a part of a file, use block comments in the following
			// format:
			format!(
				"
        /* {prefix}-disable */
        ...
        "
			...
			// To disable rule warnings in an entire file, put a /* eslint-disable */ block comment
        ...
        "
			...
			// To ensure that a rule is never applied (regardless of any future enable/disable
			// lines): This is not supported.
			// "
			// /* eslint no-debugger: \"off\" */
			//     debugger;
			// "),
			// To disable all rules on a specific line, use a line or block comment in one of the
			// following formats:
			format!(
