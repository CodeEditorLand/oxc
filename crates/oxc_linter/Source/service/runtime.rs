...
impl Runtime {
	...
	pub(super) fn process_path(&self, path:&Path, tx_error:&DiagnosticSender) {
		...
		// If there are fixes, we will accumulate all of them and write to the file at
		// the end. This means we do not write multiple times to the same file if
		// there are multiple sources in the same file (for example, multiple scripts
		// in an `.astro` file).
	...
	pub(super) fn number_of_dependencies(&self) -> usize { self.modules.len() - self.paths.len() }

	pub(super) fn iter_paths(&self) -> impl Iterator<Item = &Box<Path>> + '_ { self.paths.iter() }
}
