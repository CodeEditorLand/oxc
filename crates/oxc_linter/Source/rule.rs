...
pub trait Rule: Sized + Default + fmt::Debug {
	...
	fn from_configuration(_value:serde_json::Value) -> Self { Self::default() }
	...
	/// This is only called if the Jest plugin is enabled and the file is a test
	/// file. It should be used to run rules that are specific to Jest or
	/// Vitest.
	#[expect(unused_variables)]
	#[inline]
...
pub enum RuleCategory {
	...
	/// The restriction category should, emphatically, not be enabled as a
	/// whole. The contained lints may lint against perfectly reasonable code,
	/// may not have an alternative suggestion, and may contradict any other
	/// lints (including other categories). Lints should be considered on a
...
impl RuleCategory {
	pub fn description(self) -> &'static str {
		match self {
			...
			Self::Restriction => {
				"Lints which prevent the use of language and library features. Must not be enabled \
				 as a whole, should be considered on a case-by-case basis before enabling."
			},
...
impl RuleFixMeta {
	...
	pub const fn fix_kind(self) -> FixKind {
		match self {
			Self::Conditional(kind) | Self::Fixable(kind) => {
				debug_assert!(
					!kind.is_none(),
					"This lint rule indicates that it provides an auto-fix but its FixKind is \
					 None. This is a bug. If this rule does not provide a fix, please use \
					 RuleFixMeta::None. Otherwise, please provide a valid FixKind"
				);
				kind
			},
	...
	#[inline]
	pub fn has_fix(self) -> bool { matches!(self, Self::Fixable(_) | Self::Conditional(_)) }

	#[inline]
	pub fn is_pending(self) -> bool { matches!(self, Self::FixPending) }
		...
		match self {
			...
			Self::Fixable(kind) | Self::Conditional(kind) => {
				...
				let noun = match (kind.contains(FixKind::Fix), kind.contains(FixKind::Suggestion)) {
					...
					_ => {
						unreachable!(
							"Fix kinds must contain Fix and/or Suggestion, but {self:?} has \
							 neither."
						)
					},
				};
...
impl From<RuleFixMeta> for FixKind {
	fn from(value:RuleFixMeta) -> Self { value.fix_kind() }
}
...
impl Hash for RuleWithSeverity {
	fn hash<H:Hasher>(&self, state:&mut H) { self.rule.hash(state); }
}

impl PartialEq for RuleWithSeverity {
	fn eq(&self, other:&Self) -> bool { self.rule == other.rule }
}
...
impl Deref for RuleWithSeverity {
	...
	fn deref(&self) -> &Self::Target { &self.rule }
}

impl Borrow<RuleEnum> for RuleWithSeverity {
	fn borrow(&self) -> &RuleEnum { &self.rule }
}

impl RuleWithSeverity {
	pub fn new(rule:RuleEnum, severity:AllowWarnDeny) -> Self { Self { rule, severity } }
}
