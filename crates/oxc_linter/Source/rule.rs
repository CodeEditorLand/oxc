...
impl RuleFixMeta {
	...
	pub fn description(self) -> Cow<'static, str> {
		match self {
			Self::None => {
				Cow::Borrowed("No auto-fix is available for this rule.")
			},
			Self::FixPending => {
				Cow::Borrowed("An auto-fix is still under development.")
			},
			Self::Fixable(kind) | Self::Conditional(kind) => {
				...
				let mut message = if kind.is_dangerous() {
					format!("dangerous {noun}")
				} else {
					noun.into()
				};
...
mod test {
	...
	fn ensure_documentation() {
		...
		for rule in RULES.iter() {
			...
			let html =
				to_html_with_options(rule.documentation().unwrap(), &options)
					.unwrap();
