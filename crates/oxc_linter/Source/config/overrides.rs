...
use crate::{LintPlugins, config::OxlintRules};
...
impl OxlintOverrides {
	#[inline]
	pub fn empty() -> Self { Self(IndexVec::new()) }
...
impl JsonSchema for OxlintOverrides {
	fn schema_name() -> String { "OxlintOverrides".to_owned() }

	fn schema_id() -> Cow<'static, str> { Cow::Borrowed("OxlintOverrides") }
...
/// A glob pattern.
///
/// Thin wrapper around [`globset::GlobSet`] because that struct doesn't
/// implement Serialize or schemars traits.
#[derive(Clone, Debug, Default)]
pub struct GlobSet {
	/// Raw patterns from the config. Inefficient, but required for
	/// [serialization](Serialize), which in turn is required for
	/// `--print-config`.
	raw:Vec<String>,
...
impl GlobSet {
	pub fn new<S:AsRef<str>, I:IntoIterator<Item = S>>(patterns:I) -> Result<Self, globset::Error> {
	...
	pub fn is_match<P:AsRef<Path>>(&self, path:P) -> bool { self.globs.is_match(path) }
}
...
impl JsonSchema for GlobSet {
	fn schema_name() -> String { Self::schema_id().into() }

	fn schema_id() -> Cow<'static, str> { Cow::Borrowed("GlobSet") }

	fn json_schema(gen:&mut gen::SchemaGenerator) -> Schema { gen.subschema_for::<Vec<String>>() }
}
