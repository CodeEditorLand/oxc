...
// Note: when update document comment, also update `DummyRuleMap`'s description
// in this file.
...
impl OxlintRules {
	pub fn new(rules:Vec<ESLintRule>) -> Self { Self { rules } }

	/// Returns `true` if there are no rules.
	pub fn is_empty(&self) -> bool { self.rules.is_empty() }
}

/// A fully qualified rule name, e.g. `eslint/no-console` or
/// `react/rule-of-hooks`. Includes the plugin name, the rule name, and the
/// configuration for the rule (if any). This does not imply the rule is known
/// to the linter as that, only that it is configured.
#[derive(Debug, Clone)]
#[cfg_attr(test, derive(PartialEq))]
			...
			match rule_configs.len() {
				...
				1 => {
					...
					match severity {
						...
						AllowWarnDeny::Allow => {
							...
							// If the given rule is not found in the rule list (for example, if all
							// rules are disabled), then look it up in the entire rules list
							// and add it.
				...
				_ => {
						...
						rule_configs.iter().find(|r| r.severity.is_warn_deny())
					{
						...
						// If the given rule is not found in the rule list (for example, if all
						// rules are disabled), then look it up in the entire rules list and
						// add it.
...
impl JsonSchema for OxlintRules {
	fn schema_name() -> String { "OxlintRules".to_owned() }

	fn schema_id() -> Cow<'static, str> { Cow::Borrowed("OxlintRules") }
...
fn parse_rule_value(
	...
	match value {
		...
		_ => {
			Err(failed_to_parse_rule_value(
				&value.to_string(),
...
impl ESLintRule {
	/// Returns `<plugin_name>/<rule_name>` for non-eslint rules. For eslint
	/// rules, returns `<rule_name>`.
...
mod test {
	...
	// FIXME
	#[test]
	#[should_panic(expected = "eslint rules should be configurable by their typescript-eslint \
	                           reimplementations:")]
	fn test_override_empty_fixme() {
		...
		assert_eq!(
			rules.len(),
			1,
			"eslint rules should be configurable by their typescript-eslint reimplementations: \
			 {config:?}"
		);
		let rule = rules.iter().next().unwrap();
		assert_eq!(
