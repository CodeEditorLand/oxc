...
fn parse_rule_key(name:&str) -> (String, String) {
	...
	let (oxlint_plugin_name, rule_name) = match plugin_name {
		...
		// For backwards compatibility, react hook rules reside in the react
		// plugin.
...
impl ESLintRule {
	...
	/// rules, returns `<rule_name>`. This is effectively the inverse operation
	/// for [`parse_rule_key`].
...
mod test {
	...
	fn test_override_plugin_prefix_duplicates() {
		...
		for config in configs {
			...
			// FIXME: this fails, meaning the behavior with two rules (in
			// different plugins) does not match the behavior of a single
			// rule in a oxlintrc. assert_eq!(rules.len(), 1, "{config:?}");
			// let rule = rules.iter().next().unwrap();
