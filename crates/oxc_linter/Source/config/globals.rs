...
/// to allow the variable to be overwritten or `"readonly"` to disallow
/// overwriting.
///
/// Globals can be disabled by setting their value to `"off"`. For example, in
/// an environment where most Es2015 globals are available but `Promise` is
/// unavailable, you might use this config:
///
/// ```json
/// 
/// {
