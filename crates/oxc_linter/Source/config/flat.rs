...
use crate::{LintPlugins, RuleWithSeverity, rules::RULES};
...
/// Keeps track of a list of config deltas, lazily applying them to a base
/// config as requested by [`ConfigStore::resolve`]. This struct is [`Sync`] +
/// [`Send`] since the linter runs on each file in parallel.
#[derive(Debug)]
pub struct ConfigStore {
	...
	// with nested configs.
	/// Resolved override cache. The key is a hash of each override's ID that
	/// matched the list of file globs in order to avoid re-allocating the same
	/// set of rules multiple times.
	cache:FxDashMap<AppliedOverrideHash, ResolvedLinterState>,
	/// "root" level configuration. In the future this may just be the first
	/// entry in `overrides`.
...
impl ConfigStore {
	pub fn new(
		...
		// best-best case: no overrides are provided & config is initialized with 0
		// capacity best case: each file matches only a single override, so we only
		// need `overrides.len()` capacity worst case: files match more than one
		// override. In the most ridiculous case, we could end up needing
		// (overrides.len() ** 2) capacity. I don't really want to pre-allocate that
		// much space unconditionally. Better to re-alloc if we end up needing it.
	...
	pub fn number_of_rules(&self) -> usize { self.base.rules.len() }

	pub fn rules(&self) -> &Arc<[RuleWithSeverity]> { &self.base.rules }
	...
	pub(crate) fn resolve(&self, path:&Path) -> ResolvedLinterState {
		...
		// Compute the path of the file relative to the configuration file for glob
		// matching. Globs should match relative to the location of the configuration
		// file.
	...
	/// NOTE: this function must not borrow any entries from `self.cache` or
	/// DashMap will deadlock.
