...
use crate::{
	config::OxlintConfig, fixer::FixKind, rules::RULES,
	utils::is_jest_rule_adapted_to_vitest, FrameworkFlags, RuleCategory,
...
impl LintOptions {
	...
	pub fn derive_rules_and_config(
		&self,
	) -> Result<(Vec<RuleWithSeverity>, OxlintConfig), Error> {
		let config = self
			.config_path
			.as_ref()
			.map(|path| OxlintConfig::from_file(path))
			.transpose()?;
		...
		RULES
			...
			.filter(|rule| match rule.plugin_name() {
				...
				"jest" => {
					...
					if self.plugins.vitest
						&& is_jest_rule_adapted_to_vitest(rule.name())
					{
						return true;
					}
					false
