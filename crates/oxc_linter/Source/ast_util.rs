...
/// Checks if a branch node of `LogicalExpression` short circuits the whole
/// condition
			...
			Self::AssignmentExpression(expr) => {
				match expr.operator {
					AssignmentOperator::Assign => {
						expr.right.is_constant(in_boolean_position, semantic)
					},
...
pub fn outermost_paren<'a, 'b>(node:&'b AstNode<'a>, semantic:&'b Semantic<'a>) -> &'b AstNode<'a> {
	let mut node = node;
...
///
/// For example, when called on the right-hand side of this
