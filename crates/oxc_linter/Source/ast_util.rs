...
pub fn is_static_boolean<'a>(
	...
	match expr {
		...
		Expression::CallExpression(call_expr) => {
			call_expr.is_constant(true, ctx)
		},
	...
	match expr {
		...
		Expression::UnaryExpression(unary_expr) => {
			op == LogicalOperator::And
				&& unary_expr.operator == UnaryOperator::Void
		},
		...
		Expression::ParenthesizedExpression(expr) => {
			is_logical_identity(op, &expr.expression)
		},
		_ => false,
	}
}
		...
		match self {
				...
				let test_quasis = in_boolean_position
					&& template.quasis.iter().any(|quasi| {
						quasi
							...
							.cooked
							.as_ref()
							.map_or(false, |cooked| !cooked.is_empty())
					});
				let test_expressions = template
					.expressions
					.iter()
					.all(|expr| expr.is_constant(false, ctx));
			...
			Self::LogicalExpression(expr) => {
				let is_left_constant =
					expr.left.is_constant(in_boolean_position, ctx);
				let is_right_constant =
					expr.right.is_constant(in_boolean_position, ctx);
				let is_left_short_circuit = is_left_constant
					&& is_logical_identity(expr.operator, &expr.left);
			...
			Self::SequenceExpression(sequence_expr) => {
				sequence_expr.expressions.iter().last().map_or(false, |last| {
					last.is_constant(in_boolean_position, ctx)
				})
			},
			Self::CallExpression(call_expr) => {
				call_expr.is_constant(in_boolean_position, ctx)
			},
	...
	fn is_constant(
		...
		match self {
			Self::SpreadElement(element) => {
				element.is_constant(in_boolean_position, ctx)
			},
			match_expression!(Self) => {
				self.to_expression().is_constant(in_boolean_position, ctx)
			},
		}
	}
}
...
pub fn outermost_paren_parent<'a, 'b>(
	...
	ctx.nodes().iter_parents(node.id()).skip(1).find(|parent| {
		!matches!(parent.kind(), AstKind::ParenthesizedExpression(_))
	})
}
...
pub fn is_method_call<'a>(
	...
	let Some(member_expr) =
		call_expr.callee.without_parentheses().as_member_expression()
	else {
		return false;
	};
	...
	if let Some(methods) = methods {
		let Some(static_property_name) = member_expr.static_property_name()
		else {
			return false;
		};
...
pub fn is_new_expression<'a>(
	...
	let Expression::Identifier(ident) = new_expr.callee.without_parentheses()
	else {
		return false;
	};
...
pub fn get_new_expr_ident_name<'a>(
