...
impl FixKind {
	...
	#[inline]
	pub const fn is_some(self) -> bool { self.bits() > 0 }

	#[inline]
	pub const fn is_dangerous(self) -> bool { self.contains(Self::Dangerous) }
	...
	/// ```
	#[inline]
	pub fn can_apply(self, rule_fix:Self) -> bool { self.contains(rule_fix) }
...
#[must_use = "Fixes must be used. If you don't need a fix, use `LintContext::diagnostic`, or \
              create an empty fix using `RuleFixer::noop`."]
...
impl<'a> From<RuleFix<'a>> for CompositeFix<'a> {
	#[inline]
	fn from(val:RuleFix<'a>) -> Self { val.fix }
}
...
impl<'a> RuleFix<'a> {
	...
	#[inline]
	pub fn fix(fix:CompositeFix<'a>) -> Self { Self { kind:FixKind::Fix, message:None, fix } }
	...
	#[inline]
	pub fn kind(&self) -> FixKind { self.kind }

	#[inline]
	pub fn message(&self) -> Option<&str> { self.message.as_deref() }

	#[inline]
	pub fn into_fix(self, source_text:&str) -> Fix<'a> { self.fix.normalize_fixes(source_text) }
	...
	#[inline]
	pub fn push<F:Into<CompositeFix<'a>>>(&mut self, fix:F) { self.fix.push(fix.into()); }
}

impl GetSpan for RuleFix<'_> {
	fn span(&self) -> Span { self.fix.span() }
}

impl<'a> Deref for RuleFix<'a> {
	type Target = CompositeFix<'a>;

	fn deref(&self) -> &Self::Target { &self.fix }
}
...
impl<'a> Fix<'a> {
	pub const fn delete(span:Span) -> Self { Self { content:Cow::Borrowed(""), span } }
	...
	#[inline]
	pub const fn empty() -> Self { Self { content:Cow::Borrowed(""), span:SPAN } }
}
...
impl<'a> From<Fix<'a>> for CompositeFix<'a> {
	fn from(fix:Fix<'a>) -> Self { CompositeFix::Single(fix) }
}
...
impl<'a> From<Vec<CompositeFix<'a>>> for CompositeFix<'a> {
	fn from(fixes:Vec<Self>) -> Self { fixes.into_iter().reduce(Self::concat).unwrap_or_default() }
}
...
impl<'a> CompositeFix<'a> {
	...
	pub fn len(&self) -> usize {
		match self {
			...
			Self::Multiple(fs) => {
				debug_assert!(
					fs.len() > 1,
					"Single-element or empty composite fix vecs should have been turned into \
					 CompositeFix::None or CompositeFix::Single"
				);
	...
	//             match fix {
	//                 Self::None => {},
	//                 Self::Single(fix2) => *self =
	// Self::Multiple(vec![fix1.clone(), fix2]),                 Self::Multiple(mut
	// fixes) => {                     fixes.insert(0, fix1.clone());
	//                     *self = Self::Multiple(fixes);
	//                 }
	//             }
	//         }
	...
	/// Gets one fix from the fixes. If we retrieve multiple fixes, this merges
	/// those into one. <https://github.com/eslint/eslint/blob/v9.9.1/lib/linter/report-translator.js#L181-L203>
	...
	/// Merges multiple fixes to one, returns an [`Fix::empty`] (which will not
	/// fix anything) if:
	///
	/// 1. `fixes` is empty
...
mod test {
	...
	fn test_emojis_invalid() { FixKind::Dangerous.emoji(); }
}
