...
impl<T> Stack<T> {
	...
	pub unsafe fn last_unchecked(&self) -> &T {
		...
		// `T`, and `self.current.sub(1)` points to a valid initialized `T`, if stack
		// is not empty. Caller guarantees stack is not empty.
	...
	pub fn push(&mut self, value:T) {
		// The distance between `self.cursor` and `self.end` is always a multiple of
		// `size_of::<T>()`, so `==` check is sufficient to detect when full to
		// capacity.
	...
	pub unsafe fn pop_unchecked(&mut self) -> T {
		...
		debug_assert!(self.cursor <= self.end);
		// SAFETY: Caller guarantees stack is not empty, so subtracting 1 cannot be out
		// of bounds
		self.cursor = self.cursor.sub(1);
		// SAFETY: All methods ensure `self.cursor` is always in bounds, is aligned for
		// `T`, and points to a valid initialized `T`, if stack is not empty.
