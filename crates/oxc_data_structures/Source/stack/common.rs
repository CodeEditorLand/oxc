...
pub trait StackCommon<T>: StackCapacity<T> {
	...
	unsafe fn allocate(capacity_bytes:usize) -> (NonNull<T>, NonNull<T>) {
		...
		// SAFETY: `layout_for` produces a layout with `T`'s alignment, so pointers are
		// aligned for `T`
	...
	unsafe fn grow(&mut self) {
		...
		// ensure that). So `old_start_ptr` and `old_layout` accurately describe the
		// current allocation. `grow` creates new allocation with byte size double
		// what it currently is, or caps it at `MAX_CAPACITY_BYTES`.
		// Old capacity in bytes was a multiple of `size_of::<T>()`, so double that must
		// be too. `MAX_CAPACITY_BYTES` is also a multiple of `size_of::<T>()`.
		...
		// Old capacity and new capacity in bytes are both multiples of
		// `size_of::<T>()`, so distances `end - start` and `current - start` are both
		// multiples of `size_of::<T>()`.
	...
	unsafe fn deallocate(&self) {
		...
		// `start` and `end` are boundaries of that allocation (`allocate` and `grow`
		// ensure that). So `start` and `layout` accurately describe the current
		// allocation.
	...
	unsafe fn drop_contents(&self) {
		...
		// SAFETY: Caller guarantees stack contains `len` initialized entries, starting
		// at `start`.
	...
	unsafe fn layout_for(capacity_bytes:usize) -> Layout {
		...
		// `capacity_bytes` must be a multiple of `size_of::<T>()` so that `cursor ==
		// end` checks in `push` methods accurately detects when full to capacity
		debug_assert!(capacity_bytes % size_of::<T>() == 0);
		// `capacity_bytes` must not exceed `Self::MAX_CAPACITY_BYTES` to prevent
		// creating an allocation of illegal size
	...
	unsafe fn cursor_offset(&self) -> usize {
		...
		// This implies that both pointers are always within bounds of a single
		// allocation. Caller guarantees `cursor >= start`.
		// Caller guarantees distance between pointers is a multiple of
		// `size_of::<T>()`. `assert_unchecked!` is to help compiler to optimize.
...
unsafe fn grow(
	...
	// Reallocate.
	// SAFETY: Caller guarantees `old_start` and `old_layout` describe an existing
	// allocation. Caller guarantees that `max_capacity_bytes <= isize::MAX`.
	// `new_capacity_bytes` is capped above at `max_capacity_bytes`, so is a legal
	// allocation size.

	// `start` and `end` are boundaries of that allocation (`alloc` and `grow`
	// ensure that). So `start` and `old_layout` accurately describe the current
	// allocation. `old_capacity_bytes` was a multiple of `size_of::<T>()`, so
	// double that must be too. `MAX_CAPACITY_BYTES` is also a multiple of
	// `size_of::<T>()`. So `new_capacity_bytes` must be a multiple of
	// `size_of::<T>()`. `new_capacity_bytes` is `<= MAX_CAPACITY_BYTES`, so is a
	// legal allocation size. `layout_for` produces a layout with `T`'s alignment,
	// so `new_ptr` is aligned for `T`.
	...
	// remained in-place." <https://doc.rust-lang.org/std/alloc/trait.GlobalAlloc.html#method.realloc>
	...
	// `old_capacity_bytes < new_capacity_bytes` (ensured above), so `new_cursor`
	// must be in bounds.
