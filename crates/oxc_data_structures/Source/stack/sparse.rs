...
///
/// Functionally equivalent to a stack implemented as `Vec<Option<T>>`, but more
/// memory-efficient in cases where majority of entries in the stack will be
/// empty (`None`).
///
/// Stack is initialized with a single entry which can never be popped off.
/// If `Program` has a entry on the stack, can use this initial entry for it.
/// Get value for `Program` in `exit_program` visitor with
/// `SparseStack::take_last` instead of `SparseStack::pop`.
///
/// The stack is stored as 2 arrays:
/// 1. `has_values` - Records whether an entry on the stack has a value or not
///    (`Some` or `None`).
/// 2. `values` - Where the stack entry *does* have a value, it's stored in this
///    array.
///
/// Memory is only consumed for values where values exist.
///
/// Where value (`T`) is large, and most entries have no value, this will be a
...
///
/// When the stack grows and reallocates, `SparseStack` has less memory to copy,
/// which is a performance win too.
///
/// To simplify implementation, zero size types are not supported
/// (`SparseStack<()>`).
...
impl<T> SparseStack<T> {
	...
	///
	/// Unless `size_of::<T>() == 1`, `MAX_FILLED_CAPACITY` is lower than
	/// [`MAX_TOTAL_CAPACITY`].
	///
	...
	pub fn new() -> Self {
		// `has_values` starts with a single empty entry, which will never be popped
		// off. This means `take_last`, `last_or_init`, and `last_mut_or_init` can all
		// be infallible, as there's always an entry on the stack to read.
		Self { has_values:NonEmptyStack::new(false), values:Stack::new() }
	}

	/// Create new `SparseStack` with pre-allocated capacity.
	///
	/// * `total_capacity` is capacity for any entries (either `Some` or
	///   `None`). Cannot be 0.
	...
	pub fn pop(&mut self) -> Option<T> {
		...
		if has_value {
			...
			// SAFETY: Last `self.has_values` is only `true` if there's a corresponding
			// value in `self.values`. This invariant is maintained in `push`,
			// `take_last`, `last_or_init`, and `last_mut_or_init`. We maintain it here
			// too because we just popped from `self.has_values`, so that `true` has been
			// consumed at the same time we consume its corresponding value from
			// `self.values`.
	...
	pub fn last(&self) -> Option<&T> {
		...
		if has_value {
			debug_assert!(!self.values.is_empty());
			// SAFETY: Last `self.has_values` is only `true` if there's a corresponding
			// value in `self.values`. This invariant is maintained in `push`, `pop`,
			// `take_last`, `last_or_init`, and `last_mut_or_init`.
	...
	pub fn take_last(&mut self) -> Option<T> {
		...
		if *has_value {
			*has_value = false;
			...
			// SAFETY: Last `self.has_values` is only `true` if there's a corresponding
			// value in `self.values`. This invariant is maintained in `push`, `pop`,
			// `last_or_init`, and `last_mut_or_init`. We maintain it here too because we
			// just set last `self.has_values` to `false` at the same time as we consume
			// the corresponding value from `self.values`.
			let value = unsafe { self.values.pop_unchecked() };
	...
	/// Initialize the value for last entry on the stack, if it has no value
	/// already. Return reference to value.
	#[inline]
	pub fn last_or_init<I:FnOnce() -> T>(&mut self, init:I) -> &T {
		...
		// SAFETY: Last `self.has_values` is only `true` if there's a corresponding
		// value in `self.values`. This invariant is maintained in `push`, `pop`,
		// `take_last`, and `last_mut_or_init`. Here either last `self.has_values` was
		// already `true`, or it's just been set to `true` and a value pushed to
		// `self.values` above.
		unsafe { self.values.last_unchecked() }
	}

	/// Initialize the value for last entry on the stack, if it has no value
	/// already. Return mutable reference to value.
	#[inline]
	pub fn last_mut_or_init<I:FnOnce() -> T>(&mut self, init:I) -> &mut T {
		...
		// SAFETY: Last `self.has_values` is only `true` if there's a corresponding
		// value in `self.values`. This invariant is maintained in `push`, `pop`,
		// `take_last`, and `last_or_init`. Here either last `self.has_values` was
		// already `true`, or it's just been set to `true` and a value pushed to
		// `self.values` above.
	...
	#[inline]
	pub fn filled_capacity(&self) -> usize { self.values.capacity() }

	/// Get filled entries of stack as a slice `&[T]`.
	#[inline]
	pub fn as_slice(&self) -> &[T] { self.values.as_slice() }

	/// Get filled entries of stack as a mutable slice `&mut [T]`.
	#[inline]
	pub fn as_mut_slice(&mut self) -> &mut [T] { self.values.as_mut_slice() }
}
