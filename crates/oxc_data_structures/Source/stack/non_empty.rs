...
///
/// `NonEmptyStack` is created initially with 1 entry, and `pop` does not allow
/// removing it (though that initial entry can be mutated with `last_mut`).
///
/// The fact that the stack is never empty makes all operations except `pop`
/// infallible. `last` and `last_mut` are branchless.
///
/// The trade-off is that you cannot create a `NonEmptyStack` without
/// allocating. If that is not a good trade-off for your use case, prefer
/// [`Stack`], which can be empty.
///
/// To simplify implementation, zero size types are not supported (e.g.
/// `NonEmptyStack<()>`).
///
/// ## Design
/// Designed for maximally efficient `push`, `pop`, and reading/writing the last
/// value on stack.
///
/// The alternative would likely be to use a `Vec`. But `Vec` is optimized for
/// indexing into at arbitrary positions, not for `push` and `pop`. `Vec` stores
/// `len` and `capacity` as integers, so requires pointer maths on every
/// operation: `let entry_ptr = base_ptr + index * size_of::<T>();`.
///
/// In comparison, `NonEmptyStack` contains a `cursor` pointer, which always
/// points to last entry on stack, so it can be read/written with a minimum of
/// operations.
///
...
/// * `push` is 1 instruction shorter than `Vec`'s equivalent, and uses 1 less
///   register.
///
...
///
/// 2. Stack could grow downwards, like `bumpalo` allocator does. This would
///    probably make `pop` use 1 less register, but at the cost that the stack
///    can never grow in place, which would incur more memory copies when the
///    stack grows.
///
/// [`Stack`]: super::Stack
...
impl<T> StackCommon<T> for NonEmptyStack<T> {
	#[inline]
	fn start(&self) -> NonNull<T> { self.start }

	#[inline]
	fn end(&self) -> NonNull<T> { self.end }

	#[inline]
	fn cursor(&self) -> NonNull<T> { self.cursor }

	#[inline]
	fn set_start(&mut self, start:NonNull<T>) { self.start = start; }

	#[inline]
	fn set_end(&mut self, end:NonNull<T>) { self.end = end; }

	#[inline]
	fn set_cursor(&mut self, cursor:NonNull<T>) { self.cursor = cursor; }
	...
	fn len(&self) -> usize {
		...
		// When stack has 1 entry, `start - cursor == 0`, so add 1 to get number of
		// entries. SAFETY: Capacity cannot exceed `Self::MAX_CAPACITY`, which is `<=
		// isize::MAX`, and offset can't exceed capacity, so `+ 1` cannot wrap around.
...
impl<T> NonEmptyStack<T> {
	...
	/// Create new [`NonEmptyStack`] with default pre-allocated capacity, and
	...
	/// Create new [`NonEmptyStack`] with pre-allocated capacity for `capacity`
	/// entries, and initial value `initial_value`.
	///
	...
	/// Create new [`NonEmptyStack`] with pre-allocated capacity for `capacity`
	/// entries, and initial value `initial_value`, without checks.
	///
	/// `capacity` cannot be 0.
	///
	/// # Panics
	...
	pub unsafe fn with_capacity_unchecked(capacity:usize, initial_value:T) -> Self {
		...
		// SAFETY: Safety invariants which caller must satisfy guarantee that
		// `capacity_bytes` satisfies requirements
		Self::new_with_capacity_bytes_unchecked(capacity_bytes, initial_value)
	}

	/// Create new [`NonEmptyStack`] with provided capacity in bytes, and
	/// initial value `initial_value`, without checks.
	///
	/// # Panics
	/// Panics if `T` is a zero-sized type.
	///
	...
	unsafe fn new_with_capacity_bytes_unchecked(capacity_bytes:usize, initial_value:T) -> Self {
		...
		// SAFETY: Allocation was created with alignment of `T`, and with capacity for
		// at least 1 entry, so `start` is valid for writing a `T`.
	...
	#[inline]
	pub fn last(&self) -> &T {
		// SAFETY: All methods ensure `self.cursor` is always in bounds, is aligned for
		// `T`, and points to a valid initialized `T`
	...
	pub fn push(&mut self, value:T) {
		// SAFETY: Stack is never empty and `self.cursor` is always less than
		// `self.end`, which is end of allocation. So advancing by a `T` cannot be out
		// of bounds. The distance between `self.cursor` and `self.end` is always a
		// multiple of `size_of::<T>()`, so `==` check is sufficient to detect when
		// full to capacity.
		...
		if new_cursor == self.end {
			...
			// SAFETY: We checked there is capacity for 1 more entry, so `self.cursor` is in
			// bounds. `self.cursor` was aligned for `T`, and we added `size_of::<T>()`
			// to pointer. `size_of::<T>()` is always a multiple of `T`'s alignment, so
			// `self.cursor` must still be aligned for `T`.
	...
	///
	/// This is the slow branch of `push`, which is rarely taken, so marked as
	/// `#[cold]` and `#[inline(never)]` to make `push` as small as possible,
	...
	pub fn pop(&mut self) -> T {
		...
		//
		// Putting the panic in an `#[inline(never)]` + `#[cold]` function removes a
		// 6-byte `lea` instruction vs `assert!(self.cursor != self.start, "Cannot pop
		// all entries")`. This reduces this function on x86_64 from 32 bytes to 26
		// bytes. This function is commonly used, and we want it to be inlined, so
		// every byte counts. https://godbolt.org/z/5587z99rM
		#[inline(never)]
		#[cold]
		fn error() -> ! {
	...
	/// # Safety
	///
	/// * Stack must have at least 2 entries, so that after pop, it still has at
	///   least 1.
	#[inline]
	pub unsafe fn pop_unchecked(&mut self) -> T {
		...
		// SAFETY: Caller guarantees there's at least 2 entries on stack, so subtracting
	...
	#[inline]
	pub fn len(&self) -> usize { <Self as StackCommon<T>>::len(self) }

	/// Get if stack is empty. Always returns `false`.
	#[inline]
	pub fn is_empty(&self) -> bool {
		// This method is pointless, as the stack is never empty. But provide it to
		// override the default method from `slice::is_empty` which is inherited via
		// `Deref`
		false
	}
	...
	/// Get capacity.
	#[inline]
	pub fn capacity(&self) -> usize { <Self as StackCommon<T>>::capacity(self) }

	/// Get contents of stack as a slice `&[T]`.
	#[inline]
	pub fn as_slice(&self) -> &[T] { <Self as StackCommon<T>>::as_slice(self) }

	/// Get contents of stack as a mutable slice `&mut [T]`.
	#[inline]
	pub fn as_mut_slice(&mut self) -> &mut [T] { <Self as StackCommon<T>>::as_mut_slice(self) }
}

impl<T> Drop for NonEmptyStack<T> {
	fn drop(&mut self) {
		// Drop contents.
		// SAFETY: Stack is always allocated, and contains `self.len()` initialized
		// entries, starting at `self.start`.
...
mod tests {
	...
	fn with_capacity_zero() { NonEmptyStack::with_capacity(0, 10u64); }
	...
	fn drop() {
		...
		impl Drop for Droppy {
			fn drop(&mut self) { DROPS.get().unwrap().lock().unwrap().push(self.0); }
		}
