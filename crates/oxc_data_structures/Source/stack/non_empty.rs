...
impl<T> StackCommon<T> for NonEmptyStack<T> {
	...
	#[inline]
	fn len(&self) -> usize {
		// SAFETY: `self.start` and `self.cursor` are both derived from same
		// pointer. `self.cursor` is always >= `self.start`.
		// Distance between pointers is always a multiple of `size_of::<T>()`.
		let offset = unsafe { self.cursor_offset() };

		// When stack has 1 entry, `start - cursor == 0`, so add 1 to get number
		// of entries. SAFETY: Capacity cannot exceed `Self::MAX_CAPACITY`,
		// which is `<= isize::MAX`, and offset can't exceed capacity, so `+
		// 1` cannot wrap around. `checked_add(1).unwrap_unchecked()` instead
		// of just `+ 1` to hint to compiler that return value can never be
		// zero.
...
impl<T> NonEmptyStack<T> {
	...
	unsafe fn new_with_capacity_bytes_unchecked(capacity_bytes:usize, initial_value:T) -> Self {
		...
		// SAFETY: Allocation was created with alignment of `T`, and with
		// capacity for at least 1 entry, so `start` is valid for writing a
		// `T`.
	...
	pub fn push(&mut self, value:T) {
		...
		// `self.end`, which is end of allocation. So advancing by a `T`
		// cannot be out of bounds. The distance between `self.cursor` and
		...
		if new_cursor == self.end {
			...
			// SAFETY: We checked there is capacity for 1 more entry, so
			// `self.cursor` is in bounds. `self.cursor` was aligned for `T`,
			// and we added `size_of::<T>()` to pointer. `size_of::<T>()` is
	...
	unsafe fn push_slow(&mut self, value:T) {
		...
		// Write value.
		// SAFETY: We just allocated additional capacity, so `self.cursor` is in
		// bounds. `self.cursor` is aligned for `T`.
	...
	pub fn is_empty(&self) -> bool {
		// This method is pointless, as the stack is never empty. But provide it
		// to override the default method from `slice::is_empty` which is
		// inherited via `Deref`
		false
	}
...
impl<T> Drop for NonEmptyStack<T> {
	fn drop(&mut self) {
		// Drop contents.
		// SAFETY: Stack is always allocated, and contains `self.len()`
		// initialized entries, starting at `self.start`.
