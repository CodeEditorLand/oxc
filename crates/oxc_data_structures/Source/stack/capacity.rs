...
pub trait StackCapacity<T> {
	...
	const MAX_CAPACITY:usize = {
		...
		// As it's always true that `size_of::<T>() >= align_of::<T>()` and `/` rounds
		// down, this fulfills `Layout`'s alignment requirement
	...
	const DEFAULT_CAPACITY:usize = {
		// It's impossible for this to exceed `MAX_CAPACITY` because `size_of::<T>() >=
		// align_of::<T>()`
		match size_of::<T>() {
			1 => 16,
			size if size <= 1024 => 4,
