...
use crate::{CompactStr, cmp::ContentEq, hash::ContentHash};
...
impl<'a> Atom<'a> {
	...
	///
	/// This is the explicit form of [`Into<String>`], which [`Atom`] also
	/// implements.
	#[inline]
	pub fn into_string(self) -> String { String::from(self.as_str()) }

	/// Convert this [`Atom`] into a [`CompactStr`].
	///
	/// This is the explicit form of [`Into<CompactStr>`], which [`Atom`] also
	/// implements.
	#[inline]
	pub fn into_compact_str(self) -> CompactStr { CompactStr::new(self.as_str()) }

	/// Convert this [`Atom`] into a [`CompactStr`] without consuming `self`.
	#[inline]
	pub fn to_compact_str(&self) -> CompactStr { CompactStr::new(self.as_str()) }
}
...
impl<'alloc> FromIn<'alloc, &Atom<'alloc>> for Atom<'alloc> {
	fn from_in(s:&Atom<'alloc>, _:&'alloc Allocator) -> Self { Self::from(s.0) }
}
...
impl<'a> From<Atom<'a>> for &'a str {
	fn from(s:Atom<'a>) -> Self { s.as_str() }
}

impl<'a> From<Atom<'a>> for CompactStr {
	#[inline]
	fn from(val:Atom<'a>) -> Self { val.into_compact_str() }
}

impl<'a> From<Atom<'a>> for String {
	#[inline]
	fn from(val:Atom<'a>) -> Self { val.into_string() }
}

impl<'a> From<Atom<'a>> for Cow<'a, str> {
	#[inline]
	fn from(value:Atom<'a>) -> Self { Cow::Borrowed(value.as_str()) }
}
...
impl<'a> PartialEq<Atom<'a>> for &str {
	fn eq(&self, other:&Atom<'a>) -> bool { *self == other.as_str() }
}
...
impl<'a> PartialEq<Atom<'a>> for Cow<'_, str> {
	fn eq(&self, other:&Atom<'a>) -> bool { self.as_ref() == other.as_str() }
}
...
impl ContentHash for Atom<'_> {
	fn content_hash<H:hash::Hasher>(&self, state:&mut H) { hash::Hash::hash(self, state); }
}
