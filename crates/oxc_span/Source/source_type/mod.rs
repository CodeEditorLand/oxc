...
pub enum ModuleKind {
	...
	/// `import.meta`.
	///
	/// Note1: This value is only valid as a parser input, and does not appear
	/// on a valid AST's `Program::source_type`. Note2: Dynamic import
	/// expression is not ESM syntax.
	///
...
impl SourceType {
	...
	/// A [`SourceType`] that will be treated as a module if it contains ESM
	/// syntax.
	///
	/// After a file is parsed with an `unambiguous` source type, it will have a
	/// final [`ModuleKind`] of either [`Module`] or [`Script`].
	///
	/// [`Module`]: ModuleKind::Module
	...
	/// Mark this [`SourceType`] as using [JavaScript] if `yes` is `true`. No
	/// change will occur if `yes` is `false`.
	///
	/// [JavaScript]: Language::JavaScript
	#[must_use]
	...
	/// Mark this [`SourceType`] as using [TypeScript] if `yes` is `true`. No
	/// change will occur if `yes` is `false`.
	///
	/// [TypeScript]: Language::TypeScript
	#[must_use]
	...
	/// Mark this [`SourceType`] as a [TypeScript definition] file if `yes` is
	/// `true`.
	#[must_use]
