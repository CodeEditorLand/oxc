...
/// Lifetimeless version of [`Atom<'_>`] which owns its own string data
/// allocation.
///
/// [`CompactStr`] is immutable. Use [`CompactStr::into_string`] for a mutable
/// [`String`].
///
/// Currently implemented as just a wrapper around
/// [`compact_str::CompactString`], but will be reduced in size with a custom
...
impl CompactStr {
	...
	/// ```
	#[inline]
	pub fn new(s:&str) -> Self { Self(CompactString::new(s)) }
	...
	#[inline]
	pub fn into_compact_string(self) -> CompactString { self.0 }
...
impl From<&str> for CompactStr {
	fn from(s:&str) -> Self { Self(CompactString::from(s)) }
}

impl From<String> for CompactStr {
	fn from(s:String) -> Self { Self(CompactString::from(s)) }
}

impl From<CompactString> for CompactStr {
	fn from(s:CompactString) -> Self { Self(s) }
}

impl<'s> From<&'s CompactStr> for Cow<'s, str> {
	fn from(value:&'s CompactStr) -> Self { Self::Borrowed(value.as_str()) }
}

impl From<CompactStr> for Cow<'_, str> {
	fn from(value:CompactStr) -> Self { value.0.into() }
}
...
impl<T:AsRef<str>> PartialEq<T> for CompactStr {
	fn eq(&self, other:&T) -> bool { self.as_str() == other.as_ref() }
}

impl PartialEq<CompactStr> for &str {
	fn eq(&self, other:&CompactStr) -> bool { *self == other.as_str() }
}
...
impl PartialEq<CompactStr> for Cow<'_, str> {
	fn eq(&self, other:&CompactStr) -> bool { self.as_ref() == other.as_str() }
}
...
impl Index<Span> for CompactStr {
	...
	fn index(&self, index:Span) -> &Self::Output { &self.0[index] }
}

impl hash::Hash for CompactStr {
	fn hash<H:hash::Hasher>(&self, hasher:&mut H) { self.as_str().hash(hasher); }
}
...
impl schemars::JsonSchema for CompactStr {
	...
	fn schema_name() -> std::string::String { "String".to_string() }

	fn schema_id() -> Cow<'static, str> { Cow::Borrowed("String") }
...
///
/// Additional parameters passed to `format_compact_str!` replace the `{}`s
/// within the formatting string in the order given unless named or
