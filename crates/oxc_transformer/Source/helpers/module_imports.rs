...
impl<'a> ImportType<'a> {
	fn new(kind:ImportKind, source:Atom<'a>) -> Self { Self { kind, source } }
}
...
impl<'a> ModuleImports<'a> {
	pub fn new() -> ModuleImports<'a> { Self { imports:RefCell::new(IndexMap::default()) } }
	...
	pub fn get_import_statements(&self, ctx:&mut TraverseCtx<'a>) -> Vec<'a, Statement<'a>> {
		ctx.ast
			.vec_from_iter(self.imports.borrow_mut().drain(..).map(|(import_type, names)| {
