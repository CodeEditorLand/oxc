...
pub trait StackCommon<T>: StackCapacity<T> {
	...
	///
	/// # Panics
	/// Panics if out of memory (or may abort, depending on global allocator's
	/// behavior).
	///
	/// # SAFETY
	...
	///
	/// # SAFETY
	/// Stack must have already allocated. i.e. `start` is not a dangling
	/// pointer.
	#[inline]
	unsafe fn grow(&mut self) {
		// Grow allocation.
		// SAFETY: Caller guarantees stack has allocated.
		// `start` and `end` are boundaries of that allocation (`alloc` and
		// `grow` ensure that). So `old_start_ptr` and `old_layout` accurately
		// describe the current allocation. `grow` creates new allocation with
		// byte size double what it currently is, or caps it
		// at `MAX_CAPACITY_BYTES`.
		// Old capacity in bytes was a multiple of `size_of::<T>()`, so double
		// that must be too. `MAX_CAPACITY_BYTES` is also a multiple of
		...
		// Update pointers.
		// SAFETY: `start`, `end`, and `current` are all `NonNull` - just
		// casting them. All pointers returned from `grow` are aligned for
		// `T`. Old capacity and new capacity in bytes are both multiples of
	...
	///
	/// Note: Does *not* drop the contents of the stack (the `T`s), only the
	/// memory allocated by `allocate` / `grow`. If stack is not empty, also
	/// call `drop_contents()` before calling this.
	///
	/// # SAFETY
	/// Stack must have already allocated. i.e. `start` is not a dangling
	/// pointer.
	...
	unsafe fn deallocate(&self) {
		...
		// `start` and `end` are boundaries of that allocation (`allocate` and
	...
	///
	/// This function will be optimized out if `T` is non-drop, as
	/// `drop_in_place` calls `std::mem::needs_drop` internally and is a no-op
	/// if it returns true.
	///
	/// # SAFETY
	/// * Stack must be allocated.
	/// * Stack must contain `self.len()` initialized entries, starting at
	///   `self.start()`.
	#[inline]
	unsafe fn drop_contents(&self) {
		// Drop contents. Next line copied from `std`'s `Vec`.
		// SAFETY: Caller guarantees stack contains `len` initialized entries,
		// starting at `start`.
	...
	unsafe fn layout_for(capacity_bytes:usize) -> Layout {
		// `capacity_bytes` must not be 0 because cannot make 0-size
		// allocations.
		debug_assert!(capacity_bytes > 0);
		// `capacity_bytes` must be a multiple of `size_of::<T>()` so that
		// `cursor == end` checks in `push` methods accurately detects when
		// full to capacity
		debug_assert!(capacity_bytes % size_of::<T>() == 0);
		// `capacity_bytes` must not exceed `Self::MAX_CAPACITY_BYTES` to
		// prevent creating an allocation of illegal size
		...
		// `MAX_CAPACITY_BYTES` takes into account the rounding-up by alignment
		// requirement.
	...
	/// * Byte distance between `self.cursor()` and `self.start()` must be a
	///   multiple of `size_of::<T>()`.
	unsafe fn cursor_offset(&self) -> usize {
		// `offset_from` returns offset in units of `T`.
		// SAFETY: Caller guarantees `cursor` and `start` are derived from same
		// pointer. This implies that both pointers are always within bounds
		// of a single allocation. Caller guarantees `cursor >= start`.
		// Caller guarantees distance between pointers is a multiple of
		// `size_of::<T>()`. `assert_unchecked!` is to help compiler to
		// optimize. See: https://doc.rust-lang.org/std/primitive.pointer.html#method.sub_ptr
	...
	fn capacity(&self) -> usize {
		...
		// * `end` is always >= `start`.
		// * Distance between `start` and `end` is always a multiple of
		//   `size_of::<T>()`.
	...
	#[inline]
	fn as_slice(&self) -> &[T] {
		// SAFETY: Stack always contains `self.len()` entries, starting at
		// `self.start()`
...
unsafe fn grow(
	...
	// Reallocate.
	// SAFETY: Caller guarantees `old_start` and `old_layout` describe an
	// existing allocation. Caller guarantees that `max_capacity_bytes <=
	// isize::MAX`. `new_capacity_bytes` is capped above at
	// `max_capacity_bytes`, so is a legal allocation size.

	// `start` and `end` are boundaries of that allocation (`alloc` and `grow`
	// ensure that). So `start` and `old_layout` accurately describe the
	// current allocation. `old_capacity_bytes` was a multiple of
	// `size_of::<T>()`, so double that must be too. `MAX_CAPACITY_BYTES` is
	// also a multiple of `size_of::<T>()`. So `new_capacity_bytes` must be a
	// multiple of `size_of::<T>()`. `new_capacity_bytes` is `<=
	// MAX_CAPACITY_BYTES`, so is a legal allocation size. `layout_for`
	...
	// Update pointers.
	//
	// Stack was full to capacity, so new last index after push is the old
	// capacity. i.e. `new_cursor - new_start == old_end - old_start`.
	...
	// allocation remained in-place." <https://doc.rust-lang.org/std/alloc/trait.GlobalAlloc.html#method.realloc>
	// `end` changes whatever happens, so always need to be updated.
	// `cursor` needs to be derived from `start` to make `offset_from` valid, so
	// also needs updating.
	//
