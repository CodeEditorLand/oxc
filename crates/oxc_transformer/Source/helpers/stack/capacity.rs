...
///
/// `MAX_CAPACITY` and `MAX_CAPACITY_BYTES` being calculated correctly is
/// required for soundness of stack types.
pub trait StackCapacity<T> {
	/// Maximum capacity of stack.
	///
	/// This is guaranteed to be a legal size for a stack of `T`s, without
	/// exceeding Rust's allocation size limits.
	///
	...
	const MAX_CAPACITY:usize = {
		// This assertion is not needed as next line will cause a compile
		// failure anyway if `size_of::<T>() == 0`, due to division by zero.
	...
	///
	/// Same defaults as [`std::vec::Vec`] uses, except 16 bytes for types with
	/// size 1 (instead of 8). Allocators will usually allocate minimum 16
	/// bytes anyway.
	const DEFAULT_CAPACITY:usize = {
		// It's impossible for this to exceed `MAX_CAPACITY` because
