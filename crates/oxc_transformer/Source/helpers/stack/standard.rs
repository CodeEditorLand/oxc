...
/// A simple stack.
///
/// If a non-empty stack is viable for your use case, prefer [`NonEmptyStack`],
/// which is cheaper for all operations.
///
...
/// value on stack (although, unlike [`NonEmptyStack`], `last` and `last_mut`
/// are fallible, and not branchless).
///
...
///
/// In comparison, `Stack` uses a `cursor` pointer, so avoids these
/// calculations. This is similar to how `std`'s slice iterators work.
///
...
impl<T> Stack<T> {
	...
	/// Get reference to last value on stack, without checking stack isn't
	/// empty.
	///
	/// # Safety
	///
	...
	pub unsafe fn last_unchecked(&self) -> &T {
		...
		// aligned for `T`, and `self.current.sub(1)` points to a valid
		// initialized `T`, if stack is not empty. Caller guarantees stack is
		// not empty.
	...
	/// Get mutable reference to last value on stack, without checking stack
	/// isn't empty.
	///
	/// # Safety
	///
	...
	pub fn push(&mut self, value:T) {
		// The distance between `self.cursor` and `self.end` is always a
		// multiple of `size_of::<T>()`, so `==` check is sufficient to detect
		...
		if self.cursor == self.end {
			...
			// SAFETY: Cursor is not at end, so `self.cursor` is in bounds for
			// writing
			unsafe { self.cursor.as_ptr().write(value) };
			// SAFETY: Cursor is not at end, so advancing by a `T` cannot be out
			// of bounds
	...
	unsafe fn push_slow(&mut self, value:T) {
		...
		unsafe { self.cursor.as_ptr().write(value) }
		// SAFETY: Cursor is not at end, so advancing by a `T` cannot be out of
		// bounds
		self.cursor = unsafe { self.cursor.add(1) };
	}
	...
	pub unsafe fn pop_unchecked(&mut self) -> T {
		...
		debug_assert!(self.cursor <= self.end);
		// SAFETY: Caller guarantees stack is not empty, so subtracting 1 cannot
		// be out of bounds
		self.cursor = self.cursor.sub(1);
		// SAFETY: All methods ensure `self.cursor` is always in bounds, is
		// aligned for `T`, and points to a valid initialized `T`, if stack is
		// not empty. Caller guarantees stack was not empty.
	...
	#[inline]
	pub fn is_empty(&self) -> bool { self.cursor == self.start }
...
impl<T> Drop for Stack<T> {
	fn drop(&mut self) {
		...
		if !self.is_empty() {
			...
			// Stack contains `self.len()` initialized entries, starting at
			// `self.start`
