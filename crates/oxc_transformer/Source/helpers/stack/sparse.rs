...
impl<T> SparseStack<T> {
	...
	pub fn new() -> Self {
		// `has_values` starts with a single empty entry, which will never be
		// popped off. This means `take_last`, `last_or_init`, and
		// `last_mut_or_init` can all be infallible, as there's always an
		// entry on the stack to read.
	...
	pub fn pop(&mut self) -> Option<T> {
		...
		if has_value {
			...
			// corresponding value in `self.values`. This invariant is
			// maintained in `push`, `take_last`, `last_or_init`, and
			// `last_mut_or_init`. We maintain it here too because we just
			// popped from `self.has_values`, so that `true`
	...
	pub fn take_last(&mut self) -> Option<T> {
		...
		if *has_value {
			*has_value = false;
			...
			// SAFETY: Last `self.has_values` is only `true` if there's a
			// corresponding value in `self.values`. This invariant is
			// maintained in `push`, `pop`, `last_or_init`, and
			// `last_mut_or_init`. We maintain it here too because we just
	...
	pub fn last_or_init<I:FnOnce() -> T>(&mut self, init:I) -> &T {
		...
		debug_assert!(!self.values.is_empty());
		// SAFETY: Last `self.has_values` is only `true` if there's a
		// corresponding value in `self.values`. This invariant is maintained
		// in `push`, `pop`, `take_last`, and `last_mut_or_init`. Here either
		// last `self.has_values` was already `true`, or it's just been set to
		// `true` and a value pushed to `self.values` above.
	...
	pub fn last_mut_or_init<I:FnOnce() -> T>(&mut self, init:I) -> &mut T {
		...
		// SAFETY: Last `self.has_values` is only `true` if there's a
		// corresponding value in `self.values`. This invariant is maintained
		// in `push`, `pop`, `take_last`, and `last_or_init`. Here either last
		// `self.has_values` was already `true`, or it's just been set to `true`
