...
impl<'a, 'ctx> Traverse<'a> for NullishCoalescingOperator<'a, 'ctx> {
	fn enter_expression(&mut self, expr:&mut Expression<'a>, ctx:&mut TraverseCtx<'a>) {
		...
		if is_parent_formal_parameter {
			// Replace `function (a, x = a.b ?? c) {}` to `function (a, x = (()
			// => a.b ?? c)() ){}` so the temporary variable can be injected
			// in correct scope
...
impl<'a, 'ctx> NullishCoalescingOperator<'a, 'ctx> {
	...
	/// ```
	///
	/// reference and assignment are the same in this case, but they can be
	/// different
