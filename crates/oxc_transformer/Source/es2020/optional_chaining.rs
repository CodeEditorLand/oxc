...
impl<'a, 'ctx> OptionalChaining<'a, 'ctx> {
	...
	fn set_this_context(&mut self) { self.call_context = CallContext::This; }
	...
	/// existing variable/global. If temp variable is required, returns `None`.
	...
	fn wrap_arrow_function(expr:&mut Expression<'a>, ctx:&mut TraverseCtx<'a>) -> Expression<'a> {
		...
		let statements = ctx
			.ast
			.vec1(ctx.ast.statement_return(SPAN, Some(ctx.ast.move_expression(expr))));
	...
	fn transform_update_expression(&mut self, expr:&mut Expression<'a>, ctx:&mut TraverseCtx<'a>) {
		*expr = if self.is_inside_function_parameter {
	...
	/// expression elements, and then joins the transformed elements with the
	...
	fn transform_chain_expression_impl(
		...
		// transformed into                 a pure non-optional expression and it's
		// the last part of the chain expression.
		...
		// is `delete`, we need to wrap the last part with a `delete` unary expression
	...
	///    transform `foo` to
	/// `foo === null || foo === void 0` and return the transformed expression
	/// back to the parent
	///
	/// 4. Got to here, we now have a left expression as the above transformed
	///    expression, and the current expression
	/// is `foo.bar?.baz`, and it's also an optional [`StaticMemberExpression`],
	/// so transform `foo.bar` to `(_foo$bar = foo.bar) === null || _foo$bar
	/// === void 0` and join it with the left expression, and return the joined
	/// expression back to the parent.
	///
			...
			Expression::CallExpression(call) => {
				...
				if call.optional {
					...
					if !self.ctx.assumptions.pure_getters {
						...
						// original context, so we need to check if we need to specify the
						// context.
	...
	fn transform_and_join_expression(
		...
		if is_call {
			...
			// temp binding will become the call context, we take the current temp
			// binding and set it as the call context.
