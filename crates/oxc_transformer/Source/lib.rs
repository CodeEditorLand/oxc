...
use crate::{
	context::TransformCtx, es2015::ES2015, react::React, typescript::TypeScript,
};
...
impl<'a> Transformer<'a> {
	pub fn new(
		...
		let ctx =
			TransformCtx::new(source_path, source_text, trivias, &options);
	...
	pub fn build_with_symbols_and_scopes(
		...
		let (symbols, scopes) =
			traverse_mut(&mut transformer, allocator, program, symbols, scopes);
	...
	fn exit_arrow_function_expression(
		...
		if arrow.expression && arrow.body.statements.len() > 1 {
			let Statement::ExpressionStatement(statement) =
				arrow.body.statements.pop().unwrap()
			else {
				unreachable!(
