...
impl<'a, 'ctx> AsyncGeneratorExecutor<'a, 'ctx> {
	pub fn new(helper:Helper, ctx:&'ctx TransformCtx<'a>) -> Self { Self { helper, ctx } }
	...
	pub fn transform_function_for_method_definition(
		...
		let (generator_scope_id, wrapper_scope_id) = {
			...
			// function's body inside the wrapper function, and then the new scope id
			// will be wrapper function's scope id.
	...
	pub fn transform_function_declaration(
		...
		// TODO: Needs a better way to handle the function SymbolFlags in different
		// ModuleKind.
		...
		match ctx.parent() {
			...
			Ancestor::ObjectPropertyValue(property) if !*property.method() => {
				property
					.key()
					.static_name()
					.map(|key| Self::normalize_function_name(&key, ctx.ast))
			},
