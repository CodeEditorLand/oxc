...
struct IdentifierDefine {
	...
	/// Whether user want to replace `ThisExpression`, avoid linear scan for
	/// each `ThisExpression`
...
struct ReplaceGlobalDefinesConfigImpl {
	...
	/// extra field to avoid linear scan `meta_property` to check if it has
	/// `import.meta` every time
...
impl DotDefine {
	fn new(parts:Vec<CompactStr>, value:CompactStr) -> Self { Self { parts, value } }
}
	...
	fn check_key(key:&str) -> Result<IdentifierType, Vec<OxcDiagnostic>> {
		...
		if is_import_meta {
			match normalized_parts_len {
				...
				_ => {
					Ok(IdentifierType::ImportMetaWithParts {
						parts:parts
							.iter()
							.skip(2)
...
pub struct ReplaceGlobalDefines<'a> {
	...
	/// Since `Traverse` did not provide a way to skipping visiting sub tree of
	/// the AstNode, Use `Option<Address>` to lock the current node when it is
	/// `Some`. during visiting sub tree, the `Lock` will always be `Some`, and
	/// we can early return, this could acheieve same effect as skipping
	/// visiting sub tree. When `exit` the node, reset the `Lock` to `None` to
	/// make sure not affect other transformation.
...
impl<'a> ReplaceGlobalDefines<'a> {
	...
	fn replace_dot_defines(&mut self, expr:&mut Expression<'a>, ctx:&mut TraverseCtx<'a>) -> bool {
		match expr {
	...
	pub fn is_meta_property_define(
		...
		loop {
			...
			// Considering try replace `import.meta.env` to `undefined`, for the first loop
			// the i is already 0, if it did not match part name and still reach here,
			// that means current_part_member_expression is still something, and possible
			// to match in the further loop
			if i == 0 && matched {
				break;
			}
...
fn destructing_dot_define_optimizer<'ast>(
	...
	let Expression::ObjectExpression(ref mut obj) = expr else {
		return expr;
	};
	...
	// but need to save the checkpoint(to return the original Expr if there are any
	// dynamic key exists) which is a memory clone, cpu is faster than memory
	// allocation
	...
	// we could ensure `should_preserved_keys` has the same length as
	// `obj.properties` the method copy from std doc https://doc.rust-lang.org/std/vec/struct.Vec.html#examples-26
...
impl<'ast> VisitMut<'ast> for RemoveSpans {
	fn visit_span(&mut self, span:&mut Span) { *span = SPAN; }
}
