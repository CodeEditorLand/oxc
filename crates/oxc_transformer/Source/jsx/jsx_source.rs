...
impl<'a, 'ctx> JsxSource<'a, 'ctx> {
	...
	/// columnNumber: 1 } } />`
	///           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	fn add_source_attribute(&mut self, elem:&mut JSXOpeningElement<'a>, ctx:&mut TraverseCtx<'a>) {
		...
		// expensive. Build a table of byte indexes of each line's start on first
		// usage, and save it. Then calculate line and column from that.
	...
	pub fn get_filename_var_declarator(
		...
		let init =
			ctx.ast
				.expression_string_literal(SPAN, self.ctx.source_path.to_string_lossy(), None);
