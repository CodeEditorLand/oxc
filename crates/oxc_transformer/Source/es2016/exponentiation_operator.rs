		...
		match expr {
			...
			Expression::AssignmentExpression(assign_expr) => {
				...
				match &assign_expr.left {
					...
					// name, please install `@babel/plugin-transform-class-properties`".
...
impl<'a, 'ctx> ExponentiationOperator<'a, 'ctx> {
	...
	fn get_second_member_expression_object(
		...
		// it multiple times will not trigger getters or setters. `super` cannot be
		// directly assigned, so use it directly too. TODO(improve-on-babel): We could
		// also skip creating a temp var for `this.x **= 2`.
		match obj {
			...
			Expression::Identifier(ident) => {
				...
				// to only evaluate it once. Assign to a temp var.
			},
