...
impl<'a, 'ctx> ExponentiationOperator<'a, 'ctx> {
	...
	/// If the lhs is a member expression `obj.ref` or `obj[ref]`, assign them
	/// to a temporary variable so side-effects are not computed twice.
	/// For `obj.ref`, change it to `var _obj; _obj = obj, _obj["ref"] =
	/// Math.pow(_obj["ref"], 2)`. For `obj[ref]`, change it to `var _obj,
	/// _ref; _obj = obj, _ref = ref, _obj[_ref] = Math.pow(_obj[_ref], 2);`.
	fn explode(
		&mut self,
			...
			match_member_expression!(SimpleAssignmentTarget) => {
				...
				let (reference, uid) = match &prop {
					Expression::Identifier(ident) if !computed => {
						...
						(
							// TODO:
							// Both of these are the same, but it's in order to
							// avoid after cloning without reference_id. Related: https://github.com/oxc-project/oxc/issues/4804
	...
	/// Make sure side-effects of evaluating `obj` of `obj.ref` and `obj[ref]`
	/// only happen once.
	fn get_obj_ref(
		&mut self,
	...
	/// Make sure side-effects of evaluating `ref` of `obj.ref` and `obj[ref]`
	/// only happen once.
	fn get_prop_ref(
		&mut self,
		...
		let expr = match node {
			...
			MemberExpression::PrivateFieldExpression(_) => {
				// From babel: "We can't generate property ref for private name,
