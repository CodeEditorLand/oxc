...
//!
//! This plugin transform logical assignment operators `&&=`, `||=`, and `??=`
...
impl<'a, 'ctx> Traverse<'a> for LogicalAssignmentOperators<'a, 'ctx> {
	fn enter_expression(&mut self, expr:&mut Expression<'a>, ctx:&mut TraverseCtx<'a>) {
		...
		let (left_expr, assign_target) = match &mut assignment_expr.left {
			...
			// LeftHandSideExpression is not simple. So safe to return here.
			_ => return,
		};
...
impl<'a, 'ctx> LogicalAssignmentOperators<'a, 'ctx> {
	...
	fn convert_static_member_expression(
		...
		if let Some(ident) = self.maybe_generate_memoised(&static_expr.object, ctx) {
			...
			*reference.flags_mut() = ReferenceFlags::Read;
			...
			// TODO: We should use static_expr.clone_in instead of cloning the
			// properties, but currently clone_in will get rid of
			// IdentifierReference's reference_id
	...
	fn convert_computed_member_expression(
		...
		if let Some(ident) = self.maybe_generate_memoised(&computed_expr.object, ctx) {
			...
			// transform `obj[++key] ||= 1` to `obj[_key = ++key] || (obj[_key]
			// = 1)`
			...
			// TODO: ideally we should use computed_expr.clone_in instead of
			// cloning the properties, but currently clone_in will get rid of
			// IdentifierReference's reference_id
	...
	/// Clone an expression
	///
	/// If it is an identifier, clone the identifier by
	/// [TraverseCtx::clone_identifier_reference], otherwise, use [CloneIn].
	///
