...
use crate::{
	common::module_imports::NamedImport,
...
impl<'a, 'ctx> Bindings<'a, 'ctx> {
	#[inline]
	fn is_classic(&self) -> bool { matches!(self, Self::Classic(_)) }
}
...
impl<'a, 'ctx> AutomaticScriptBindings<'a, 'ctx> {
	...
	fn require_create_element(&mut self, ctx:&mut TraverseCtx<'a>) -> IdentifierReference<'a> {
		if self.require_create_element.is_none() {
			...
			// We have to insert this `require` above
			// `require("react/jsx-runtime")` just to pass one of Babel's
			// tests, but the order doesn't actually matter.
			// TODO(improve-on-babel): Remove this once we don't need our output
			// to match Babel exactly.
...
impl<'a, 'ctx> AutomaticModuleBindings<'a, 'ctx> {
	...
	// Inline so that compiler can see in `import_jsx` and `import_jsxs` that
	// fields are always `Some` after calling this function, and can elide the
	// `unwrap()`s
	#[inline]
...
impl<'a, 'ctx> ReactJsx<'a, 'ctx> {
	fn is_script(&self) -> bool { self.ctx.source_type.is_script() }
	...
	fn insert_filename_var_statement(&mut self, ctx:&mut TraverseCtx<'a>) {
		...
		// If is a module, add filename statements before `import`s. If script,
		// then after `require`s. This is the same behavior as Babel.
		// If in classic mode, then there are no import statements, so it
		// doesn't matter either way. TODO(improve-on-babel): Simplify this
		// once we don't need to follow Babel exactly.
		if self.bindings.is_classic() || !self.is_script() {
			...
			// Insert after imports - add to `var_declarations`, which are
			// inserted after `require` statements
	...
	fn transform_jsx<'b>(
		...
		// If has_key_after_props_spread is true, we need to fallback to
		// `createElement` same behavior as classic runtime
		...
		// The object properties for the second argument of
		// `React.createElement`
				...
				match attribute {
					JSXAttributeItem::Attribute(attr) => {
						...
						if attr.is_key() {
							...
							// In automatic mode, extract the key before spread
							// prop, and add it to the third argument
							// later.
							if is_automatic {
		...
		// It would be better to push to `arguments` earlier, rather than use
		// `insert`. But we have to do it here to replicate the same import
		// order as Babel, in order to pass Babel's conformance tests.
		// TODO(improve-on-babel): Change this if we can handle differing output
		// in tests.
		...
		// If runtime is automatic that means we always to add `{ .. }` as the
		// second argument even if it's empty
		if is_automatic || !properties.is_empty() {
			...
			// If not and second argument doesn't exist, we should add `null` as
			// the second argument
...
impl<'a, 'b> JSXElementOrFragment<'a, 'b> {
	...
	fn is_fragment(&self) -> bool { matches!(self, Self::Fragment(_)) }
