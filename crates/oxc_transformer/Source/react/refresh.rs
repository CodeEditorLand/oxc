...
impl<'a, 'ctx> Traverse<'a> for ReactRefresh<'a, 'ctx> {
	...
	fn exit_expression(&mut self, expr:&mut Expression<'a>, ctx:&mut TraverseCtx<'a>) {
		let signature = match expr {
			Expression::FunctionExpression(func) => {
				self.create_signature_call_expression(
			...
			Expression::ArrowFunctionExpression(arrow) => {
				...
				// If the signature is found, we will push a new statement to
				// the arrow function body. So it's not an expression anymore.
				if call_fn.is_some() {
	...
	fn exit_function(&mut self, func:&mut Function<'a>, ctx:&mut TraverseCtx<'a>) {
		...
		self.extra_statements
			.entry(id.symbol_id.get().unwrap())
			.or_insert(ctx.ast.vec())
			.push(ctx.ast.statement_expression(
				SPAN,
				ctx.ast.expression_call(
					SPAN,
					Self::create_identifier_reference_from_binding_identifier(
	...
	fn enter_call_expression(
		...
		if !is_builtin_hook(&hook_name) {
			// Check if a corresponding binding exists where we emit the
			// signature.
		...
		self.hook_calls
			...
			.or_default()
			.push((hook_name, ctx.ast.atom(&key)));
	}
}
...
impl<'a, 'ctx> ReactRefresh<'a, 'ctx> {
	...
	fn create_signature_call_expression(
		...
		if force_reset || !custom_hooks_in_scope.is_empty() {
			arguments.push(
				ctx.ast
					.argument_expression(ctx.ast.expression_boolean_literal(SPAN, force_reset)),
			);
		}
		...
		// _s = refresh_sig();
		self.signature_declarator_items
			.last_mut()
			.unwrap()
			.push(
				ctx.ast.variable_declarator(
		...
		// Following is the signature call expression, will be generated in call
		// site. _s(App, signature_key, false, function() { return [] });
		//                        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ custom hooks
		// only
				...
				match &mut stmt_decl.declaration {
					declaration @ match_expression!(ExportDefaultDeclarationKind) => {
						...
						if !matches!(expression, Expression::CallExpression(_)) {
							...
							// Named function declarations are handled in
							// FunctionDeclaration. Anonymous direct
							// exports like export default function() {}
						...
						// export default memo(() => {})
						// In those cases it is more plausible people will omit
						// names so they're worth handling despite
						// possible false positives. More importantly, it
						// handles the named case: export default
						// memo(function Named() {})
	...
	fn handle_variable_declaration(
		...
		let found_inside = self.replace_inner_components(
			&id.name, init, // is_variable_declarator
			true, ctx,
		);

		if !found_inside {
			// See if this identifier is used in JSX. Then it's a component.
			// TODO: Here we should check if the variable is used in JSX. But
			// now we only check if it has value references. https://github.com/facebook/react/blob/ba6a9e94edf0db3ad96432804f9931ce9dc89fec/packages/react-refresh/src/ReactFreshBabelPlugin.js#L161-L199
