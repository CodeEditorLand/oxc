...
impl<'a> ReactJsx<'a> {
	...
	fn ast(&self) -> &AstBuilder<'a> { &self.ctx.ast }
}
...
impl<'a> ReactJsx<'a> {
	pub fn add_runtime_imports(&mut self, program:&mut Program<'a>) {
		...
		if self.can_add_filename_statement {
			...
			// If source type is module then we need to add the import statement
			// after the var file name statement Follow the same behavior as
			// babel
			if !self.is_script() {
				index += 1;
			}
...
impl<'a> ReactJsx<'a> {
	...
	/// - Development: React.jsxDEV(type, arguments, key, isStaticChildren,
	///   source, self)
	///
	/// ### Fragment
	...
	/// - Development: React.createElement(type, arguments, children, source,
	///   self)
	///
	/// ### Fragment
			...
			JSXChild::ExpressionContainer(e) => {
				match &e.expression {
					e @ match_expression!(JSXExpression) => {
						Some(self.ast().copy(e.to_expression()))
					},
