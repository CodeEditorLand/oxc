...
impl<'a, 'ctx> ReactJsxSource<'a, 'ctx> {
	...
	fn add_source_attribute(&mut self, elem:&mut JSXOpeningElement<'a>, ctx:&mut TraverseCtx<'a>) {
		...
		// TODO: We shouldn't calculate line + column from scratch each time as
		// it's expensive. Build a table of byte indexes of each line's start
		// on first usage, and save it. Then calculate line and column from
		// that.
