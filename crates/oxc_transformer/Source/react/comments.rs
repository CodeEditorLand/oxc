...
fn find_jsx_pragma<'a>(
	...
	// This allows `find_at_sign` functions to search in chunks of 8 bytes without
	// hitting end of string.
...
fn find_at_sign_in_block_comment(str:&str) -> Option<&str> {
	// Note: Neither `accept` nor `skip` matches `/`, so will not search beyond end
	// of the comment
	let accept = |byte| byte == b'@';
...
fn split_at_whitespace(str:&str) -> Option<(&str, &str)> {
	...
	// Don't use `find` to search in chunks here, as usually there's only a single
	// space and this loop will exit on first turn.
...
where
	...
	// Check chunks for any non-ASCII bytes in one go, and deopt to unicode handler
	// if so.
