...
fn update_options_with_comment(options:&mut TransformOptions, comment:&Comment, source_text:&str) {
	let Some((keyword, remainder)) = find_jsx_pragma(comment, source_text) else {
		return;
	};
...
fn find_jsx_pragma<'a>(
	...
	// This allows `find_at_sign` functions to search in chunks of 8 bytes
	// without hitting end of string.
...
/// Returns `None` if any other character except space, or tab, or irregular
/// whitespace is found first. That includes line breaks, since this is a
/// single-line comment.
fn find_at_sign_in_line_comment(str:&str) -> Option<&str> {
	// Note: Neither `accept` nor `skip` matches line breaks, so will not search
	// beyond end of the comment
...
///
/// Returns the remainder of the string after the `@`.
/// Returns `None` if any other character except whitespace, line breaks, or `*`
/// is found first.
///
...
fn find_at_sign_in_block_comment(str:&str) -> Option<&str> {
	// Note: Neither `accept` nor `skip` matches `/`, so will not search beyond
	// end of the comment
	let accept = |byte| byte == b'@';
...
/// Returns `None` if no whitespace found, or if no further characters after the
/// whitespace.
fn split_at_whitespace(str:&str) -> Option<(&str, &str)> {
	...
	// Don't use `find` to search in chunks here, as usually there's only a
	// single space and this loop will exit on first turn.
...
fn trim_end(str:&str) -> &str {
	...
	// SAFETY: `index` came from a safe iterator, so must be before end of
	// `str`. Therefore `index + 1` must be in bounds (or at end of string).
	// We have only seen ASCII bytes, so `index + 1` must be on a UTF-8 char
	// boundary.
...
/// Non-ASCII chars are considered unlikely and are handled in a cold fallback
/// path.
///
/// Search occurs in batches of 8 bytes, with a slower fallback for the last 7
/// bytes. Provide the longest string possible to be able to avoid the slower
/// fallback.
///
/// Iterates through string byte-by-byte, calling `accept` and `skip` for each
/// byte.
/// * If a non-ASCII byte is found, hands control to `find_unicode` and returns
///   whatever it returns.
...
where
	...
	// Check chunks for any non-ASCII bytes in one go, and deopt to unicode
	// handler if so.
	...
	for (chunk_index, chunk) in chunks.by_ref().enumerate() {
		...
		// Compiler will unroll this loop if `accept` and `skip` are small
		// enough
...
#[cold]
#[inline(never)]
pub fn cold_branch<F:FnOnce() -> T, T>(f:F) -> T { f() }
