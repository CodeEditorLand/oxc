	...
	fn enter_call_expression(
		...
		let name = loop {
			...
			match ancestor {
				...
				Ancestor::AssignmentExpressionRight(assign_expr) => {
					match assign_expr.left() {
						...
						// Babel does not handle computed member expressions
						// e.g. `foo["bar"]`, so we diverge from Babel
						// here, but that's probably an improvement
				...
				Ancestor::ObjectPropertyValue(prop) => {
					// Babel only handles static identifiers e.g. `{foo:
					// React.createClass({})}`, whereas we also handle e.g.
					// `{"foo-bar": React.createClass({})}`, so we diverge
					// from Babel here, but that's probably an improvement
...
impl<'a, 'ctx> ReactDisplayName<'a, 'ctx> {
	...
	fn add_display_name(obj_expr:&mut ObjectExpression<'a>, name:Atom<'a>, ctx:&TraverseCtx<'a>) {
