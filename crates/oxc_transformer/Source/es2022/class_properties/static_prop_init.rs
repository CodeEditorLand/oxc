...
/// `ScopeFlags` need updating, then we only need to transform `this`, and
/// re-parent first-level scopes. So can skip traversing into functions and
/// other contexts which have their own `this`.
///
...
struct StaticInitializerVisitor<'a, 'ctx, 'v> {
	...
	/// accurately. Only requirement is to ensure that `scope_depth == 0` only
	/// when we're in first-level scope. So we don't bother incrementing +
	/// decrementing for scopes which are definitely not first level.
	/// e.g. `BlockStatement` or `ForStatement` must be in a function, and
	/// therefore we're already in a nested scope.
	scope_depth:u32,
...
impl<'a, 'ctx, 'v> VisitMut<'a> for StaticInitializerVisitor<'a, 'ctx, 'v> {
	...
	fn visit_property_definition(&mut self, prop:&mut PropertyDefinition<'a>) {
		...
		// `PropertyDefinition` can only be in a class, and that class would be the
		// first-level scope. So no need to call `reparent_scope_if_first_level`.
		...
		// `PropertyDefinition` must be in a class, and traversal would have stopped
		// in `visit_class` if it wasn't
	...
	fn visit_accessor_property(&mut self, prop:&mut AccessorProperty<'a>) {
		...
		// `AccessorProperty` must be in a class, and traversal would have stopped in
		// `visit_class` if it wasn't
		if let Some(value) = &mut prop.value {
			self.this_depth += 1;
			self.visit_expression(value);
			self.this_depth -= 1;
