...
impl<'a, 'ctx> ClassProperties<'a, 'ctx> {
	...
	/// the returned `InstanceInitsInsertLocation`, and `ScopeId` for `_super`
	/// function is returned as `insert_in_scope_id` in returned
	/// `InstanceInitScopes`.
	///
	...
	pub(super) fn replace_super_in_constructor(
		...
		// has some bindings. If it doesn't, no need to check for shadowed symbols in
		// instance prop initializers, because no bindings to clash with.
	...
	///
	/// `scope_id` has different meaning depending on type of
	/// `insertion_location`.
	pub(super) fn insert_instance_inits(
		...
		// TODO: Handle where vars used in property init clash with vars in top scope of
		// constructor. (or maybe do that earlier?)
	...
	fn create_super_function_inside_constructor(
		...
		// `_defineProperty(this, propName, value)`. `_defineProperty` returns `this`,
		// so last statement could be `return _defineProperty(this, propName, value)`,
		// rather than an additional `return this` statement.
		// Actually this wouldn't work at present, as `_classPrivateFieldInitSpec(this,
		// _prop, value)` does not return `this`. We could alter it so it does when we
		// have our own helper package.
		let args_binding =
	...
	fn create_super_function_outside_constructor(
		...
		// Note: Inserting it after class not before, so that other transforms run on
		// it. TODO: That doesn't work - other transforms do not run on it.
		// TODO: If static block transform is not enabled, it's possible to construct
		// the class within the static block `class C { static { new C() } }` and
		// that'd run before `_super` is defined. So it needs to go before the class,
		// not after, in that case.
	...
	/// initializers
	fn rename_clashing_symbols(&mut self, constructor:&mut Function<'a>, ctx:&mut TraverseCtx<'a>) {
	...
	/// Get `Function` for constructor, given constructor's index within class
	/// elements.
...
impl<'a, 'c> ConstructorParamsSuperReplacer<'a, 'c> {
	fn new(ctx:&'c mut TraverseCtx<'a>) -> Self { Self { super_binding:None, ctx } }
	...
	fn visit_expression(&mut self, expr:&mut Expression<'a>) {
		if let Expression::CallExpression(call_expr) = expr {
			if call_expr.callee.is_super() {
				...
				// `walk_expression` below to avoid infinite loop as `super()` gets visited
				// over and over
	...
	fn replace(
		...
		loop {
			...
			if let Some((index, stmt)) = body_stmts_iter_enumerated.next() {
				...
				// `_super` function which is never called. That is pointless, but not
				// inserting the initializers anywhere would leave `Semantic` in an
				// inconsistent state. What we get is completely legal output and correct
				// `Semantic`, just longer than it could be. But this should very rarely
				// happen in practice, and minifier will delete the `_super` function as
				// dead code. TODO: Delete the initializers instead.
		...
		// `super()` found in nested position. There may be more `super()`s in
		// constructor. Convert them all to `_super()`.
