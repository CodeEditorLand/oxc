...
use super::{FxIndexMap, class_bindings::ClassBindings};

/// Stack of private props defined by classes.
///
/// Pushed to when entering a class (`None` if class has no private props,
/// `Some` if it does). Entries contain a mapping from private prop name to
/// binding for temp var for that property, and details of the class that
/// defines the private prop.
///
...
impl<'a> PrivatePropsStack<'a> {
	...
	#[inline]
	pub fn push(&mut self, props:Option<PrivateProps<'a>>) { self.stack.push(props); }

	#[inline]
	pub fn pop(&mut self) -> Option<PrivateProps<'a>> { self.stack.pop() }

	#[inline]
	pub fn last(&self) -> Option<&PrivateProps<'a>> { self.stack.last() }

	#[inline]
	pub fn last_mut(&mut self) -> Option<&mut PrivateProps<'a>> { self.stack.last_mut() }
	...
	pub fn find<'b>(
		...
		// TODO: This should be unreachable. Only returning `None` because
...
pub(super) struct PrivateProps<'a> {
	...
	// TODO(improve-on-babel): Order that temp vars are created in is not important. Use
	// `FxHashMap` instead.
