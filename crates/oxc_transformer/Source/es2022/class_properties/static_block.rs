...
use super::{super::ClassStaticBlock, ClassProperties};
...
impl<'a, 'ctx> ClassProperties<'a, 'ctx> {
	...
	pub(super) fn convert_static_block(
		...
		// `x = class C { static { this.C = C; } }` -> `x = (_C = class C {}, _C.C = _C,
		// _C)` TODO: Scope of static block contents becomes outer scope, not scope of
		// class.

		// If class expression, assignment in static block moves to a position where
		// it's read from. e.g.: `x` here now has read+write `ReferenceFlags`:
