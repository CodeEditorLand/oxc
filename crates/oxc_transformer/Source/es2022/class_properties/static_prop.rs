...
impl<'a, 'ctx> ClassProperties<'a, 'ctx> {
	...
	/// Transform private fields. See below for full details of transforms.
	...
	/// Set flag on `ClassBindings` that we are/are not currently transforming
	/// static prop initializers.
	///
	/// The logic around which bindings are used for transforming private fields
	/// is complex, so we use this to make sure the logic is correct.
	///
	/// In debug builds, `ClassBindings::get_or_init_temp_binding` will panic if
	/// we end up transforming a static private field, and there's no `temp`
	/// binding - which should be impossible.
...
///    * Class declaration: `class C { static x = this.y; }` -> `var _C; class C
///      {}; _C = C; C.x = _C.y;`
///    * Class expression: `x = class C { static x = this.y; }` -> `var _C; x =
///      (_C = class C {}, _C.x = _C.y, _C)`
/// 2. Reference to class name to class temp var.
///    * Class declaration: `class C { static x = C.y; }` -> `var _C; class C
///      {}; _C = C; C.x = _C.y;`
///    * Class expression: `x = class C { static x = C.y; }` -> `var _C; x = (_C
///      = class C {}, _C.x = _C.y, _C)`
/// 3. Private fields which refer to private props of this class.
///    * Class declaration: `class C { static #x = 123; static y = this.#x; }`
///      -> `var _C; class C {}; _C = C; var _x = { _: 123 }; C.y =
///      _assertClassBrand(_C, _C, _x)._;`
///    * Class expression: `x = class C { static #x = 123; static y = this.#x;
///      }` -> `var _C, _x; x = (_C = class C {}, _x = { _: 123 }, _C.y =
///      _assertClassBrand(_C, _C, _x)._), _C)`
///
/// Also:
/// * Updates parent `ScopeId` of first level of scopes in initializer.
/// * Sets `ScopeFlags` of scopes to sloppy mode if code outside the class is
///   sloppy mode.
///
/// Reason we need to transform `this` is because the initializer is being moved
/// from inside the class to outside. `this` outside the class refers to a
/// different `this`, and private fields are only valid within the class body.
...
/// `this` or class name, and private fields. This is because class binding `C`
/// could be mutated, and the initializer may contain functions which
/// are not executed immediately, so the mutation occurs before that initializer
/// code runs.
///
/// ```js
/// class C {
...
/// ```
///
/// If this class defines no private properties, class has no name, and no
/// `ScopeFlags` need updating, then we only need to transform `this`. So can
/// skip traversing into functions and other contexts which have their own
/// `this`.
///
/// Note: Those functions could contain private fields referring to a *parent*
/// class's private props, but we don't need to transform them here as they
/// remain in same class scope.
...
// initializer in a strict mode IIFE so that initializer code runs in strict
// mode, as it was before within class body.
struct StaticInitializerVisitor<'a, 'ctx, 'v> {
	/// `true` if class has name, or class has private properties, or
	/// `ScopeFlags` need updating. Any of these neccesitates walking the whole
	/// tree. If none of those apply, we only need to walk as far as functions
	/// and other constructs which define a `this`.
	walk_deep:bool,
...
impl<'a, 'ctx, 'v> VisitMut<'a> for StaticInitializerVisitor<'a, 'ctx, 'v> {
	// TODO: Also need to call class visitors so private props stack is in correct
	// state. Otherwise, in this example, `#x` in `getInnerX` is resolved
	// incorrectly and `getInnerX()` will return 1 instead of 2.
	// We have to visit the inner class now rather than later after exiting outer
	// class so that `#y` in `getOuterY` resolves correctly too.
	// ```js
	// class Outer {
	//   #x = 1;
	//   #y = 1;
	...
	// ```
	//
	// Need to save all per-class state (`insert_before` etc), and restore it again
	// after. Using a stack would be overkill because nested classes in static
	// blocks will be rare.
				...
				if unary_expr.operator == UnaryOperator::Delete {
					match &unary_expr.argument {
						...
						Expression::ChainExpression(_) => {
							...
							// main entry point `transform_unary_expression`. We already checked
							// that `expr` is `delete <chain expression>`, so can avoid
							// checking that again.
	...
	fn enter_scope(&mut self, _flags:ScopeFlags, scope_id:&Cell<Option<ScopeId>>) {
		...
		// In JS, only `Function`, `ArrowFunctionExpression` or `Class` can be the
		// first-level scope, as all other types which have a scope are statements or
		// `StaticBlock` which would need to be inside a function or class. But some
		// TS types with scopes could be first level via
		// e.g. `TaggedTemplateExpression::type_parameters`, which contains `TSType`.
		// Not sure if that matters though, as they'll be stripped out anyway by TS
		// transform.
	...
	// be transformed.
	//
	// Or, if class has no name, class has no private properties, and `ScopeFlags`
	// don't need updating, stop traversing entirely. No private field accesses
	// need to be transformed, and no scopes need flags updating, so no point
	// searching for them.
	//
	...
	fn visit_static_block(&mut self, block:&mut StaticBlock<'a>) {
		if self.walk_deep {
			...
			// `StaticBlock` can only be in a class, and that class would be
	...
	fn visit_ts_module_block(&mut self, block:&mut TSModuleBlock<'a>) {
		...
		if self.walk_deep {
			...
			// Not possible that `self.scope_depth == 0` here, because a
			// `TSModuleBlock` can only be in a function, and that function
			// would be the first-level scope
		}
	...
	fn visit_property_definition(&mut self, prop:&mut PropertyDefinition<'a>) {
		...
		// }
		// ```
		// Don't visit `type_annotation` field because can't contain `this` or private
		// props. TODO: Are decorators in scope?
		...
		if self.walk_deep {
			...
			// Not possible that `self.scope_depth == 0` here, because a
			// `PropertyDefinition` can only be in a class, and that class
			// would be the first-level scope
		}
	}
	...
	fn visit_accessor_property(&mut self, prop:&mut AccessorProperty<'a>) {
		...
		if self.walk_deep {
			...
			// Not possible that `self.scope_depth == 0` here, because an
			// `AccessorProperty` can only be in a class, and that class
			// would be the first-level scope
		}
	}
}
...
impl<'a, 'ctx> ClassProperties<'a, 'ctx> {
	pub(super) fn get_temp_binding(&mut self, ctx:&mut TraverseCtx<'a>) -> &BoundIdentifier<'a> {
		...
		// because other visitors which transform private fields may create a temp
		// binding and store it on `PrivateProps`
