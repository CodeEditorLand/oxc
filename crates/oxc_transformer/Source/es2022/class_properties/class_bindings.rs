...
/// * Class expression has static properties. e.g. `C = class { x = 1; }`
/// * Class declaration has static properties and one of the static prop's
///   initializers contains: a. `this` e.g. `class C { x = this; }` b. Reference
///   to class name e.g. `class C { x = C; }` c. A private field referring to
///   one of the class's static private props. e.g. `class C { static #x; static
///   y = obj.#x; }`
///
/// An instance of `ClassBindings` is stored in main `ClassProperties`
/// transform, and a 2nd is stored in `PrivateProps` for the class, if the class
/// has any private properties. If the class has private props, the instance of
/// `ClassBindings` in `PrivateProps` is the source of truth.
///
...
/// * Class name when field is within class body and class has a name e.g.
///   `class C { static #x; method() { return obj.#x; } }`
/// * Temp var when field is within class body and class has no name e.g. `C =
///   class { static #x; method() { return obj.#x; } }`
/// * Temp var when field is within a static prop initializer. e.g. `class C {
///   static #x; y = obj.#x; }`
///
/// To cover all these cases, the meaning of `temp` binding here changes while
/// traversing the class body. [`ClassProperties::transform_class`] sets `temp`
/// binding to be a copy of the `name` binding before that traversal begins. So
/// the name `temp` is misleading at that point.
///
...
impl<'a> ClassBindings<'a> {
	...
	pub fn get_or_init_temp_binding(&mut self, ctx:&mut TraverseCtx<'a>) -> &BoundIdentifier<'a> {
		if self.temp.is_none() {
			// This should only be possible if we are currently transforming static prop
			// initializers
			#[cfg(debug_assertions)]
			{
				assert!(
