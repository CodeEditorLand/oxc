...
impl<'a, 'ctx> ClassProperties<'a, 'ctx> {
	...
	pub(super) fn transform_private_field_expression(
		...
		let Expression::PrivateFieldExpression(field_expr) = owned_expr else {
			unreachable!()
		};
	...
	fn transform_private_field_expression_impl(
		...
		// TODO: Move this to top of function once `lookup_private_property` does not
		// return `Option`
		...
		if is_static {
			...
			// static props of outer class inside inner class, to make sure we're getting
			// the right `class_bindings`.
	...
	// expressions too. TODO: Take `&ClassBindings` instead of `Option<SymbolId>`.
	...
	fn transform_call_expression_impl(
		...
		if self.private_fields_as_properties {
			...
			let prop_details = self.private_props_stack.find(&field_expr.field);
			// TODO: Should never be `None` - only because implementation is incomplete.
			let Some(ResolvedPrivateProp { prop_binding, .. }) = prop_details else {
				return;
			};
	...
	fn transform_private_field_callee(
		...
		let replacement = if is_static {
			...
			// _prop)._.call(object, arg)` or shortcut `_prop._.call(object, arg)`
			...
			// If `object` is reference to class name, there's no need for the class brand
			// assertion TODO: Combine this check with `duplicate_object`. Both check if
			// `object` is an identifier, and look up the `SymbolId`
	...
	fn transform_assignment_expression_impl(
		...
		// hopefully compiler will see these clones of `BoundIdentifier`s
		// can be elided.
		...
		if is_static {
			...
			// skip calling `class_bindings.get_or_init_temp_binding(ctx)` if shortcut
			// can be used. But can't pass `class_bindings` as a `&mut ClassBinding` into
			// `transform_static_assignment_expression` due to borrow-checker restrictions.
			// If clone it, then any update to `temp` field is not stored globally, so that
			// doesn't work. Solution will have to be to break up
			// `transform_static_assignment_expression` into 2 methods for shortcut/no
			// shortcut and do the "can we shortcut?" check here. Then only create temp
			// var for the "no shortcut" branch, and clone the resulting binding
	...
	///
	/// Output in all cases contains an `AssignmentExpression`, so mutate
	/// existing `AssignmentExpression` rather than creating a new one.
	...
	fn transform_static_assignment_expression(
		...
		// `Class.#prop` for `_prop._`. Or does that behave slightly differently if
		...
		if let Some((class_symbol_id, object_reference_id)) = object_reference {
			...
			if operator == AssignmentOperator::Assign {
				...
				// _prop)` TODO: Ensure there are tests for nested classes with references
				// to private static props of outer class inside inner class, to make sure
				// we're getting the right `class_binding`.
	...
	fn transform_instance_assignment_expression(
		...
		// TODO: Different output in for statements e.g.
	...
	/// _prop)[_prop]` `object.#prop++` -> `_classPrivateFieldLooseBase(object,
	/// _prop)[_prop]++`
	///
	...
	fn transform_update_expression_impl(
		...
		// `update_expr.argument` instead. Only doing this first to match the order
		// Babel creates temp vars.
		let object = ctx.ast.move_expression(&mut field_expr.object);

		if is_static {
			// TODO: If `object` is reference to class name, and class is declaration, use
			// shortcuts: `++Class.#prop` -> `_prop._ = ((_Class$prop = _prop._),
			// ++_Class$prop)` `Class.#prop++` -> `_prop._ = (_Class$prop = _prop._,
			// _Class$prop2 = _Class$prop++, _Class$prop), _Class$prop2`
			// TODO(improve-on-babel): These shortcuts could be shorter - just `_prop._++` /
			// `++_prop._`. Or does that behave slightly differently if `Class.#prop` is
			// an object with `valueOf` method? TODO(improve-on-babel): No reason not to
			// apply these shortcuts for class expressions too.
	...
	fn transform_chain_expression_impl(
		...
		if matches!(element, ChainElement::PrivateFieldExpression(_)) {
			...
			// updated directly in `element` or do nothing because haven't found any
			// private field."
			None
		}
	}
	...
	fn transform_chain_expression_element(
		...
		match element {
			expression @ match_member_expression!(ChainElement) => {
				self.transform_member_expression_of_chain_expression(
	...
	fn transform_private_field_expression_of_chain_expression(
		...
		if matches!(ctx.ancestor(1), Ancestor::CallExpressionCallee(_)) {
			...
			// _m._.bind(Foo))();` ^^^^^^^^^^^^ is a call expression, we need to bind the
			// proper context
	...
	fn transform_member_expression_of_chain_expression(
		...
		if is_optional && !object.is_identifier_reference() {
			...
			// object)?.self.unicorn`  ^^^^^^^^^^^^^^^^^ the object has transformed, if
			// the current member is optional,                    then we need to wrap it
	...
	fn transform_call_expression_of_chain_expression(
		...
		// has a proper context,                            we also need to assign
		// `o?.Foo.#self` to a temp variable, and                            then use
		// it as a first argument of `getSelf` call.
		//
	...
	///    -> `(_ref = o === null || o === void 0 ? void 0 :
	/// (_babelHelpers$assertC =       babelHelpers.assertClassBrand(Foo,
	/// o.Foo, _self)._).getSelf) === null ||       _ref === void 0 ? void 0 :
	/// babelHelpers.assertClassBrand(Foo, _ref$call       = _ref.
	/// call(_babelHelpers$assertC), _m)._?.call(_ref$call);`
	fn transform_call_expression_to_bind_proper_context(
		...
		let context = if let Some(result) = self.transform_chain_element_recursively(object, ctx) {
			if is_optional_callee {
				...
				// null || o === void 0 ?              ^^ is optional         void 0 :
				// babelHelpers.assertClassBrand(Foo, o.Foo, _self)._)`
				*object =
				...
				*object = assignment;
				...
				// we need to assign                        the parent of callee (i.e
				// `o?.Foo.#self`) to a temp variable,                        and then use
				// it as a first argument of `_ref.call`.
				let (assignment, context) =
					self.duplicate_object(ctx.ast.move_expression(object), ctx);
				*object = assignment;
				*callee =
			...
			// proper context,                  we also need to assign `Foo?.bar()` to a
			// temp variable, and then use                  it as a first argument of
			// `_Foo$bar$zoo`.
			let (assignment, context) = self.duplicate_object(ctx.ast.move_expression(object), ctx);
			*object = assignment;
			context
	...
	/// `&BoundIdentifier<'a>` returned by `self.private_props_stack.find()`,
	/// which takes a partial mut borrow of `self`.
