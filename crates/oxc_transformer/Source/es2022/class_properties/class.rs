...
impl<'a, 'ctx> ClassProperties<'a, 'ctx> {
	...
	// Want it to be inlined into `enter_expression` and for `enter_expression` to be inlined into
	// parent.
	#[inline]
	...
	fn transform_class_expression_start(
		...
		if *self.class_expression_addresses_stack.last() == class_address {
			...
			// in the sequence expression which was substituted for it. So don't transform
			// it again! Returning 0 tells `enter_expression` not to call
			// `transform_class_expression_finish` either.
		...
		if expr_count > 0 {
			...
			// Store the `Address` of class in stack. This will cause bail-out when we
			// re-visit it.
	...
	fn transform_class_expression_finish(
		...
		// example above). If class name shadows var which is referenced within class,
		// rename that var. `var C = class { prop = C }; var C2 = C;`
		// -> `var _C = class C { constructor() { this.prop = _C; } }; var C2 = _C;`
		// This is really difficult as need to rename all references too to that binding
		...
		// Or maybe should store count and increment it when create private static
		// props? They're probably pretty rare, so it'll be rarely used.
		...
		if let Some(binding) = &self.class_bindings.temp {
			...
			// static block contents goes after class expression, and temp var is called
			// `_temp` not `_Class`. `let C = class extends Unbound { static { x = 1; }
			// };` -> `var _temp; let C = ((_temp = class C extends Unbound {}), (x = 1),
			// _temp);` `let C = class extends Bound { static { x = 1; } };`
	...
	fn transform_class(&mut self, class:&mut Class<'a>, ctx:&mut TraverseCtx<'a>) {
		...
		// Check if class has any properties and get index of constructor (if class has
		// one)
		...
		let class_temp_binding = if need_temp_var {
			...
			if self.is_declaration {
				...
				// only to match Babel's output. It'd be simpler just to insert it at the
				// end and get rid of `temp_var_is_created` that tracks whether it's done
				// already or not.
				self.ctx.var_declarations.insert_var(&temp_binding, ctx);
			}
			Some(temp_binding)
		...
		if private_props.is_empty() {
			...
			// `class_bindings.temp` in the `PrivateProps` entry is the temp var (if one has
			// been created). Private fields in static prop initializers use the temp var
			// in the transpiled output e.g. `_assertClassBrand(_Class, obj, _prop)`.
			// At end of this function, if it's a class declaration, we set
			// `class_bindings.temp` to be the binding for the class name, for when the
			// class body is visited, because in the class body, private fields use the
			// class name e.g. `_assertClassBrand(Class, obj, _prop)` (note `Class` not
			// `_Class`).
		...
		// Extract properties and static blocks from class body + substitute computed
		// method keys
		...
		// Insert instance initializers into constructor, or create constructor if there
		// is none
		...
		// Update class bindings prior to traversing class body and insertion of
		// statements/expressions before/after the class. See comments on
		// `ClassBindings`.
		if let Some(private_props) = self.private_props_stack.last_mut() {
			...
			// declarations. `class Class { static #prop; method() { return obj.#prop; }
			...
			// Note: If declaration is `export default class {}` with no name, and class has
			// static props, then class has had name binding created already above. So
			// name binding is always `Some`.
			if self.is_declaration {
