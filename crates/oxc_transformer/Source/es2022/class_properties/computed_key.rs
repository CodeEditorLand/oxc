...
///
/// `this` does not have side effects, but in this context, it needs a temp var
/// anyway, because `this` in computed key and `this` within class constructor
/// resolve to different `this` bindings. So we need to create a temp var
/// outside of the class to get the correct `this`. `class C { [this] = 1; }`
/// -> `let _this; _this = this; class C { constructor() { this[_this] = 1; } }`
// TODO(improve-on-babel): Can avoid the temp var if key is for a static
// prop/method, as in that case the usage of `this` stays outside the class.
fn key_needs_temp_var(key:&Expression, ctx:&TraverseCtx) -> bool {
	match key {
		...
		// effects, but don't bother checking for that as it shouldn't occur in real world code.
