...
impl<'a, 'v> Visit<'a> for InstanceInitializerVisitor<'a, 'v> {
	...
	// functions
	#[inline]
	fn leave_scope(&mut self) { self.scope_ids_stack.pop(); }
...
impl<'a, 'v> InstanceInitializerVisitor<'a, 'v> {
	...
	fn check_for_symbol_clash(&mut self, ident:&IdentifierReference<'a>) {
		...
		// `ScopeTree::bindings` with same `ScopeId` every time here, but `ScopeTree`
		// doesn't allow that, and we also take a `&mut ScopeTree` in
		// `reparent_scope`, so borrow-checker doesn't allow that.
		...
		// initializer itself. If it's bound within the initializer, there's no clash,
		// so exit. e.g. `class C { double = (n) => n * 2; constructor(n) {} }`
		// Even though there's a binding `n` in constructor, it doesn't shadow the use
		// of `n` in init. This is an improvement over Babel.
...
/// continuing to traverse inside that scope, as by definition and nested scopes
...
impl<'a, 'v> FastInstanceInitializerVisitor<'a, 'v> {
	fn new(class_properties:&'v mut ClassProperties<'a, '_>, ctx:&'v mut TraverseCtx<'a>) -> Self {
