...
impl<'a> Traverse<'a> for ClassStaticBlock {
	fn enter_class_body(&mut self, body:&mut ClassBody<'a>, ctx:&mut TraverseCtx<'a>) {
		...
		// found, as usually there will be no matching private keys, so those checks
		// are cheap and will not allocate.
...
impl ClassStaticBlock {
	...
	/// TODO: Make this function non-pub if no longer use it for
	/// `ClassProperties`.
