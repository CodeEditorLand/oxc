...
impl<'a, 'ctx> ObjectRestSpread<'a, 'ctx> {
	...
	fn transform_object_assignment_target(
		...
		let keys =
			ctx.ast
				.vec_from_iter(object_assignment_target.properties.iter_mut().filter_map(|e| {
					match e {
						...
						match p {
							...
							AssignmentTargetProperty::AssignmentTargetPropertyProperty(p) => {
								match &p.binding {
									...
									_ => {
										Self::has_nested_target_rest(
											p.binding.to_assignment_target(),
										)
									},
								}
							},
	...
	fn recursive_walk_assignment_target_maybe_default(
		...
		match target {
			...
			_ => {
				Self::recursive_walk_assignment_target(
	...
	fn transform_variable_declarator(
		...
		if let BindingPatternKind::ObjectPattern(pat) = &mut decl.id.kind {
			...
			if let Some(rest) = pat.rest.take() {
				...
				// (babelHelpers.objectDestructuringEmpty(_foo), _foo))`. Or `rest =
				// babelHelpers.objectWithoutProperties(_foo, ["x"])`.
				let (lhs, rhs) = datum.get_lhs_rhs(
					&mut reference_builder,
					&mut self.excluded_variable_declarators,
					self.ctx,
...
impl<'a> SpreadPair<'a> {
	fn get_lhs_rhs(
		...
		let rhs = if self.has_no_properties {
			...
			// destructuring null. `function _objectDestructuringEmpty(t) { if (null ==
			// t) throw new TypeError("Cannot destructure " + t); }`
