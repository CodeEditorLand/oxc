...
use crate::{
	...
	env::{EnvOptions, Versions, can_enable_plugin},
...
impl TransformOptions {
	...
	/// # Errors
	///
	/// If there are any errors in the `options.targets``, they will be returned
	/// as a list of errors.
	...
	pub fn from_babel_options(options:&BabelOptions) -> Result<Self, Vec<Error>> {
		...
		let targets = env_options.as_ref().and_then(|env| {
			match env.get_targets() {
				Ok(res) => Some(res),
				Err(err) => {
					errors.push(OxcDiagnostic::error(err.to_string()).into());
...
fn report_error(name:&str, err:&serde_json::Error, is_preset:bool, errors:&mut Vec<Error>) {
	let message = if is_preset {
		format!("preset-{name}: {err}",)
	} else {
		format!("{name}: {err}",)
	};
