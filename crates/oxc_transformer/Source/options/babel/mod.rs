...
impl PluginPresetEntry {
	fn name(&self) -> &str {
		match self {
			Self::String(s) | Self::Tuple(s, _) | Self::Triple(s, ..) => s,
			Self::Vec1(s) => &s[0],
		}
	}
...
impl BabelOptions {
	...
	pub fn is_jsx(&self) -> bool { self.plugins.syntax_jsx }

	pub fn is_typescript(&self) -> bool { self.plugins.syntax_typescript.is_some() }
