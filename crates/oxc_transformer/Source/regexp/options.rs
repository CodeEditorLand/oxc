...
pub struct RegExpOptions {
	...
	/// Enables plugin to transform the RegExp literal has `(?<=)` or `(?<!)`
	/// lookbehind assertions
	pub look_behind_assertions:bool,
	/// Enables plugin to transform the RegExp literal has `(?<name>x)` named
	/// capture groups
	pub named_capture_groups:bool,
	/// Enables plugin to transform the RegExp literal has `\p{}` and `\P{}`
