...
fn default_for_jsx_pragma() -> Cow<'static, str> { Cow::Borrowed("React.createElement") }

fn default_for_jsx_pragma_frag() -> Cow<'static, str> { Cow::Borrowed("React.Fragment") }
...
pub struct TypeScriptOptions {
	...
	/// This is so that we know that the import is not a type import, and
	/// should not be removed. defaults to React
	...
	/// This is so that we know that the import is not a type import, and
	/// should not be removed. defaults to React.Fragment
	...
	/// allows to write complete relative specifiers in import declarations
	/// while using the same extension used by the source files.
	///
	/// When set to `true`, same as [`RewriteExtensionsMode::Rewrite`].
...
impl RewriteExtensionsMode {
	pub fn is_remove(&self) -> bool { matches!(self, Self::Remove) }
}
