...
impl<'a, 'ctx> Traverse<'a> for TypeScriptAnnotations<'a, 'ctx> {
	...
	fn enter_ts_module_declaration(
		...
		// NB: Namespace transform happens in `enter_program` visitor, and replaces
		// retained namespaces with functions. This visitor is called after, by which
		// time any remaining namespaces need to be deleted.
...
impl<'a, 'ctx> TypeScriptAnnotations<'a, 'ctx> {
	...
	fn replace_with_empty_block_if_ts(
		...
		if stmt.is_typescript_syntax() {
			...
			let block =
				ctx.ast
					.alloc_block_statement_with_scope_id(stmt.span(), ctx.ast.vec(), scope_id);
	...
	pub fn has_value_reference(&self, name:&str, ctx:&TraverseCtx<'a>) -> bool {
		if let Some(symbol_id) = ctx.scopes().get_root_binding(name) {
			...
			// then it's a value redeclaration. That means the import is shadowed, and we
			// can safely remove the import.
