...
impl<'a, 'ctx> Traverse<'a> for TypeScriptAnnotations<'a, 'ctx> {
	...
	fn enter_simple_assignment_target(
		...
		if let Some(expr) = target.get_expression_mut() {
			match expr.get_inner_expression_mut() {
				...
				_ => {
					// This should be never hit until more syntax is added to
					// the JavaScript/TypeScrips
	...
	fn exit_method_definition(&mut self, def:&mut MethodDefinition<'a>, ctx:&mut TraverseCtx<'a>) {
	...
	fn enter_property_definition(
		...
		assert!(
			...
			"Fields with the 'declare' modifier cannot be initialized here, but only in the \
			 constructor"
		);
	...
	fn exit_statements(
		...
		stmts.retain(|stmt| {
			match stmt {
				...
				// Any namespaces left after namespace transform are type only,
				// so remove them
	...
	fn enter_ts_module_declaration(
		...
		// NB: Namespace transform happens in `enter_program` visitor, and
		// replaces retained namespaces with functions. This visitor is called
		// after, by which time any remaining namespaces need to be deleted.
...
impl<'a, 'ctx> TypeScriptAnnotations<'a, 'ctx> {
	...
	pub fn has_value_reference(&self, name:&str, ctx:&TraverseCtx<'a>) -> bool {
		if let Some(symbol_id) = ctx.scopes().get_root_binding(name) {
			// `import T from 'mod'; const T = 1;` The T has a value
			// redeclaration `import T from 'mod'; type T = number;` The T
			// has a type redeclaration If the symbol is still a value symbol
			// after SymbolFlags::Import is removed, then it's a value
			// redeclaration. That means the import is shadowed, and we can
