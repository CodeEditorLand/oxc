...
impl<'a> TypeScriptEnum<'a> {
	pub fn new() -> Self { Self { enums:FxHashMap::default() } }
}
...
impl<'a> TypeScriptEnum<'a> {
	...
	fn transform_ts_enum(
		...
		ctx.scopes_mut()
			.add_binding(func_scope_id, enum_name.to_compact_str(), param_symbol_id);
			...
			let init = if let Some(initializer) = &mut member.initializer {
				...
				let init = match constant_value {
					None => {
						...
						// and it is not a binding in the current scope and
						// parent scopes, we need to rename it to the enum
						// name. e.g. `d = c` to `d = A.c` same behavior in https://github.com/babel/babel/blob/610897a9a96c5e344e77ca9665df7613d2f88358/packages/babel-plugin-transform-typescript/src/enum.ts#L145-L150
	...
	fn get_number_literal_expression(value:f64, ctx:&TraverseCtx<'a>) -> Expression<'a> {
		ctx.ast
			.expression_numeric_literal(SPAN, value, value.to_string(), NumberBase::Decimal)
	}
	...
	fn evaluate_ref(
		...
		match expr {
			...
			Expression::Identifier(ident) => {
				...
				// TODO:
				// This is a bit tricky because we need to find the
				// BindingIdentifier that corresponds to the identifier
				// reference. and then we may to evaluate the initializer of
				// the BindingIdentifier. finally, we can get the value of
				// the identifier and call the `computed_constant_value`
				// function. See https://github.com/babel/babel/blob/610897a9a96c5e344e77ca9665df7613d2f88358/packages/babel-plugin-transform-typescript/src/enum.ts#L327-L329
				None
			},
			_ => None,
		}
	...
	fn visit_expression(&mut self, expr:&mut Expression<'a>) {
		let new_expr = match expr {
			...
			Expression::Identifier(ident) => {
				...
				// TODO: shadowed case, e.g. let ident = 1; ident; // ident is
				// not an enum enum_name.identifier
