...
impl<'a, 'ctx> Traverse<'a> for TypeScriptNamespace<'a, 'ctx> {
	...
	fn enter_program(&mut self, program:&mut Program<'a>, ctx:&mut TraverseCtx<'a>) {
		...
		// Inserting the `let` declaration multiple times will reallocate the
		// whole statements vec every time a namespace declaration is
		// encountered.
...
impl<'a, 'ctx> TypeScriptNamespace<'a, 'ctx> {
		...
		for stmt in namespace_top_level {
			match stmt {
				...
				Statement::ExportNamedDeclaration(export_decl) => {
					// NB: `ExportNamedDeclaration` with no declaration (e.g.
					// `export {x}`) is not legal syntax in TS namespaces
	...
	fn transform_namespace(
		...
		// (function (_N) { var M; (function (_M) { var x; })(M || (M = _N.M ||
		// (_N.M = {})));})(N || (N = {}));
		// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^    ^^^^^^^^^^^^^^
	...
	fn add_declaration(
		...
		// This function is only called with a function, class, or enum
		// declaration, all of which are guaranteed to have an `id`
