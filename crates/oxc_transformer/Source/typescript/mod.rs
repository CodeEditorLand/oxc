...
impl<'a, 'ctx> Traverse<'a> for TypeScript<'a, 'ctx> {
	...
	fn enter_formal_parameter(&mut self, param:&mut FormalParameter<'a>, ctx:&mut TraverseCtx<'a>) {
	...
	fn enter_method_definition(&mut self, def:&mut MethodDefinition<'a>, ctx:&mut TraverseCtx<'a>) {
	...
	fn enter_accessor_property(&mut self, def:&mut AccessorProperty<'a>, ctx:&mut TraverseCtx<'a>) {
