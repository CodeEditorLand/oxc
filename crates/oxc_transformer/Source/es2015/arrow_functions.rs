...
impl<'a> ArrowFunctions<'a> {
	fn get_this_identifier(
		...
		// create a single `_this` UID and reuse it in each scope. But this
		// does not match output for some of Babel's test cases. <https://github.com/oxc-project/oxc/pull/5840>
	...
	/// Find arrow function we are currently in, if it's between current node,
	/// and where `this` is bound. Return its `ScopeId`.
	fn get_arrow_function_scope(ctx:&mut TraverseCtx<'a>) -> Option<ScopeId> {
		...
		// ```
		//
		// `this` resolves to the class / class instance (i.e. `this` defined
		// *within* the class) in:
		// * Method body
		// * Method param
		// * Property value
