...
impl<'a> ModuleImportsStore<'a> {
	...
	pub fn new() -> Self { Self { imports:RefCell::new(IndexMap::default()) } }
	...
	pub fn is_empty(&self) -> bool { self.imports.borrow().is_empty() }
}
...
impl<'a> ModuleImportsStore<'a> {
	...
	fn insert_import_statements(&self, transform_ctx:&TransformCtx<'a>, ctx:&mut TraverseCtx<'a>) {
	...
	fn insert_require_statements(&self, transform_ctx:&TransformCtx<'a>, ctx:&mut TraverseCtx<'a>) {
		let mut imports = self.imports.borrow_mut();
	...
	fn get_import(source:Atom<'a>, names:Vec<Import<'a>>, ctx:&TraverseCtx<'a>) -> Statement<'a> {
		let specifiers = ctx.ast.vec_from_iter(names.into_iter().map(|import| {
			match import {
				Import::Named(import) => {
