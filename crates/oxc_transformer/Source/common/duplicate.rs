...
impl<'a> TransformCtx<'a> {
	...
	/// for a bound identifier, if it's mutated (assigned to) anywhere in AST.
	///
		...
		let temp_var_binding = match &expr {
			Expression::Identifier(ident) => {
				...
				if let Some(symbol_id) = reference.symbol_id() {
					...
					{
						// Reading bound identifier cannot have side effects, so no need for temp
						// var
			...
			// Note: "`x${foo}`" *can* have side effects if `foo` is an object with a `toString`
			// method.
