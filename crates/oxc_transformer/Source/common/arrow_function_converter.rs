...
impl<'a> Traverse<'a> for ArrowFunctionConverter<'a> {
	...
	fn enter_identifier_reference(
		...
		// `transform_identifier_reference_for_arguments` so that the fast path for
		// "no transform required" doesn't require a function call
	...
	fn enter_binding_identifier(
		...
		// Do this check here rather than in
		// `transform_binding_identifier_for_arguments` so that the fast path for "no
		// transform required" doesn't require a function call
		let arguments_needs_transform = *self.arguments_needs_transform_stack.last();
...
impl<'a> ArrowFunctionConverter<'a> {
	...
	fn is_disabled(&self) -> bool { self.mode == ArrowFunctionConverterMode::Disabled }

	/// Check if arrow function conversion has enabled for transform async arrow
	/// functions
	#[inline]
	fn is_async_only(&self) -> bool { self.mode == ArrowFunctionConverterMode::AsyncOnly }
	...
	fn get_this_identifier(
		...
		// pointless, as only one `this` can be in scope at a time. We could create a
		// single `_this` UID and reuse it in each scope. But this does not match
		// output for some of Babel's test cases. <https://github.com/oxc-project/oxc/pull/5840>
	...
	/// function whose body includes the original `super` expression. The arrow
	/// function's name is generated based on the property name, such as
	/// `_superprop_getProperty`.
	///
	/// The `super` expressions are temporarily stored in
	/// [`Self::super_methods`] and eventually inserted by
	/// [`Self::insert_variable_statement_at_the_top_of_statements`].`
	///
	/// ## Example
	///
	...
	/// binding same as the target scope, if it's mismatch, we need to move the
	/// binding to the target scope.
	...
	fn generate_super_method(
		...
		if is_computed {
			...
			// there's nothing in `prop => super[prop]` or `(prop, value) => super[prop]
			// = value` which can clash.
			let param_binding =
		...
		if is_assignment {
			...
			// there's nothing in `value => super.prop = value` or `(prop, value) =>
	...
	fn transform_identifier_reference_for_arguments(
		...
		let binding = self.arguments_var_stack.last_or_init(|| {
			if let Some(symbol_id) = symbol_id {
				...
				// haven't been created yet, so we temporary use root scope id as the fake
				// target scope ID.
