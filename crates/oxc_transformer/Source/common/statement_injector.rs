...
impl<'a> StatementInjectorStore<'a> {
	...
	pub fn new() -> Self { Self { insertions:RefCell::new(FxHashMap::default()) } }
}
...
impl<'a> StatementInjectorStore<'a> {
	...
	where
		...
		adjacent_stmts.splice(
			...
			stmts
				.into_iter()
				.map(|stmt| AdjacentStatement { stmt, direction:Direction::Before }),
		);
	}
	...
	where
		...
		adjacent_stmts.extend(
			stmts
				.into_iter()
				.map(|stmt| AdjacentStatement { stmt, direction:Direction::After }),
		);
	}
}
