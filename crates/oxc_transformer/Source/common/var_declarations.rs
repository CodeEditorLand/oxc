...
//! entering a statement block, and when exiting the block, writes a `var`
//! statement to top of block containing the declarators.
//!
...
impl<'a, 'ctx> Traverse<'a> for VarDeclarations<'a, 'ctx> {
	fn enter_statements(&mut self, _stmts:&mut Vec<'a, Statement<'a>>, _ctx:&mut TraverseCtx<'a>) {
...
impl<'a> VarDeclarationsStore<'a> {
	...
	pub fn new() -> Self { Self { stack:RefCell::new(SparseStack::new()) } }

	/// Add a `VariableDeclarator` to be inserted at top of current enclosing
	/// statement block, given `name` and `symbol_id`.
	pub fn insert(
		&self,
		name:Atom<'a>,
		symbol_id:SymbolId,
...
impl<'a> VarDeclarationsStore<'a> {
	...
	fn insert_into_statements(&self, stmts:&mut Vec<'a, Statement<'a>>, ctx:&mut TraverseCtx<'a>) {
