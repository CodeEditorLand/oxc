...
/// If `enum_` has any non-unit variant, returns `#[repr(C, u8)]`, otherwise
/// returns `#[repr(u8)]`.
...
fn assert_generated_derives(attrs:&[Attribute]) -> TokenStream {
	// NOTE: At this level we don't care if a trait is derived multiple times,
	// It is the responsibility of the `ast_tools` to raise errors for those.
	let assertion = attrs
		.iter()
...
fn parse_attr(attr:&Attribute) -> impl Iterator<Item = Ident> {
	attr.parse_args_with(Punctuated::<Ident, Comma>::parse_terminated)
		.expect(
			"`#[generate_derive]` only accepts traits as single segment paths. Found an invalid \
			 argument.",
		)
		.into_iter()
}

// TODO: benchmark this to see if a lazy static cell containing `HashMap` would
// perform better.
#[inline]
fn abs_trait(
	ident:&Ident,
...
fn invalid_derive(ident:&Ident) -> ! {
	panic!(
		"Invalid derive trait(generate_derive): {ident}.\nHelp: If you are trying to implement a \
		 new `generate_derive` trait, make sure to add it to the list in `abs_trait` function."
	)
}
