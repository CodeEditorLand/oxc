...
use quote::quote;
use syn::{
	punctuated::Punctuated, token::Comma, Attribute, Fields, Ident, Item,
	ItemEnum,
};
...
pub fn ast(input: &Item) -> TokenStream {
	let (head, tail) = match input {
		Item::Enum(enum_) => {
			(enum_repr(enum_), assert_generated_derives(&enum_.attrs))
		},
		Item::Struct(struct_) => {
...
fn abs_trait(
	...
	/* absolute type path */ TokenStream,
	/* possible generics */ TokenStream,
) {
	if ident == "CloneIn" {
