...
use tower_lsp::lsp_types::{
	self, DiagnosticRelatedInformation, DiagnosticSeverity, Position, Range,
	Url,
};
	...
	pub fn new(
		...
		let labels_with_pos: Vec<LabeledSpanWithPosition> = labels
			...
			.map(|labeled_span| LabeledSpanWithPosition {
				start_pos: offset_to_position(
					labeled_span.offset() + start,
					text,
				)
				.unwrap_or_default(),
	...
	fn to_lsp_diagnostic(&self, path: &PathBuf) -> lsp_types::Diagnostic {
		...
		let range = related_information.as_ref().map_or(
			...
			|infos: &Vec<DiagnosticRelatedInformation>| {
				...
				for info in infos {
					if cmp_range(&ret_range, &info.location.range)
...
impl IsolatedLintHandler {
	...
	fn lint_path(
		...
		for source in javascript_sources {
			...
			let ret =
				Parser::new(&allocator, javascript_source_text, source_type)
	...
	fn should_lint_path(path: &Path) -> bool {
		static WANTED_EXTENSIONS: OnceLock<FxHashSet<&'static str>> =
			OnceLock::new();
