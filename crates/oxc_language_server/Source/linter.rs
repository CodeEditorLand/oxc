...
impl ErrorWithPosition {
	pub fn new(error:Error, text:&str, fixed_content:Option<FixedContent>, start:usize) -> Self {
...
impl IsolatedLintHandler {
	pub fn new(linter:Arc<Linter>) -> Self { Self { linter, loader:Loader } }
		...
		for source in javascript_sources {
			...
			if !ret.errors.is_empty() {
				let reports = ret
					...
					.into_iter()
					.map(|diagnostic| {
						ErrorReport { error:Error::from(diagnostic), fixed_content:None }
					})
					.collect();
			...
			let reports = result
				.into_iter()
				.map(|msg| {
					let fixed_content = msg.fix.map(|f| {
						FixedContent {
...
impl ServerLinter {
	...
	pub fn new_with_linter(linter:Linter) -> Self { Self { linter:Arc::new(linter) } }
...
fn parse_diagnostic_code(code:&str) -> Option<(&str, &str)> {
	...
	Some((
		&code[0..right_parenthesis_pos],
		&code[right_parenthesis_pos + 1..code.len() - 1],
	))
}
