...
use crate::{
	capabilities::{CODE_ACTION_KIND_SOURCE_FIX_ALL_OXC, Capabilities},
	linter::{DiagnosticReport, ServerLinter},
};
	...
	async fn did_change_configuration(&self, params:DidChangeConfigurationParams) {
		let changed_options =
			if let Ok(options) = serde_json::from_value::<Options>(params.settings) {
				...
				// Fallback if some client didn't took changed configuration in params of
		...
		*self.options.lock().await = changed_options.clone();
		...
		// revalidate the config and all open files, when lint level is not disabled and
		// the config path is changed
	...
	async fn code_action(&self, params:CodeActionParams) -> Result<Option<CodeActionResponse>> {
		...
		if let Some(value) = self.diagnostics_report_map.get(&uri.to_string()) {
			...
			for report in reports {
				// TODO: Would be better if we had exact rule name from the diagnostic instead
				...
				code_actions_vec.push(
					// TODO: This CodeAction doesn't support disabling multiple rules by name for a
					// given line.  To do that, we need to read
					// `report.diagnostic.range.start.line` and check if a disable comment already
					// exists.  If it does, it needs to be appended to instead of a completely
					// new line inserted.
								...
								vec![TextEdit {
									range:Range {
										start:Position {
											...
											// TODO: character should be set to match the first
											// non-whitespace character in the source text to match
