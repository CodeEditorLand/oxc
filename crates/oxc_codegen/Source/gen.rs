...
pub trait Gen: GetSpan {
	...
	fn print(&self, p:&mut Codegen, ctx:Context) { self.gen(p, ctx); }
}
...
pub trait GenExpr: GetSpan {
	...
	/// Generate code for an expression, respecting operator precedence. Alias
	/// for `gen_expr`.
...
impl Gen for Directive<'_> {
	fn gen(&self, p:&mut Codegen, _ctx:Context) {
		...
		// So here should print original `directive` value, the `expression` value is
		// escaped str. See https://github.com/babel/babel/blob/v7.26.2/packages/babel-generator/src/generators/base.ts#L64
...
impl GenExpr for ComputedMemberExpression<'_> {
	fn gen_expr(&self, p:&mut Codegen, _precedence:Precedence, ctx:Context) {
		...
		p.wrap(wrap, |p| {
			self.object
				.print_expr(p, Precedence::Postfix, ctx.intersection(Context::FORBID_CALL));
		});
...
impl GenExpr for StaticMemberExpression<'_> {
	fn gen_expr(&self, p:&mut Codegen, _precedence:Precedence, ctx:Context) {
		...
		if self.optional {
			...
			// `0.toExponential()` is invalid, add a space before the dot, `0
			// .toExponential()` is valid
			p.print_hard_space();
		}
...
impl GenExpr for PrivateFieldExpression<'_> {
	fn gen_expr(&self, p:&mut Codegen, _precedence:Precedence, ctx:Context) {
		self.object
			.print_expr(p, Precedence::Prefix, ctx.intersection(Context::FORBID_CALL));
...
impl GenExpr for BinaryExpression<'_> {
	fn gen_expr(&self, p:&mut Codegen, precedence:Precedence, ctx:Context) {
		let v = BinaryExpressionVisitor {
			// SAFETY:
			// The pointer is stored on the heap and all will be consumed in the binary expression
			// visitor.
...
impl GenExpr for ChainExpression<'_> {
	fn gen_expr(&self, p:&mut Codegen, precedence:Precedence, ctx:Context) {
		p.wrap(precedence >= Precedence::Postfix, |p| {
...
impl Gen for TSTypeAnnotation<'_> {
	fn gen(&self, p:&mut Codegen, ctx:Context) { self.type_annotation.print(p, ctx); }
}
...
impl Gen for TSModuleDeclaration<'_> {
	fn gen(&self, p:&mut Codegen, ctx:Context) {
		...
		// If the kind is global, then the id is also `global`, so we don't need to
		// print it
