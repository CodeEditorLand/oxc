...
use oxc_ast::{
	ast::{
		BindingIdentifier, BlockStatement, Expression, IdentifierReference,
		Program, Statement,
	},
	Trivias,
};
...
use crate::{
	binary_expr_visitor::BinaryExpressionVisitor, comment::CommentsMap,
...
impl<'a> Codegen<'a> {
	...
	pub fn with_capacity(mut self, source_text_len: usize) -> Self {
		let capacity = if self.options.minify {
			source_text_len / 2
		} else {
			source_text_len
		};
	...
	pub fn build(mut self, program: &Program<'_>) -> CodegenReturn {
		...
		let source_map =
			self.sourcemap_builder.map(SourcemapBuilder::into_sourcemap);
...
impl<'a> Codegen<'a> {
	...
	fn print_space_before_identifier(&mut self) {
		if self.peek_nth(0).is_some_and(|ch| {
			is_identifier_part(ch) || self.prev_reg_exp_end == self.code.len()
		}) {
	...
	fn print_block_statement(
		&mut self,
		stmt: &BlockStatement<'_>,
		ctx: Context,
	) {
	...
	fn print_list_with_comments<T: Gen + GetSpan>(
		&mut self,
		items: &[T],
		ctx: Context,
	) {
	...
	fn print_expressions<T: GenExpr>(
		&mut self,
		items: &[T],
		precedence: Precedence,
		ctx: Context,
	...
	fn get_binding_identifier_name(
		&self,
	...
	fn add_source_mapping_for_name(&mut self, span: Span, name: &str) {
		if let Some(sourcemap_builder) = self.sourcemap_builder.as_mut() {
			sourcemap_builder
				.add_source_mapping_for_name(&self.code, span, name);
		}
	}
}
