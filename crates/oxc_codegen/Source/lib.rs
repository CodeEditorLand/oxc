...
pub struct CodegenReturn {
	...
	///
	/// You must set [`CodegenOptions::source_map_path`] for this to be
	/// [`Some`].
	pub map:Option<oxc_sourcemap::SourceMap>,

	/// All the legal comments returned from [LegalComment::Linked] or
...
pub struct Codegen<'a> {
	...
	/// Indicates the output is JSX type, it is set in [`Program::gen`] and the
	/// result is obtained by [`oxc_span::SourceType::is_jsx`]
	is_jsx:bool,
...
impl<'a> From<Codegen<'a>> for String {
	fn from(val:Codegen<'a>) -> Self { val.into_source_text() }
}

impl<'a> From<Codegen<'a>> for Cow<'a, str> {
	fn from(val:Codegen<'a>) -> Self { Cow::Owned(val.into_source_text()) }
}
...
impl<'a> Codegen<'a> {
	...
	///
	/// A source map will be generated if [`CodegenOptions::source_map_path`] is
	/// set.
	#[must_use]
	...
	///
	/// This is more useful for cases that progressively build code using
	/// [`print_expression`].
	///
	/// [`build`]: Codegen::build
	/// [`print_expression`]: Codegen::print_expression
	#[must_use]
	pub fn into_source_text(self) -> String { self.code.into_string() }
	...
	#[inline]
	pub fn print_ascii_byte(&mut self, byte:u8) { self.code.print_ascii_byte(byte); }

	/// Push str into the buffer
	#[inline]
	pub fn print_str(&mut self, s:&str) { self.code.print_str(s); }

	/// Print a single [`Expression`], adding it to the code generator's
	/// internal buffer. Unlike [`Codegen::build`], this does not consume
	/// `self`.
	#[inline]
...
// Private APIs
impl<'a> Codegen<'a> {
	fn code(&self) -> &CodeBuffer { &self.code }

	fn code_len(&self) -> usize { self.code().len() }
	...
	#[inline]
	fn print_hard_space(&mut self) { self.print_ascii_byte(b' '); }
	...
	#[inline]
	fn print_hard_newline(&mut self) { self.print_ascii_byte(b'\n'); }

	#[inline]
	fn print_semicolon(&mut self) { self.print_ascii_byte(b';'); }

	#[inline]
	fn print_comma(&mut self) { self.print_ascii_byte(b','); }
	...
	#[inline]
	fn last_byte(&self) -> Option<u8> { self.code.last_byte() }

	#[inline]
	fn last_char(&self) -> Option<char> { self.code.last_char() }
	...
	#[inline]
	fn print_ellipsis(&mut self) { self.print_str("..."); }

	#[inline]
	fn print_colon(&mut self) { self.print_ascii_byte(b':'); }

	#[inline]
	fn print_equal(&mut self) { self.print_ascii_byte(b'='); }
