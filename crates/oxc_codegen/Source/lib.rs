...
pub struct CommentOptions {
	/// Enable preserve annotate comments, like `/* #__PURE__ */` and `/*
	/// #__NO_SIDE_EFFECTS__ */`.
...
pub struct Codegen<'a> {
	...
	/// Start of comment that needs to be moved to the before
	/// VariableDeclarator
	///
	/// For example:
	/// ```js
...
impl<'a> From<Codegen<'a>> for String {
	fn from(mut val:Codegen<'a>) -> Self { val.into_source_text() }
}

impl<'a> From<Codegen<'a>> for Cow<'a, str> {
	fn from(mut val:Codegen<'a>) -> Self { Cow::Owned(val.into_source_text()) }
}
...
impl<'a> Codegen<'a> {
	...
	pub fn with_capacity(mut self, source_text_len:usize) -> Self {
		...
		// ensure space for at least `capacity` additional bytes without
		// clobbering existing allocations.
	...
	///
	/// The source code will be used with comments or for improving the
	/// generated output. It also pre-allocates memory for the output code
	/// using [`Codegen::with_capacity`]. Note that if you use this method
	/// alongside your own call to [`Codegen::with_capacity`], the larger of the
	/// two will be used.
	#[must_use]
	...
	#[inline]
	pub fn print_char(&mut self, ch:u8) { self.code.push(ch); }

	/// Push str into the buffer
	#[inline]
	pub fn print_str(&mut self, s:&str) { self.code.extend(s.as_bytes()); }
...
impl<'a> Codegen<'a> {
	...
	#[inline]
	fn print_hard_space(&mut self) { self.print_char(b' '); }
	...
	#[inline]
	fn print_hard_newline(&mut self) { self.print_char(b'\n'); }

	#[inline]
	fn print_semicolon(&mut self) { self.print_char(b';'); }

	#[inline]
	fn print_comma(&mut self) { self.print_char(b','); }
	...
	#[inline]
	fn print_colon(&mut self) { self.print_char(b':'); }

	#[inline]
	fn print_equal(&mut self) { self.print_char(b'='); }
