...
impl CodeBuffer {
	...
	#[inline]
	pub fn with_capacity(capacity:usize) -> Self { Self { buf:Vec::with_capacity(capacity) } }
	...
	#[inline]
	pub fn last_byte(&self) -> Option<u8> { self.buf.last().copied() }

	/// Peek the last char from the end of the buffer.
	#[inline]
	pub fn last_char(&self) -> Option<char> { self.peek_nth_char_back(0) }
	...
	pub fn print_ascii_byte(&mut self, byte:u8) {
		// When this method is inlined, and the value of `byte` is known, this assertion
		// should get optimized away by the compiler. e.g.
		// `code_buffer.print_ascii_byte(b' ')`.
	...
	/// Push a byte to the buffer, without checking that the buffer still
	/// represents a valid UTF-8 string.
	///
	/// If you are looking to print a byte you know is valid ASCII, prefer
	/// [`print_ascii_byte`]. If you are not certain, you may use
	/// [`print_char`] as a safe alternative.
	///
	/// # SAFETY
	/// The caller must ensure that, after 1 or more sequential calls, the
	/// buffer represents a valid UTF-8 string.
	///
	/// It is safe for a single call to temporarily result in invalid UTF-8, as
	/// long as UTF-8 integrity is restored before calls to any other `print_*`
	/// method or [`into_string`]. This lets you, for example, print an 4-byte
	/// Unicode character using 4 separate calls to this method. However,
	/// consider using [`print_bytes_unchecked`] instead for that use case.
	///
	/// # Example
	/// ```
	/// use oxc_codegen::CodeBuffer;
	...
	pub unsafe fn print_byte_unchecked(&mut self, byte:u8) {
		// By default, `self.buf.push(byte)` results in quite verbose assembly, because
		// the default branch is for the "buf is full to capacity" case.
		//
		// That's not ideal because growth strategy is doubling, so e.g. when the `Vec`
		// has just grown from 1024 bytes to 2048 bytes, it won't need to grow again
		// until another 1024 bytes have been pushed. "Needs to grow" is a very rare
		// occurrence.
		//
		// So we use `push_slow` to move the complicated logic for the "needs to grow"
		// path out of `print_byte_unchecked`, leaving a fast path for the common
		// "there is sufficient capacity" case. https://godbolt.org/z/Kv8sEoEed
	...
	/// this method since it's much more efficient. If you really want to
	/// insert only a single character and you're certain it's ASCII, consider
	/// using [`print_ascii_byte`].
	///
	/// # Example
	/// ```
	/// use oxc_codegen::CodeBuffer;
	...
	/// ```
	#[inline]
	pub fn print_str<S:AsRef<str>>(&mut self, s:S) { self.buf.extend(s.as_ref().as_bytes()); }
	...
	/// # Safety
	///
	/// The caller must ensure that, after this method call, the buffer
	/// represents a valid UTF-8 string. In practice, this means only two cases
	/// are valid:
	///
	/// 1. Both the buffer and the byte sequence are valid UTF-8,
	/// 2. The buffer became invalid after a call to [`print_byte_unchecked`]
	///    and `bytes` completes any incomplete Unicode characters, returning
	///    the buffer to a valid state.
	///
	/// # Example
	/// ```
	/// use oxc_codegen::CodeBuffer;
	...
	/// but only advance `len` by `n` bytes. This avoids a `memset` function
	/// call.
	///
...
impl From<CodeBuffer> for String {
	#[inline]
	fn from(code:CodeBuffer) -> Self { code.into_string() }
}
