...
///
/// Most lines of source code will not contain Unicode chars, so optimize
/// storage for this common case.
///
/// Each line is represented by a `Line`.
/// Where a line is entirely ASCII, translating byte offset to UTF-16 column is
/// simple, given the byte offset of start of line. A column lookup table isn't
/// needed for that line. In this case, `Line::column_offsets_id` is `None`.
/// For rare lines which do contain Unicode chars, we store column offsets in a
/// `ColumnOffsets` which is stored in a separate `IndexVec`.
/// `Line::column_offsets_id` contains index for that line's `ColumnOffsets`.
/// Storing column offset info which is rarely used in a separate structure
/// keeps `Line` as small as possible.
...
impl SourcemapBuilder {
	...
	pub fn add_source_mapping_for_name(&mut self, output:&[u8], span:Span, name:&str) {
		...
		// If it hasn't change, name should be `None` to reduce `SourceMap`
		// size.
		let token_name =
	...
	fn update_generated_line_and_column(&mut self, output:&[u8]) {
		...
		self.generated_column += if last_line_is_ascii {
			// `iter` is now exhausted, so `iter.as_slice().as_ptr()` is pointer
			// to end of `output`
			...
			// TODO: It'd be better if could use `from_utf8_unchecked` here, but
			// we'd need to make this function unsafe and caller guarantees
			// `output` contains a valid UTF-8 string
			let last_line = std::str::from_utf8(&remaining[line_byte_offset..]).unwrap();
			// Mozilla's "source-map" library counts columns using UTF-16 code
			// units
	...
	fn generate_line_offset_tables(content:&str) -> LineOffsetTables {
		...
		// For each line, start by assuming line will be entirely ASCII, and
		// read byte-by-byte. If line is all ASCII, UTF-8 columns and UTF-16
		// columns are the same, so no need to create a `columns` Vec. This is
		// the fast path for common case. If a Unicode character found, read
		// rest of line char-by-char, populating `columns` Vec.
		// At end of line, go back to top of outer loop, and again assume ASCII
		// for next line.
		...
		'lines: loop {
			...
			for (byte_offset_from_line_start, b) in remaining.iter().enumerate() {
				...
				match b {
					...
					_ => {
						// Unicode char found.
						// Set `column_offsets_id` for line and create `columns`
						// Vec.
						...
						// `chunk_byte_offset` in this loop is byte offset from
						// start of this 1st Unicode char.
						...
						for (chunk_byte_offset, ch) in remaining.char_indices() {
							...
							match ch {
								...
								_ => {
									...
									// columns using UTF-16 code units
							...
							// Line break found.
							// `chunk_byte_offset` is now the offset of *end* of
							// the line break.
				...
				// Line break found.
				// `byte_offset_from_line_start` is now the length of line
	...
	fn assert_mapping(source:&str, mappings:&[(u32, u32, u32)]) {
		...
		for (position, expected_line, expected_col) in mappings.iter().copied() {
			...
			assert_eq!(
				...
				"Incorrect mapping for '{source}' - position {position} = line {line}, column \
				 {col}"
			);
		}
	}
		...
		fn create_mappings(source:&str, line:u32, column:u32) {
			...
			for (i, _ch) in source.char_indices() {
				...
				assert!(
					...
					"Incorrect generated mapping for '{source}' ({:?}) starting at {i} - line {}, \
					 column {}",
					source.as_bytes(),
