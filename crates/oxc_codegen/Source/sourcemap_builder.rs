...
impl SourcemapBuilder {
	pub fn with_name_and_source(&mut self, name: &str, source: &str) {
		...
		self.source_id =
			self.sourcemap_builder.set_source_and_content(name, source);
	...
	pub fn add_source_mapping_for_name(
		...
		let token_name = if original_name == Some(name) {
			None
		} else {
			original_name.map(Into::into)
		};
	...
	pub fn add_source_mapping(
		...
		let (original_line, original_column) =
			self.search_original_line_and_column(position);
	...
	fn search_original_line_and_column(&mut self, position: u32) -> (u32, u32) {
		let result = self.line_offset_tables.lines.partition_point(|table| {
			table.byte_offset_to_start_of_line <= position
		});
		...
		if let Some(column_offsets_id) = line.column_offsets_id {
			let column_offsets =
				&self.line_offset_tables.column_offsets[column_offsets_id];
			if original_column >= column_offsets.byte_offset_to_first {
				original_column = column_offsets.columns[(original_column
					- column_offsets.byte_offset_to_first)
					as usize];
			}
		}
	...
	fn update_generated_line_and_column(&mut self, output: &[u8]) {
		...
		self.generated_column += if last_line_is_ascii {
			...
			let line_byte_offset =
				line_start_ptr as usize - remaining.as_ptr() as usize;
			...
			let last_line =
				std::str::from_utf8(&remaining[line_byte_offset..]).unwrap();
			...
			for (byte_offset_from_line_start, b) in remaining.iter().enumerate()
			{
				...
				match b {
					...
					b'\r' => {
						byte_offset_from_line_start += 1;
						// Handle Windows-specific "\r\n" newlines
						if remaining.get(byte_offset_from_line_start as usize)
							== Some(&b'\n')
						{
							byte_offset_from_line_start += 1;
						}
					...
					_ => {
						...
						line.column_offsets_id = Some(
							ColumnOffsetsId::from_usize(column_offsets.len()),
						);
						...
						for (chunk_byte_offset, ch) in remaining.char_indices()
						{
							...
							match ch {
								'\r' => {
									...
									if remaining
										.as_bytes()
										.get(chunk_byte_offset as usize)
										== Some(&b'\n')
									{
										chunk_byte_offset += 1;
