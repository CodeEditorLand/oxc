...
impl SourcemapBuilder {
	...
	fn update_generated_line_and_column(&mut self, output:&[u8]) {
		...
		self.generated_column += if last_line_is_ascii {
			// `iter` is now exhausted, so `iter.as_slice().as_ptr()` is pointer to end of
			// `output`
			...
			// TODO: It'd be better if could use `from_utf8_unchecked` here, but we'd need
			// to make this function unsafe and caller guarantees `output` contains a
			// valid UTF-8 string
	...
	fn generate_line_offset_tables(content:&str) -> LineOffsetTables {
		...
		// For each line, start by assuming line will be entirely ASCII, and read
		// byte-by-byte. If line is all ASCII, UTF-8 columns and UTF-16 columns are
		// the same, so no need to create a `columns` Vec. This is the fast path for
		// common case. If a Unicode character found, read rest of line char-by-char,
		// populating `columns` Vec. At end of line, go back to top of outer loop, and
		// again assume ASCII for next line.
		...
		'lines: loop {
			...
			for (byte_offset_from_line_start, b) in remaining.iter().enumerate() {
					...
					_ => {
						...
						for (chunk_byte_offset, ch) in remaining.char_indices() {
							...
							match ch {
								...
								#[allow(clippy::cast_possible_truncation)]
								_ => {
									// Mozilla's "source-map" library counts columns using UTF-16
									// code units
				...
				// Line break found.
				// `byte_offset_from_line_start` is now the length of line *including* line
				// break.
				line_byte_offset += byte_offset_from_line_start;
