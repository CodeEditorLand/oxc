...
use crate::{Codegen, Context, Operator, gen::GenExpr};
...
impl BinaryishOperator {
	fn is_binary(self) -> bool { matches!(self, Self::Binary(_)) }
}
...
impl<'a> BinaryExpressionVisitor<'a> {
	...
	pub fn check_and_prepare(&mut self, p:&mut Codegen) -> bool {
		...
		// We don't need to print parentheses if both sides use the same logical
		// operator For example: `(a     &&     b)         && c` should be printed as
		// `a && b && c`                      ^^  e.operator()  ^^ self.operator
