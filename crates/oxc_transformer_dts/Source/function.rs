...
impl<'a> TransformerDts<'a> {
	...
	pub fn transform_formal_parameter(
		...
		if is_assignment_pattern || pattern.type_annotation.is_none() {
			let is_next_param_optional = next_param
				.map_or(true, |next_param| next_param.pattern.optional);
			...
			pattern = self.ctx.ast.binding_pattern(
				...
				pattern.optional
					|| (is_next_param_optional && is_assignment_pattern),
			);
		}
	...
	pub fn transform_formal_parameters(
		...
		if params.kind.is_signature()
			|| (params.rest.is_none() && params.items.is_empty())
		{
			return self.ctx.ast.alloc(self.ctx.ast.copy(params));
		}

		let items = self.ctx.ast.new_vec_from_iter(
			params.items.iter().enumerate().map(|(index, item)| {
				self.transform_formal_parameter(
					item,
					params.items.get(index + 1),
				)
			}),
		);
