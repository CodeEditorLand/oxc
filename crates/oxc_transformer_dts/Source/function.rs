...
impl<'a> TransformerDts<'a> {
	...
	pub fn transform_formal_parameter(
		...
		if is_assignment_pattern || pattern.type_annotation.is_none() {
			...
			let type_annotation = pattern
				...
				.map(|ts_type| {
					// jf next param is not optional and current param is
					// assignment pattern we need to add undefined to it's
					// type
					if !is_next_param_optional {
						if matches!(ts_type, TSType::TSTypeReference(_)) {
							self.ctx.error(
								OxcDiagnostic::error(
									"Declaration emit for this parameter requires implicitly \
									 adding undefined to it's type. This is not supported with \
							...
							return self.ctx.ast.ts_type_annotation(
								SPAN,
								self.ctx.ast.ts_union_type(
									SPAN,
									self.ctx.ast.new_vec_from_iter([
										ts_type,
										self.ctx.ast.ts_undefined_keyword(SPAN),
									]),
								),
							);
						}
	...
	pub fn transform_formal_parameters(
		...
		let items =
			self.ctx
				.ast
				.new_vec_from_iter(params.items.iter().enumerate().map(|(index, item)| {
