...
impl<'a> ScopeTree<'a> {
	...
	pub fn references_len(&self) -> usize {
		self.value_references.last().unwrap().len()
			+ self.type_references.last().unwrap().len()
	}
	...
	fn resolve_references(&mut self) {
		let current_value_bindings =
			self.value_bindings.pop().unwrap_or_default();
		let current_value_references =
			self.value_references.pop().unwrap_or_default();
		self.type_references.last_mut().unwrap().extend(
			current_value_references
				.difference(&current_value_bindings)
				.cloned(),
		);

		let current_type_bindings =
			self.type_bindings.pop().unwrap_or_default();
		let current_type_references =
			self.type_references.pop().unwrap_or_default();
		self.type_references.last_mut().unwrap().extend(
			current_type_references.difference(&current_type_bindings).cloned(),
		);
	}
}
