...
impl<'a> TransformerDts<'a> {
	...
	pub fn transform_object_expression_to_ts_type(
		...
		let members =
			self.ctx.ast.new_vec_from_iter(expr.properties.iter().filter_map(|property| {
				match property {
					ObjectPropertyKind::ObjectProperty(object) => {
						if object.computed {
							self.ctx.error(
								OxcDiagnostic::error(
									"Computed property names on class or object literals cannot \
					...
					ObjectPropertyKind::SpreadProperty(spread) => {
						self.ctx.error(
							OxcDiagnostic::error(
								"Objects that contain spread assignments can't be inferred with \
	...
	pub fn transform_array_expression_to_ts_type(
			...
			self.ctx.ast.new_vec_from_iter(expr.elements.iter().filter_map(|element| {
				match element {
					ArrayExpressionElement::SpreadElement(spread) => {
						self.ctx.error(
							OxcDiagnostic::error(
								"Arrays with spread elements can't inferred with \
								 --isolatedDeclarations.",
							)
							.with_label(spread.span),
						);
		...
		if is_const {
			self.ctx
				.ast
				.ts_type_operator_type(SPAN, TSTypeOperatorOperator::Readonly, ts_type)
		} else {
			ts_type
		}
	}
	...
	pub fn transform_expression_to_ts_type(&self, expr:&Expression<'a>) -> TSType<'a> {
		match expr {
			...
			Expression::BigintLiteral(lit) => {
				self.ctx
					.ast
					.ts_literal_type(SPAN, TSLiteral::BigintLiteral(self.ctx.ast.copy(lit)))
			},
			Expression::StringLiteral(lit) => {
				self.ctx
					.ast
					.ts_literal_type(SPAN, TSLiteral::StringLiteral(self.ctx.ast.copy(lit)))
			},
