...
/// `T::drop` will NOT be called on the `Box`'s contents when the `Box` is
...
impl<T:?Sized> Box<'_, T> {
	...
	/// This requirement is met if the pointer was obtained from other data in
	/// the arena.
	///
	/// Data pointed to by `ptr` must *only* be used for this `Box`. i.e. it
	/// must be unique, with no other aliases. You must not, for example,
	/// create 2 `Box`es from the same pointer.
	///
	/// `ptr` must have been created from a `*mut T` or `&mut T` (not a `*const
	/// T` / `&T`).
	#[inline]
	pub(crate) const unsafe fn from_non_null(ptr:NonNull<T>) -> Self { Self(ptr, PhantomData) }

	/// Consume a [`Box`] and return a [`NonNull`] pointer to its contents.
	#[inline]
	#[expect(clippy::needless_pass_by_value)]
	pub fn into_non_null(boxed:Self) -> NonNull<T> { boxed.0 }
}
