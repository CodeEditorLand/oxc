...
/// This trait works similarly to the standard library [`From`] trait, It comes
/// with a similar implementation containing blanket implementation for
/// [`IntoIn`], reflective implementation and a bunch of primitive conversions
/// from Rust types to their arena equivalent.
...
/// It is similar to [`FromIn`] is reflective, A [`FromIn`] implementation also
/// implicitly implements [`IntoIn`] for the opposite type.
pub trait IntoIn<'a, T>: Sized {
	/// Converts this type into the (usually inferred) input type within the
	/// given `allocator`.
...
impl<'a, T> FromIn<'a, T> for T {
	#[inline(always)]
	fn from_in(t:T, _:&'a Allocator) -> T { t }
}
...
{
	#[inline]
	fn into_in(self, allocator:&'a Allocator) -> U { U::from_in(self, allocator) }
}
...
impl<'a, T> FromIn<'a, T> for Box<'a, T> {
	#[inline(always)]
	fn from_in(value:T, allocator:&'a Allocator) -> Self { Box::new_in(value, allocator) }
}
