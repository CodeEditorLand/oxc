...
/// `T::drop` will NOT be called on the `Vec`'s contents when the `Vec` is
/// dropped. If `T` owns memory outside of the arena, this will be a memory
/// leak.
///
/// Note: This is not a soundness issue, as Rust does not support relying on
/// `drop` being called to guarantee soundness.
#[derive(PartialEq, Eq)]
...
impl<'alloc, T> Vec<'alloc, T> {
	...
	/// Create a new [`Vec`] from a fixed-size array, allocated in the given
	/// `allocator`.
	///
	/// This is preferable to `from_iter_in` where source is an array, as size
	/// is statically known, and compiler is more likely to construct the
	/// values directly in arena, rather than constructing on stack and then
	/// copying to arena.
	///
	/// # Examples
	///
	...
	pub fn from_array_in<const N: usize>(array:[T; N], allocator:&'alloc Allocator) -> Self {
		...
		// Allocated size cannot be larger than `isize::MAX`, or `Box::new_in` would
		// have failed.
	...
	/// The excess memory will be leaked in the arena (i.e. not reused by
	/// another allocation).
	///
	/// # Examples
	///
	...
	pub fn into_boxed_slice(self) -> Box<'alloc, [T]> {
		...
		// `allocator_api2::vec::Vec::leak` consumes the inner `Vec` without dropping
		// it. Lifetime of returned `Box<'alloc, [T]>` is same as lifetime of consumed
		// `Vec<'alloc, T>`, so data in the `Box` must be valid for its lifetime.
		// `Vec` uniquely owned the data, and we have consumed the `Vec`, so the new
		// `Box` has unique ownership of the data (no aliasing).
...
mod test {
	...
	fn vec_to_boxed_slice() {
		...
		let b = v.into_boxed_slice();
		// Check return value is an `oxc_allocator::Box`, not an
