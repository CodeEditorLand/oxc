...
impl Address {
	...
	///
	/// If the AST node is in a `Box`, the address is guaranteed to be a unique
	/// identifier for the duration of the arena's existence.
	/// If the node is in a `Vec`, then the `Address` may not remain accurate if
	/// the `Vec` is resized or has elements added or removed before this node.
	#[inline]
	pub fn from_ptr<T>(p:*const T) -> Self { Self(p as usize) }
}
...
impl<T> GetAddress for Box<'_, T> {
	...
	/// so this address acts as a unique identifier for the duration of the
	/// arena's existence.
	#[inline]
	fn address(&self) -> Address { Address::from_ptr(ptr::addr_of!(**self)) }
}
