...
/// The type which includes all atom nodes that Quantifier node can have as
/// children.
#[derive(Debug)]
...
/// The character class used in legacy (neither `u` nor `v` flag) and Unicode
/// mode (`u` flag). This character class is guaranteed to **not** contain
/// strings. In Unicode sets mode (`v` flag), {@link UnicodeSetsCharacterClass}
/// is used.
#[derive(Debug)]
...
/// StringsUnicodePropertyCharacterSet is Unicode property escape with property
/// of strings.
#[derive(Debug)]
...
/// StringAlternative is only used for `\q{alt}`({@link
