...
use oxc_regular_expression::{
	Parser as RegExpParser, ParserOptions as RegExpParserOptions,
};
	...
	fn enter_node(&mut self, kind: AstKind<'a>) {
		...
		match kind {
			AstKind::RegExpLiteral(re) => {
				println!(
					"🍀 {}",
					re.span.source_text(self.source_text.as_ref())
				);
					...
					.is_some() =>
			{
				...
				let (pattern, pattern_span) = match new_expr.arguments.first() {
					Some(ast::Argument::StringLiteral(sl)) => {
						(&sl.value, &sl.span)
					},
				...
				if let Err(error) = parsed {
					let error =
						error.with_source_code(Arc::clone(&self.source_text));
					println!("{error:?}");
					return;
				}
				println!("{parsed:#?}");
