...
impl<'a> Source<'a> {
	...
	pub(super) fn new(mut source_text:&'a str, _unique:UniquePromise) -> Self {
		...
		// source is very short, and has very low memory address (e.g. 16). If that's
		// the case, `end_for_batch_search_addr` will be 0, so a test whether any
		// non-null pointer is past end will always test positive, and disable batch
		// search.
	...
	pub(super) fn whole(&self) -> &'a str {
		...
		// cannot be after `end`. `start` and `end` are by definition on UTF-8 char
		// boundaries.
		unsafe {
	...
	pub(super) fn remaining(&self) -> &'a str {
		...
		// char boundary. `end` is pointer to end of original `&str`, so be definition
	...
	pub(super) fn set_position(&mut self, pos:SourcePosition) {
		...
		// created from this `Source`. `SourcePosition`s can only be created from a
		// `Source`. `Source::new` takes a `UniquePromise`, which guarantees that it's
		// the only `Source` in existence on this thread. `Source` is not `Sync` or
		// `Send`, so no possibility another `Source` originated on another thread can
		// "jump" onto this one. This is sufficient to guarantee that any
		// `SourcePosition` that parser/lexer holds must be from this `Source`.
	...
	pub(super) unsafe fn str_between_positions_unchecked(
		...
		// `Source` in existence on this thread. `Source` is not `Sync` or `Send`, so
		// no possibility another `Source` originated on another thread can "jump"
		// onto this one. This is sufficient to guarantee that any `SourcePosition`
		// that parser/lexer holds must be from this `Source`, therefore `start.ptr`
		// and `end.ptr` must both be within the same allocation, and derived from the
		// same original pointer. Invariants of `Source` and `SourcePosition` types
		// guarantee that both are positioned on UTF-8 character boundaries. So
		// slicing source text between these 2 points will always yield a valid UTF-8
		// string.
	...
	pub(super) fn back(&mut self, n:usize) {
		...
		// `ptr`, so `new_ptr` cannot be outside of allocation of original `&str`
		let new_pos = unsafe { self.position().sub(n) };

		// Enforce invariant that `ptr` must be positioned on a UTF-8 character
		// boundary. SAFETY: `new_ptr` is in bounds of original `&str`, and `n > 0`
		// assertion ensures not at the end, so valid to read a byte.
		// `Source`'s invariants guarantee that `self.start` - `self.end` contains
		// allocated memory. `Source::new` takes an immutable ref `&str`, guaranteeing
		// that the memory `new_ptr` addresses cannot be aliased by a `&mut` ref as
	...
	pub(super) unsafe fn next_byte_unchecked(&mut self) -> u8 {
		...
		// `end`. Therefore always valid to read a byte from `ptr`, and incrementing
		// `ptr` cannot result in `ptr > end`.
	...
	pub(super) fn peek_char(&self) -> Option<char> {
		...
		let mut chars = self.remaining().chars();
		// SAFETY: We know that there's a byte to be consumed, so `chars.next()` must
		// return `Some(_)`. Could just return `chars.next()` here, but making it
		// clear to compiler that this branch always returns `Some(_)` may help it
		// optimize the caller. Compiler seems to have difficulty "seeing into"
		// `Chars` iterator and making deductions.
		let c = unsafe { chars.next().unwrap_unchecked() };
	...
	pub(super) unsafe fn peek_byte_unchecked(&self) -> u8 {
		...
		// `Source`'s invariants guarantee that `self.start` - `self.end` contains
		// allocated memory. `Source::new` takes an immutable ref `&str`, guaranteeing
		// that the memory `self.ptr` addresses cannot be aliased by a `&mut` ref as
		// long as `Source` exists.
...
impl SourcePosition<'_> {
	...
	pub(super) unsafe fn read(self) -> u8 {
		...
		// guarantees no mutable references to the same memory can exist, as that
		// would violate Rust's aliasing rules. Pointer is "dereferenceable" by
		// definition as a `u8` is 1 byte and cannot span multiple objects.
		// Alignment is not relevant as `u8` is aligned on 1 (i.e. no alignment
		// requirements).
	...
	pub(super) unsafe fn read2(self) -> [u8; 2] {
		...
		// text. `Source` is created from a valid `&str`, so points to allocated,
		// initialized memory. `Source` conceptually holds the source text `&str`,
		// which guarantees no mutable references to the same memory can exist, as
		// that would violate Rust's aliasing rules. Pointer is "dereferenceable" by
		// definition as a `u8` is 1 byte and cannot span multiple objects.
		// Alignment is not relevant as `u8` is aligned on 1 (i.e. no alignment
		// requirements).
		debug_assert!(!self.ptr.is_null());
