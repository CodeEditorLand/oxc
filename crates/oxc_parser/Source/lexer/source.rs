...
impl<'a> Source<'a> {
	...
	pub(super) fn new(mut source_text:&'a str, _unique:UniquePromise) -> Self {
		...
		// which will fail to parse. `Parser::parse` will convert error to
		...
		// dereferenced, only checked for direct pointer equality with `ptr`
		// to check if at end of file.
		let end = unsafe { start.add(source_text.len()) };

		// `saturating_sub` not `wrapping_sub` so that value doesn't wrap around
		// if source is very short, and has very low memory address (e.g. 16).
		// If that's the case, `end_for_batch_search_addr` will be 0, so a
		// test whether any non-null pointer is past end will always test
		// positive, and disable batch search.
	...
	pub(super) fn whole(&self) -> &'a str {
		...
		// `start` cannot be after `end`. `start` and `end` are by definition
	...
	pub(super) fn remaining(&self) -> &'a str {
		...
		// UTF-8 char boundary. `end` is pointer to end of original `&str`, so
		// be definition a UTF-8 char boundary.
		unsafe {
			self.str_between_positions_unchecked(
	...
	#[inline]
	pub(super) fn is_eof(&self) -> bool { self.ptr == self.end }

	/// Get end address.
	#[inline]
	pub(super) fn end_addr(&self) -> usize { self.end as usize }

	/// Get last memory address at which a batch of
	/// `Lexer::search::SEARCH_BATCH_SIZE` bytes can be read without going out
	/// of bounds.
	#[inline]
	pub(super) fn end_for_batch_search_addr(&self) -> usize { self.end_for_batch_search_addr }
	...
	pub(super) fn position(&self) -> SourcePosition<'a> {
		...
		// is always valid, if caller has upheld safety conditions of other
		// unsafe methods of this type.
	...
	pub(super) fn set_position(&mut self, pos:SourcePosition) {
		...
		// as it's created from this `Source`. `SourcePosition`s can only be
		// created from a `Source`. `Source::new` takes a `UniquePromise`,
		// which guarantees that it's the only `Source` in existence on this
		// thread. `Source` is not `Sync` or `Send`, so no possibility another
		// `Source` originated on another thread can "jump" onto this one.
		// This is sufficient to guarantee that any `SourcePosition` that
		// parser/lexer holds must be from this `Source`.
	...
	#[inline]
	pub(super) fn advance_to_end(&mut self) { self.ptr = self.end; }

	/// Advance `Source`'s cursor by one byte if it is equal to the given ASCII
	/// value.
	///
	/// # SAFETY
	///
	...
	pub(super) unsafe fn advance_if_ascii_eq(&mut self, ascii_byte:u8) -> bool {
		...
		if matched {
			// SAFETY: next byte exists and is a valid ASCII char (and thus
			// UTF-8 char boundary).
	...
	pub(super) unsafe fn str_from_pos_to_current_unchecked(&self, pos:SourcePosition) -> &'a str {
		...
		// `Source`. `self.ptr` is always a valid `SourcePosition` due to
		// invariants of `Source`.
	...
	pub(super) unsafe fn str_between_positions_unchecked(
		...
		// only `Source` in existence on this thread. `Source` is not `Sync`
		// or `Send`, so no possibility another `Source` originated on another
		// thread can "jump" onto this one. This is sufficient to guarantee
		// that any `SourcePosition` that parser/lexer holds must be from this
		// `Source`, therefore `start.ptr` and `end.ptr` must both be within the
		// same allocation, and derived from the same original pointer.
		// Invariants of `Source` and `SourcePosition` types guarantee that both
		// are positioned on UTF-8 character boundaries. So slicing source
		// text between these 2 points will always yield a valid UTF-8 string.
	...
	#[inline]
	pub(super) fn offset(&self) -> u32 { self.offset_of(self.position()) }
	...
	pub(super) fn back(&mut self, n:usize) {
		...
		// below. Without this check, calling `back(0)` on an empty `Source`
		// would cause reading out of bounds.
		...
		// `start` and `ptr`, so `new_ptr` cannot be outside of allocation of
		// original `&str`
		...
		// contains allocated memory. `Source::new` takes an immutable ref
		// `&str`, guaranteeing that the memory `new_ptr` addresses cannot be
		// aliased by a `&mut` ref as long as `Source` exists.
	...
	pub(super) fn next_char(&mut self) -> Option<char> {
		...
		// `self.ptr` to match `Chars` iterator's updated pointer afterwards.
		// `Chars` iterator upholds same invariants as `Source`, so its pointer
		// is guaranteed to be valid as `self.ptr`.
	...
	pub(super) fn next_2_chars(&mut self) -> Option<[char; 2]> {
		...
		if byte1.is_ascii() && byte2.is_ascii() {
			// SAFETY: We just checked that there are at least 2 bytes
			// remaining, and next 2 bytes are ASCII, so advancing by 2 bytes
			// must put `ptr` in bounds and on a UTF-8 character boundary
		...
		// Create a `Chars` iterator, get next 2 chars from it, and then update
		// `self.ptr` to match `Chars` iterator's updated pointer afterwards.
		// `Chars` iterator upholds same invariants as `Source`, so its pointer
		// is guaranteed to be valid as `self.ptr`.
		let mut chars = self.remaining().chars();
	...
	/// character sequence, which would violate one of `Source`'s invariants.
	///
	...
	/// by `Source::next_char`, which are safe methods, and compiler will often
	/// reduce to equally efficient code, if calling code tests the byte
	/// returned. e.g.:
	///
	/// ```
	/// // Consume a space
	...
	pub(super) unsafe fn next_byte_unchecked(&mut self) -> u8 {
		...
		// after `end`. Therefore always valid to read a byte from `ptr`, and
	...
	pub(super) fn peek_char(&self) -> Option<char> {
		...
		let mut chars = self.remaining().chars();
		// SAFETY: We know that there's a byte to be consumed, so `chars.next()`
		// must return `Some(_)`. Could just return `chars.next()` here, but
		// making it clear to compiler that this branch always returns
		// `Some(_)` may help it optimize the caller. Compiler seems to have
		// difficulty "seeing into" `Chars` iterator and making deductions.
		let c = unsafe { chars.next().unwrap_unchecked() };
		Some(c)
	}
	...
	pub(super) fn peek_2_bytes(&self) -> Option<[u8; 2]> {
		...
		if (self.end as usize) - (self.ptr as usize) >= 2 {
			// SAFETY: The check above ensures that there are at least 2 bytes
			// to read from `self.ptr` without reading past `self.end`
	...
	pub(super) unsafe fn peek_byte_unchecked(&self) -> u8 {
		...
		// file). Methods of this type provide no way to allow `ptr` to be
		// before `start`. `Source`'s invariants guarantee that `self.start` -
		// `self.end` contains allocated memory. `Source::new` takes an
		// immutable ref `&str`, guaranteeing that the memory `self.ptr`
		// addresses cannot be aliased by a `&mut` ref as long as `Source`
		// exists.
...
impl<'a> SourcePosition<'a> {
	...
	#[inline]
	pub(super) unsafe fn new(ptr:*const u8) -> Self { Self { ptr, _marker:PhantomData } }

	/// Get memory address of `SourcePosition` as a `usize`.
	#[inline]
	pub(super) fn addr(self) -> usize { self.ptr as usize }
	...
	/// from. NB: It is legal to use `add` to create a `SourcePosition` which
	/// is *on* the end of `Source`, just not past it.
	#[inline]
	pub(super) unsafe fn add(self, n:usize) -> Self { Self::new(self.ptr.add(n)) }
	...
	/// make it before the start of `Source` this `SourcePosition` was created
	/// from.
	#[inline]
	pub(super) unsafe fn sub(self, n:usize) -> Self { Self::new(self.ptr.sub(n)) }
	...
	pub(super) unsafe fn read(self) -> u8 {
		...
		// initialized memory. `Source` conceptually holds the source text
		// `&str`, which guarantees no mutable references to the same memory
		// can exist, as that would violate Rust's aliasing rules. Pointer is
		// "dereferenceable" by definition as a `u8` is 1 byte and cannot span
		// multiple objects. Alignment is not relevant as `u8` is aligned on 1
		// (i.e. no alignment requirements).
	...
	/// of source text. i.e. if source length is 10, `self` must be on position
	/// 8 max.
	#[inline]
	pub(super) unsafe fn read2(self) -> [u8; 2] {
		// SAFETY:
		// Caller guarantees `self` is not at no later than 2 bytes before end
		// of source text. `Source` is created from a valid `&str`, so points
		// to allocated, initialized memory. `Source` conceptually holds the
		// source text `&str`, which guarantees no mutable references
		// to the same memory can exist, as that would violate Rust's aliasing
		// rules. Pointer is "dereferenceable" by definition as a `u8` is 1
		// byte and cannot span multiple objects. Alignment is not relevant as
		// `u8` is aligned on 1 (i.e. no alignment requirements).
		debug_assert!(!self.ptr.is_null());
