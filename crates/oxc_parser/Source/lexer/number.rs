	...
	match kind {
		...
		Kind::Octal => {
			...
			let s = if second_byte == b'o' || second_byte == b'O' {
				// SAFETY: We just checked that 2nd byte is ASCII, so slicing
				// off 2 bytes must be in bounds and on a UTF-8 character
				// boundary.
...
/// cycles, which doesn't include addition. Some platforms support mul + add in
/// a single instruction, but many others do not.
///
