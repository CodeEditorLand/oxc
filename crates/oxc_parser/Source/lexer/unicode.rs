...
impl<'a> Lexer<'a> {
	...
	fn string_unicode_escape_sequence(
		...
		// `"\uD83D\uDE00" === 😀` values are interpreted as is if they fall
		// out of range
		match value {
	...
	fn hex_digit(&mut self) -> Option<u32> {
		// Reduce instructions and remove 1 branch by comparing against `A-F`
		// and `a-f` simultaneously https://godbolt.org/z/9caMMzvP3
		let value = if let Some(b) = self.peek_byte() {
			if b.is_ascii_digit() {
				b - b'0'
			} else {
				// Match `A-F` or `a-f`. `b | 32` converts uppercase letters to
				// lowercase, but leaves lowercase as they are
				let lower_case = b | 32;
		...
		// Because of `b | 32` above, compiler cannot deduce that next byte is
		// definitely ASCII so `next_byte_unchecked` is necessary to produce
		// compact assembly, rather than `consume_char`. SAFETY: This code is
		// only reachable if there is a byte remaining, and it's ASCII.
		// Therefore it's safe to consume that byte, and will leave position on
