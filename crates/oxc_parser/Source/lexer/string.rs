			...
			while let Some(b) = $lexer.peek_byte() {
				match b {
					b if !$table.matches(b) => {
						...
						// violates `Source`'s invariant temporarily, but the guarantees of
						// `SafeByteMatchTable` mean `!table.matches(b)` on this branch
						// prevents exiting this loop until `source` is positioned on a UTF-8
