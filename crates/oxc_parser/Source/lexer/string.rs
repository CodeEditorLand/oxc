...
macro_rules! handle_string_literal {
	($lexer:ident, $delimiter:expr, $table:ident) => {{
		...
		if $lexer.context == LexerContext::JsxAttributeValue {
			// SAFETY: Caller guarantees `$delimiter` is ASCII, and next char is
			// ASCII
			return $lexer.read_jsx_string_literal($delimiter);
		}

		// Skip opening quote.
		// SAFETY: Caller guarantees next byte is ASCII, so safe to advance past
		// it.
		...
		match next_byte {
			$delimiter => {
				// SAFETY: Macro user guarantees delimiter is ASCII, so
				// consuming it cannot move `lexer.source` off a UTF-8
				// character boundary.
			...
			while let Some(b) = $lexer.peek_byte() {
				match b {
					b if !$table.matches(b) => {
						...
						// continuation byte, which violates `Source`'s
						// invariant temporarily, but the guarantees of
						// `SafeByteMatchTable` mean `!table.matches(b)`
						// on this branch prevents exiting this loop until
					...
					b if b == $delimiter => {
						...
						// consuming it cannot move `lexer.source` off a
...
impl<'a> Lexer<'a> {
	...
	/// size small Strings without escaped values can be retrieved as is from
	/// the token span
