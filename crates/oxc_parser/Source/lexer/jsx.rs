	...
	fn read_jsx_child(&mut self) -> Kind {
		match self.peek_byte() {
			...
			Some(_) => {
				...
				// Did you mean `{'>'}` or `&gt;`?". Where as the Babel
				// compiler does not raise any errors. The following check
				// omits `>` and `}` so that more Babel tests can be passed.
				let len = memchr2(b'{', b'<', self.remaining().as_bytes());
				if let Some(len) = len {
					// SAFETY: `memchr2` guarantees `len` will be offset from
					// current position of a `{` or `<` byte. So must be a
					// valid UTF-8 boundary, and within bounds of source.
