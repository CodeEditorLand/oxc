...
/// # Examples
/// ```
/// use crate::lexer::search::{byte_match_table, ByteMatchTable};
///
...
impl ByteMatchTable {
	...
	/// provided, which makes using the macro unsafe for `ByteMatchTable`, but
	/// safe for `SafeByteMatchTable`.
	...
	#[inline]
	pub const fn matches(&self, b:u8) -> bool { self.0[b as usize] }
}
...
/// # Examples
/// ```
/// use crate::lexer::search::{safe_byte_match_table, SafeByteMatchTable};
///
...
macro_rules! byte_search {
	...
	(lexer: $lexer:ident,table: $table:ident,handle_eof: $eof_handler:expr,) => {{
	...
	(
		lexer:
		$lexer:ident,table:
		$table:ident,continue_if:
		($byte:ident, $pos:ident)
		$should_continue:expr,handle_eof:
		$eof_handler:expr,
	) => {{
		let start = $lexer.source.position();
		byte_search! {
	...
	(
		lexer: $lexer:ident,table: $table:ident,start: $start:ident,handle_eof: $eof_handler:expr,
	) => {
		byte_search! {
			lexer: $lexer,
			table: $table,
	...
	(
		...
		// `lexer.source` will be positioned on a UTF-8 character boundary
		// before `handle_match` is called. If `$table` is a `ByteMatchTable`,
		// no such guarantee is given, but call to `$table.use_table()` here
		// makes using this macro unsafe, and it's the user's responsibility
		// to uphold this invariant. Therefore we can assume this is taken
		// care of one way or another, and wrap the calls to unsafe functions
		// in this function with `unsafe {}`.
		$table.use_table();
		...
		'outer: loop {
			let $byte = if $pos.addr() <= $lexer.source.end_for_batch_search_addr() {
				...
				// first turn. Only using `loop {}` so that can use
				// `break 'inner` to get out of it. This allows complex
				// logic of `$should_continue` and `$match_handler` to be
				// outside the `for` loop, keeping it as minimal as
				// possible, to encourage compiler to unroll it.
				//
				// SAFETY:
				...
				'inner: loop {
					for _i in 0..crate::lexer::search::SEARCH_BATCH_SIZE {
						// SAFETY: `$pos` cannot go out of bounds in this
						// loop (see above)
						...
						// SAFETY: `$pos` cannot go out of bounds in this
						// loop (see above). Also see above about
						// UTF-8 character boundaries invariant.
				...
				// Not enough bytes remaining for a batch. Process
				// byte-by-byte. Same as above, `'inner: loop {}` is
				// not a real loop here - always exits on first turn.
				...
				'inner: loop {
					while $pos.addr() < end_addr {
						...
						// SAFETY: `pos` is not at end of source, so safe to
						// advance 1 byte. See above about UTF-8
			...
			if $should_continue {
				...
				// byte. See above about UTF-8 character boundaries
				// invariant.
				$pos = unsafe { $pos.add(1) };
				continue;
			}
			...
			// unmatched bytes. SAFETY: See above about UTF-8 character
