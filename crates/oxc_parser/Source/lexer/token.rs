...
pub struct Token {
	...
	// dword + 1 x qword and `Token::default()` is 1 x xmmword store, rather
	// than 1 x dword + 1 x qword.
	_padding2:u32,
}
...
impl Token {
	pub(super) fn new_on_new_line() -> Self { Self { is_on_new_line:true, ..Self::default() } }

	pub fn span(&self) -> Span { Span::new(self.start, self.end) }

	pub fn escaped(&self) -> bool { self.escaped }
