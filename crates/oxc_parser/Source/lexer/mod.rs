...
impl<'a> Lexer<'a> {
	...
	pub fn lookahead(&mut self, n:u8) -> Token {
		...
		// of this function is if we were at very start of file, before any tokens
		// have been read, when `token.is_on_new_line` is `true`. But `lookahead`
		// isn't called before the first token is read, so that's not possible. So no
		// need to restore `self.token` here. It's already in same state as it was at
