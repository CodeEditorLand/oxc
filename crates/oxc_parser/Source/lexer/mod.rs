...
impl<'a> Lexer<'a> {
	...
	pub fn remaining(&self) -> &'a str { self.source.remaining() }
	...
	pub fn lookahead(&mut self, n:u8) -> Token {
		...
		for _i in self.lookahead.len()..n {
			...
			self.lookahead
				.push_back(Lookahead { position:self.source.position(), token:peeked });
		}

		// Call to `finish_next` in loop above leaves `self.token =
		// Token::default()`. Only circumstance in which `self.token` wouldn't
		// have been default at start of this function is if we were at very
		// start of file, before any tokens have been read, when
		// `token.is_on_new_line` is `true`. But `lookahead` isn't called before
		// the first token is read, so that's not possible. So no need to
		// restore `self.token` here. It's already in same state as it was at
	...
	/// Set context
	pub fn set_context(&mut self, context:LexerContext) { self.context = context; }
	...
	fn offset(&self) -> u32 { self.source.offset() }

	/// Get the current unterminated token range
	fn unterminated_range(&self) -> Span { Span::new(self.token.start, self.offset()) }

	/// Consume the current char if not at EOF
	#[inline]
	fn next_char(&mut self) -> Option<char> { self.source.next_char() }

	/// Consume the current char
	#[inline]
	fn consume_char(&mut self) -> char { self.source.next_char().unwrap() }

	/// Consume the current char and the next if not at EOF
	#[inline]
	fn next_2_chars(&mut self) -> Option<[char; 2]> { self.source.next_2_chars() }

	/// Consume the current char and the next
	#[inline]
	fn consume_2_chars(&mut self) -> [char; 2] { self.next_2_chars().unwrap() }

	/// Peek the next byte without advancing the position
	#[inline]
	fn peek_byte(&self) -> Option<u8> { self.source.peek_byte() }

	/// Peek the next two bytes without advancing the position
	#[inline]
	fn peek_2_bytes(&self) -> Option<[u8; 2]> { self.source.peek_2_bytes() }

	/// Peek the next char without advancing the position
	#[inline]
	fn peek_char(&self) -> Option<char> { self.source.peek_char() }
