...
impl<'a> Lexer<'a> {
	...
	unsafe fn template_literal_backslash(
		...
		// after `\`. SAFETY: Caller guarantees next byte is `\`, which is
		// ASCII, so `pos + 1` is UTF-8 char boundary.
	...
	unsafe fn template_literal_create_string(&self, pos:SourcePosition) -> String<'a> {
		...
		// guess that total length will be double what we've seen so far, or
