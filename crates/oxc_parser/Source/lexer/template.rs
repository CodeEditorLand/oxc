...
impl<'a> Lexer<'a> {
	...
	/// passed to `template_literal_escaped` which builds the unescaped string.
	/// This division keeps the path for common case as fast as possible.
	...
	unsafe fn template_literal_backslash(
		...
		// `\`. SAFETY: Caller guarantees next byte is `\`, which is ASCII, so `pos +
		// 1` is UTF-8 char boundary.
	...
	unsafe fn template_literal_create_string(&self, pos:SourcePosition) -> String<'a> {
		...
		// `MIN_ESCAPED_TEMPLATE_LIT_LEN` minimum. SAFETY: Caller guarantees `pos` is
		// not before `self.source.position()`.
