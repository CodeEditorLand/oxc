...
#[inline]
fn is_identifier_start_ascii_byte(byte:u8) -> bool { ASCII_ID_START_TABLE.matches(byte) }
...
impl<'a> Lexer<'a> {
	...
	/// which handle those cases, but those paths are marked `#[cold]` to keep
	/// the ASCII fast path as fast as possible.
	///
	...
	pub(super) unsafe fn identifier_name_handler(&mut self) -> &'a str {
		...
		if !next_byte.is_ascii() {
			return cold_branch(|| {
				...
				// subtracting 1 makes `start_pos` `source`'s position as it
				// was at start of this function
	...
	/// `self.source` prior to it. `self.source` should be positioned at start
	...
	/// handled on paths marked `#[cold]` to keep the common ASCII fast path as
	/// fast as possible.
