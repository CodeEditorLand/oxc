...
impl<'a> Lexer<'a> {
	...
	fn skip_multi_line_comment_after_line_break(&mut self, pos:SourcePosition) -> Kind {
		...
		// creating it. `Finder::new` isn't a const function, so can't make it
		// a `static`, and `lazy_static!` has a cost each time it's deref-ed.
		// Creating `Finder` unconditionally in `Lexer::new`
		// would be efficient for files containing multi-line comments, but
		// would impose pointless cost on files which don't. So this is the
		// fastest solution.
