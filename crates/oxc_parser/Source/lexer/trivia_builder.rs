...
pub struct TriviaBuilder {
	...
	/// Previous token kind, used to indicates comments are trailing from what
	/// kind
	previous_kind:Kind,
}
...
impl TriviaBuilder {
	...
	// For block comments only. This function is not called after line comments
	// because the lexer skips newline after line comments.
	...
	pub fn handle_token(&mut self, token:Token) {
		...
		if self.processed < len {
			// All unprocessed preceding comments are leading comments attached to this
			// token start.
	...
	/// Determines if the current line comment should be treated as a trailing
	/// comment.
	///
	/// A line comment should be treated as trailing when both of the following
	/// conditions are met:
	///
	...
	/// ```
	///
	/// 2. It does not immediately follow an `=` [`Kind::Eq`] or `(`
	///    [`Kind::LParen`] token.
	///
	/// ```javascript
	...
	fn add_comment(&mut self, comment:Comment) {
		...
		// before, to avoid situations where the parser needs to rewind and tries to
		// reinsert the comment.
		...
		if comment.is_line() {
			// A line comment is always followed by a newline. This is never set in
			// `handle_newline`.
