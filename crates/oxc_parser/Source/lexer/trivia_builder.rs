...
impl TriviaBuilder {
	...
	pub fn handle_token(&mut self, token_start:u32) {
		let len = self.comments.len();
		if self.processed < len {
			// All unprocessed preceding comments are leading comments attached
			// to this token start.
	...
	fn add_comment(&mut self, comment:Comment) {
		...
		// added before, to avoid situations where the parser needs to rewind
		// and tries to reinsert the comment.
		...
		if comment.is_line() {
			// A line comment is always followed by a newline. This is never set
			// in `handle_newline`.
