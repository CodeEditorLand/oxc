...
impl<'a> ParserImpl<'a> {
	...
	fn parse_postfix_type_or_higher(&mut self) -> Result<TSType<'a>> {
		...
		while !self.cur_token().is_on_new_line {
			match self.cur_kind() {
				...
				Kind::Question => {
					// If next token is start of a type we have a conditional
					// type
		...
		if self.lookahead(Self::is_tuple_element_name) {
			...
			return Ok(if dotdotdot {
				let type_annotation = self.ast.ts_type_named_tuple_member(
					...
					element_type,
					label,
					// TODO: A tuple member cannot be both optional and rest.
					// (TS5085) See typescript suite
	...
	fn parse_return_type(&mut self, return_kind:Kind, is_type:bool) -> Result<Option<TSType<'a>>> {
	...
	fn should_parse_return_type(&mut self, return_kind:Kind, _is_type:bool) -> bool {
		...
		// // This is easy to get backward, especially in type contexts, so
		// parse the type anyway parseErrorAtCurrentToken(Diagnostics.
		// _0_expected, tokenToString(SyntaxKind.ColonToken)); nextToken();
		// return true;
		// }
		false
	}
	...
	fn parse_ts_index_signature_name(&mut self) -> Result<TSIndexSignatureName<'a>> {
		...
		Ok(self
			.ast
			.ts_index_signature_name(self.end_span(span), name, type_annotation.unwrap()))
	}
