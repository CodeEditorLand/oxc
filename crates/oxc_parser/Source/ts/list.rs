...
impl<'a> SeparatedList<'a> for TSEnumMemberList<'a> {
	fn new(p:&ParserImpl<'a>) -> Self { Self { members:p.ast.new_vec() } }

	fn open(&self) -> Kind { Kind::LCurly }

	fn close(&self) -> Kind { Kind::RCurly }
...
impl<'a> SeparatedList<'a> for TSTupleElementList<'a> {
	fn new(p:&ParserImpl<'a>) -> Self { Self { elements:p.ast.new_vec() } }

	fn open(&self) -> Kind { Kind::LBrack }

	fn close(&self) -> Kind { Kind::RBrack }
...
impl<'a> SeparatedList<'a> for TSTypeParameterList<'a> {
	fn new(p:&ParserImpl<'a>) -> Self { Self { params:p.ast.new_vec() } }

	fn open(&self) -> Kind { Kind::LAngle }

	fn close(&self) -> Kind { Kind::RAngle }
...
impl<'a> TSInterfaceOrObjectBodyList<'a> {
	pub(crate) fn new(p:&ParserImpl<'a>) -> Self { Self { body:p.ast.new_vec() } }
}
...
impl<'a> SeparatedList<'a> for TSTypeArgumentList<'a> {
	fn new(p:&ParserImpl<'a>) -> Self { Self { params:p.ast.new_vec(), in_expression:false } }
