...
pub fn modifier_already_seen(modifier:&Modifier) -> OxcDiagnostic {
	// OxcDiagnostic::error(format!("TS1030: '{}' modifier already seen.",
	// modifier.kind))
...
pub fn cannot_appear_on_a_type_parameter(modifier:&Modifier) -> OxcDiagnostic {
	ts_error(
		"1273",
		format!("'{}' modifier cannot be used on a type parameter.", modifier.kind),
	)
	.with_label(modifier.span)
}
...
pub fn cannot_appear_on_an_index_signature(modifier:&Modifier) -> OxcDiagnostic {
	ts_error(
		"1071",
		format!("'{}' modifier cannot appear on an index signature.", modifier.kind),
	)
	.with_label(modifier.span)
}
