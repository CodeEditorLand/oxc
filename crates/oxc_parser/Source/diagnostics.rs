...
pub fn redeclaration(
	...
	OxcDiagnostic::error(format!("Identifier `{x0}` has already been declared"))
		.with_labels([
			declare_span
				.label(format!("`{x0}` has already been declared here")),
...
pub fn ts_constructor_this_parameter(span: Span) -> OxcDiagnostic {
	ts_error("2681", "A constructor cannot have a `this` parameter.")
		.with_label(span)
}
...
pub fn a_set_accessor_cannot_have_a_return_type_annotation(
	...
	ts_error("1095", " A 'set' accessor cannot have a return type annotation.")
		.with_label(span)
}
...
pub fn cannot_appear_on_a_parameter(modifier: &Modifier) -> OxcDiagnostic {
	ts_error(
		"1090",
		format!("'{}' modifier cannot appear on a parameter.", modifier.kind),
	)
	.with_label(modifier.span)
}
...
#[cold]
pub fn computed_property_names_not_allowed_in_enums(
	span: Span,
) -> OxcDiagnostic {
	ts_error("1164", "Computed property names are not allowed in enums.")
		.with_label(span)
}
/// An enum member cannot have a numeric name.ts(2452)
#[cold]
pub fn enum_member_cannot_have_numeric_name(span: Span) -> OxcDiagnostic {
	ts_error("2452", "An enum member cannot have a numeric name.")
		.with_label(span)
}
