...
impl FunctionKind {
	pub(crate) fn is_id_required(self) -> bool { matches!(self, Self::Declaration) }

	pub(crate) fn is_expression(self) -> bool { self == Self::Expression }
}
	...
	pub(crate) fn parse_yield_expression(&mut self) -> Result<Expression<'a>> {
		...
		if !self.cur_token().is_on_new_line {
			...
			let not_assignment_expr = matches!(
				self.cur_kind(),
				Kind::Semicolon
					| Kind::Eof | Kind::RCurly
					| Kind::RParen | Kind::RBrack
