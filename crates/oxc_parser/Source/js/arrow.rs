...
impl<'a> ParserImpl<'a> {
	...
	fn is_parenthesized_arrow_function_expression_worker(&mut self) -> Tristate {
		...
		match first {
			Kind::LParen => {
				...
				// identifier. This isn't actually allowed, but we want to
				// treat it as a lambda so we can provide a good error
				// message.
				...
				// not valid, but we want to parse it and then give a
				// semantic error.
	...
	fn is_un_parenthesized_async_arrow_function_worker(&mut self) -> Tristate {
		if self.at(Kind::Async) {
			...
			// beginning of an async arrow-function but instead a simple
			// arrow-function which will be parsed inside
