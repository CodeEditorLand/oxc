...
use super::{
	...
	operator::{
		kind_to_precedence,
		map_assignment_operator,
		map_binary_operator,
		map_logical_operator,
		map_unary_operator,
		map_update_operator,
	},
};
...
impl<'a> ParserImpl<'a> {
	...
	pub(crate) fn parse_literal_number(&mut self) -> Result<NumericLiteral<'a>> {
		...
		Ok(self
			.ast
			.numeric_literal(self.end_span(span), value, Some(Atom::from(src)), base))
	}
	...
	pub(crate) fn parse_literal_string(&mut self) -> Result<StringLiteral<'a>> {
		...
		// SAFETY:
		// range comes from the lexer, which are ensured to meeting the criteria of
		// `get_unchecked`.
	...
	pub(crate) fn parse_binary_expression_or_higher(
		...
		let lhs = if self.ctx.has_in() && self.at(Kind::PrivateIdentifier) {
			...
			self.ast
				.expression_private_in(self.end_span(lhs_span), left, BinaryOperator::In, right)
		} else {
	...
	fn parse_binary_expression_rest(
		...
		loop {
			...
			let Some(left_precedence) = kind_to_precedence(kind, self.is_ts) else {
				break;
			};
