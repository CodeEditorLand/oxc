...
impl<'a> ParserImpl<'a> {
	...
	fn parse_primary_expression(&mut self) -> Result<Expression<'a>> {
		...
		match self.cur_kind() {
			...
			Kind::Slash | Kind::SlashEq => {
				self.parse_literal_regexp().map(|literal| {
					self.ast.expression_from_reg_exp_literal(literal)
				})
			},
	...
	pub(crate) fn parse_literal_regexp(&mut self) -> Result<RegExpLiteral<'a>> {
		...
		Ok(self
			.ast
			.reg_exp_literal(self.end_span(span), EmptyObject, RegExp { pattern, flags }))
	}
