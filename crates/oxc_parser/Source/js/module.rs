...
impl<'a> ParserImpl<'a> {
		...
		if source.is_none() {
			for specifier in specifiers.iter_mut() {
				match &specifier.local {
					...
					// NamedExports: It is a Syntax Error if StringValue of
					// n is a ReservedWord or the StringValue of n
					// is one of "implements", "interface", "let", "package",
					...
					// No prior code path should lead to parsing
					// `ModuleExportName` as `IdentifierReference`.
	...
	fn parse_export_default_declaration(
		...
		let declaration = match self.cur_kind() {
			...
			_ if self.at_function_with_async() => {
				self.parse_function_impl(FunctionKind::DefaultExport)
	...
	pub(crate) fn parse_import_specifier(&mut self) -> Result<ImportDeclarationSpecifier<'a>> {
		...
		let (imported, local) = if self.peek_at(Kind::As) {
			...
			let local = self.parse_binding_identifier()?;
			(
				self.ast.module_export_name_identifier_name(local.span, local.name.clone()),
				local,
			)
		};
	...
	fn parse_export_named_specifier(&mut self) -> Result<ExportSpecifier<'a>> {
		...
		Ok(self
			.ast
			.export_specifier(self.end_span(specifier_span), local, exported, export_kind))
	}
}
