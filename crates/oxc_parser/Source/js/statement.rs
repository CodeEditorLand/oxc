...
impl<'a> ParserImpl<'a> {
	...
	pub(crate) fn parse_directives_and_statements(
		...
		while !self.at(Kind::Eof) {
			...
			// The only way to get a correct directive is to parse the statement
			// first and check if it is a string literal. All other method are flawed, see test cases in [babel](https://github.com/babel/babel/blob/main/packages/babel-parser/test/fixtures/core/categorized/not-directive/input.js)
			if expecting_directives {
				if let Statement::ExpressionStatement(expr) = &stmt {
					if let Expression::StringLiteral(string) = &expr.expression {
						// span start will mismatch if they are parenthesized
						// when `preserve_parens = false`
