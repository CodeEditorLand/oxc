...
//! # Usage
//!
//! The parser has a minimal API with three inputs (a [memory
//! arena](oxc_allocator::Allocator), a source string, and a [`SourceType`]) and
//! one return struct (a [ParserReturn]).
//!
//! ```rust
...
//! Oxc's AST is located in a separate [`oxc_ast`] crate. You can find type
//! definitions for AST nodes [here][`oxc_ast::ast`].
//!
//! # Performance
//!
...
//!
//! <div class="warning">
//! Because [`oxc_span::Span`] uses `u32` instead of `usize`, Oxc can only parse
//! files up to 4 GiB in size. This shouldn't be a limitation in almost all
//! cases. </div>
//!
//! # Examples
//!
...
/// ## AST Validity
///
/// [`program`] will always contain a structurally valid AST, even if there are
/// syntax errors. However, the AST may be semantically invalid. To ensure a
/// valid AST,
/// 1. Check that [`errors`] is empty
/// 2. Run semantic analysis with [syntax error checking enabled](https://docs.rs/oxc_semantic/latest/oxc_semantic/struct.SemanticBuilder.html#method.with_check_syntax_error)
///
/// ## Errors
/// Oxc's [`Parser`] is able to recover from some syntax errors and continue
/// parsing. When this happens,
...
/// [`program`] will be empty and [`panicked`] will be `true`.
///
...
pub struct ParserReturn<'a> {
	/// The parsed AST.
	///
	/// Will be empty (e.g. no statements, directives, etc) if the parser
	/// panicked.
	///
	/// ## Validity
	/// It is possible for the AST to be present and semantically invalid. This
	/// will happen if
	/// 1. The [`Parser`] encounters a recoverable syntax error
	/// 2. The logic for checking the violation is in the semantic analyzer
	///
	/// To ensure a valid AST, check that [`errors`](ParserReturn::errors) is
	/// empty. Then, run semantic analysis with syntax error checking enabled.
	pub program:Program<'a>,

	/// Syntax errors encountered while parsing.
	///
	/// This list is not comprehensive. Oxc offloads more-expensive checks to
	/// [semantic analysis](https://docs.rs/oxc_semantic), which can be enabled using
	...
	///
	/// This will be `false` if parsing was successful, or if parsing was able
	/// to recover from a syntax error. When `true`, [`program`] will be empty
	/// and [`errors`] will contain at least one error.
	///
	/// [`program`]: ParserReturn::program
	/// [`errors`]: ParserReturn::errors
	pub panicked:bool,
...
pub struct ParseOptions {
	...
	/// (`false`). Set this to `true` to accept such code.
	///
...
impl<'a> Parser<'a> {
	...
	///
	/// # Parameters
	/// - `allocator`: [Memory arena](oxc_allocator::Allocator) for allocating
	///   AST nodes
	/// - `source_text`: Source code to parse
	/// - `source_type`: Source type (e.g. JavaScript, TypeScript, JSX, ESM
	///   Module, Script)
...
mod parser_parse {
	...
	/// that only a single `ParserImpl`, `Lexer` and `lexer::Source` can exist
	/// at any time on a thread. This constraint is required to guarantee the
	/// soundness of some methods of these types e.g. `Source::set_position`.
	///
	...
	/// and it only calls `ParserImpl::new` once. This enforces the invariant
	/// throughout the entire parser.
	///
	/// `UniquePromise` is a zero-sized type and has no runtime cost. It's
	/// purely for the type-checker.
	///
	/// `UniquePromise::new_for_benchmarks` is a backdoor for benchmarks, so
	/// they can create a `ParserImpl` or `Lexer`, and manipulate it directly,
	...
	impl UniquePromise {
		...
		/// above). This function must NOT be exposed outside of tests and
		/// benchmarks, as it allows circumventing safety invariants of the
		/// parser.
		#[cfg(feature = "benchmarking")]
		pub fn new_for_benchmarks() -> Self { Self(()) }
	}
	...
	impl<'a> Parser<'a> {
		...
		///
		/// See the [module-level documentation](crate) for examples and more
		/// information.
...
impl<'a> ParserImpl<'a> {
	...
	/// Push a Syntax Error
	fn error(&mut self, error:OxcDiagnostic) { self.errors.push(error); }

	fn errors_count(&self) -> usize { self.errors.len() + self.lexer.errors.len() }

	fn ts_enabled(&self) -> bool { self.source_type.is_typescript() }
...
mod test {
	...
	// release mode. `not(debug_assertions)` is a proxy for detecting release
	// mode.
