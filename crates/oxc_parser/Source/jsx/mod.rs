...
impl<'a> ParserImpl<'a> {
	...
	fn parse_jsx_element_name(&mut self) -> Result<JSXElementName<'a>> {
		...
		// References begin with a capital letter, `_` or `$` e.g. `<Foo>`,
		// `<_foo>`, `<$foo>`. https://babeljs.io/repl#?code_lz=DwMQ9mAED0B8DcAoYAzCMHIPpqnJwAJLhkkA&presets=react
		// The identifier has already been checked to be valid, so when first
		// char is ASCII, it can only be `a-z`, `A-Z`, `_` or `$`. But
		// compiler doesn't know that, so we can help it create faster code by
		// taking that invariant into account. `b < b'a'` matches `A-Z`, `_`
		// and `$`. Use a fast path for common case of ASCII characters, to
		// avoid the more expensive `char::is_uppercase` in most cases.
