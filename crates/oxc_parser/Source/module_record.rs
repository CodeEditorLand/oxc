...
impl<'a> ModuleRecordBuilder<'a> {
	...
	pub fn build(mut self) -> (ModuleRecord<'a>, Vec<OxcDiagnostic>) {
		// The `ParseModule` algorithm requires `importedBoundNames` (import entries) to
		// be resolved before resolving export entries.
	...
	fn add_export_entry(&mut self, entry:ExportEntry<'a>) { self.export_entries.push(entry); }
		...
		for ee in export_entries {
			...
			if ee.module_request.is_none() {
				...
				match found_import_entry {
					...
					// ii. Else,
					// 1. Let ie be the element of importEntries whose [[LocalName]] is the same as
					//    ee.[[LocalName]].
					Some(ie) => {
						match &ie.import_name {
							// 2. If ie.[[ImportName]] is namespace-object, then
							ImportImportName::NamespaceObject => {
								// a. NOTE: This is a re-export of an imported module namespace
								// object. b. Append ee to localExportEntries.
								self.append_local_export_entry(ee);
							},
							// 3. Else,
	...
	fn visit_export_named_declaration(&mut self, decl:&ExportNamedDeclaration<'a>) {
		...
		let module_request = decl
			.source
			.as_ref()
			.map(|source| NameSpan::new(source.value.clone(), source.span));
...
mod module_record_tests {
	...
	// Table 55 gives examples of ImportEntry records fields used to represent the
	// syntactic import forms: `https://tc39.es/ecma262/#table-import-forms-mapping-to-importentry-records`
	...
	// Table 57 gives examples of the ExportEntry record fields used to represent
	// the syntactic export forms `https://tc39.es/ecma262/#table-export-forms-mapping-to-exportentry-records`
