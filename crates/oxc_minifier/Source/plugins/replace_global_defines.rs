...
impl ReplaceGlobalDefinesConfig {
	...
	pub fn new<S:AsRef<str>>(defines:&[(S, S)]) -> Result<Self, Vec<OxcDiagnostic>> {
		...
		// see test case replace_global_definitions_dot_with_postfix_mixed as an
		// example
...
impl<'a> ReplaceGlobalDefines<'a> {
	...
	fn parse_value(&self, source_text:&str) -> Expression<'a> {
		...
		Parser::new(self.allocator, source_text, SourceType::default())
			.parse_expression()
			.unwrap()
	}
	...
	pub fn is_meta_property_define(
		...
		loop {
				...
				match &member.object {
					...
					Expression::MetaProperty(_) => {
						if meta_define.postfix_wildcard {
							// `import.meta.env` should not match
							// `import.meta.env.*`
			...
			// Considering try replace `import.meta.env` to `undefined`, for the
			// first loop the i is already 0, if it did not match part name
			// and still reach here, that means
			// current_part_member_expression is still something, and possible
