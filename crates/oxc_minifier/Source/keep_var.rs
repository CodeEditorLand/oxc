use oxc_ast::{
	ast::*, syntax_directed_operations::BoundNames, AstBuilder, Visit, NONE,
};
...
impl<'a> KeepVar<'a> {
	...
	pub fn get_variable_declaration_statement(self) -> Option<Statement<'a>> {
		...
		let decls = self.ast.vec_from_iter(self.vars.into_iter().map(
			|(name, span)| {
		...
		let stmt = self
			.ast
			.statement_declaration(self.ast.declaration_from_variable(decl));
		Some(stmt)
	}
}
