...
impl<'a> Visit<'a> for KeepVar<'a> {
	fn visit_statement(&mut self, it:&Statement<'a>) {
		...
		match it {
			...
			// visitor.visit_declaration(it.to_declaration()),
...
impl<'a> KeepVar<'a> {
	pub fn new(ast:AstBuilder<'a>) -> Self { Self { ast, vars:std::vec![], all_hoisted:true } }

	pub fn all_hoisted(&self) -> bool { self.all_hoisted }
