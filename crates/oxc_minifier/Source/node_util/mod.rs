...
pub use self::{
	is_literal_value::IsLiteralValue,
	may_have_side_effects::MayHaveSideEffects,
...
pub trait NodeUtil {
	...
	/// Returns the number value of the node if it has one and it cannot have
	/// side effects.
	fn get_side_free_number_value(&self, expr:&Expression) -> Option<NumberValue> {
		...
		// calculating side effects, and there are only a very few cases where
		// we can compute a number value, but there could also be side
		// effects. e.g. `void doSomething()` has value NaN, regardless of the
	...
	fn get_side_free_bigint_value(&self, expr:&Expression) -> Option<BigInt> {
		...
		// calculating side effects, and there are only a very few cases where
		// we can compute a bigint value, but there could also be side
		// effects. e.g. `void doSomething()` has value NaN, regardless of the
		// behavior of `doSomething()`
		if value.is_some() && expr.may_have_side_effects() { None } else { value }
	}
	...
	/// Gets the value of a node as a String, or `None` if it cannot be
	/// converted. This method effectively emulates the `String()` JavaScript
	/// cast function when possible and the node has no side effects.
	...
	fn get_side_free_string_value<'a>(&self, expr:&'a Expression) -> Option<Cow<'a, str>> {
		...
		// calculating side effects, and there are only a very few cases where
		// we can compute a string value, but there could also be side
		// effects. e.g. `void doSomething()` has value 'undefined', regardless
	...
	/// Gets the boolean value of a node that represents an expression, or
	/// `None` if no such value can be determined by static analysis.
		...
		match expr {
			...
			Expression::SequenceExpression(sequence_expr) => {
				...
				sequence_expr
					.expressions
					.last()
					.map_or(Tri::Unknown, |e| self.get_boolean_value(e))
			},
	...
	fn get_bigint_value(&self, expr:&Expression) -> Option<BigInt> {
		match expr {
			...
			Expression::UnaryExpression(unary_expr) => {
				match unary_expr.operator {
					UnaryOperator::LogicalNot => {
						self.get_boolean_value(expr).map(|boolean| {
							if boolean.is_true() { BigInt::one() } else { BigInt::zero() }
						})
					},
					UnaryOperator::UnaryNegation => {
			...
			Expression::TemplateLiteral(_) => {
				self.get_string_value(expr)
					.and_then(|value| self.get_string_bigint_value(&value))
			},
			_ => None,
		}
	}
	...
	/// Gets the value of a node as a String, or `None` if it cannot be
	/// converted. When it returns a String, this method effectively emulates
	/// the `String()` JavaScript cast function. This method does not consider
	/// whether `expr` may have side effects.
	fn get_string_value<'a>(&self, expr:&'a Expression) -> Option<Cow<'a, str>> {
		match expr {
			...
			Expression::TemplateLiteral(template_literal) => {
				// TODO: I don't know how to iterate children of TemplateLiteral
				// in order,so only checkout string like `hi`. Closure-compiler do more: [case TEMPLATELIT](https://github.com/google/closure-compiler/blob/e13f5cd0a5d3d35f2db1e6c03fdf67ef02946009/src/com/google/javascript/jscomp/NodeUtil.java#L241-L256).
				template_literal
					.quasis
					.first()
					.filter(|quasi| quasi.tail)
	...
	/// Evaluate  and attempt to determine which primitive value type it could
	/// resolve to. Without proper type information some assumptions had to be
	/// made for operations that could result in a BigInt or a Number. If there
	/// is not enough information available to determine one or the other then
	/// we assume Number in order to maintain historical behavior of the
	/// compiler and avoid breaking projects that relied on this behavior.
