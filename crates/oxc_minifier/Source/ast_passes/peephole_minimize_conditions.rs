...
/// Minimize Conditions
///
/// A peephole optimization that minimizes conditional expressions according to
/// De Morgan's laws. Also rewrites conditional statements as expressions by
/// replacing them with `? :` and short-circuit binary operators.
///
...
impl<'a> PeepholeMinimizeConditions {
	pub fn new() -> Self { Self { changed:false } }
...
mod test {
	...
	fn test_same(source_text:&str) { test(source_text, source_text); }

	fn fold_same(js:&str) { test_same(js); }

	fn fold(js:&str, expected:&str) { test(js, expected); }
	...
	#[test]
	#[ignore]
	fn test_combine_ifs3() { fold_same("function f() {if (x) return 1; if (y) {g();f()}}"); }
	...
	fn test_remove_duplicate_statements() {
		// enableNormalize();
		// TODO(bradfordcsmith): Stop normalizing the expected output or
		// document why it is necessary. enableNormalizeExpectedOutput();
		fold("if (a) { x = 1; x++ } else { x = 2; x++ }", "x=(a) ? 1 : 2; x++");
		// fold(
		// "if (a) { x = 1; x++; y += 1; z = pi; }" + " else  { x = 2; x++; y +=
		// 1; z = pi; }", "x=(a) ? 1 : 2; x++; y += 1; z = pi;",
		// );
		// fold(
		// "function z() {" + "if (a) { foo(); return !0 } else { goo(); return
		// !0 }" + "}", "function z() {(a) ? foo() : goo(); return !0}",
		// );
		// fold(
	...
	fn test_dont_remove_duplicate_statements_without_normalization() {
		// In the following test case, we can't remove the duplicate "alert(x);"
		// lines since each "x" refers to a different variable.
		// We only try removing duplicate statements if the AST is normalized
	...
	fn test_minimize_while_condition() {
		// This test uses constant folding logic, so is only here for
		// completeness.
	...
	#[test]
	#[ignore]
	fn test_minimize_demorgan1() { fold("if(!a&&!b)foo()", "(a||b)||foo()"); }
	...
	#[test]
	#[ignore]
	fn test_minimize_demorgan3() { fold("if((!a||!b)&&(c||d)) foo()", "(a&&b||!c&&!d)||foo()"); }

	#[test]
	#[ignore]
	fn test_minimize_demorgan5() { fold("if((!a||!b)&&c) foo()", "(a&&b||!c)||foo()"); }
	...
	#[test]
	#[ignore]
	fn test_preserve_if() { fold_same("if(!a&&!b)for(;f(););"); }
	...
	fn test_minimize_for_condition() {
		// This test uses constant folding logic, so is only here for
		// completeness. These could be simplified to "for(;;) ..."
	...
	fn test_substitute_return() {
		...
		// It doesn't matter if x is changed between them. We are still
		// returning x at whatever x value current holds. The whole x = 1 is
		// skipped.
		fold(
		...
		// fold_same("function f() {" + "  try { while(x) {return f()}} catch
		// (e) { } return f()}");

		// fold_same("function f() {" + "  try { while(x) {return f()}} finally
		// {alert(1)} return f()}");

		// Both returns has the same handler
		// fold(
		// "function f() {" + "  try { while(x) { return f() } return f() }
		// catch (e) { } }", "function f() {" + "  try { while(x) { break }
		// return f() } catch (e) { } }", );

		// We can't fold this because it'll change the order of when foo is
		// called. fold_same(
		// "function f() {"
		...
		// fold(
		// "function f() {" + "  try { while(x) { return 1 } } finally {
		// alert(1) } return 1}", "function f() {" + "  try { while(x) { break
		// } } finally { alert(1) } return 1}", );
	...
	fn test_substitute_break_for_throw() {
		...
		// fold_same("function f() {" + "  try { while(x) {throw f()}} catch (e)
		// { } throw f()}");

		// fold_same("function f() {" + "  try { while(x) {throw f()}} finally
		// {alert(1)} throw f()}");

		// Both throws has the same handler
		// fold(
		// "function f() {" + "  try { while(x) { throw f() } throw f() } catch
		// (e) { } }", "function f() {" + "  try { while(x) { break } throw
		// f() } catch (e) { } }", );
		...
		// fold(
		// "function f() {" + "  try { while(x) { throw 1 } } finally { alert(1)
		// } throw 1}", "function f() {" + "  try { while(x) { break    } }
		// finally { alert(1) } throw 1}", );
	...
	fn test_remove_duplicate_return() {
		...
		// but they don't matter if the result doesn't have side effects and
		// can't be affect by side-effects.
		fold(
		...
		// fold(
		// "function f() { switch(a){ " + "  case 1: return a; case 2: return a;
		// } return a; }", "function f() { switch(a){ " + "  case 1: break;
		// case 2: } return a; }", );
	}
	...
	fn test_remove_duplicate_throw() {
		...
		// fold(
		// "function f() { switch(a){ " + "case 1: throw a; case 2: throw a; }
		// throw a; }", "function f() { switch(a){ case 1: break; case 2: }
		// throw a; }", );
	}
	...
	fn test_remove_else_cause() {
		// test(
		// "function f() {" + " if(x) return 1;" + " else if(x) return 2;" + "
		// else if(x) return 3 }", "function f() {" + " if(x) return 1;" + "{
		// if(x) return 2;" + "{ if(x) return 3 } } }", );
	}
