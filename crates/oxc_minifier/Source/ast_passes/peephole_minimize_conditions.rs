...
mod test {
	...
	fn test_fold_one_child_blocks() {
		...
		// fold("if(x){do{foo()}while(y)}else bar()", "if(x){do foo();while(y)}else
		// bar()");
	...
	fn test_remove_duplicate_statements() {
		// enableNormalize();
		// TODO(bradfordcsmith): Stop normalizing the expected output or document why it
		// is necessary. enableNormalizeExpectedOutput();
	...
	fn test_dont_remove_duplicate_statements_without_normalization() {
		// In the following test case, we can't remove the duplicate "alert(x);" lines
		// since each "x" refers to a different variable.
		// We only try removing duplicate statements if the AST is normalized and names
		// are unique.
		test_same(
