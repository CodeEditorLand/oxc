...
impl PeepholeReplaceKnownMethods {
	...
	fn try_fold_known_string_methods<'a>(
		...
		let Expression::CallExpression(call_expr) = node else {
			return;
		};
		...
		if let Expression::StringLiteral(string_lit) = &member.object {
			let replacement = match member.property.name.as_str() {
				...
				"substring" | "slice" => {
					Self::try_fold_string_substring_or_slice(
						call_expr.span,
						call_expr,
						string_lit,
						ctx,
				...
				"replace" | "replaceAll" => {
					Self::try_fold_string_replace_or_string_replace_all(
						call_expr.span,
						call_expr,
						member,
						string_lit,
	...
	fn try_fold_string_substring_or_slice<'a>(
		...
		let start_idx = call_expr.arguments.first().and_then(|arg| {
			match arg {
		...
		let end_idx = call_expr.arguments.get(1).and_then(|arg| {
			match arg {
				Argument::SpreadElement(_) => None,
				_ => Ctx(ctx).get_side_free_number_value(arg.to_expression()),
			}
	...
	fn try_fold_string_char_code_at<'a>(
		...
		let char_at_index = call_expr.arguments.first().and_then(|arg| {
			match arg {
				Argument::SpreadElement(_) => None,
				_ => Ctx(ctx).get_side_free_number_value(arg.to_expression()),
			}
		...
		// TODO: if `result` is `None`, return `NaN` instead of skipping the
		// optimization
...
mod test {
	...
	#[test]
	#[ignore]
	fn test_string_join_add_sparse() { fold("x = [,,'a'].join(',')", "x = ',,a'"); }
	...
	fn test_fold_string_substring() {
		...
		// we should be leaving negative, out-of-bound, and inverted indices alone for
		// now
	...
	#[test]
	#[ignore]
	fn test_fold_math_functions_bug() { fold_same("Math[0]()"); }
	...
	fn test_fold_math_functions_fround_j2cl() { fold_same("Math.fround(1.2)"); }
	...
	fn test_fold_parse_numbers() {
		...
		fold("x = parseInt(-15.99, 10)", "x = -15");
		// Java's Integer.parseInt("-15.99", 10) throws an exception, because of the
		// decimal point.
	...
	#[test]
	#[ignore]
	fn test_array_of_no_args() { fold("x = Array.of()", "x = []"); }
	...
	#[test]
	#[ignore]
	fn test_fold_array_bug() { fold_same("Array[123]()"); }

	fn fold_same_string_typed(js:&str) { fold_string_typed(js, js); }
