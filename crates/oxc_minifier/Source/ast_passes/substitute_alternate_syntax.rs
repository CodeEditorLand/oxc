...
impl<'a> Traverse<'a> for SubstituteAlternateSyntax {
	...
	fn exit_binary_expression(&mut self, expr:&mut BinaryExpression<'a>, ctx:&mut TraverseCtx<'a>) {
		self.compress_typeof_undefined(expr, ctx);
	}
}

impl<'a> SubstituteAlternateSyntax {
	pub fn new(options:CompressOptions) -> Self { Self { options, in_define_export:false } }
	...
	fn is_object_define_property_exports(call_expr:&CallExpression<'a>) -> bool {
		...
		// Use tighter check than
		// `call_expr.callee.is_specific_member_access("Object",
		// "defineProperty")` because we're looking for
		// `Object.defineProperty` specifically, not e.g.
	...
	fn compress_block(&self, stmt:&mut Statement<'a>) {
		if let Statement::BlockStatement(block) = stmt {
			// Avoid compressing `if (x) { var x = 1 }` to `if (x) var x = 1`
			// due to different semantics according to AnnexB, which lead to
			// different semantics.
	...
	fn compress_typeof_undefined(&self, expr:&mut BinaryExpression<'a>, ctx:&mut TraverseCtx<'a>) {
