...
impl<'a> FoldConstants {
	pub fn new() -> Self { Self { evaluate:false } }
	...
	fn try_fold_binary_operator(
		...
		match binary_expr.operator {
			...
			// NOTE: string concat folding breaks our current evaluation of
			// Test262 tests. The minifier is tested by comparing output of
			// running the minifier once and twice, respectively. Since
			// Test262Error messages include string concats, the outputs
			// don't match (even though the produced code is valid).
			// Additionally, We'll likely want to add `evaluate` checks for
			// all constant folding, not just additions, but we're adding
			// this here until a decision is made.
