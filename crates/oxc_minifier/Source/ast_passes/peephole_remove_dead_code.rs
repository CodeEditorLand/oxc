...
use crate::{CompressorPass, keep_var::KeepVar, node_util::Ctx};
...
impl<'a, 'b> PeepholeRemoveDeadCode {
	...
	/// Removes dead code thats comes after `return` statements after inlining
	/// `if` statements
	...
	fn compress_block(&mut self, stmt:&mut Statement<'a>, ctx:Ctx<'a, 'b>) {
		if let Statement::BlockStatement(block) = stmt {
			// Avoid compressing `if (x) { var x = 1 }` to `if (x) var x = 1` due to
			// different semantics according to AnnexB, which lead to different
			// semantics.
					...
					Some(ForStatementInit::VariableDeclaration(var_init)) => {
						...
						if var_init.kind.is_var() {
							if let Some(var_decl) = &mut var_decl {
								...
								var_decl = Some(
									ctx.ast.alloc(ctx.ast.move_variable_declaration(var_init)),
								);
							}
						}
	...
	fn try_fold_expression_stmt(
		...
		// This is the only scenario where we can't remove it even if
		// `ExpressionStatement`. TODO find a better way to handle this.
	...
	fn try_fold_array_expression(
		...
		if transformed_elements.is_empty() {
			...
			return Some(
				ctx.ast
					.statement_expression(array_expr.span, transformed_elements.pop().unwrap()),
			);
		}
	...
	/// Try folding conditional expression (?:) if the condition results of the
	/// condition is known.
	fn try_fold_conditional_expression(
		...
		match ctx.get_boolean_value(&expr.test) {
			Some(true) => {
				// Bail `let o = { f() { assert.ok(this !== o); } }; (true ? o.f : false)();
				// (true ? o.f : false)``;`
