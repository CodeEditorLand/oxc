	...
	pub fn try_fold_and_or(
		...
		if let Some(lval) = left_val {
			...
			if if lval { op == LogicalOperator::Or } else { op == LogicalOperator::And } {
				...
				// Bail `let o = { f() { assert.ok(this !== o); } }; (true && o.f)(); (true &&
				// o.f)``;`
...
mod test {
	...
	fn test_fold_bitwise_op() {
		...
		// these cases look strange because bitwise OR converts unsigned numbers to be
		// signed
	...
	fn test_fold_arithmetic() {
		...
		// test_same("x = 3 ** -1"); // backs off because 3**-1 is shorter than
		// 0.3333333333333333
