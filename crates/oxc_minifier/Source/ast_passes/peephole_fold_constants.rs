...
impl<'a> PeepholeFoldConstants {
	...
	fn try_fold_unary_expression(
		...
		fn is_valid(x:f64) -> bool { x.is_finite() && x.fract() == 0.0 }
		match expr.operator {
			...
			UnaryOperator::UnaryPlus => {
				match &expr.argument {
					...
					// `+NaN` -> `NaN`
					_ if expr.argument.is_nan() => {
						Some(ctx.ast.move_expression(&mut expr.argument))
					},
					_ if expr.argument.is_number() => {
						Some(ctx.ast.move_expression(&mut expr.argument))
					},
					_ => None,
				}
			...
			UnaryOperator::BitwiseNot => {
				match &mut expr.argument {
					Expression::BigIntLiteral(n) => {
						let value =
							ctx.get_string_bigint_value(n.raw.as_str().trim_end_matches('n'));
						value.map(|value| {
							let value = !value;
						...
						match un.operator {
							...
							UnaryOperator::UnaryNegation if un.argument.is_big_int_literal() => {
								...
								if let Expression::BigIntLiteral(n) = &mut un.argument {
									let value = ctx.get_string_bigint_value(
										n.raw.as_str().trim_end_matches('n'),
									);
	...
	pub fn try_fold_and_or(
		...
		if let Some(lval) = left_val {
			...
			if if lval { op == LogicalOperator::Or } else { op == LogicalOperator::And } {
				...
				// Bail `let o = { f() { assert.ok(this !== o); } }; (true &&
				// o.f)(); (true && o.f)``;`
		...
		match left_val {
			ValueType::Null | ValueType::Void => {
				Some(if left.may_have_side_effects() {
					...
					// nullish condition => this expression evaluates to the
					// right side.
					ctx.ast.move_expression(&mut logical_expr.right)
				})
			},
			...
			| ValueType::Boolean
			| ValueType::Object => {
				// non-nullish condition => this expression evaluates to the
				// left side.
	...
	fn try_fold_binary_expression(
		...
		match e.operator {
			...
			| BinaryOperator::Exponential => self.try_fold_arithmetic_op(e.span, &e.left, &e.right, ctx),
	...
	pub fn bigint_less_than_number(
		...
		// tryAbstractRelationalComparison if it's true, then the number is on
		// the left
		match number_value {
...
mod test {
	...
	fn test_bigint_number_comparison() {
		...
		// Don't fold unsafely large numbers because there might be
	...
	fn unary_ops() {
		// TODO: need to port
		// These cases are handled by PeepholeRemoveDeadCode in
		// closure-compiler. test_same("!foo()");
		// test_same("~foo()");
		// test_same("-foo()");
	...
	fn test_unary_ops_string_compare() {
		...
		// More tests added by Ethan, which aligns with Google Closure
		// Compiler's behavior
