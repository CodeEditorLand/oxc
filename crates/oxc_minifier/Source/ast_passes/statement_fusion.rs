...
impl<'a> StatementFusion {
	...
	fn can_fuse_into_one_statement(stmts:&[Statement<'a>]) -> bool {
		...
		if stmts[0..len - 1]
			.iter()
			.any(|s| !matches!(s, Statement::ExpressionStatement(_)))
		{
			return false;
		}
...
mod test {
	...
	#[test]
	fn fold_switch() { fuse("a;b;c;switch(x){}", "switch(a,b,c,x){}"); }

	#[test]
	fn fuse_into_for_in1() { fuse("a;b;c;for(x in y){}", "for(x in a,b,c,y){}"); }

	#[test]
	fn fuse_into_for_in2() {
		// This test case causes a parse warning in ES5 strict out, but is a parse error
		// in ES6+ out. setAcceptedLanguage(CompilerOptions.LanguageMode.
		// ECMASCRIPT5_STRICT); set_expect_parse_warnings_in_this_test();
	...
	#[test]
	fn no_fuse_into_while() { fuse_same("a;b;c;while(x){}"); }

	#[test]
	fn no_fuse_into_do() { fuse_same("a;b;c;do{}while(x)"); }

	#[test]
	fn no_fuse_into_block() {
		// Never fuse a statement into a block that contains let/const/class
		// declarations, or you risk colliding variable names. (unless the AST is
		// normalized).
		fuse("a; {b;}", "{a,b;}");
	...
	#[test]
	fn no_global_scope_changes() { test_same("a,b,c"); }

	#[test]
	fn no_function_block_changes() { test_same("function foo() { a,b,c }"); }
}
