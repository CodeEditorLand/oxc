...
impl Tri {
	...
	pub fn xor(self, other:Self) -> Self { self.for_int(-self.value() * other.value()) }
	...
	pub fn for_boolean(boolean:bool) -> Self { if boolean { Self::True } else { Self::False } }
...
fn bigint_less_than_number(
	...
	// if invert is false, then the number is on the right in
	// tryAbstractRelationalComparison if it's true, then the number is on the
	// left
	match number_value {
...
impl<'a> Compressor<'a> {
		...
		let folded_expr = match expr {
			Expression::BinaryExpression(binary_expr) => {
				match binary_expr.operator {
					...
					// NOTE: string concat folding breaks our current evaluation
					// of Test262 tests. The minifier is tested by
					// comparing output of running the minifier once and twice,
					// respectively. Since Test262Error messages include string
					// concats, the outputs don't match (even though the
					// produced code is valid). Additionally, We'll likely
					// want to add `evaluate` checks for all constant folding,
					// not just additions, but we're adding this here until
					// a decision is made.
					BinaryOperator::Addition if self.options.evaluate => {
						self.try_fold_addition(
							binary_expr.span,
							&binary_expr.left,
							&binary_expr.right,
						)
	...
	fn try_abstract_relational_comparison<'b>(
				...
				if (left.operator, right.operator) == (UnaryOperator::Typeof, UnaryOperator::Typeof)
				{
						...
						(&left.argument, &right.argument)
					{
						if left.name == right.name {
							// Special case: `typeof a < typeof a` is always
							// false.
							return Tri::False;
						}
					}
		...
		match (left_bigint, right_bigint, left_num, right_num) {
			// Next, try to evaluate based on the value of the node. Try
			// comparing as BigInts first.
			(Some(l_big), Some(r_big), ..) => {
			...
			(_, _, Some(l_num), Some(r_num)) => {
				match (l_num, r_num) {
					...
					(NumberValue::Number(l), NumberValue::Number(r)) => {
						return Tri::for_boolean(l < r);
					},
					_ => {},
				}
			},
				...
				Ty::Str => {
						...
						(left_expr, right_expr)
					{
							...
							== (UnaryOperator::Typeof, UnaryOperator::Typeof)
						{
							...
							{
								if left.name == right.name {
									// Special case, typeof a == typeof a is
									// always true.
									return Tri::True;
								}
							}
			...
			let type_name = match argument {
				...
				Expression::UnaryExpression(unary_expr) => {
					match unary_expr.operator {
						...
						// `unary_expr.argument` is literal value, so it's safe
						// to fold
	...
	fn try_fold_unary_operator(
		...
		if let Some(boolean) = get_boolean_value(&unary_expr.argument) {
			match unary_expr.operator {
				// !100 -> false
				// !100n -> false
				// after this, it will be compressed to !1 or !0 in
				// `compress_boolean`
				UnaryOperator::LogicalNot => {
						...
						Expression::NumericLiteral(number_literal) => {
							if number_literal.value.fract() == 0.0 {
								...
								let literal = self.ast.number_literal(
									...
									f64::from(!int_value),
									number_literal.raw,
									NumberBase::Decimal, /* since it be
									                      * converted to
									                      * i32, it should
									                      * always be decimal.
									                      */
	...
	fn try_fold_shift<'b>(
			...
			(left_num, right_num)
		{
			...
			// only the lower 5 bits are used when shifting, so don't do
			// anything if the shift amount is outside [0,32)
	...
	fn try_fold_and_or(
		...
		if let Some(boolean_value) = boolean_value {
			...
			// e.g. true_with_sideeffects || foo() => true_with_sideeffects,
	...
	fn fold_expression_in_condition(&mut self, expr:&mut Expression<'a>) -> Option<Expression<'a>> {
		let folded_expr = match expr {
			Expression::UnaryExpression(unary_expr) => {
				match unary_expr.operator {
					UnaryOperator::LogicalNot => {
