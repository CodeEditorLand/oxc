...
/// Returns true if this is a literal value. We define a literal value as any
/// node that evaluates to the same thing regardless of when or where it is
/// evaluated. So `/xyz/` and `[3, 5]` are literals, but the name a is not.
///
/// Function literals do not meet this definition, because they lexically
/// capture variables. For example, if you have `function() { return a; }`.
/// If it is evaluated in a different scope, then it captures a different
/// variable. Even if the function did not read any captured variables directly,
/// it would still fail this definition, because it affects the lifecycle of
/// variables in the enclosing scope.
///
/// However, a function literal with respect to a particular scope is a literal.
/// If `include_functions` is true, all function expressions will be treated as
/// literals.
	...
	fn is_literal_value(&self, include_functions:bool) -> bool {
		match self {
			...
			Self::ObjectExpression(expr) => {
				expr.properties
					.iter()
					.all(|property| property.is_literal_value(include_functions))
			},
...
where
	...
	fn may_have_side_effects(&self) -> bool { self.check_for_state_change(false) }
}
...
/// `check_for_new_objects` is true, we assume that newly created mutable
/// objects (like object literals) change state. Otherwise, we assume that they
...
impl<'a, 'b> CheckForStateChange<'a, 'b> for SpreadElement<'a> {
	fn check_for_state_change(&self, _check_for_new_objects:bool) -> bool {
		...
		// TODO: Closure Compiler assumes that getters may side-free when set
		// `assumeGettersArePure`. https://github.com/google/closure-compiler/blob/a4c880032fba961f7a6c06ef99daa3641810bfdd/src/com/google/javascript/jscomp/AstAnalyzer.java#L282
		true
	}
}
...
/// A "simple" operator is one whose children are expressions, has no direct
/// side-effects.
fn is_simple_unary_operator(operator:UnaryOperator) -> bool { operator != UnaryOperator::Delete }
...
pub fn is_exact_int64(num:f64) -> bool { num.fract() == 0.0 }
		...
		Expression::UnaryExpression(unary_expr) => {
			match unary_expr.operator {
				...
				UnaryOperator::UnaryNegation => {
					get_number_value(&unary_expr.argument).map(|v| v.not())
				},
				UnaryOperator::BitwiseNot => {
					get_number_value(&unary_expr.argument).map(|value| {
						match value {
							NumberValue::Number(num) => {
								NumberValue::Number(f64::from(
...
/// Returns the number value of the node if it has one and it cannot have side
/// effects.
pub fn get_side_free_number_value(expr:&Expression) -> Option<NumberValue> {
	let value = get_number_value(expr);
	// Calculating the number value, if any, is likely to be faster than
	// calculating side effects, and there are only a very few cases where we
	// can compute a number value, but there could also be side effects. e.g.
	// `void doSomething()` has value NaN, regardless of the behavior
	// of `doSomething()`
...
pub fn get_side_free_bigint_value(expr:&Expression) -> Option<BigInt> {
	...
	// Calculating the bigint value, if any, is likely to be faster than
	// calculating side effects, and there are only a very few cases where we
	// can compute a bigint value, but there could also be side effects. e.g.
	// `void doSomething()` has value NaN, regardless of the behavior
	// of `doSomething()`
	if value.is_some() && expr.may_have_side_effects() { None } else { value }
}
...
/// Gets the boolean value of a node that represents an expression, or `None` if
/// no such value can be determined by static analysis.
...
/// When it returns a String, this method effectively emulates the `String()`
/// JavaScript cast function. This method does not consider whether `expr` may
/// have side effects.
pub fn get_string_value<'a>(expr:&'a Expression) -> Option<Cow<'a, str>> {
	match expr {
		...
		Expression::TemplateLiteral(template_literal) => {
			// TODO: I don't know how to iterate children of TemplateLiteral in
			// order,so only checkout string like `hi`. Closure-compiler do more: [case TEMPLATELIT](https://github.com/google/closure-compiler/blob/e13f5cd0a5d3d35f2db1e6c03fdf67ef02946009/src/com/google/javascript/jscomp/NodeUtil.java#L241-L256).
			template_literal
				.quasis
				.first()
				.filter(|quasi| quasi.tail)
...
/// Gets the value of a node as a String, or `None` if it cannot be converted.
/// This method effectively emulates the `String()` JavaScript cast function
/// when possible and the node has no side effects. Otherwise, it returns
/// `None`.
pub fn get_side_free_string_value<'a>(expr:&'a Expression) -> Option<Cow<'a, str>> {
	let value = get_string_value(expr);
	// Calculating the string value, if any, is likely to be faster than
	// calculating side effects, and there are only a very few cases where we
	// can compute a string value, but there could also be side effects. e.g.
	// `void doSomething()` has value 'undefined', regardless of the
