...
impl<'a> IsolatedDeclarations<'a> {
	...
	pub fn transform_ts_signatures(
			...
			if let TSSignature::TSMethodSignature(method) = signature {
				...
				match method.kind {
					...
					TSMethodSignatureKind::Set => {
						let Some(first_param) = method.params.items.first_mut()
						else {
							return;
						};

						let entry =
							method_annotations.entry(name.clone()).or_default();
						entry.0 |=
							first_param.pattern.type_annotation.is_none();
						entry.1 =
							Some(&mut first_param.pattern.type_annotation);
					},
		...
		for (requires_inference, param, return_type) in
			method_annotations.into_values()
		{
