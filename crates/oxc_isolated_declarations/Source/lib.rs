...
impl<'a> IsolatedDeclarations<'a> {
	...
	pub fn build(
		...
		let program =
			self.ast.program(SPAN, source_type, None, directives, stmts);
	...
	fn build_internal_annotations(
		...
		for comment in trivias.comments() {
			let has_internal =
				comment.span.source_text(source_text).contains("@internal");
...
impl<'a> IsolatedDeclarations<'a> {
	...
	pub fn transform_program_without_module_declaration(
		...
		self.ast.vec_from_iter(stmts.iter().map(|stmt| {
			if let Some(new_decl) =
				self.transform_declaration(stmt.to_declaration(), false)
			{
	...
	pub fn transform_statements_on_demand(
		...
		let mut transformed_stmts: FxHashMap<Span, Statement<'a>> =
			FxHashMap::default();
		let mut transformed_variable_declarator: FxHashMap<
			Span,
						...
						if is_global || decl.id.is_string_literal() {
							...
							if !is_global {
								if let Some(body) = decl
									.body
									.as_mut()
									.and_then(|body| body.as_module_block_mut())
								{
							...
							self.scope
								.visit_ts_module_declaration(decl.as_ref());
					...
					match stmt.to_module_declaration() {
						ModuleDeclaration::ExportDefaultDeclaration(decl) => {
								...
								self.transform_export_default_declaration(decl)
							{
								...
								self.scope.visit_export_default_declaration(
									&new_decl,
								);
								transformed_stmts.insert(
                                    decl.span,
								...
								self.scope
									.visit_export_default_declaration(decl);
							}
						...
						ModuleDeclaration::ExportNamedDeclaration(decl) => {
							...
							{
								self.scope
									.visit_export_named_declaration(&new_decl);
								transformed_stmts.insert(
                                    decl.span,
						...
						module_declaration => {
							...
							self.scope
								.visit_module_declaration(module_declaration);
						},
					}
				},
				_ => {},
		...
		loop {
			...
			for stmt in &stmts {
				...
				let Some(decl) = stmt.as_declaration() else {
					continue;
				};
				...
				if let Declaration::VariableDeclaration(declaration) = decl {
					...
					for declarator in &declaration.declarations {
						...
						{
							self.scope
								.visit_variable_declarator(&new_declarator);
							transformed_variable_declarator
								.insert(declarator.span, new_declarator);
						} else {
		...
		if last_transformed_len != 0
			&& last_transformed_len == transformed_spans.len()
		{
			need_empty_export_marker = false;
		}

		// 6. Transform variable/using declarations, import statements, remove unused imports
		let mut new_stm = self.ast.vec_with_capacity(
			stmts.len() + usize::from(export_default_var.is_some()),
		);
			...
			match stmt {
				Statement::ImportDeclaration(decl) => {
						...
						self.transform_import_declaration(decl)
					{
						new_stm.push(Statement::from(
							self.ast
								.module_declaration_from_import_declaration(
									new_decl,
								),
						));
					}
				...
				Statement::VariableDeclaration(decl) => {
					if decl.declarations.len() > 1 {
						...
						new_stm.push(Statement::from(
							self.ast.declaration_from_variable(
		...
		if need_empty_export_marker {
			...
			let empty_export = self.ast.alloc_export_named_declaration(
				SPAN, None, specifiers, None, kind, NONE,
			);
			new_stm.push(Statement::from(
				ModuleDeclaration::ExportNamedDeclaration(empty_export),
			));
	...
	fn get_assignable_properties_for_namespaces(
		...
		for stmt in stmts {
			...
			let TSModuleDeclarationName::Identifier(ident) = &decl.id else {
				continue;
			};
			let Some(TSModuleDeclarationBody::TSModuleBlock(block)) =
				&decl.body
			else {
				continue;
			};
	...
	pub fn report_error_for_expando_function(
		...
		let assignable_properties_for_namespace =
			IsolatedDeclarations::get_assignable_properties_for_namespaces(
				stmts,
			);
			...
			match stmt {
							...
							for declarator in &decl.declarations {
								if declarator.id.type_annotation.is_none()
									&& declarator
										.init
										.as_ref()
										.is_some_and(Expression::is_function)
								{
									if let Some(name) =
										declarator.id.get_identifier()
									{
										can_expando_function_names
											.insert(name.clone());
									}
				...
				Statement::ExpressionStatement(stmt) => {
					if let Expression::AssignmentExpression(assignment) =
						&stmt.expression
					{
						if let AssignmentTarget::StaticMemberExpression(
							static_member_expr,
						) = &assignment.left
						{
								...
								&static_member_expr.object
							{
								if can_expando_function_names
									...
									self.error(
										function_with_assigning_properties(
