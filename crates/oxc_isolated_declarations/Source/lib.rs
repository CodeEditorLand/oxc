...
pub struct IsolatedDeclarationsOptions {
	...
	/// its JSDoc comment. This is an internal compiler option; use at your
	/// own risk, because the compiler does not check that the result is valid. <https://www.typescriptlang.org/tsconfig/#stripInternal>
...
impl<'a> IsolatedDeclarations<'a> {
	...
	fn take_errors(&self) -> Vec<OxcDiagnostic> { mem::take(&mut self.errors.borrow_mut()) }

	/// Add an Error
	fn error(&self, error:OxcDiagnostic) { self.errors.borrow_mut().push(error); }
	...
	fn build_internal_annotations(source_text:&str, trivias:&Trivias) -> FxHashSet<u32> {
		...
		for comment in trivias.comments() {
			...
			// Use the first jsdoc comment if there are multiple jsdoc comments
			// for the same node.
...
impl<'a> IsolatedDeclarations<'a> {
	...
	pub fn transform_statements_on_demand(
				...
				match_declaration!(Statement) => {
					if let Statement::TSModuleDeclaration(decl) = stmt {
						...
						if is_global || decl.id.is_string_literal() {
							...
							// Remove export keyword from all statements in
							// `declare module "xxx" { ... }`
							if !is_global {
								if let Some(body) =
							...
							// We need to visit the module declaration to
				...
				match_module_declaration!(Statement) => {
					match stmt.to_module_declaration() {
						...
						ModuleDeclaration::ImportDeclaration(_) => {
							// We must transform this in the end, because we
							// need to know all references
						},
						module_declaration => {
		...
		// If any declaration is transformed in previous step, we don't need to
		// add empty export marker
		...
		let mut new_stm = self
			.ast
			.vec_with_capacity(stmts.len() + usize::from(export_default_var.is_some()));
		...
		if need_empty_export_marker {
			...
			let empty_export = self
				.ast
				.alloc_export_named_declaration(SPAN, None, specifiers, None, kind, NONE);
			new_stm.push(Statement::from(ModuleDeclaration::ExportNamedDeclaration(empty_export)));
		} else if self.scope.is_ts_module_block() {
			// If we are in a module block and we don't need to add `export {}`,
			// in that case we need to remove `export` keyword from all
			// ExportNamedDeclaration <https://github.com/microsoft/TypeScript/blob/a709f9899c2a544b6de65a0f2623ecbbe1394eab/src/compiler/transformers/declarations.ts#L1556-L1563>
			...
			match stmt {
				Statement::FunctionDeclaration(ref func) => {
					let name = &func
						...
						.unwrap_or_else(|| {
							unreachable!(
								"Only export default function declaration is allowed to have no \
								 name"
							)
						})
						.name;
				...
				Statement::ExportNamedDeclaration(ref decl) => {
					if let Some(Declaration::FunctionDeclaration(ref func)) = decl.declaration {
						let name = &func
							...
							.unwrap_or_else(|| {
								unreachable!(
									"Only export default function declaration is allowed to have \
									 no name"
								)
							})
							.name;
						if func.body.is_some() {
							if last_function_name
								.as_ref()
								.is_some_and(|last_name| last_name == name)
							{
								return false;
							}
						} else {
