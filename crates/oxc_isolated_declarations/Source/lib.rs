...
pub struct IsolatedDeclarationsOptions {
	...
	/// risk, because the compiler does not check that the result is valid.
	///
	/// Default: `false`
	///
	/// ## References
...
impl<'a> IsolatedDeclarations<'a> {
	...
	fn build_internal_annotations(program:&Program<'a>) -> FxHashSet<u32> {
		...
		for comment in &program.comments {
			...
			// Use the first jsdoc comment if there are multiple jsdoc comments for the same
			// node.
...
impl<'a> IsolatedDeclarations<'a> {
	fn transform_program(&mut self, program:&Program<'a>) -> oxc_allocator::Vec<'a, Statement<'a>> {
	...
	fn transform_statements_on_demand(
		...
		// we will emit an extra variable declaration to store the inferred type of
		// expression
		...
		// If any declaration is transformed in previous step, we don't need to add
		// empty export marker
		...
		if need_empty_export_marker {
			...
			// If we are in a module block and we don't need to add `export {}`, in that
			// case we need to remove `export` keyword from all ExportNamedDeclaration <https://github.com/microsoft/TypeScript/blob/a709f9899c2a544b6de65a0f2623ecbbe1394eab/src/compiler/transformers/declarations.ts#L1556-L1563>
	...
	fn remove_function_overloads_implementation(stmts:&mut oxc_allocator::Vec<'a, &Statement<'a>>) {
