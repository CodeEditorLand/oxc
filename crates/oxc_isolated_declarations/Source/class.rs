...
impl<'a> IsolatedDeclarations<'a> {
	...
	pub(crate) fn transform_class(
		...
		if has_private_key {
			...
			// When the class has at least one private identifier, create a unique constant
			// identifier to retain the nominal typing behavior Prevents other classes
			// with the same public members from being used in place of the current class
