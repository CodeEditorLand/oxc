...
pub fn implicitly_adding_undefined_to_type(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::error(
		"TS9025: Declaration emit for this parameter requires implicitly adding undefined to it's \
...
pub fn function_with_assigning_properties(span:Span) -> OxcDiagnostic {
	OxcDiagnostic::error(
		"TS9023: Assigning properties to functions without declaring them is not supported with \
		 --isolatedDeclarations. Add an explicit declaration for the properties assigned to this \
		 function.",
	)
	.with_label(span)
}

// TS9026: Declaration emit for this file requires preserving this import for
// augmentations. This is not supported with --isolatedDeclarations. This error
// requires cross-file checking, which we cannot support.
