...
use oxc_ast::ast::{
	...
	Function, Statement, TSType, TSTypeAnnotation, UnaryExpression,
};
use oxc_span::SPAN;
...
impl<'a> IsolatedDeclarations<'a> {
	...
	pub fn infer_type_from_expression(
		...
		match expr {
			Expression::BooleanLiteral(_) => {
				Some(self.ast.ts_type_boolean_keyword(SPAN))
			},
			Expression::NullLiteral(_) => {
				Some(self.ast.ts_type_null_keyword(SPAN))
			},
			Expression::NumericLiteral(_) => {
				Some(self.ast.ts_type_number_keyword(SPAN))
			},
			Expression::BigIntLiteral(_) => {
				Some(self.ast.ts_type_big_int_keyword(SPAN))
			},
	...
	pub fn infer_function_return_type(
		...
		function.body.as_ref().and_then(|body| {
			FunctionReturnType::infer(self, body).map(|type_annotation| {
	...
	pub fn is_need_to_infer_type_from_expression(
		expr: &Expression<'a>,
	) -> bool {
		match expr {
			Expression::NumericLiteral(_)
			...
			Expression::UnaryExpression(expr) => {
				!Self::can_infer_unary_expression(expr)
			},
			_ => true,
		}
	}
