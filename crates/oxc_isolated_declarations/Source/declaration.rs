...
use crate::{
	diagnostics::{
		binding_element_export, inferred_type_of_expression,
		signature_computed_property_name, variable_must_have_explicit_type,
	},
...
impl<'a> IsolatedDeclarations<'a> {
		...
		if decl.id.type_annotation.is_none() {
			...
			if init.is_none() && binding_type.is_none() {
				...
				if !decl.init.as_ref().is_some_and(Expression::is_function) {
					self.error(variable_must_have_explicit_type(
						decl.id.span(),
					));
				}
			}
	...
	pub fn transform_declaration(
		...
		match decl {
			Declaration::FunctionDeclaration(func) => {
				...
				{
					self.transform_function(func, None)
			...
			Declaration::ClassDeclaration(decl) => {
				if !check_binding
					|| decl
						...
						.is_some_and(|id| self.scope.has_reference(&id.name))
				{
					self.transform_class(decl, None)
			...
			Declaration::TSTypeAliasDeclaration(alias_decl) => {
				if !check_binding
					|| self.scope.has_reference(&alias_decl.id.name)
				{
			...
			Declaration::TSInterfaceDeclaration(interface_decl) => {
				if !check_binding
					|| self.scope.has_reference(&interface_decl.id.name)
				{
					let mut decl = decl.clone_in(self.ast.allocator);
					self.visit_declaration(&mut decl);
					Some(decl)
			...
			Declaration::TSEnumDeclaration(enum_decl) => {
				if !check_binding
					|| self.scope.has_reference(&enum_decl.id.name)
				{
			...
			Declaration::TSImportEqualsDeclaration(decl) => {
				if !check_binding || self.scope.has_reference(&decl.id.name) {
					// SAFETY: `ast.copy` is unsound! We need to fix.
					Some(Declaration::TSImportEqualsDeclaration(unsafe {
						self.ast.copy(decl)
					}))
				} else {
					None
	...
	fn report_signature_property_key(
		&self,
		key: &PropertyKey<'a>,
		computed: bool,
	) {
		...
		let is_not_allowed = match key {
			PropertyKey::StaticIdentifier(_) | PropertyKey::Identifier(_) => {
				false
			},
...
impl<'a> VisitMut<'a> for IsolatedDeclarations<'a> {
	fn visit_ts_signatures(
		&mut self,
		signatures: &mut Vec<'a, TSSignature<'a>>,
	) {
