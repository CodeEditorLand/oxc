...
use crate::{
	diagnostics::{
		...
		implicitly_adding_undefined_to_type, parameter_must_have_explicit_type,
	},
...
impl<'a> IsolatedDeclarations<'a> {
				...
				.map(|ts_type| {
					...
					if is_remaining_params_have_required {
						if matches!(ts_type, TSType::TSTypeReference(_)) {
							self.error(implicitly_adding_undefined_to_type(
								param.span,
							));
							...
							return self.ast.ts_type_annotation(
								...
								self.ast.ts_type_union_type(
									...
									self.ast.vec_from_iter([
										ts_type,
										self.ast
											.ts_type_undefined_keyword(SPAN),
									]),
								),
							);
						}
	...
	pub fn transform_formal_parameters(
		...
		let items = self.ast.vec_from_iter(
			params.items.iter().enumerate().filter_map(|(index, item)| {
