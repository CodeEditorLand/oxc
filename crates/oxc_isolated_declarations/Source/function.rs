...
use crate::{
	diagnostics::{
		...
		implicitly_adding_undefined_to_type,
...
impl<'a> IsolatedDeclarations<'a> {
	...
	pub fn transform_formal_parameter(
		...
		Some(
			self.ast
				.formal_parameter(param.span, self.ast.vec(), pattern, None, false, false),
		)
	}
	...
	pub fn transform_formal_parameters(
		...
		let items =
			self.ast
				.vec_from_iter(params.items.iter().enumerate().filter_map(|(index, item)| {
