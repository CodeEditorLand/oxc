...
use oxc_ast::visit::walk::{
	walk_export_all_declaration, walk_export_named_declaration,
	walk_import_declaration, walk_import_expression, walk_meta_property,
	walk_module_declaration, walk_statement,
};
	...
	fn visit_import_declaration(&mut self, decl: &ImportDeclaration<'a>) {
		let assertions = decl
			...
			.filter(|c| {
				c.with_entries
					.first()
					.is_some_and(|a| a.key.as_atom() == "type")
			})
			.map(|c| c.span.start);
		self.imports.push(ImportSpecifier {
