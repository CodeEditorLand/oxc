...
/// Info about a binding, from which one can create a `BindingIdentifier` or
...
/// Notes:
///
/// * `BoundIdentifier` is smaller than `BindingIdentifier`, so takes less
///   memory when you store it for later use.
/// * `BoundIdentifier` is `Clone` (unlike `BindingIdentifier`).
/// * `BoundIdentifier` re-uses the same `Atom` for all `BindingIdentifier` /
...
impl<'a> BoundIdentifier<'a> {
	...
	pub fn new(name:Atom<'a>, symbol_id:SymbolId) -> Self { Self { name, symbol_id } }
	...
	pub fn create_binding_identifier(&self, ctx:&TraverseCtx<'a>) -> BindingIdentifier<'a> {
		ctx.ast
			.binding_identifier_with_symbol_id(SPAN, self.name.clone(), self.symbol_id)
	}
