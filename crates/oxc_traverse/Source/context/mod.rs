...
impl<'a> TraverseCtx<'a> {
	...
	#[inline]
	pub fn current_hoist_scope_id(&self) -> ScopeId { self.scoping.current_hoist_scope_id() }
	...
	///
	/// If you know if there's a `SymbolId` or not, prefer
	/// `TraverseCtx::create_bound_ident_reference`
	...
	///
	/// If you know if there's a `SymbolId` or not, prefer
	/// `TraverseCtx::create_bound_ident_expr`
...
impl<'a> TraverseCtx<'a> {
	...
	///
	/// # SAFETY
	/// This function must not be public to maintain soundness of
	/// [`TraverseAncestry`].
	...
	/// Shortcut for `ctx.scoping.set_current_hoist_scope_id`, to make `walk_*`
	/// methods less verbose.
	#[inline]
