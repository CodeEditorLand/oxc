...
///   [`scopes_mut`], [`symbols_mut`], [`ancestor_scopes`].
...
impl<'a> TraverseCtx<'a> {
	...
	#[inline]
	pub fn parent<'t>(&'t self) -> Ancestor<'a, 't> { self.ancestry.parent() }
	...
	#[inline]
	pub fn ancestor<'t>(&'t self, level:usize) -> Ancestor<'a, 't> { self.ancestry.ancestor(level) }
	...
	#[inline]
	pub fn current_scope_flags(&self) -> ScopeFlags { self.scoping.current_scope_flags() }
	...
	/// used in the program.
	///
	/// See [`TraverseScoping::generate_uid_name`] for important information on
	/// how UIDs are generated. There are some potential "gotchas".
	///
	...
	/// `ctx.scoping.generate_uid_in_current_scope_based_on_node`.
	#[inline]
	...
	/// `IdentifierReference`s with `TraverseCtx::create_reference_id`.
	///
	...
	#[inline]
	pub fn is_static(&self, expr:&Expression) -> bool { self.scoping.is_static(expr) }
}
...
impl<'a> TraverseCtx<'a> {
	...
	/// break safety invariants.
	#[inline]
	pub(crate) unsafe fn pop_stack(&mut self, token:PopToken) { self.ancestry.pop_stack(token); }
