...
impl TraverseScoping {
	...
	#[inline]
	pub(crate) fn current_hoist_scope_id(&self) -> ScopeId { self.current_hoist_scope_id }
	...
	///
	/// Use this when removing an expression which owns a scope, without
	/// removing its children. For example when unwrapping `(() => foo)()` to
	/// just `foo`. `foo` here could be an expression which itself contains
	/// scopes.
	...
	/// Generate binding.
	///
	/// Creates a symbol with the provided name and flags and adds it to the
	/// specified scope.
	...
	/// output.
	///
	/// `get_uid_names` iterates through every single binding and unresolved
...
impl TraverseScoping {
	...
	/// Consume [`TraverseScoping`] and return [`SymbolTable`] and
	/// [`ScopeTree`].
	...
	///
	/// Once this set is created, generating a UID is a relatively quick
...
fn get_uid_name_base(name:&str) -> &str {
	...
	// SAFETY: We started with a valid UTF8 `&str` and have only trimmed off ASCII
	// characters, so remainder must still be valid UTF8
...
fn get_unique_name_impl(base:&str, uid_names:&FxHashSet<CompactStr>) -> CompactString {
	...
	// capacity. The extra byte is to avoid reallocation if need to add a digit on
	// the end later, which will not be too uncommon.
	// Having to add 2 digits will be uncommon, so we don't allocate 2 extra bytes
	// for 2 digits.
	...
	// It's fairly common that UIDs may need a numerical postfix, so we try to keep
	// string operations to a minimum for postfixes up to 99 - reusing a single
	// `CompactString`, rather than generating a new string on each attempt.
	...
	// with ASCII digits. These bytes are already ASCII chars, so cannot produce an
	// invalid UTF-8 string. Writes are always in bounds (`bytes` is redefined
	// after string grows due to `push`).
	unsafe {
	...
	// sufficient capacity to push?" in the loop below completely predictable for
	// `n < 1000`.
	name.reserve(1);
	...
	for n in 100..=u32::MAX {
		...
		// SAFETY: `base_len` is always shorter than current `name.len()`, on a UTF-8
		// char boundary, and `name` contains at least `base_len` initialized bytes
	...
	// `u32::MAX` identifier names in the AST. So loop above cannot fail to find an
	// unused name.
	unreachable!();
}
