...
impl TraverseScoping {
	...
	#[inline]
	pub fn current_scope_flags(&self) -> ScopeFlags { self.scopes.get_flags(self.current_scope_id) }
	...
	/// All child scopes of the statement are reassigned to be children of the
	/// new scope.
	///
	/// `flags` provided are amended to inherit from parent scope's flags.
	...
	/// New scope is created as child of current scope.
	/// All child scopes of the expression are reassigned to be children of the
	/// new scope.
	///
	/// `flags` provided are amended to inherit from parent scope's flags.
	pub fn insert_scope_below_expression(&mut self, expr:&Expression, flags:ScopeFlags) -> ScopeId {
		let mut collector = ChildScopeCollector::new();
	...
	///
	/// Finds a unique variable name which does clash with any other variables
	/// used in the program.
	///
	...
	///
	/// This implementation aims to replicate Babel's behavior, but differs from
	/// Babel in the following ways:
	///
	/// 1. Does not check that name is a valid JS identifier name.
	/// In most cases, we'll be creating a UID based on an existing variable
	/// name, in which case this check is redundant.
	/// Caller must ensure `name` is a valid JS identifier, after a `_` is
	/// prepended on start. The fact that a `_` will be prepended on start
	/// means providing an empty string or a string starting with a digit (0-9)
	/// is fine.
	///
	...
	/// No globals or "contextVariables" start with `_` anyway, so no need for
	/// this check.
	///
	/// 4. Does not check this name is unique if used as a named statement
	///    label, only that it's unique
	/// as an identifier.
	/// If we need to generate unique labels for named statements, we should
	/// create a separate method `generate_uid_label`.
	///
	/// 5. Does not check against list of other UIDs that have been created.
	/// `TraverseScoping::generate_uid` adds this name to symbols table, so when
	/// creating next UID, this one will be found and avoided, like any other
	/// existing binding. So it's not needed.
	///
	/// # Potential improvements
	///
	/// TODO(improve-on-babel):
	///
	/// This function is fairly expensive, because it aims to replicate Babel's
	/// output.
	///
	/// `init_uid_names` iterates through every single binding and unresolved
	/// reference in the entire AST, and builds a hashset of symbols which
	/// could clash with UIDs. Once that's built, it's cached, but
	/// `find_uid_name` still has to do at least one hashset lookup,
	/// and a hashset insert. If the first name tried is already in use, it will
	/// do another hashset lookup, potentially multiple times until a name
	/// which isn't taken is found.
	///
	/// We could improve this in one of 3 ways:
	///
	/// 1. Build the hashset in `SemanticBuilder` instead of iterating through
	///    all symbols again here.
	///
	/// 2. Use a much simpler method:
	///
	/// * During initial semantic pass, check for any existing identifiers
	///   starting with `_`.
	/// * Calculate what is the highest postfix number on `_...` identifiers
	///   (e.g. `_foo1`, `_bar8`).
	/// * Store that highest number in a counter which is global across the
	///   whole program.
	/// * When creating a UID, increment the counter, and make the UID
	///   `_<name><counter>`.
	///
	/// i.e. if source contains identifiers `_foo1` and `_bar15`, create UIDs
	/// named `_qux16`, `_temp17` etc. They'll all be unique within the
	/// program.
	///
	/// Minimal cost in semantic, and generating UIDs extremely cheap.
	///
	/// This is a slightly different method from Babel, and unfortunately
	/// produces UID names which differ from Babel for some of its test cases.
	///
	/// 3. If output is being minified anyway, use a method which produces less
	///    debuggable output,
	/// but is even simpler:
	///
	/// * During initial semantic pass, check for any existing identifiers
	...
	/// * Generate UIDs with a counter starting at 0, prefixed with number of
	///   `_`s one greater than what was found in AST.
	/// i.e. if source contains identifiers `_foo` and `__bar`, create UIDs
	/// names `___0`, `___1`, `___2` etc. They'll all be unique within the
	/// program.
	...
	///
	/// See also comments on [`TraverseScoping::generate_uid_name`] for
	/// important information on how UIDs are generated. There are some
	/// potential "gotchas".
	...
	///
	/// If you know if there's a `SymbolId` or not, prefer
	/// `TraverseCtx::create_bound_reference`
	...
	///
	/// If you know if there's a `SymbolId` or not, prefer
	/// `TraverseCtx::create_bound_reference_id`
	...
	pub fn delete_reference_for_identifier(&mut self, ident:&IdentifierReference) {
		// `unwrap` should never panic as `IdentifierReference`s should always
		// have a `ReferenceId`
		self.delete_reference(ident.reference_id().unwrap(), &ident.name);
	}

	/// Clone `IdentifierReference` based on the original reference's `SymbolId`
	/// and name.
	///
	/// This method makes a lookup of the `SymbolId` for the reference. If you
	/// need to create multiple `IdentifierReference`s for the same binding, it
	/// is better to look up the `SymbolId` only once, and generate
	/// `IdentifierReference`s with `TraverseScoping::create_reference_id`.
	...
	/// Determine whether evaluating the specific input `node` is a
	/// consequenceless reference.
	///
	/// I.E evaluating it won't result in potentially arbitrary code from being
	/// ran. The following are allowed and determined not to cause side
	/// effects:
	///
	/// - `this` expressions
	/// - `super` expressions
	...
	///
	/// # Panics
	/// Can only panic if [`IdentifierReference`] does not have a reference_id,
...
impl TraverseScoping {
	...
	///
	/// Iterate through all symbols and unresolved references in AST and
	/// identify any var names which could clash with UIDs (start with `_`).
	/// Build a hash set containing them.
	///
	/// Once this map is created, generating a UID is a relatively quick
	/// operation, rather than iterating over all symbols and unresolved
	/// references every time generate a UID.
	fn init_uid_names(&mut self) {
		let uid_names = self
			...
			.filter_map(|name| {
				if name.as_bytes().first() == Some(&b'_') { Some(name.clone()) } else { None }
			})
			.collect();
...
fn get_uid_name_base(name:&str) -> &str {
	// Equivalent to `name.trim_start_matches('_').trim_end_matches(|c: char|
	// c.is_ascii_digit())` but more efficient as operates on bytes not chars
	...
	// SAFETY: We started with a valid UTF8 `&str` and have only trimmed off
	// ASCII characters, so remainder must still be valid UTF8
...
// TODO: We could make this function more performant, especially when it checks
// a lot of names before it reaches one that is unused.
// This function repeatedly creates strings which have only differ from each
// other by digits added on end, and then hashes each of those strings to test
// them against the hash set `uid_names`. Hashing strings is fairly expensive.
// As here only the end of the string changes on each iteration,
// we could calculate an "unfinished" hash not including the last block, and
// then just add the final block to "finish" the hash on each iteration. With
// `FxHash` this would be straight line code and only a few operations.
fn get_unique_name_impl(base:&str, uid_names:&FxHashSet<CompactStr>) -> CompactString {
	// Create `CompactString` prepending name with `_`, and with 1 byte excess
	// capacity. The extra byte is to avoid reallocation if need to add a digit
	// on the end later, which will not be too uncommon.
	// Having to add 2 digits will be uncommon, so we don't allocate 2 extra
	// bytes for 2 digits.
	let mut name = CompactString::with_capacity(base.len() + 2);
	name.push('_');
	name.push_str(base);

	// It's fairly common that UIDs may need a numerical postfix, so we try to
	// keep string operations to a minimum for postfixes up to 99 - reusing a
	// single `CompactString`, rather than generating a new string on each
	// attempt. For speed we manipulate the string as bytes.
	// Postfixes greater than 99 should be very uncommon, so don't bother
	// optimizing.
	//
	// SAFETY: Only modifications to string are replacing last byte/last 2 bytes
	// with ASCII digits. These bytes are already ASCII chars, so cannot
	// produce an invalid UTF-8 string. Writes are always in bounds (`bytes` is
	// redefined after string grows due to `push`).
	unsafe {
	...
	// Do this here so that `name.push_str(digits)` will never need to grow the
	// string until it reaches `n == 1000`, which makes the branch on "is there
	// sufficient capacity to push?" in the loop below completely predictable
	// for `n < 1000`.
	name.reserve(1);

	// At this point, `name` has had 2 digits added on end. `base_len` is length
	// without those 2 digits.
	let base_len = name.len() - 2;

	let mut buffer = ItoaBuffer::new();
	for n in 100..=u32::MAX {
		let digits = buffer.format(n);
		// SAFETY: `base_len` is always shorter than current `name.len()`, on a
		// UTF-8 char boundary, and `name` contains at least `base_len`
		// initialized bytes
	...
	// Limit for size of source text is `u32::MAX` bytes, so there cannot be
	// `u32::MAX` identifier names in the AST. So loop above cannot fail to
	// find an unused name.
	unreachable!();
}
...
fn test_get_unique_name() {
	let cases:&[(&[&str], &str, &str)] = &[
		...
		(
			&["_foo", "_foo2", "_foo3", "_foo4", "_foo5", "_foo6", "_foo7", "_foo8"],
			"foo",
			"_foo9",
		),
		(
