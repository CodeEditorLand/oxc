...
///
/// `'t` is constrained in `parent`, `ancestor` and `ancestors` methods to only
/// live as long as the `&'t TraverseAncestry` passed to the method.
/// i.e. `Ancestor`s can only live as long as `enter_*` or `exit_*` method in
/// which they're obtained, and cannot "escape" those methods.
/// This is required for soundness. If an `Ancestor` could be retained longer,
/// the references that can be got from it could alias a `&mut` reference to the
/// same AST node.
///
/// # SAFETY
...
/// ## Soundness hole
///
/// Strictly speaking, there is still room to abuse the API and cause UB as
/// follows:
///
/// * Initiate a 2nd traversal of a different AST inside a `Traverse` visitor
///   method.
/// * `mem::swap` the 2 x `&mut TraverseCtx`s from the 2 different traversals.
///
/// The 2 ASTs would have to be different, but borrowed for same lifetime, so I
/// (@overlookmotel) don't think it's possible by this method to produce
/// aliasing violations, or to over-extend AST node lifetimes to cause a
/// use-after-free. But it *could* produce buffer underrun in `pop_stack`, when
/// it tries to pop from a stack which is already empty.
///
/// In practice, this would be a completely bizarre thing to do, and would
/// basically require you to write malicious code specifically designed to cause
/// UB. So it's not a particularly real risk.
///
/// To close this hole and make the API 100% sound, we'd need branded lifetimes
/// so that all `TraverseCtx`s have unique lifetimes, and so cannot be swapped
/// for any other without the borrow-checker complaining.
...
impl<'a> TraverseAncestry<'a> {
	...
	pub fn parent<'t>(&'t self) -> Ancestor<'a, 't> {
		...
		// SAFETY: The `Ancestor` is guaranteed valid for `'t`. It is not possible to
		// obtain a `&mut` ref to any AST node which this `Ancestor` gives access to
		// during `'t`.
	...
	/// `ancestor(0)` is equivalent to `parent()` (but better to use `parent()`
	...
	pub fn ancestor<'t>(&'t self, level:usize) -> Ancestor<'a, 't> {
		...
		// `len = 1, level = 1` -> return `Ancestor::None` from else branch (out of
		// bounds) `len = 3, level = 0` -> return parent (index 2)
		// `len = 3, level = 1` -> return grandparent (index 1)
		// `len = 3, level = 2` -> return `Ancestor::None` from else branch
		// `len = 3, level = 3` -> return `Ancestor::None` from else branch (out of
		// bounds)

		// `self.stack.len()` is always at least 1, so `self.stack.len() - 1` cannot
		// wrap around. `level <= last_index` would also work here, but `level <
		// last_index` avoids a read from memory when that read would just get
		// `Ancestor::None` anyway.
		let last_index = self.stack.len() - 1;

		if level < last_index {
			// SAFETY: We just checked that `level < last_index` so `last_index - level`
			// cannot wrap around, and `last_index - level` must be a valid index
	...
	///
	/// Last `Ancestor` returned will be `Program`. `Ancestor::None` is not
...
impl<'a> TraverseAncestry<'a> {
	...
	pub(crate) fn pop_stack(&mut self, token:PopToken) {
		// SAFETY: `PopToken`s are only created in `push_stack`, so the fact that caller
		// provides one guarantees that a push has happened. This method consumes the
		// token which guarantees another pop hasn't occurred already corresponding to
		// that push. Therefore the stack cannot by empty.
		// The stack starts with 1 entry, so also it cannot be left empty after this
		// pop.
	...
	///
	/// # SAFETY
	/// * Stack must have length of at least 2 (so we are not retagging dummy
	///   root `Ancestor`).
...
/// Zero sized token which allows popping from stack. Used to ensure push and
/// pop always correspond. Inner field is private to this module so can only be
/// created by methods in this file. It is not `Clone` or `Copy`, so no way to
/// obtain one except in this file. Only method which generates a `PopToken` is
/// `push_stack`, and `pop_stack` consumes one, which guarantees you can't have
