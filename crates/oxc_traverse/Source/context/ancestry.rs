...
impl<'a> TraverseAncestry<'a> {
	...
	pub fn parent<'t>(&'t self) -> Ancestor<'a, 't> {
		...
		// SAFETY: The `Ancestor` is guaranteed valid for `'t`. It is not
		// possible to obtain a `&mut` ref to any AST node which this
		// `Ancestor` gives access to during `'t`.
	...
	pub fn ancestor<'t>(&'t self, level:usize) -> Ancestor<'a, 't> {
		...
		// `len = 1, level = 1` -> return `Ancestor::None` from else branch (out
		// of bounds) `len = 3, level = 0` -> return parent (index 2)
		// `len = 3, level = 1` -> return grandparent (index 1)
		// `len = 3, level = 2` -> return `Ancestor::None` from else branch
		// `len = 3, level = 3` -> return `Ancestor::None` from else branch (out
		// of bounds)

		// `self.stack.len()` is always at least 1, so `self.stack.len() - 1`
		// cannot wrap around. `level <= last_index` would also work here, but
		// `level < last_index` avoids a read from memory when that read would
		// just get `Ancestor::None` anyway.
		let last_index = self.stack.len() - 1;
		if level < last_index {
			// SAFETY: We just checked that `level < last_index` so `last_index
			// - level` cannot wrap around, and `last_index - level` must be
			// a valid index
...
impl<'a> TraverseAncestry<'a> {
	...
	pub(crate) fn pop_stack(&mut self, token:PopToken) {
		// SAFETY: `PopToken`s are only created in `push_stack`, so the fact
		// that caller provides one guarantees that a push has happened. This
		// method consumes the token which guarantees another pop hasn't
		// occurred already corresponding to that push. Therefore the stack
		// cannot by empty. The stack starts with 1 entry, so also it cannot
		// be left empty after this pop.
