...
/// Notes:
///
/// * The original `IdentifierReference` must also be used in the AST, or it'll
///   be a dangling reference.
/// * `MaybeBoundIdentifier` is smaller than `IdentifierReference`, so takes
///   less memory when you store it for later use.
/// * `MaybeBoundIdentifier` re-uses the same `Atom` for all `BindingIdentifier`
///   / `IdentifierReference`s created from it.
/// * `MaybeBoundIdentifier` looks up the `SymbolId` for the reference only
///   once.
#[derive(Debug, Clone)]
...
impl<'a> MaybeBoundIdentifier<'a> {
	...
	pub fn new(name:Atom<'a>, symbol_id:Option<SymbolId>) -> Self { Self { name, symbol_id } }

	/// Create `MaybeBoundIdentifier` from an `IdentifierReference`
	pub fn from_identifier_reference(ident:&IdentifierReference<'a>, ctx:&TraverseCtx<'a>) -> Self {
	...
	pub fn to_bound_identifier(&self) -> Option<BoundIdentifier<'a>> {
		self.symbol_id
			.map(|symbol_id| BoundIdentifier::new(self.name.clone(), symbol_id))
	}

	// --- Read only ---

	/// Create `IdentifierReference` referencing this binding, which is read
	/// from, with dummy `Span`
	pub fn create_read_reference(&self, ctx:&mut TraverseCtx<'a>) -> IdentifierReference<'a> {
		self.create_spanned_read_reference(SPAN, ctx)
	}

	/// Create `Expression::Identifier` referencing this binding, which is read
	/// from, with dummy `Span`
	...
	/// Create `IdentifierReference` referencing this binding, which is written
	/// to, with dummy `Span`
	...
	/// Create `Expression::Identifier` referencing this binding, which is
	/// written to, with specified `Span`
	...
	/// Create `IdentifierReference` referencing this binding, which is read
	/// from + written to, with dummy `Span`
	pub fn create_read_write_reference(&self, ctx:&mut TraverseCtx<'a>) -> IdentifierReference<'a> {
	...
	/// Create `AssignmentTarget` referencing this binding, which is read from +
	/// written to, with dummy `Span`
	...
	/// Create `SimpleAssignmentTarget` referencing this binding, which is read
	/// from + written to, with dummy `Span`
	...
	/// Create `IdentifierReference` referencing this binding, which is read
	/// from + written to, with specified `Span`
	...
	/// Create `IdentifierReference` referencing this binding, with specified
	/// `ReferenceFlags`
	...
	/// Create `Expression::Identifier` referencing this binding, with specified
	/// `ReferenceFlags`
	...
	/// Create `AssignmentTarget` referencing this binding, with specified
	/// `ReferenceFlags`
	...
	/// Create `SimpleAssignmentTarget` referencing this binding, with specified
	/// `ReferenceFlags`
	...
	/// Create `AssignmentTarget::AssignmentTargetIdentifier` referencing this
	/// binding, with specified `Span` and `ReferenceFlags`
	...
	/// Create `SimpleAssignmentTarget::AssignmentTargetIdentifier` referencing
	/// this binding, with specified `Span` and `ReferenceFlags`
