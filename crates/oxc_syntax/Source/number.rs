...
impl NumberBase {
	pub fn is_base_10(&self) -> bool { matches!(self, Self::Float | Self::Decimal) }
}
...
impl BigintBase {
	pub fn is_base_10(&self) -> bool { self == &Self::Decimal }
}
...
impl ToJsInt32 for f64 {
	...
	fn to_js_int_32(&self) -> i32 {
		...
		fn is_denormal(number:f64) -> bool { (number.to_bits() & EXPONENT_MASK) == 0 }
		...
		fn sign(number:f64) -> i64 { if (number.to_bits() & SIGN_MASK) == 0 { 1 } else { -1 } }
