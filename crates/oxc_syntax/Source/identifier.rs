...
pub fn is_identifier_name(name:&str) -> bool {
	// This function contains a fast path for ASCII (common case), iterating over
	// bytes and using the cheap `is_identifier_start_ascii` and
	// `is_identifier_part_ascii` to test bytes. Only if a Unicode char is found,
	// fall back to iterating over `char`s, and using the more
	// expensive `is_identifier_start_unicode` and `is_identifier_part`.
	// As a further optimization, we test if bytes are ASCII in blocks of 8 or 4
	// bytes, rather than 1 by 1.
	...
	let mut chars = if first_byte.is_ascii() {
		...
		'outer: loop {
			...
			if bytes_remaining >= 8 {
				...
				// SAFETY: We checked above that there are at least 8 bytes to read starting at
				// `index`
				...
				// SAFETY: We checked above that there are at least 4 bytes to read starting at
				// `index`
				#[allow(clippy::cast_ptr_alignment)]
		...
		// Unicode byte found - search rest of string (from this byte onwards) as
		// Unicode
		name[index..].chars()
	} else {
		// First char is Unicode.
		// NB: `unwrap()` cannot fail because we already checked the string is not
		// empty.
