...
impl ScopeId {
	...
	pub const fn new(idx:u32) -> Self {
		// We could use `NonMaxU32::new(idx).unwrap()` but `Option::unwrap` is
		// not a const function and we want this function to be
		assert!(idx != u32::MAX);
...
impl ScopeFlags {
	...
	#[inline]
	pub fn is_strict_mode(&self) -> bool { self.contains(Self::StrictMode) }

	#[inline]
	pub fn is_block(&self) -> bool { self.is_empty() || *self == Self::StrictMode }

	#[inline]
	pub fn is_top(&self) -> bool { self.contains(Self::Top) }

	#[inline]
	pub fn is_function(&self) -> bool { self.contains(Self::Function) }

	#[inline]
	pub fn is_arrow(&self) -> bool { self.contains(Self::Arrow) }

	#[inline]
	pub fn is_constructor(&self) -> bool { self.contains(Self::Constructor) }

	#[inline]
	pub fn is_class_static_block(&self) -> bool { self.contains(Self::ClassStaticBlock) }

	#[inline]
	pub fn is_ts_module_block(&self) -> bool { self.contains(Self::TsModuleBlock) }

	#[inline]
	pub fn is_var(&self) -> bool { self.intersects(Self::Var) }

	#[inline]
	pub fn is_set_accessor(&self) -> bool { self.contains(Self::SetAccessor) }
	...
	#[inline]
	pub fn is_catch_clause(&self) -> bool { self.contains(Self::CatchClause) }
}
