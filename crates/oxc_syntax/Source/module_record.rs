...
pub struct ModuleRecord {
	...
	pub loaded_modules:
		DashMap<CompactStr, Arc<ModuleRecord>, BuildHasherDefault<FxHasher>>,
	...
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> std::fmt::Result {
		...
		f.debug_struct("ModuleRecord")
			...
			.field(
				...
				&self.exported_bindings_from_star_export,
			)
...
mod test {
	...
	use super::{
		ExportExportName, ExportLocalName, ImportImportName, NameSpan,
	};
	...
	fn import_import_name() {
		...
		assert!(
			!ImportImportName::Default(Span::new(0, 0)).is_namespace_object()
		);
	}
