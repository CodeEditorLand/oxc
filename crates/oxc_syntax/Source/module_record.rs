...
pub struct ModuleRecord {
	...
	///
	/// A List of [`ExportEntry`] records derived from the code of this module
	/// that correspond to `export *` declarations that occur within the
...
impl fmt::Debug for ModuleRecord {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> std::fmt::Result {
		// recursively formatting loaded modules can crash when the module graph
		// is cyclic
...
impl NameSpan {
	...
	pub const fn name(&self) -> &CompactStr { &self.name }
...
impl RequestedModule {
	...
	pub fn is_import(&self) -> bool { self.is_import }
}
