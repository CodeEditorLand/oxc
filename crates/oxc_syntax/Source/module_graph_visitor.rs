...
type EventFn<'a> =
	dyn FnMut(ModuleGraphVisitorEvent, ModulePair, &ModuleRecord) + 'a;
...
impl<'a, T> ModuleGraphVisitorBuilder<'a, T> {
	...
	#[must_use]
	pub fn filter<F: (Fn(ModulePair, &ModuleRecord) -> bool) + 'a>(
		mut self,
		filter: F,
	) -> Self {
	...
	#[must_use]
	pub fn event<
		F: FnMut(ModuleGraphVisitorEvent, ModulePair, &ModuleRecord) + 'a,
	>(
		mut self,
		event: F,
	) -> Self {
	...
	#[must_use]
	pub fn enter<F: FnMut(ModulePair, &ModuleRecord) + 'a>(
		mut self,
		enter: F,
	) -> Self {
	...
	#[must_use]
	pub fn leave<F: FnMut(ModulePair, &ModuleRecord) + 'a>(
		mut self,
		leave: F,
	) -> Self {
	...
	pub fn visit_fold<
		V: Fn(T, ModulePair, &ModuleRecord) -> VisitFoldWhile<T>,
	>(
		self,
		initial_value: T,
		module: &ModuleRecord,
		...
		let result = visitor.filter_fold_while(
			initial_value,
			module,
			filter,
			visit,
