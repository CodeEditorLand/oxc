...
impl Runner for LintRunner {
	...
	fn run(self) -> CliRunResult {
		...
		if let Some(path) = tsconfig.as_ref() {
			if path.is_file() {
				...
				return CliRunResult::InvalidOptions {
					message:format!(
						"The tsconfig file {path:?} does not exist, Please provide a valid \
						 tsconfig file.",
					),
				};
			}
		...
		// Spawn linting in another thread so diagnostics can be printed immediately
		// from diagnostic_service.run.
		rayon::spawn({
...
impl LintRunner {
		...
		for (severity, filter_arg) in filters_arg {
			match LintFilter::new(severity, filter_arg) {
				...
				Err(InvalidFilterKind::PluginMissing(filter)) => {
					return Err(CliRunResult::InvalidOptions {
						message:format!(
							"Failed to {severity} filter {filter}: Plugin name is missing. \
							 Expected <plugin>/<rule>"
						),
					});
				},
				...
				Err(InvalidFilterKind::RuleMissing(filter)) => {
					return Err(CliRunResult::InvalidOptions {
						message:format!(
							"Failed to {severity} filter {filter}: Rule name is missing. Expected \
							 <plugin>/<rule>"
						),
					});
				},
	...
	// when config is provided, but not found, an CliRunResult is returned, else the
	// oxlintrc config file is returned when no config is provided, it will search
	// for the default file names in the current working directory when no file is
...
mod test {
	...
	use crate::cli::{CliRunResult, LintResult, Runner, lint_command};
	...
	fn test_tsconfig_option() {
		...
		// failed
		assert!(
			test_invalid_options(&["--tsconfig", "oxc/tsconfig.json"]).contains(
				"oxc/tsconfig.json\" does not exist, Please provide a valid tsconfig file."
			)
		);
	}
