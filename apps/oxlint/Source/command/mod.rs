...
pub use self::{
	...
	lint::{LintCommand, OutputFormat, OutputOptions, WarningOptions, lint_command},
};
...
pub struct MiscOptions {
	...
	/// When present, no linting is performed and only config-related options
	/// are valid.
	#[bpaf(switch, hide_usage)]
...
fn validate_paths(paths:&Vec<PathBuf>) -> bool {
	if paths.is_empty() {
		...
		paths
			.iter()
			.all(|p| p.components().all(|c| c != std::path::Component::ParentDir))
	}
}
