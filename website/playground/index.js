...
class Playground {
	...
	initEditor() {
		const stateListener = EditorView.updateListener.of((view) => {
			...
			if (
				!view.docChanged &&
				view.selectionSet &&
				this.currentView === "ast"
			) {
				const ranges = view.state.selection.ranges;
				if (ranges.length === 1 && ranges[0].empty) {
					this.editorRange = view.state.selection.ranges;
					const { from } = this.editorRange[0];
					const viewerText = this.viewer.state.doc.toString();
					const ast = parser.parse(viewerText);
					const root = ast.cursor().node;
					const targetNode = findMostInnerNodeForPosition(
						root.node,
						from,
						viewerText,
					);
	...
	updateDiagnostics() {
		const diagnostics = (this.oxc ? this.oxc.getDiagnostics() : []).map(
			(d) => ({
				...
				EditorState.transactionExtender.of((tr) => {
					...
					switch (this.currentLanguage()) {
						case "json":
							...
							break;
						case "javascript":
							if (currentLanguage == javascriptLanguage)
								return null;
	...
	updateView(view) {
		...
		switch (this.currentView) {
			...
			case "symbol":
				...
				break;
			case "codegen":
				document.getElementById("codegen-controls").style.display =
					"block";
				this.run();
				...
				for (const e of tr.effects) {
					if (e.is(addHighlight)) {
						highlights = highlights.update({
							add: [
								Playground.highlightMark.range(
									e.value.from,
									e.value.to,
								),
							],
	...
	highlightEditorFromViewer(e, view) {
		if (this.currentView === "symbol") {
			...
			const [start, end] = getStartAndEnd.call(this, view, cursor);
			// if we didn't find a start or an end, return early
			if (start == undefined || end == undefined) return;
			const ranges = getSymbolAndReferencesSpan(start, end);
...
async function main() {
	...
	document.getElementById("transform").onchange = () => {
	...
	document.getElementById("codegen-ts").onchange = () => {
	...
	document.getElementById("mangle").onchange = () => {
	...
	document.getElementById("file-type-select").onchange = (e) => {
	...
	document.getElementById("syntax").onchange = () => {
	...
	document.getElementById("lint").onchange = () => {
